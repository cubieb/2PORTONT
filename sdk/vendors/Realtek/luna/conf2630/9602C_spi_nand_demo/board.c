...
static int board_01_pb_is_pushed(int which) {
	...
	case PB_RESET:
		return GPIO_READ(GPIO_38);

	case PB_WIFISW:
		return GPIO_READ(GPIO_37);
#if defined (CONFIG_RTL8192CD) || defined (CONFIG_RTL8192CD_MODULE)
	case PB_WPS:
		return GPIO_READ(GPIO_41);
#endif
	default:
		return 0;
	}
...
//	REG32(BSP_MISC_PINMUX) |= (BSP_ZSI_ISI_PINMUX_C0 | BSP_ZSI_ISI_PINMUX_C1);
...

    {
        ...
        rtk_led_parallelEnable_set(15,ENABLED);	// FE LAN
	rtk_led_parallelEnable_set(2,ENABLED);	// GE LAN
...
//      ledConfig.ledEnable[LED_CONFIG_SPD1000] = ENABLED;
        ...
        rtk_led_config_set(15,LED_TYPE_UTP0,&ledConfig);    

//      memset(&ledConfig,0x0,sizeof(rtk_led_config_t));
        ledConfig.ledEnable[LED_CONFIG_SPD1000ACT] = ENABLED;    
        ledConfig.ledEnable[LED_CONFIG_SPD1000] = ENABLED;
	
        rtk_led_config_set(2,LED_TYPE_UTP1,&ledConfig);    
    }
