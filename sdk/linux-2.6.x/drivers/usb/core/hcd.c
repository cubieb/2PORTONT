...
////leroy ori////	0x02, 0x00, /*  __le16 idProduct; device 0x0002 */
...
#ifdef REALTEK_ONCHIP_USB
		strlcpy (buf, usb_bus_name, sizeof buf);
#else
			   ...
			   gfp_t mem_flags)
{
	...
	if(hcd->self.uses_dma && hcd->driver->rtl8672_dma_process) {
			hcd->driver->rtl8672_dma_process(hcd, urb, 1);
	}
...
static void unmap_urb_for_dma(struct usb_hcd *hcd, struct urb *urb)
{
	...
	#ifdef CONFIG_USB_PATCH_RTL8672
	if(hcd->self.uses_dma && hcd->driver->rtl8672_dma_process)
		hcd->driver->rtl8672_dma_process(hcd, urb, 3);
	#endif	//CONFIG_USB_PATCH_RTL8672
}
...
static int rtl8672_hcd_out_64_check(struct urb *urb)
{
	int retval = 0;
...
static int rtl8672_hcd_out_1K_boundary_check(struct urb *urb, bool setup_pkt)
{
	int retval = 0;
...
static int rtl8672_hcd_in_alignment_check(struct urb *urb)
{
	int retval = 0;
	unsigned char aligned;
...
static void rtl8672_hcd_dma_check(struct usb_hcd *hcd, struct urb *urb)
{
	#define RETRYCNT	5
	int retval = 0;
	...
	if(usb_pipecontrol(urb->pipe)) {
		...
		do {
			...
			retval = rtl8672_hcd_out_1K_boundary_check(urb, 1);
			if(retval < 0) {
...
parse_data_pkt:
	...
	if(usb_pipeout(urb->pipe)) {
		...
		do {
			...
			retval = rtl8672_hcd_out_64_check(urb);
			if(retval < 0) {
				printk("%s %d fail! retval=%d\n", __func__, __LINE__, retval);
				goto patch_data_pkt_fail; 
			}

			retval = rtl8672_hcd_out_1K_boundary_check(urb, 0);
			if(retval < 0) {
				printk("%s %d fail! retval=%d\n", __func__, __LINE__, retval);
				goto patch_data_pkt_fail;
			}
		...
		retval = rtl8672_hcd_in_alignment_check(urb);
...
#if 0
static void rtl8672_hcd_dma_giveback_abnormal(struct usb_hcd *hcd, struct urb *urb)
{
	if(unlikely(urb->temp_setup_buf)) {
		if(usb_pipecontrol(urb->pipe))
			dma_unmap_single(hcd->self.controller, urb->setup_dma,
...
static void rtl8672_hcd_dma_giveback_normal(struct usb_hcd *hcd, struct urb *urb)
{
	if(unlikely(urb->temp_setup_buf)) {
...
void rtl8672_hcd_dma_process(struct usb_hcd *hcd, struct urb *urb, unsigned char option)
{
	switch(option) {
		case 1:
			rtl8672_hcd_dma_check(hcd, urb);
			break;
		#if 0
		case 2:
			rtl8672_hcd_dma_giveback_abnormal(hcd, urb);
			break;
		#endif
		case 3:
			rtl8672_hcd_dma_giveback_normal(hcd, urb);
			break;
		default:
			break;
	}
...
EXPORT_SYMBOL(rtl8672_hcd_dma_process);
#endif	//CONFIG_USB_PATCH_RTL8672
