...
int32 rtl865x_initNxtHopTable(void)
{
	...
	for(i = 0; i < NXTHOP_ENTRY_NUM; i++)	
		rtl865x_nxtHopTable[i].entryIndex = i;	
...
static int32 _rtl865x_synNxtHopToAsic(rtl865x_nextHopEntry_t *entry_t)
{
	...
	if(!entry_t->valid)
		goto clear_asic_entry;	

	dstNetif = _rtl865x_getNetifByName(entry_t->dstNetif);
	if(dstNetif==NULL)
		goto clear_asic_entry;	
	...
	if (entry_t->nextHopType == IF_ETHER)
	{
		...
		rtl865x_getVlanFilterDatabaseId(dstNetif->vid,&fid);
...
static int32 _rtl865x_addNxtHop(uint32 attr, void *ref_ptr, char *if_name, uint32 nexthop,uint32 srcIp)
{
	int entryIdx;
	...
	netif = _rtl865x_getSWNetifByName(if_name);
	if(netif == NULL)
		return RTL_EINVALIDINPUT;
...
static int32 _rtl865x_delNxtHop(uint32 attr, uint32 entryIdx)
{
	...
	index_bak = entry->entryIndex;
	memset(entry,0,sizeof(rtl865x_nextHopEntry_t));
	entry->entryIndex = index_bak;
...
static int32 _rtl865x_eventHandle_swnetif_change(void *para)
{
	int32 retval = EVENT_CONTINUE_EXECUTE;
	_rtl865x_arrangeNxtHop(0,NXTHOP_ENTRY_NUM);
...
static int32 _rtl865x_nextHop_register_event(void)
{
	...
	eventParam.eventPriority=0;
	eventParam.event_action_fn=_rtl865x_eventHandle_swnetif_change;
	rtl865x_registerEvent(&eventParam);
...
int32 rtl865x_addNxtHop(uint32 attr, void *ref_ptr, char *if_name, uint32 nexthop,uint32 srcIp)
{
	...
	ret = _rtl865x_addNxtHop(attr,ref_ptr,if_name,nexthop,srcIp);	
	//rtl_up(&nxthop_sem);	
...
int32 rtl865x_getNxtHopIdx(uint32 attr, char *netif_name, uint32 nexthop,uint32 srcIp)
{
	...
	for(i = 0; i < NXTHOP_ENTRY_NUM; i++,entry++)
	{
		if(	(entry->valid) &&
			//(entry->flag == attr)&&
			!strcmp(entry->dstNetif,netif_name) &&
			(entry->nexthop== nexthop) &&
			(entry->srcIp == srcIp))
		{
			return i;
...
int32 rtl865x_delNxtHopBySessionID(uint32 attr, char *netif_name, uint32 session)
{
	...
	for(i = 0; i < NXTHOP_ENTRY_NUM; i++,entry++)
	{
		if(	(entry->valid) &&
			!strcmp(entry->dstNetif,netif_name) &&
			(entry->nexthop== session) )
		{
			_rtl865x_delNxtHop(attr, i);
			return SUCCESS;
		}		
	}
