...
{
    int32   ret,i;
    const int32 tcontDebugArraySize=16;
    rtk_qos_priSelWeight_t  priSelWeight;
    rtk_port_t  port, max_port;
    uint32  priority, dscpVal,debugQosVal;
    ...
    if ((ret = apollo_raw_qos_wfqBurstSize_set(0x3FFF)) != RT_ERR_OK)
    {
    ...
    */
	...
	for(i=0; i<tcontDebugArraySize;i++)
	{
		/*get debug bit*/
		 if((ret = reg_array_field_read(RSVD_EGR_SCHr, REG_ARRAY_INDEX_NONE, i,RSVD_MEMf,&debugQosVal))!=RT_ERR_OK)
		 {
		 	   RT_ERR(ret, (MOD_QOS|MOD_DAL), "");
		            qos_init = INIT_NOT_COMPLETED;
		            return ret;
		 ...
		 debugQosVal |= (1 << 16);
		 debugQosVal &= ~(1 << 19);
		 /*set debug bit to 0 for enabling auto-detection when CIR exceed*/
		 if((ret = reg_array_field_write(RSVD_EGR_SCHr, REG_ARRAY_INDEX_NONE, i,RSVD_MEMf,&debugQosVal))!=RT_ERR_OK)
		 {
		 	   RT_ERR(ret, (MOD_QOS|MOD_DAL), "");
		            qos_init = INIT_NOT_COMPLETED;
		            return ret;
		 }
		  if((ret = reg_array_field_read(RSVD_ALE_METERr, REG_ARRAY_INDEX_NONE, i,RSVD_MEMf,&debugQosVal))!=RT_ERR_OK)
		 {
		 	   RT_ERR(ret, (MOD_QOS|MOD_DAL), "");
		            qos_init = INIT_NOT_COMPLETED;
		            return ret;
		 ...
		 debugQosVal |= (1 << 16);
		 debugQosVal &= ~(1 << 19);
		  if((ret = reg_array_field_write(RSVD_ALE_METERr, REG_ARRAY_INDEX_NONE, i,RSVD_MEMf,&debugQosVal))!=RT_ERR_OK)
		 {
		 	   RT_ERR(ret, (MOD_QOS|MOD_DAL), "");
		            qos_init = INIT_NOT_COMPLETED;
		            return ret;
