...
int32 rtl9601b_raw_flowctrl_queueEegressDropThreshold_set(uint32 queue, uint32 threshold)
{
    ...
    /* Bad example here, this is a raw function called by user space application */
    if ((ret = drv_swcore_cid_get(&chipId, &rev)) != RT_ERR_OK)
    {
    ...
    if(rev == CHIP_REV_ID_0)
    {
        ...
        ioal_mem32_read(0x2D008 + 4 * (queue / 3), &value);
        switch(queue % 3)
        {
        case 0:
            value &= ~(0x1ff);
            value |= threshold & 0x1ff;
            break;
        case 1:
            value &= ~(0x1ff << 9);
            value |= (threshold & 0x1ff) << 9;
            break;
        case 2:
            value &= ~(0x1ff << 18);
            value |= (threshold & 0x1ff) << 18;
            break;
        default:
            break;
        }
        ioal_mem32_write(0x2D008 + 4 * (queue / 3), value);
    }
    else
    {
...
int32 rtl9601b_raw_flowctrl_queueEegressDropThreshold_get(uint32 queue, uint32 *pThreshold)
{
    ...
    if(rev == CHIP_REV_ID_0)
    {
        ...
        {
        case 0:
            *pThreshold = value & 0x1ff;
            break;
        case 1:
            *pThreshold = (value >> 9) & 0x1ff;
            break;
        case 2:
            *pThreshold = (value >> 18) & 0x1ff;
            break;
        default:
            break;
        }
...
int32 rtl9601b_raw_flowctrl_portEegressDropThreshold_set(uint32 port, uint32 threshold)
{
    ...
    if(rev == CHIP_REV_ID_0)
    {
        ...
        ioal_mem32_write(0x2D014, value);
    }
    else
    {
...
/* Function Name:
 *      rtl9601b_raw_flowctrl_ponSidState_set
 * Description:
 ...
 *      None
 */
int32 rtl9601b_raw_flowctrl_ponSidState_set(uint32 sid, rtk_enable_t state)
{
    int32   ret;
    uint32 reg;
    ...
    if ((ret = reg_array_field_write(RTL9601B_FC_PON_Q_EGR_FC_ENr, REG_ARRAY_INDEX_NONE, sid, RTL9601B_ENf, &reg)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of rtl9601b_raw_flowctrl_ponSidState_set */

/* Function Name:
 *      rtl9601b_raw_flowctrl_ponSidState_get
 * Description:
 ...
 *      None
 */
int32 rtl9601b_raw_flowctrl_ponSidState_get(uint32 sid, rtk_enable_t *pState)
{
    int32   ret;
    uint32 reg;
    ...
    if ((ret = reg_array_field_read(RTL9601B_FC_PON_Q_EGR_FC_ENr, REG_ARRAY_INDEX_NONE, sid, RTL9601B_ENf, &reg)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }
    ...
    *pState = (ENABLED == reg) ? 1 : 0;
    ...
    return RT_ERR_OK;
} /* end of rtl9601b_raw_flowctrl_ponSidState_get */
...
/* Function Name:
 *      rtl9601b_raw_flowctrl_portUsedPageCnt_get
 * Description:
 ...
 *      None
 */
int32 rtl9601b_raw_flowctrl_portUsedPageCnt_get(uint32 port, uint32 *pCurrent, uint32 *pMaximum)
{
    int32   ret;
    ...
    if ((ret = reg_array_field_read(RTL9601B_FC_P_USED_PAGE_CNTr, port, REG_ARRAY_INDEX_NONE, RTL9601B_P_USED_PAGE_CNTf, pCurrent)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_array_field_read(RTL9601B_FC_P_USED_PAGE_CNTr, port, REG_ARRAY_INDEX_NONE, RTL9601B_P_MAX_USED_PAGE_CNTf, pMaximum)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL|MOD_DAL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of rtl9601b_raw_flowctrl_portUsedPageCnt_get */
...
/* Function Name:
 *      rtl9601b_raw_flowctrl_ponUsedPageCtrl_set
 * Description:
 *      Set flow control tuning debug control
 * Input:
 *      port                   - Physical port number (0~6)
 ...
 *      None
 */
int32 rtl9601b_raw_flowctrl_ponUsedPageCtrl_set(uint32 sid, uint32 clear)
{
    int32   ret;
    uint32  data;
    uint32  retry;
    ...
    /*set sid_no*/
    if ((ret = reg_field_write(RTL9601B_PONIP_DBG_CTRLr, RTL9601B_SID_NOf, &sid)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
    }
    ...
    /*trigger clear*/
    data = 1;
    if ((ret = reg_field_write(RTL9601B_PONIP_DBG_CTRLr, RTL9601B_CLR_SID_MAX_PAGE_CNTf, &data)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
    }
    ...
    do
    {
        if ((ret = reg_field_read(RTL9601B_PONIP_DBG_CTRLr, RTL9601B_SIDCNT_ACC_BUSYf, &data)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /* end of rtl9601b_raw_flowctrl_ponUsedPageCtrl_set */

/* Function Name:
 *      rtl9601b_raw_flowctrl_ponUsedPage_get
 * Description:
 *      Set flow control tuning debug control
 * Input:
 ...
 *      None
 */
int32 rtl9601b_raw_flowctrl_ponUsedPage_get(uint32 sid, uint32 *pCurrent, uint32 *pMaximum)
{
    int32   ret;
    uint32  data;
    uint32  retry;
    ...
    /*trigger read*/
    data = 1;
    if ((ret = reg_field_write(RTL9601B_PONIP_DBG_CTRLr, RTL9601B_RD_SID_MAX_PAGE_CNTf, &data)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
    }
    ...
    /*read data*/
    if ((ret = reg_field_read(RTL9601B_PONIP_SID_USED_PAGE_CNTr, RTL9601B_USED_PAGE_CNTf, pCurrent)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
    }

    if ((ret = reg_field_read(RTL9601B_PONIP_SID_USED_PAGE_CNTr, RTL9601B_MAX_PAGE_CNTf, pMaximum)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_FLOWCTRL), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of rtl9601b_raw_flowctrl_ponUsedPage_get */
...
static int32 _patch_default(void)
{
    ...
    {
        if ((ret = rtl9601b_raw_pbo_sidThreshold_set(i, 250, 200)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
            return ret;
        }
    ...
    /* PBO stop all 8000 */
    if ((ret = rtl9601b_raw_pbo_stopAllThreshold_set(8000)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }

    /* PBO system on 6600 off 6500 */
    if ((ret = rtl9601b_raw_pbo_globalThreshold_set(6600, 6500)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }

    /* CPU port egress drop threshold 200 */
    if ((ret = rtl9601b_raw_flowctrl_portEegressDropThreshold_set(HAL_GET_CPU_PORT(), 200)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }

#if 0 /* remove switch threshold setting, use chip default value */
    /* Switch PON port system hign on 216 off 196 */
    if ((ret = rtl9601b_raw_flowctrl_ponGlobalHighThreshold_set(216, 196)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }
    ...
    if ((ret = rtl9601b_raw_flowctrl_ponGlobalLowThreshold_set(192, 184)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }
    ...
    if ((ret = rtl9601b_raw_flowctrl_ponPortHighThreshold_set(176, 168)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }
    ...
    if ((ret = rtl9601b_raw_flowctrl_ponPortLowThreshold_set(16, 12)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }
    ...
    {
        if ((ret = rtl9601b_raw_flowctrl_queueEegressDropThreshold_set(i, 36)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
            return ret;
        }
    ...
    if ((ret = rtl9601b_raw_flowctrl_queueEegressGapThreshold_set(8)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_FLOWCTRL | MOD_DAL), "");
        return ret;
    }
