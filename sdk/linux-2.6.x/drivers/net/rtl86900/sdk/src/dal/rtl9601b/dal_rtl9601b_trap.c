...
dal_rtl9601b_trap_reasonTrapToCpuPriority_get(rtk_trap_reason_type_t type, rtk_pri_t *pPriority)
{
    ...
    switch (type)
    {
        ...
        case TRAP_REASON_IPV6MLD:
            if ((ret = reg_field_read(RTL9601B_IGMP_GLB_CTRLr, RTL9601B_TRAP_PRIORITYf, &pri)) != RT_ERR_OK)
		    {
...
dal_rtl9601b_trap_reasonTrapToCpuPriority_set(rtk_trap_reason_type_t type, rtk_pri_t priority)
{
    ...
    switch (type)
    {
        ...
        case TRAP_REASON_IPV4IGMP:
        case TRAP_REASON_IPV6MLD:
            if ((ret = reg_field_write(RTL9601B_IGMP_GLB_CTRLr, RTL9601B_TRAP_PRIORITYf, &pri)) != RT_ERR_OK)
		    {
		        RT_ERR(ret, (MOD_DOT1X|MOD_DAL), "");
		        return ret;
		    }
