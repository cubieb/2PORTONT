...
static dal_rtl9602c_phy_data_t  phyPatchArray_20160122[] = {
/* Port 0 */
{ 0, 0xa43, 16, 0x019C},
/* Port 1 */
{ 0, 0xbce, 18, 0x0000},
...
dal_rtl9602c_switch_phyPatch(void)
{
    int i;
    int ret;
    dal_rtl9602c_phy_data_t *patchArray;
    uint32   patchSize;

    patchArray = phyPatchArray_20160122;
    patchSize = sizeof(phyPatchArray_20160122)/sizeof(dal_rtl9602c_phy_data_t);
...
#if 1
    value = 0xb;
    if ((ret = reg_field_write(RTL9602C_BOND_INFOr,RTL9602C_BOND_INFO_ENf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }

    if ((ret = reg_field_read(RTL9602C_BOND_INFOr,RTL9602C_BOND_CHIP_MODEf, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
...
dal_rtl9602c_switch_init(void)
{
    ...
    wData = 0x1100;
    if ((ret = reg_write(RTL9602C_CFG_PHY_POLL_CMDr,&wData)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
	...
	/* TBD remove field
    wData = 0;
    if ((ret = reg_field_write(RTL9602C_L34_GLB_CFGr, RTL9602C_CFG_CPU_ORG_OPf, (uint32 *)&wData)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
...
{
    dal_rtl9602c_switch_asic_type_t asicType;
    ...
    asicType = RTL9602C_ASIC_PON;

    switch(asicType)
    {
        case RTL9602C_ASIC_PON:
        default:
