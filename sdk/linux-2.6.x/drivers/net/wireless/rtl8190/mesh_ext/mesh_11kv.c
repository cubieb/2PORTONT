...
unsigned char *fillLinkMeasureReportMain(struct rtl8190_priv *priv, struct stat_info *pstat, unsigned char *pbuf, int *retlen)
{
	*pbuf = 1; // dialog (1)
	pbuf++;
	(*retlen)++;
...
unsigned char *fillActionHeader(struct rtl8190_priv * priv, struct stat_info * pstat, UINT8 uiCategory, UINT8 uiField, struct tx_insn *txinsn)
{
	unsigned short  seq;
	UINT8                   tmp;
...
void issue_11kv_LinkMeasureReq(struct rtl8190_priv * priv, struct stat_info * pstat, UINT16 lenTst, UINT16 cntTst, UINT8 prioTst)
{
	UINT8	tmp;
	UINT16          val;
	unsigned char	*pbuf;
...
void issue_11kv_LinkMeasureRepAck(struct rtl8190_priv * priv, struct stat_info * pstat)
{
	unsigned char           *pbuf;
...
void issue_11kv_LinkMeasureRepRep(struct rtl8190_priv * priv, struct stat_info * pstat)
{
	unsigned char	*pbuf;
	UINT16		val;
...
unsigned int On11kvLinkMeasureReq(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
...
void issue_11kv_TestTraffic(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	UINT8 i;
	UINT8 t;
	UINT16 len_t;
...
unsigned int On11kvLinkMeasureRepAck(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, unsigned char *pFrameBody, unsigned char *pSubElement)
{
...
unsigned int On11kvLinkMeasureRepRep(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, unsigned char *pFrameBody, unsigned char *pSubElement)
{
	UINT16 len, cnt;
...
unsigned int On11kvTestTraffic(struct rtl8190_priv * priv, struct rx_frinfo *pfrinfo, unsigned char *pFrameBody, unsigned char *pSubElement)
{
...
unsigned int On11kvLinkMeasureRep(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	unsigned char  *pframe, *pFrameBody;
...
UINT32 computeMetric(struct rtl8190_priv * priv, struct stat_info * pstat, UINT8 rate, UINT8 priority,
...
UINT32 getMetric(struct rtl8190_priv * priv, struct stat_info * pstat)
{
	return (pstat->mesh_neighbor_TBL.metric == 0)? MESH_METRIC_UNSPECIFIED : pstat->mesh_neighbor_TBL.metric;
}


void metric_update(struct rtl8190_priv *priv)
{
	struct stat_info        *pstat;
