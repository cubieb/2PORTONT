...
unsigned short getMeshSeq(struct rtl8190_priv *priv)
{
	unsigned long flags;
...
//pepsi
#ifdef PU_STANDARD
UINT8 getPUSeq(struct rtl8190_priv *priv)
{
	unsigned long flags;
	SAVE_INT_AND_CLI(flags);
...
// return 0: duplicate
// return 1: ok
unsigned short chkMeshSeq(struct rtl8190_priv *priv, unsigned char *srcMac, unsigned short seq)
{
...
void insert_PREQ_entry(char *targetMac, struct rtl8190_priv *priv)
{
...
void GEN_PREQ_PACKET(char *targetMac, struct rtl8190_priv *priv, char insert)
{
...
unsigned char* getMeshHeader(struct rtl8190_priv *priv, int wep_mode, unsigned char* pframe)
{
	INT		payload_offset;
 ...
 */
 // chuangch 10.19
 void route_maintenance(struct rtl8190_priv *priv)
 { 
...
// Gallardo test 2008.0901			
 ...
 void aodv_expire(struct rtl8190_priv *priv)
 {
	 struct sk_buff *pskb;
	 struct mesh_rreq_retry_entry *retryEntry;
...
struct path_sel_entry *pathsel_query_table(struct rtl8190_priv *priv ,unsigned char destaddr[MACADDRLEN] )
{
...
//int pathsel_modify_table_entry( struct rtl8190_priv *priv, unsigned char destaddr[MACADDRLEN], struct path_sel_entry *pEntry)
int pathsel_modify_table_entry(struct rtl8190_priv *priv, struct path_sel_entry *pEntry)
{
	int i, tbl_sz;
...
//			entry->routeMaintain = xtime; 
 ...
 int pathsel_table_entry_insert_tail( struct rtl8190_priv *priv, struct path_sel_entry *pEntry)
{
...
int remove_path_entry(struct rtl8190_priv *priv,unsigned char *invalid_addr)
{
