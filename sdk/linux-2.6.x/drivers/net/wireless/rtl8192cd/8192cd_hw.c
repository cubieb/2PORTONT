...
#endif
		{
		    panic_printk("[%s][TXPWR_LMT_92EE_new]\n", __FUNCTION__);
...
void SwChnl(struct rtl8192cd_priv *priv, unsigned char channel, int offset)
{
	...
	if(priv->pmib->dot11StationConfigEntry.sc_enabled == 1 && priv->pmib->dot11StationConfigEntry.sc_fix_channel)
	{
		channel = priv->pmib->dot11StationConfigEntry.sc_fix_channel;
	}
#ifdef UNIVERSAL_REPEATER
	if(IS_ROOT_INTERFACE(priv) && 
		IS_DRV_OPEN(GET_VXD_PRIV(priv))&& 
		GET_VXD_PRIV(priv)->pmib->dot11StationConfigEntry.sc_enabled && 
		(GET_VXD_PRIV(priv)->pmib->dot11StationConfigEntry.sc_fix_channel)
	)
		{
		channel = priv->pmib->dot11StationConfigEntry.sc_fix_channel;
		//return;
...
//			ODM_RF_CALIBRATION		|
    ...
    if(priv->pmib->dot11RFEntry.rfe_type)
...
#endif
				{
					//4 int_pa OFDM base index -3 dB for 5G
					if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G) {
						PHY_SetBBReg(priv, rA_TxScale_Jaguar, 0xffe00000, TxScalingTable_Jaguar[18]);
						PHY_SetBBReg(priv, rB_TxScale_Jaguar, 0xffe00000, TxScalingTable_Jaguar[18]);
						PHY_SetBBReg(priv, rC_TxScale_Jaguar2, 0xffe00000, TxScalingTable_Jaguar[18]);
						PHY_SetBBReg(priv, rD_TxScale_Jaguar2, 0xffe00000, TxScalingTable_Jaguar[18]);
						priv->pshare->OFDM_index0[0] = 18;
						priv->pshare->OFDM_index0[1] = 18;
						priv->pshare->OFDM_index0[2] = 18;
						priv->pshare->OFDM_index0[3] = 18;
...
#ifdef USE_OUT_SRC
		if (IS_OUTSRC_CHIP(priv))
			set_CCK_swing_index(priv, ODMPTR->DefaultCckIndex);
#endif
	}
#endif	
    if (get_rf_mimo_mode(priv) == MIMO_1T1R){
...
#ifdef CONFIG_RTL_88E_SUPPORT && (defined(__KERNEL__) && (defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)))
	if (GET_CHIP_VER(priv) == VERSION_8188E) {
...
int ReadAdapterInfo8192CE(struct rtl8192cd_priv *priv)
{
	...
	ReadTxPowerInfoFromHWPG(priv);
#if defined(CONFIG_WLAN_HAN_8814AE) //1 Not to load thermal temporarily
		return 0;
#endif
