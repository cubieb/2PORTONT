...
#ifdef CONFIG_WLAN_HAL
                 || (total_num >= (WLAN_HAL_TX_AMSDU_NUM-1))
#endif
        ) {
...
int rtl88XX_signin_txdesc(struct rtl8192cd_priv *priv, struct tx_insn* txcfg, unsigned char convHdr)
{
	...
	for (i=0; i < txcfg->frg_num; i++)
	{
		...
		if(txcfg->pstat)
		{
			ODM_SetTxAntByTxInfo_HAL(priv, (PVOID)&desc_data, txcfg, txcfg->pstat->aid);   //HAL Set TX antenna	
		}
...
__inline__ static int rtl8192cd_swq_enqueue(struct rtl8192cd_priv *priv, struct sk_buff *skb, struct stat_info	*pstat)
{
    ...
    if(priv->pshare->swq_use_hw_timer) {
        ...
        if(pstat->swq.swq_en[q_num]) {
            if((pstat->tx_avarage < 625000 && !rtl8192cd_swq_bdfull(priv, pstat, q_num)) || 20000000 < pstat->tx_avarage) { /* 5M < TP  or 160M < TP*/
            ...
            if(687500 < pstat->tx_avarage && pstat->tx_avarage < 19375000) { /* 5.5M < TP < 155M*/
	...
	if (!da)
	{
		...
		if (GetPrivacy(txcfg->phdr))
		{
			...
			if(txcfg->isPMF) {
				...
				if(txcfg->isPMF) printk("check_txdesc PATH37, privacy=%d\n",txcfg->privacy);
				txcfg->iv = 8;
				txcfg->icv = 0;
				txcfg->mic = 8;
			} else			
