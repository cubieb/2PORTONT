	...
	)
{
	...
	if(pDM_Odm->bLinked && bPerformance)
	{
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("FA %d wifi_rx_pkts_2 %u wifi_rx_pkts_1 %u \n", pFalseAlmCnt->Cnt_all, priv->pshare->wifi_rx_pkts[2], priv->pshare->wifi_rx_pkts[1]));
		if (pFalseAlmCnt->Cnt_all <= 30 && (UINT32_DIFF(priv->pshare->wifi_rx_pkts[2], priv->pshare->wifi_rx_pkts[1]) < 30)) {
		    pDM_DigTable->rx_gain_range_max = 0x1c;
		}else 
#endif
