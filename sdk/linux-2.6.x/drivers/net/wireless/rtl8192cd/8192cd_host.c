...
struct pcie_para ePHY[][29] = {
	...
	{0xff,0xff,0xffff}}, //8685S 25M clk	
     {  {0, 3, 0x3031}, {0, 6, 0xe0b8}, {0, 0xf, 0x400f}, {0, 0x19, 0xfc70},
	{0xff,0xff,0xffff}}, //9602C 25M clk
     {  {0, 3, 0x7b31}, {0, 6, 0xe2b8}, {0, 0xf, 0x400f}, {0, 0x19, 0xfc70},
...
int PCIE_reset_procedure(int portnum, int Use_External_PCIE_CLK, int mdio_reset, unsigned long conf_addr)
{
	...
	if (mdio_reset)
	{
		...
		else if(IS_RTL9602C && (REG32(MISC_PINSR) & CLKSEL)){
			printk("RTL9602C PCIe Port 0 40MHz \n");
                        idx = CLK40_9602C;
		}
		else if(IS_RTL9602C && !(REG32(MISC_PINSR) & CLKSEL)) {
			printk("RTL9602C PCIe Port 0 25MHz \n");
			idx = CLK25_9602C;
		}
		else
			idx = NOT_DEFINED_CLK;
