...
int should_forbid_Nmode(struct rtl8192cd_priv *priv)
{
	...
	if (priv->pmib->dot11nConfigEntry.dot11nLgyEncRstrct & BIT(1))	{
                if (priv->pmib->dot1180211AuthEntry.dot11EnablePSK == 0
                        ...
                        if (priv->pmib->dot1180211AuthEntry.dot11WPACipher == 0	// WPA2 TKIP Only
                                && priv->pmib->dot1180211AuthEntry.dot11WPA2Cipher == 2)
                                return 1;
						else if (priv->pmib->dot1180211AuthEntry.dot11WPACipher == 2	// WPA TKIP Only
...
u8* rtk_get_ie_with_oui(const u8 *in_ie, int in_len, u8 ie_to_search, u8* oui_input ,int oui_len, int *report_ielen)
{
	int cnt=0;
	u8 *tar_ie_ptr=NULL;
	u8 eid=0;
