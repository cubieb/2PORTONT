...
)
{
	...
	char *q_name[] = {"MGQ", "BKQ", "BEQ", "VIQ", "VOQ",
	                  "HI0Q", "HI1Q", "HI2Q", "HI3Q",
	                  "HI4Q", "HI5Q", "HI6Q", "HI7Q", "BCNQ"};

    u4Byte                      beacon_offset=0;
	...
	printk("q_num=%d, hw_idx=%d, host_idx=%d\n",
		q_num, ptx_dma->tx_queue[q_num].hw_idx,
		ptx_dma->tx_queue[q_num].host_idx);

	printk("total_txbd_num=%d, avail_txbd_num=%d, reg_rwptr_idx=%x\n",
		ptx_dma->tx_queue[q_num].total_txbd_num,
		ptx_dma->tx_queue[q_num].avail_txbd_num,
		ptx_dma->tx_queue[q_num].reg_rwptr_idx);
...
#endif
	{
		...
		if (q_num != HCI_TX_DMA_QUEUE_BCN) {
			for (i=0;i<pTXBD_NUM[q_num];i++ ){
				...
				printk("  Desc: %08x %08x %08x %08x  %08x %08x %08x %08x  %08x %08x (%08x)\n",
					GET_DESC(ptx_desc_head[i].Dword0), GET_DESC(ptx_desc_head[i].Dword1),
					GET_DESC(ptx_desc_head[i].Dword2), GET_DESC(ptx_desc_head[i].Dword3),
					GET_DESC(ptx_desc_head[i].Dword4), GET_DESC(ptx_desc_head[i].Dword5),
					GET_DESC(ptx_desc_head[i].Dword6), GET_DESC(ptx_desc_head[i].Dword7),
					GET_DESC(ptx_desc_head[i].Dword8), GET_DESC(ptx_desc_head[i].Dword9), &ptx_desc_head[i]);
			}
		}else{
			for (i = 0; i < 1+HAL_NUM_VWLAN; i++)
			{
				ptxbd = (pu1Byte)ptxbd + beacon_offset * i;
				printk("BCN [%d] ptxbd addr=%08x/%08x, ",
	                    i,
	                    (u4Byte)&ptxbd,
	                    (u4Byte)HAL_VIRT_TO_BUS1(Adapter, (PVOID)&ptxbd[i],sizeof(TX_BUFFER_DESCRIPTOR), HAL_PCI_DMA_TODEVICE));
				printk("ptx_desc_head[%d] addr=%08x/%08x\n",
...
)
{
	...
	switch (queueIndex) {
    	case HCI_TX_DMA_QUEUE_HI0:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
    	...
    	case HCI_TX_DMA_QUEUE_HI1:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    	case HCI_TX_DMA_QUEUE_HI2:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 2, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    	case HCI_TX_DMA_QUEUE_HI3:	
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 3, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    	case HCI_TX_DMA_QUEUE_HI4:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 4, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    	case HCI_TX_DMA_QUEUE_HI5:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 5, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    	case HCI_TX_DMA_QUEUE_HI6:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 6, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    	case HCI_TX_DMA_QUEUE_HI7:
            SET_DESC_FIELD_NO_CLR(val, 1, TX_DW1_MOREDATA_MSK, TX_DW1_MOREDATA_SH);
            SET_DESC_FIELD_NO_CLR(val, 7, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
            q_select = TXDESC_QSEL_HIGH;
    		break;
    ...
    SET_DESC_FIELD_NO_CLR(val, q_select, TX_DW1_QSEL_MSK, TX_DW1_QSEL_SH);
    ptx_desc->Dword1 |= val;
}
...
//    u1Byte  TX_DESC_PKT_OFFSET       = 0; // early mode ?
    ...
    //4 Set Dword0
    val = 0;
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_TXPKTSIZE, TX_DW0_TXPKSIZE_MSK, TX_DW0_TXPKSIZE_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_OFFSET, TX_DW0_OFFSET_MSK, TX_DW0_OFFSET_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_BMC, TX_DW0_BMC_MSK, TX_DW0_BMC_SH);
    ptx_desc->Dword0 |= val;
    ...
    if ( (queueIndex >= HCI_TX_DMA_QUEUE_HI0) && (queueIndex <= HCI_TX_DMA_QUEUE_HI7) ) {
        ...
        SET_DESC_FIELD_NO_CLR(val, TX_DESC_MACID, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
    }
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_RATE_ID, TX_DW1_RATE_ID_MSK, TX_DW1_RATE_ID_SH);
    ptx_desc->Dword1 |= val;

    //4 Set Dword2
    val = 0;
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_AGG_EN, TX_DW2_AGG_EN_MSK, TX_DW2_AGG_EN_SH);    
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_BK, TX_DW2_BK_MSK, TX_DW2_BK_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_MOREFRAG, TX_DW2_MOREFRAG_MSK, TX_DW2_MOREFRAG_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_AMPDU_DENSITY, TX_DW2_AMPDU_DENSITY_MSK, TX_DW2_AMPDU_DENSITY_SH);    
	ptx_desc->Dword2 |= val;

    //4 Set Dword3
    val = 0;
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_USERATE, TX_DW3_USERATE_MSK, TX_DW3_USERATE_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DISRTSFB, TX_DW3_DISRTSFB_MSK, TX_DW3_DISRTSFB_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DISDATAFB, TX_DW3_DISDATAFB_MSK, TX_DW3_DISDATAFB_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_CTS2SELF, TX_DW3_CTS2SELF_MSK, TX_DW3_CTS2SELF_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_RTS_EN, TX_DW3_RTSEN_MSK, TX_DW3_RTSEN_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_HW_RTS_EN, TX_DW3_HW_RTS_EN_MSK, TX_DW3_HW_RTS_EN_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_NAVUSEHDR, TX_DW3_NAVUSEHDR_MSK, TX_DW3_NAVUSEHDR_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_MAX_AGG_NUM, TX_DW3_MAX_AGG_NUM_MSK, TX_DW3_MAX_AGG_NUM_SH);
    ptx_desc->Dword3 |= val;

    //4 Set Dword4
    val = 0;
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATERATE, TX_DW4_DATARATE_MSK, TX_DW4_DATARATE_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATA_RATEFB_LMT, TX_DW4_DATA_RATEFB_LMT_MSK, TX_DW4_DATA_RATEFB_LMT_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_RTY_LMT_EN, TX_DW4_RTY_LMT_EN_MSK, TX_DW4_RTY_LMT_EN_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATA_RT_LMT, TX_DW4_DATA_RT_LMT_MSK, TX_DW4_DATA_RT_LMT_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_RTSRATE, TX_DW4_RTSRATE_MSK, TX_DW4_RTSRATE_SH);
    ptx_desc->Dword4 |= val;

    //4 Set Dword5
    val = 0;
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATA_SC, TX_DW5_DATA_SC_MSK, TX_DW5_DATA_SC_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATA_SHORT, TX_DW5_DATA_SHORT_MSK, TX_DW5_DATA_SHORT_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATA_BW, TX_DW5_DATA_BW_MSK, TX_DW5_DATA_BW_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_DATA_STBC, TX_DW5_DATA_STBC_MSK, TX_DW5_DATA_STBC_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_RTS_SHORT, TX_DW5_RTS_SHORT_MSK, TX_DW5_RTS_SHORT_SH);
    SET_DESC_FIELD_NO_CLR(val, TX_DESC_RTS_SC, TX_DW5_RTS_SC_MSK, TX_DW5_RTS_SC_SH);
	SET_DESC_FIELD_NO_CLR(val, TX_DESC_POWER_OFFSET, TX_DW5_TXPWR_OFSET_MSK, TX_DW5_TXPWR_OFSET_SH);
    ptx_desc->Dword5 |= val;
    ...
    switch(CtrlFlag) {
        ...
        case TxPktFinalIO88XX_CHECK:
            if (0 == GET_DESC_FIELD(cur_txbd->TXBD_ELE[0].Dword0, 
                        TXBD_DW0_PSLEN_MSK, TXBD_DW0_PSLEN_SH)) {
                RT_TRACE(COMP_SEND, DBG_SERIOUS, 
                    ("cur_txbd.TXBD_ELE[0].Dword0 address(0x%lx) Error\n", cur_txbd->TXBD_ELE[0].Dword0));
...
)
{
    ...
    if (RT_STATUS_FAILURE == TxPktFinalIO88XX(Adapter, (cur_q->pTXBD_head + LastHostIdx), TxPktFinalIO88XX_CHECK, NULL)) {
...
)
{
	...
	TXBD_Dword1[1] = HAL_VIRT_TO_BUS1(Adapter, (PVOID)pdesc_data->pHdr, hdrLen, HAL_PCI_DMA_TODEVICE);
    ...
    if (payloadLen) {
		TXBD_Dword1[2] = HAL_VIRT_TO_BUS1(Adapter, (PVOID)pdesc_data->pBuf, payloadLen, HAL_PCI_DMA_TODEVICE);
    ...
    if (pdesc_data->pIcv != NULL) {
		TotalLen += pdesc_data->icv;
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[3].Dword0,
                    pdesc_data->icv,
                    TXBD_DW0_TXBUFSIZE_MSK, TXBD_DW0_TXBUFSIZE_SH);
        
		TXBD_Dword1[3] = HAL_VIRT_TO_BUS1(Adapter, (PVOID)pdesc_data->pIcv, pdesc_data->icv, HAL_PCI_DMA_TODEVICE);
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[3].Dword1,
                    TXBD_Dword1[3],
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);

    } else if (pdesc_data->pMic != NULL) {
		TotalLen += pdesc_data->mic;
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[3].Dword0,
                    pdesc_data->mic,
                    TXBD_DW0_TXBUFSIZE_MSK, TXBD_DW0_TXBUFSIZE_SH);
        
		TXBD_Dword1[3] = HAL_VIRT_TO_BUS1(Adapter, (PVOID)pdesc_data->pMic, pdesc_data->mic, HAL_PCI_DMA_TODEVICE);
        SET_DESC_FIELD_NO_CLR(cur_txbd->TXBD_ELE[3].Dword1,
                    TXBD_Dword1[3],
                    TXBD_DW1_PHYADDR_LOW_MSK, TXBD_DW1_PHYADDR_LOW_SH);
    } else {
...
//    }
...
#if IS_EXIST_RTL8192EE
    if ( IS_HARDWARE_TYPE_8192EE(Adapter) ) {
        PSBLen   = (TotalLen+PBP_PSTX_SIZE-1) / PBP_PSTX_SIZE;
    }
#endif
#if IS_EXIST_RTL8881AEM
    if ( IS_HARDWARE_TYPE_8881AEM(Adapter) ) {
    ...
    if ((pdesc_data->pIcv != NULL) || (pdesc_data->pMic != NULL)) {
        HAL_CACHE_SYNC_WBACK(Adapter, 
            TXBD_Dword1[3], 
            pdesc_data->icv, 
            HAL_PCI_DMA_TODEVICE);
...
)
{
    ...
    //Dword0
    val = 0;
    SET_DESC_FIELD_NO_CLR(val, 1, TX_DW0_BMC_MSK, TX_DW0_BMC_SH);
    SET_DESC_FIELD_NO_CLR(val, HAL_TXDESC_OFFSET_SIZE, TX_DW0_OFFSET_MSK, TX_DW0_OFFSET_SH);
    SET_DESC_FIELD_NO_CLR(val, txLength, TX_DW0_TXPKSIZE_MSK, TX_DW0_TXPKSIZE_SH);
    pdesc->Dword0 |= val;

    //Dword1
    val = 0;
    SET_DESC_FIELD_NO_CLR(pdesc->Dword1, TXDESC_QSEL_BCN, TX_DW1_QSEL_MSK, TX_DW1_QSEL_SH);
#if CFG_HAL_SUPPORT_MBSSID
        if (HAL_IS_VAP_INTERFACE(Adapter)) {
        	// set MBSSID for each VAP_ID
	        SET_DESC_FIELD_NO_CLR(pdesc->Dword1, HAL_VAR_VAP_INIT_SEQ, TX_DW1_MACID_MSK, TX_DW1_MACID_SH);
        }         
#endif //#if CFG_HAL_SUPPORT_MBSSID
	pdesc->Dword1 |= val;
    ...
    //Dword3
    val = 0;
    SET_DESC_FIELD_NO_CLR(pdesc->Dword3, 1, TX_DW3_DISDATAFB_MSK, TX_DW3_DISDATAFB_SH);
    SET_DESC_FIELD_NO_CLR(pdesc->Dword3, 1, TX_DW3_USERATE_MSK, TX_DW3_USERATE_SH);
	pdesc->Dword3 |= val;

    //Dword5
    if (HAL_VAR_IS_40M_BW) {
        if (HAL_VAR_OFFSET_2ND_CHANNEL == HT_2NDCH_OFFSET_BELOW) {
            SET_DESC_FIELD_NO_CLR(pdesc->Dword5, TXDESC_DATASC_LOWER, TX_DW5_DATA_SC_MSK, TX_DW5_DATA_SC_SH);
        }
		else {
            SET_DESC_FIELD_NO_CLR(pdesc->Dword5, TXDESC_DATASC_UPPER, TX_DW5_DATA_SC_MSK, TX_DW5_DATA_SC_SH);
		}
	}
    ...
    //Dword6
        ...
        if (HAL_IS_VAP_INTERFACE(Adapter)) {
        	// set MBSSID for each VAP_ID
	        SET_DESC_FIELD_NO_CLR(pdesc->Dword6, HAL_VAR_VAP_INIT_SEQ, TX_DW6_MBSSID_MSK, TX_DW6_MBSSID_SH);  
        }         
#endif //#if CFG_HAL_SUPPORT_MBSSID
    ...
    //Dword7
    SET_DESC_FIELD_NO_CLR(pdesc->Dword7, txLength, TX_DW7_SW_TXBUFF_MSK, TX_DW7_SW_TXBUFF_SH);
    ...
    //Dword9
	val = 0;
    SET_DESC_FIELD_NO_CLR(pdesc->Dword9, GetSequence(dat_content), TX_DW9_SEQ_MSK, TX_DW9_SEQ_SH);
    // Group Bit Control
    SET_DESC_FIELD_NO_CLR(pdesc->Dword9, (HAL_VAR_TIM_OFFSET-24), TX_DW9_GROUPBIT_IE_OFFSET_MSK, TX_DW9_GROUPBIT_IE_OFFSET_SH);
    SET_DESC_FIELD_NO_CLR(pdesc->Dword9, 1, TX_DW9_GROUPBIT_IE_ENABLE_MSK, TX_DW9_GROUPBIT_IE_ENABLE_SH);
	pdesc->Dword9 |= val;
