...
)
{
	...
	printk("q_num:%d, hw_idx=%d, host_idx=%d, cur_host_idx=%d, rxtag_seq_num=%d\n",
		q_num,
		prx_dma->rx_queue[q_num].hw_idx,
		prx_dma->rx_queue[q_num].host_idx, 
		prx_dma->rx_queue[q_num].cur_host_idx,
		prx_dma->rx_queue[q_num].rxtag_seq_num);
#else
	...
	printk("total_rxbd_num=%d, avail_rxbd_num=%d, reg_rwptr_idx=%x\n",
		prx_dma->rx_queue[q_num].total_rxbd_num,
		prx_dma->rx_queue[q_num].avail_rxbd_num,
		prx_dma->rx_queue[q_num].reg_rwptr_idx);
...
#endif
	{
		printk("pRXBD_head=%08x/%08x, reg(%x)=%08x\n",
			...
			REG_RXQ_RXBD_DESA,
			HAL_RTL_R32(REG_RXQ_RXBD_DESA));

		for (i=0; i<RX_Q_RXBD_NUM; i++)
		{
			printk("pRXBD_head[%d] addr=%08x/%08x: Dword0=%08x, Dword1=%08x\n", 
				i,
				(u4Byte)&prx_dma->rx_queue[q_num].pRXBD_head[i],
...
)
{
    ...
    PlatformZeroMemory(desc_dma_buf, DESC_DMA_PAGE_SIZE_MAX);
        ...
        printk("%s(%d):pdesc_dma_buf:%08x, desc_dma_buf:%08x \n", __FUNCTION__,__LINE__, pdesc_dma_buf, desc_dma_buf);
    prx_dma = (PHCI_RX_DMA_MANAGER_88XX)(_GET_HAL_DATA(Adapter)->PRxDMA88XX);
...
#ifdef __MIPSEB__
	pdesc_dma_buf = (unsigned char *)KSEG1ADDR(pdesc_dma_buf);
#endif
	printk("desc_dma_buf @ 0x%x\n", pdesc_dma_buf);
