				...
				)
{
	...
	if (bcnInt) {
		...
		if (
			...
			(GET_CHIP_VER(priv)==VERSION_8192E)?(status & HIMR_92E_BcnInt):
#endif
			(status & HIMR_BCNDMA0)) {
#ifdef UNIVERSAL_REPEATER
		if ((OPMODE & WIFI_STATION_STATE) && GET_VXD_PRIV(priv) &&
...
#endif
#ifdef CONFIG_RTL_92E_SUPPORT
						(GET_CHIP_VER(priv)==VERSION_8192E)?(status_ext & (HIMRE_92E_BCNDMAINT1 <<
						(priv->pvap_priv[i]->vap_init_seq-1))):
#endif
...
#ifndef NOT_RTK_BSP
		// for non-RTK platforms, those may use share interrupt. interrupt signal may from other device, so don't print this warring message
...
#endif
#if defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE)
	if (GET_CHIP_VER(priv)==VERSION_8192E) {
...
retry_process:
	...
	if (
		...
		(GET_CHIP_VER(priv)==VERSION_8192E) ?
	 	((CIRC_CNT_RTK(phw->txbufhead0, phw->txbuftail0, CURRENT_NUM_TX_DESC) > 10) ||
		(CIRC_CNT_RTK(phw->txbufhead1, phw->txbuftail1, CURRENT_NUM_TX_DESC) > 10) ||
		(CIRC_CNT_RTK(phw->txbufhead2, phw->txbuftail2, CURRENT_NUM_TX_DESC) > 10) ||
		(CIRC_CNT_RTK(phw->txbufhead3, phw->txbuftail3, CURRENT_NUM_TX_DESC) > 10) ||
		(CIRC_CNT_RTK(phw->txbufhead4, phw->txbuftail4, CURRENT_NUM_TX_DESC) > 10) ||
		(CIRC_CNT_RTK(phw->txbufhead5, phw->txbuftail5, CURRENT_NUM_TX_DESC) > 10)
#ifdef MBSSID
		|| (CIRC_CNT_RTK(phw->txbufhead6, phw->txbuftail6, CURRENT_NUM_TX_DESC) > 10)
		|| (CIRC_CNT_RTK(phw->txbufhead7, phw->txbuftail7, CURRENT_NUM_TX_DESC) > 10)
		|| (CIRC_CNT_RTK(phw->txbufhead8, phw->txbuftail8, CURRENT_NUM_TX_DESC) > 10)
		|| (CIRC_CNT_RTK(phw->txbufhead9, phw->txbuftail9, CURRENT_NUM_TX_DESC) > 10)
		|| (CIRC_CNT_RTK(phw->txbufhead10, phw->txbuftail10, CURRENT_NUM_TX_DESC) > 10)
		|| (CIRC_CNT_RTK(phw->txbufhead11, phw->txbuftail11, CURRENT_NUM_TX_DESC) > 10)
		|| (CIRC_CNT_RTK(phw->txbufhead12, phw->txbuftail12, CURRENT_NUM_TX_DESC) > 10)
#endif	
		):	
#endif	
...
static int rtl8192cd_init_sw(struct rtl8192cd_priv *priv)
{
...
#endif
	{
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			phw->tx_ring0_addr = phw->ring_dma_addr + NUM_RX_DESC * sizeof(struct rx_bufdesc);
		} else
#endif
		{
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			phw->tx_bufring0_addr = phw->txcmd_ring_addr + NUM_CMD_DESC * sizeof(struct tx_desc);
			phw->tx_bufring1_addr = phw->tx_bufring0_addr + NUM_TX_DESC * sizeof(struct tx_bufdesc);
			phw->tx_bufring2_addr = phw->tx_bufring1_addr + NUM_TX_DESC * sizeof(struct tx_bufdesc);
			phw->tx_bufring3_addr = phw->tx_bufring2_addr + NUM_TX_DESC * sizeof(struct tx_bufdesc);
			phw->tx_bufring4_addr = phw->tx_bufring3_addr + NUM_TX_DESC * sizeof(struct tx_bufdesc);
			phw->tx_bufring5_addr = phw->tx_bufring4_addr + NUM_TX_DESC * sizeof(struct tx_bufdesc);
			phw->tx_bufringB_addr = phw->tx_bufring5_addr + NUM_TX_DESC * sizeof(struct tx_bufdesc);
			phw->tx_ringB_addr = phw->tx_bufringB_addr + 16 * sizeof(struct tx_bufdesc);
		} else
#endif
		{
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			phw->rx_bufdescL = phw->rx_descL;
			phw->tx_desc0 = (struct rx_bufdesc *)((unsigned long)phw->rx_bufdescL + NUM_RX_DESC * sizeof(struct rx_bufdesc));
		} else 
#endif
		{
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			phw->txbuf_desc0 = ((unsigned long)phw->txcmd_desc + NUM_CMD_DESC * sizeof(struct tx_desc));
			phw->txbuf_desc1 = (struct tx_bufdesc *)((unsigned long)phw->txbuf_desc0 + NUM_TX_DESC * sizeof(struct tx_bufdesc));
			phw->txbuf_desc2 = (struct tx_bufdesc *)((unsigned long)phw->txbuf_desc1 + NUM_TX_DESC * sizeof(struct tx_bufdesc));
			phw->txbuf_desc3 = (struct tx_bufdesc *)((unsigned long)phw->txbuf_desc2 + NUM_TX_DESC * sizeof(struct tx_bufdesc));
			phw->txbuf_desc4 = (struct tx_bufdesc *)((unsigned long)phw->txbuf_desc3 + NUM_TX_DESC * sizeof(struct tx_bufdesc));
			phw->txbuf_desc5 = (struct tx_bufdesc *)((unsigned long)phw->txbuf_desc4 + NUM_TX_DESC * sizeof(struct tx_bufdesc));
			phw->txbuf_descB = (struct tx_bufdesc *)((unsigned long)phw->txbuf_desc5 + NUM_TX_DESC * sizeof(struct tx_bufdesc));
			phw->tx_descB = (struct tx_desc *)((unsigned long)phw->txbuf_descB + 16 * sizeof(struct tx_bufdesc));
		} else
#endif		
		{
		...
		/* To set the DMA address for both RX/TX ring */
		{
			...
			if (GET_CHIP_VER(priv) == VERSION_8192E)
			for (i=0; i<NUM_RX_DESC; i++) {
				phw->rx_bufdescL_dma_addr[i] = get_physical_addr(priv, (void *)(&phw->rx_bufdescL[i]),
					sizeof(struct rx_bufdesc), PCI_DMA_TODEVICE);
			}
#endif			
#endif
...
#if defined(CONFIG_WLAN_HAL)
				if (max_qnum == HIGH_QUEUE7 && tx_info[i].buf_pframe && (tx_info[i].type == _SKB_FRAME_TYPE_)) { // should be buf_paddr
...
#if defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) //eric_8812 ??
		if(GET_CHIP_VER(priv)==VERSION_8812E || GET_CHIP_VER(priv)==VERSION_8812E)
...
#if defined(CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE)
		{
...
#if defined (CONFIG_RTL_92D_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE)
#ifdef CONFIG_RTL_92D_SUPPORT
				if (vendor_deivce_id == ((unsigned long)((0x8193<<16)|PCI_VENDOR_ID_REALTEK)))
					priv->pshare->version_id = VERSION_8192D;
				else
...
#if defined(CONFIG_RTL_88E_SUPPORT) || defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE)
