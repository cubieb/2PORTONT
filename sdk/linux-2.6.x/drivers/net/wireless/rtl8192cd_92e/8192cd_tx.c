...
//					is_MCS_rate(txcfg->tx_rate) && ((txcfg->tx_rate & 0x7f)>7))
	...
	if (
#ifdef CONFIG_RTL_92E_SUPPORT	
		(GET_CHIP_VER(priv) == VERSION_8192E) ? (get_desc(pdesc->Dword3 ) & TXdesc_92E_RtsEn) :
#endif	
		(get_desc(pdesc->Dword4) & (TX_RtsEn | TX_CTS2Self)) 
	){
		if (erp_protection)
			rate = (unsigned char)find_rate(priv, NULL, 1, 3);
		else
...
#ifdef CONFIG_WLAN_HAL
        // TODO:... dma_txhead.... and rtl_cache_sync_wback
#else
		pfrst_dma_desc = dma_txhead[*tx_head];

		if (i != 0) {
#ifndef CONFIG_RTL_92E_SUPPORT			
...
{
#ifdef CONFIG_RTL_92E_SUPPORT
	struct tx_bufdesc		*phbufdesc, *pfrstbufdesc;
#endif	
...
#ifdef CONFIG_RTL_92E_SUPPORT	
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		dma_txhead	= get_txbufdma_addr(phw, q_num);
		tx_head		= get_txbufhead_addr(phw, q_num);
		phbufdesc	= get_txbufdesc(phw, q_num);
...
#ifdef CONFIG_RTL_92E_SUPPORT	
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			pfrstbufdesc= phbufdesc + (*tx_head);	
			memset(pdesc, 0, 40);
		} else
#endif
		{
...
#ifdef CONFIG_RTL_92E_SUPPORT	
			if (GET_CHIP_VER(priv) == VERSION_8192E) {
				pfrstbufdesc->Dword0 = set_desc((sizeof(struct tx_desc)) & TX_TxBufSizeMask);			
				pfrstbufdesc->Dword0 |= set_desc((((get_desc(pdesc->Dword0) & TX_TxBufSizeMask) + sizeof(struct tx_desc)+256)/256) << 16);
				pfrstbufdesc->Dword1 = set_desc(virt_to_bus(pdesc)); 
				pfrstbufdesc->Dword4 = set_desc(get_desc(pdesc->Dword7) & TX_TxBufSizeMask);
			} else
#endif
			{
...
#ifdef CONFIG_RTL_92E_SUPPORT	
			if (GET_CHIP_VER(priv) == VERSION_8192E) {
				pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
				pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			}
#endif
			q_select = 0x11;// High Queue
			break;
#if defined(MBSSID) && 	defined(CONFIG_RTL_92E_SUPPORT)
		case HIGH_QUEUE1:
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(1 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue1
			break;
		case HIGH_QUEUE2:
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(2 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue2
			break;
		case HIGH_QUEUE3:	
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(3 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue3
			break;
		case HIGH_QUEUE4:
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(4 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue4
			break;
		case HIGH_QUEUE5:
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(5 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue5
			break;
		case HIGH_QUEUE6:
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(6 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue6
			break;
		case HIGH_QUEUE7:						
			pdesc->Dword1 |= set_desc(TXdesc_92E_MOREDATA);
			pdesc->Dword1 &= set_desc(~TXdesc_92E_MacIdMask);
			pdesc->Dword1 |= set_desc(7 & TXdesc_92E_MacIdMask);
			q_select = 0x11;// High Queue7
			break;
#endif			
		case MGNT_QUEUE:
			q_select = 0x12;
...
#ifdef CONFIG_RTL_92E_SUPPORT
			if (GET_CHIP_VER(priv) == VERSION_8192E)		
				pfrstbufdesc->Dword5 = set_desc(get_physical_addr(priv, pwlhdr,
					(get_desc(pdesc->Dword7)&TX_TxBufSizeMask), PCI_DMA_TODEVICE));
			else
#endif
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E)		
			pdescinfo->paddr  = get_desc(pfrstbufdesc->Dword5);
		else
#endif
...
#ifdef CONFIG_RTL_92E_SUPPORT	
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			flush_addr[flush_num]  = (unsigned long)virt_to_bus(pfrstbufdesc);
			flush_len[flush_num++]= sizeof(struct tx_bufdesc);
			flush_addr[flush_num]  = (unsigned long)(get_desc(pfrstbufdesc->Dword1));
			flush_len[flush_num++]= sizeof(struct tx_desc);
			flush_addr[flush_num]  = (unsigned long)(get_desc(pfrstbufdesc->Dword5));
		} else
#endif
		{	
...
*/
		...
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			...
			if (i == 0)
				pndescinfo->buf_type[0] = txcfg->fr_type;
			else
				pndescinfo->buf_type[0] = _RESERVED_FRAME_TYPE_;
		} else
#endif
		{
		/*------------------------------------------------------------*/
...
#ifdef CONFIG_RTL_92E_SUPPORT	
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			//pfrstbufdesc->Dword8 = set_desc(txcfg->fr_len & TX_TxBufSizeMask);
			pfrstbufdesc->Dword8 = set_desc(fr_len & TX_TxBufSizeMask);
			pfrstbufdesc->Dword9 = set_desc(tmpphyaddr);
		
			pndescinfo->buf_paddr[0] = get_desc(pfrstbufdesc->Dword9);
			pndescinfo->buf_pframe[0] = txcfg->pframe;
			pndescinfo->buf_len[0] = txcfg->fr_len;
		} else
#endif		
		{
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
			flush_addr[flush_num]=(unsigned long)(get_desc(pfrstbufdesc->Dword9));
			//flush_len[flush_num++] = txcfg->fr_len & TX_TxBufSizeMask;
			flush_len[flush_num++] = fr_len & TX_TxBufSizeMask;
			txdesc_rollover(pfrstbufdesc, (unsigned int *)tx_head);
		} else
#endif
		{
...
init_deschead:
	...
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		...
		{
			case MGNT_QUEUE:
				RTL_W32(REG_92E_MGQ_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case BK_QUEUE:
				RTL_W32(REG_92E_BKQ_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;		
			case BE_QUEUE:
				//printk("q2=0x%x\n", RTL_R32(REG_92E_BEQ_IDX));	
				//RTL_W32(REG_92E_BEQ_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				RTL_W16(REG_92E_BEQ_IDX, *tx_head & 0xfff);
				#if 0
				//if ((*tx_head % 10) == 0)
				{
					int cnt = 0;
					while(1)
					{
						if ((RTL_R16(REG_92E_BEQ_IDX+2) & 0xfff) == *tx_head)
						//if ((RTL_R32(0x300) & 0x02) == 0)
						{
							//if (cnt > 50)
							//	printk("%d\n", cnt);
							break;
							...
							priv->pshare->rf_ft_var.swq_dbg = 3;
							panic_printk("tx buf desc:0x%.8x,0x%.8x\n",pfrstbufdesc,virt_to_bus(pfrstbufdesc));
		
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword0));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword1));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword2));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword3));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword4));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword5));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword6));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword7));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword8));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword9));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword10));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword11));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword12));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword13));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword14));
							panic_printk("0x%.8x\n", get_desc(pfrstbufdesc->Dword15));
				
							panic_printk("tx wifi info:0x%.8x,0x%.8x\n", pdesc, virt_to_bus(pdesc));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword0));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword1));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword2));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword3));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword4));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword5));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword6));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword7));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword8));
							panic_printk("0x%.8x\n", get_desc(pdesc->Dword9));
							break;
						}
						#endif
						else
				...
				break;
			case VO_QUEUE:
				RTL_W32(REG_92E_VOQ_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case VI_QUEUE:
				RTL_W32(REG_92E_VIQ_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;		
			case HIGH_QUEUE:
				//printk("q0=0x%x\n", RTL_R32(REG_92E_HI0Q_IDX));						
				RTL_W32(REG_92E_HI0Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;	
	#ifdef MBSSID
			case HIGH_QUEUE1:
				//printk("q1=0x%x,0x%x\n", RTL_R32(REG_92E_HI1Q_IDX), RTL_R32(0x300));						
				RTL_W32(REG_92E_HI1Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				#if 0
				printk("0x%x,0x%x\n", REG_92E_HI0Q_IDX,RTL_R32(REG_92E_HI0Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI1Q_IDX,RTL_R32(REG_92E_HI1Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI2Q_IDX,RTL_R32(REG_92E_HI2Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI3Q_IDX,RTL_R32(REG_92E_HI3Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI4Q_IDX,RTL_R32(REG_92E_HI4Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI5Q_IDX,RTL_R32(REG_92E_HI5Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI6Q_IDX,RTL_R32(REG_92E_HI6Q_IDX));
				printk("0x%x,0x%x\n", REG_92E_HI7Q_IDX,RTL_R32(REG_92E_HI7Q_IDX));
				#endif
				break;
			case HIGH_QUEUE2:	
				RTL_W32(REG_92E_HI2Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case HIGH_QUEUE3:	
				RTL_W32(REG_92E_HI3Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case HIGH_QUEUE4:	
				RTL_W32(REG_92E_HI4Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case HIGH_QUEUE5:	
				RTL_W32(REG_92E_HI5Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case HIGH_QUEUE6:	
				RTL_W32(REG_92E_HI6Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);
				break;
			case HIGH_QUEUE7:	
				RTL_W32(REG_92E_HI7Q_IDX, (*tx_head & 0xfff)%CURRENT_NUM_TX_DESC);						
				break;
	#endif				
		}
	} else
...
//mark_test , remove EAP padding by ethernet
	...
	priv = (struct rtl8192cd_priv *)dev->priv;
        ...
        if (!netif_running(priv->dev)) {
                priv->ext_stats.tx_drops++;
                DEBUG_ERR("TX DROP: Can't send packet due to wlan interface is down!\n");
                goto free_and_stoptx;
        }
...
#ifdef USE_RTL8186_SDK
	rtl_cache_sync_wback(priv, (unsigned long)get_desc(pfrstdesc->Dword8), (get_desc(pfrstdesc->Dword7)&TX_TxBufSizeMask), PCI_DMA_TODEVICE);
#endif
...
#if defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_WLAN_HAL)
...
int __rtl8192cd_firetx(struct rtl8192cd_priv *priv, struct tx_insn* txcfg)
{
	...
	if (txcfg->aggre_en < FG_AGGRE_MSDU_FIRST) {
...
#endif
		{
			...
			if (GET_CHIP_VER(priv) == VERSION_8192E) {
				tx_head	= get_txbufhead_addr(phw, q_num);
				tx_tail = get_txbuftail_addr(phw, q_num);
			} else 
#endif			
			{
				tx_head = get_txhead_addr(phw, q_num);
		...
		(GET_CHIP_VER(priv) == VERSION_8192E)?((val32 + 1) > CIRC_SPACE_RTK(*tx_head, *tx_tail, CURRENT_NUM_TX_DESC)):
#endif		
...
static int rtl8192cd_tx_recycle(struct rtl8192cd_priv *priv, unsigned int txRingIdx, int *recycleCnt_p)
{
	struct tx_desc	*pdescH, *pdesc;
#ifdef CONFIG_RTL_92E_SUPPORT			
	struct tx_bufdesc *pbufdescH, *pbufdesc;
	int	cnt = 0;
#endif	
	struct tx_desc_info *pdescinfoH, *pdescinfo;
	volatile int	head, tail;
	...
	if (GET_CHIP_VER(priv) == VERSION_8192E) {
		...
		{
			case MGNT_QUEUE:
				head 		= (RTL_R32(REG_92E_MGQ_IDX)>>16) & 0xfff;
				break;
			case BK_QUEUE:
				head 		= (RTL_R32(REG_92E_BKQ_IDX)>>16) & 0xfff;
				break;		
			case BE_QUEUE:
				head 		= (RTL_R32(REG_92E_BEQ_IDX)>>16) & 0xfff;
				break;
			case VO_QUEUE:
				head 		= (RTL_R32(REG_92E_VOQ_IDX)>>16) & 0xfff;
				break;
			case VI_QUEUE:
				head 		= (RTL_R32(REG_92E_VIQ_IDX)>>16) & 0xfff;
				break;		
			case HIGH_QUEUE:				
				head 		= (RTL_R32(REG_92E_HI0Q_IDX)>>16) & 0xfff;
				break;
	#ifdef MBSSID				
			case HIGH_QUEUE1:
				head 		= (RTL_R32(REG_92E_HI1Q_IDX)>>16) & 0xfff;
				break;
			case HIGH_QUEUE2:
				head 		= (RTL_R32(REG_92E_HI2Q_IDX)>>16) & 0xfff;
				break;
			case HIGH_QUEUE3:
				head 		= (RTL_R32(REG_92E_HI3Q_IDX)>>16) & 0xfff;
				break;
			case HIGH_QUEUE4:
				head 		= (RTL_R32(REG_92E_HI4Q_IDX)>>16) & 0xfff;
				break;
			case HIGH_QUEUE5:
				head 		= (RTL_R32(REG_92E_HI5Q_IDX)>>16) & 0xfff;
				break;
			case HIGH_QUEUE6:
				head 		= (RTL_R32(REG_92E_HI6Q_IDX)>>16) & 0xfff;
				break;
			case HIGH_QUEUE7:
				head 		= (RTL_R32(REG_92E_HI7Q_IDX)>>16) & 0xfff;
				break;
	#endif				
		}
	
		tail		= get_txbuftail(phw, txRingIdx);
		pbufdescH	= get_txbufdesc(phw, txRingIdx);
	} else
#endif
	{
	...
	while (CIRC_CNT_RTK(head, tail, CURRENT_NUM_TX_DESC))
	{
...
#ifdef CONFIG_RTL_92E_SUPPORT	
		if (GET_CHIP_VER(priv) == VERSION_8192E)
			pbufdesc = (struct tx_bufdesc *)KSEG1ADDR(pbufdesc);
		else
#endif		
		...
		if (
#ifdef CONFIG_RTL_92E_SUPPORT			
		(GET_CHIP_VER(priv) == VERSION_8192E) ? !pbufdesc :
#endif		
		(!pdesc || (get_desc(pdesc->Dword0) & TX_OWN))
		)
			break;
...
#ifdef CONFIG_RTL_92E_SUPPORT
		if (GET_CHIP_VER(priv) == VERSION_8192E) {
		for (cnt =0;cnt<RTL_92E_BUF_SEGMENT-2;cnt++) {
			//if (txRingIdx == 2)
			//panic_printk("buf type[%d]=%d\n", cnt, pdescinfo->buf_type[cnt]);
			if (pdescinfo->buf_type[cnt] == _SKB_FRAME_TYPE_)
			{
...
#ifdef CONFIG_RTL_92E_SUPPORT
	if (GET_CHIP_VER(priv) == VERSION_8192E)
		*get_txbuftail_addr(phw, txRingIdx) = tail;
	else
#endif	
...
void rtl8192cd_tx_dsr(unsigned long task_priv)
{
	...
	SMP_LOCK_XMIT(flags);
#if defined(MBSSID) && 	defined(CONFIG_RTL_92E_SUPPORT)
	if ((GET_CHIP_VER(priv) == VERSION_8192E) && (GET_ROOT(priv)->pmib->miscEntry.vap_enable))
