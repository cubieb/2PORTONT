...
#define CFG_UPDATE_RX_SWBD_IDX_EARLY    0
...
#else		
		/* for small packet RX test, there may comes too many packets from the air and cause our RX ISR while(1) loop run
		   too long and then cause the system watch dog timer timeout. So we break the while loop when it takes to long */
...
//        printk("%s(%d): (%d,%d), rxbdOkCnt:%d \n", __FUNCTION__, __LINE__, rx_desc_status.FS, rx_desc_status.LS, rxbdOkCnt);
...
#endif
#if 0
				// Workaround for MIC error of the time interval from set_key to key_active
				if ((pstat->rx_pkts < 100) && ((pstat->rx_tkip_mic_err < 100)))
				{
					pstat->rx_tkip_mic_err++;
					goto free_skb_in_defrag;
				}
#endif					
