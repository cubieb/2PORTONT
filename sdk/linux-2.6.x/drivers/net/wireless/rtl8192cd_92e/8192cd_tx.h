...
static __inline__ void init_txbufdesc(struct rtl8192cd_priv *priv, struct tx_bufdesc *pdesc,
				unsigned long ringaddr, unsigned int i)
{
	//if (i == (CURRENT_NUM_TX_DESC - 1))
	//	(pdesc + i)->Dword10 = set_desc(ringaddr); // NextDescAddress
	//else
	(pdesc + i)->Dword1 = set_desc(ringaddr + (i) * sizeof(struct tx_bufdesc)); // Physical address Low
}
#endif
