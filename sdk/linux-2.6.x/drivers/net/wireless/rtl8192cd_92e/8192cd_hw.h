...
#if defined(CONFIG_RTL_92E_SUPPORT) || defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8192EE)
#include "./8192e_reg.h"
#endif
#include "./8192cd_cfg.h"

// Justin: To enable the IQ-Calibration, we need to include this 8812_reg.h; no Reg Map defination for  8192E ?
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_WLAN_HAL_8881A) || defined(CONFIG_WLAN_HAL_8192EE)
#include "./8812_reg.h"
#endif
...
#else
#ifdef CONFIG_RTL_92E_SUPPORT	
	unsigned int		buf_type[RTL_92E_BUF_SEGMENT-2];
	unsigned int		buf_paddr[RTL_92E_BUF_SEGMENT-2];
	void				*buf_pframe[RTL_92E_BUF_SEGMENT-2];
	unsigned int		buf_len[RTL_92E_BUF_SEGMENT-2];
#endif	
...
struct tx_bufdesc {
	...
	volatile unsigned int Dword14;
	volatile unsigned int Dword15;
};
#endif
...
#ifdef CONFIG_RTL_92E_SUPPORT
#define DESC_DMA_SIZE_92E	(NUM_RX_DESC *(sizeof(struct rx_bufdesc))+\
												NUM_TX_DESC *(sizeof(struct tx_bufdesc))*5 +\
												NUM_TX_DESC *(sizeof(struct tx_bufdesc))*8 +\
												NUM_CMD_DESC *(sizeof(struct rx_bufdesc)) + \
						 NUM_CMD_DESC *(sizeof(struct tx_bufdesc))) +\
						 NUM_TX_DESC *(sizeof(struct tx_desc))*5 +\
						 NUM_TX_DESC *(sizeof(struct tx_desc))*8 +\
						 16 *(sizeof(struct tx_bufdesc)) +\
						 8 *(sizeof(struct tx_desc)) 
#else
#define DESC_DMA_SIZE_92E	0						 
#endif
...
#ifndef __KERNEL__
#define DESC_DMA_PAGE_SIZE ((MAX_NUM(DESC_DMA_SIZE, DESC_DMA_SIZE_92E) + (PAGE_SIZE - 1)))
#else
//#define DESC_DMA_PAGE_SIZE ((DESC_DMA_SIZE + (2*PAGE_SIZE - 1)) & (~(PAGE_SIZE - 1)))
#define DESC_DMA_PAGE_SIZE ((MAX_NUM(DESC_DMA_SIZE, DESC_DMA_SIZE_92E) + PAGE_SIZE))
#endif
...
#ifdef CONFIG_RTL_92E_SUPPORT	
	struct tx_desc	*tx_desc6;
	struct tx_desc	*tx_desc7;
	struct tx_desc	*tx_desc8;
	struct tx_desc	*tx_desc9;
	struct tx_desc	*tx_desc10;
	struct tx_desc	*tx_desc11;
	struct tx_desc	*tx_desc12;
#endif	
	struct tx_desc	*tx_descB;	

#ifdef CONFIG_RTL_92E_SUPPORT		
	unsigned int	txhead6;
	unsigned int	txhead7;
	unsigned int	txhead8;
	unsigned int	txhead9;
	unsigned int	txhead10;
	unsigned int	txhead11;
	unsigned int	txhead12;
	unsigned int	txtail6;
	unsigned int	txtail7;
	unsigned int	txtail8;
	unsigned int	txtail9;
	unsigned int	txtail10;
	unsigned int	txtail11;
	unsigned int	txtail12;
	unsigned long		rx_bufring_addr;
	struct	rx_bufdesc	*rx_bufdescL;
	unsigned long		rx_bufdescL_dma_addr[NUM_RX_DESC];
	
	unsigned int	txbufhead0;
	unsigned int	txbufhead1;
	unsigned int	txbufhead2;
	unsigned int	txbufhead3;
	unsigned int	txbufhead4;
	unsigned int	txbufhead5;
	unsigned int	txbufhead6;
	unsigned int	txbufhead7;
	unsigned int	txbufhead8;
	unsigned int	txbufhead9;
	unsigned int	txbufhead10;
	unsigned int	txbufhead11;
	unsigned int	txbufhead12;
	
	unsigned int	txbuftail0;
	unsigned int	txbuftail1;
	unsigned int	txbuftail2;
	unsigned int	txbuftail3;
	unsigned int	txbuftail4;
	unsigned int	txbuftail5;
	unsigned int	txbuftail6;
	unsigned int	txbuftail7;
	unsigned int	txbuftail8;
	unsigned int	txbuftail9;
	unsigned int	txbuftail10;
	unsigned int	txbuftail11;
	unsigned int	txbuftail12;
	
	struct tx_bufdesc	*txbuf_desc0;
	struct tx_bufdesc	*txbuf_desc1;
	struct tx_bufdesc	*txbuf_desc2;
	struct tx_bufdesc	*txbuf_desc3;
	struct tx_bufdesc	*txbuf_desc4;
	struct tx_bufdesc	*txbuf_desc5;
	struct tx_bufdesc	*txbuf_desc6;
	struct tx_bufdesc	*txbuf_desc7;
	struct tx_bufdesc	*txbuf_desc8;
	struct tx_bufdesc	*txbuf_desc9;
	struct tx_bufdesc	*txbuf_desc10;
	struct tx_bufdesc	*txbuf_desc11;
	struct tx_bufdesc	*txbuf_desc12;
	struct tx_bufdesc	*txbuf_descB;
...
#ifdef CONFIG_RTL_92E_SUPPORT
	unsigned long	tx_desc6_dma_addr[NUM_TX_DESC];
	unsigned long	tx_desc7_dma_addr[NUM_TX_DESC];
	unsigned long	tx_desc8_dma_addr[NUM_TX_DESC];
	unsigned long	tx_desc9_dma_addr[NUM_TX_DESC];
	unsigned long	tx_desc10_dma_addr[NUM_TX_DESC];
	unsigned long	tx_desc11_dma_addr[NUM_TX_DESC];
	unsigned long	tx_desc12_dma_addr[NUM_TX_DESC];
#endif	
...
#ifdef CONFIG_RTL_92E_SUPPORT
	unsigned long	tx_ring6_addr;
	unsigned long	tx_ring7_addr;
	unsigned long	tx_ring8_addr;
	unsigned long	tx_ring9_addr;
	unsigned long	tx_ring10_addr;
	unsigned long	tx_ring11_addr;
	unsigned long	tx_ring12_addr;
	unsigned long	txbuf_desc0_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc1_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc2_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc3_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc4_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc5_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc6_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc7_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc8_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc9_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc10_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc11_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_desc12_dma_addr[NUM_TX_DESC];
	unsigned long	txbuf_descB_dma_addr[NUM_TX_DESC];
	
	unsigned long	tx_bufring0_addr;
	unsigned long	tx_bufring1_addr;
	unsigned long	tx_bufring2_addr;
	unsigned long	tx_bufring3_addr;
	unsigned long	tx_bufring4_addr;
	unsigned long	tx_bufring5_addr;
	unsigned long	tx_bufring6_addr;
	unsigned long	tx_bufring7_addr;
	unsigned long	tx_bufring8_addr;
	unsigned long	tx_bufring9_addr;
	unsigned long	tx_bufring10_addr;
	unsigned long	tx_bufring11_addr;
	unsigned long	tx_bufring12_addr;
	unsigned long	tx_bufringB_addr;
#endif	
...
#ifdef CONFIG_RTL_92E_SUPPORT
static __inline__ int get_txbuftail(struct rtl8192cd_hw *phw, int q_num)
{
	return *(int *)((unsigned int)&(phw->txbuftail0) + sizeof(int)*q_num);
}

static __inline__ struct tx_bufdesc *get_txbufdesc(struct rtl8192cd_hw *phw, int q_num)
{
	return (struct tx_bufdesc *)(*(unsigned int *)((unsigned int)&(phw->txbuf_desc0) + sizeof(struct tx_bufdesc *)*q_num));
}

static __inline__ int *get_txbufhead_addr(struct rtl8192cd_hw *phw, int q_num)
{
	return (int *)((unsigned int)&(phw->txbufhead0) + sizeof(int)*q_num);
}

static __inline__ int *get_txbuftail_addr(struct rtl8192cd_hw *phw, int q_num)
{
	return (int *)((unsigned int)&(phw->txbuftail0) + sizeof(int)*q_num);
}

static __inline__ unsigned int *get_txbufdma_addr(struct rtl8192cd_hw *phw, int q_num)
{
	return (unsigned int *)((unsigned int)&(phw->txbuf_desc0_dma_addr) + (sizeof(unsigned long)*q_num*NUM_TX_DESC));
}
#endif
