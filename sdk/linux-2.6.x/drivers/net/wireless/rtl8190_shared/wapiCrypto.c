...
#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
EXPORT_SYMBOL(init_SMS4_CK_Sbox);
#endif
#endif
...
#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
EXPORT_SYMBOL(WapiSMS4ForMNKEncrypt);
#endif
#ifdef IRAM_FOR_WIRELESS_AND_WAPI_PERFORMANCE
//__MIPS16
__IRAM_WLAN_HI
...
EXPORT_SYMBOL(WapiIncreasePN);
int (*wapiUpdateMSK_hook)(struct rtl8190_priv *priv, struct stat_info *pstat)=NULL;
EXPORT_SYMBOL(wapiUpdateMSK_hook);
int (*wapiUpdateUSK_hook)(struct rtl8190_priv *priv, struct stat_info *pstat);
EXPORT_SYMBOL(wapiUpdateUSK_hook);
#endif
void SecSWSMS4Encryption(struct rtl8190_priv	*priv, struct tx_insn* txcfg)
{
	uint16				OutputLength;
	...
	pHeader = txcfg->phdr;	/*	txcfg->hdr_len	*/
	{
		...
		{ 
			...
			if (bPNOverflow==WAPI_RETURN_SUCCESS)
			{
				/*	MSK update	*/
				#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
				if(wapiUpdateMSK_hook != NULL)	
					wapiUpdateMSK_hook(priv, NULL);
				#else
		...
		else
		{
			...
			if (bPNOverflow==WAPI_RETURN_SUCCESS)
			{
				/*	USK update	*/
				#if defined(CONFIG_RTL_WAPI_MODULE_SUPPORT)
				if(wapiUpdateUSK_hook != NULL)	wapiUpdateUSK_hook(priv, txcfg->pstat);
				#else
...
int32 SecSWSMS4Decryption(
	struct rtl8190_priv	*priv, struct stat_info *pstat, struct rx_frinfo* pfrinfo)
{
	uint8				PNOffset;	
	uint8				DataOffset;	
	uint16				OutputLength;
