...
*/
int rtl8190_rx_dispatch_mesh(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	int reuse = 1;
...
void mesh_shortcut_update(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
/*
	unsigned char *pframe = get_pframe(pfrinfo);
...
*/
int process_11s_datafrme(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat)
{
	unsigned char 	*pframe;
	...
	{	
		if (!priv->pmib->dot11OperationEntry.block_relay)
		{
			...
			if (pnewskb) {
				...
				if (rtl8190_start_relay_11s_dataframe(pnewskb, /*pskb->dev,*/ privacy, &newfrinfo))
				{
				...
				if(pProxyEntry != NULL) // addr5 is a proxy sta
				{
					...
					// stanley
					{
						...
						printk("root relay pkt to dev:%02X:%02X:%02X:%02X:%02X:%02X (source:%02X:%02X:%02X:%02X:%02X:%02X)\n", 
							...
							addr6[0], addr6[1], addr6[2], addr6[3], addr6[4], addr6[5]);
#endif
						if(rtl8190_start_relay_11s_dataframe(pskb, /*pskb->dev,*/ privacy, pfrinfo)) 
						{
#if 0
...
#ifdef _11s_TEST_MODE_
int mesh_debug_rx1(struct rtl8190_priv *priv, struct sk_buff *pskb)
{
...
int mesh_debug_rx2(struct rtl8190_priv *priv, unsigned int cmd)
{
