...
UINT16 updateMeshMetric(struct rtl8190_priv *priv, struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
	/*
...
UINT16 getMeshMetric(struct rtl8190_priv *priv, struct stat_info *pstat,  struct rx_frinfo *pfrinfo)
{
	/*
 ...
 *	@retval	void
 */
void peer_expire(struct rtl8190_priv* priv)
{
	unsigned long		flags;
...
// chuangch 2007.09.19
int pathsel_table_expire(struct rtl8190_priv* priv){
 ...
 */
void proxy_table_chkcln(struct rtl8190_priv* priv, struct sk_buff *pskb){
...
int proxy_table_expire(struct rtl8190_priv* priv){
...
#ifdef PU_STANDARD
int proxyupdate_table_expire(struct rtl8190_priv* priv){
 ...
 *	@retval	void
 */
void mesh_expire(struct rtl8190_priv* priv)
{
		peer_expire(priv);
...
static int testMesh_OnAuth(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	// we don't have any knowledge about it on the draft of 802.11s D0.02
	return 0;
}

static int testMesh_OnDeAuth(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	// we don't have any knowledge about it on the draft of 802.11s D0.02
	return 0;
}
...
int is_11s_mgt_frame(int num, struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	int isMesh = 0;
 ...
 */
unsigned int mesh_ie_MeshID(struct rtl8190_priv *priv, UINT8 meshiearray[], UINT8 isWildcard)
{
	int	len;
 ...
 */
unsigned int mesh_ie_WLANMeshCAP(struct rtl8190_priv *priv, UINT8 meshiearray[])
{
	int	counter = 0;
 ...
 *	@retval	counter		: Write in array byte count.
 */
static int mesh_ie_ActiveProfileANNOU(struct rtl8190_priv *priv, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT8	i, *meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
static int mesh_ie_LocalLinkStateANNOU(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT16	tmp;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
unsigned int mesh_ie_DTIM(struct rtl8190_priv *priv, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
unsigned int mesh_ie_MKDDIE(struct rtl8190_priv *priv, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT8	size;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
unsigned int mesh_ie_EMSAIE(struct rtl8190_priv *priv, struct stat_info *pstat, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT8	size;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
static int mesh_ie_PeerLinkOpen(struct rtl8190_priv *priv, struct stat_info *pstat, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT8	size;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
static int mesh_ie_PeerLinkConfirm(struct rtl8190_priv *priv, struct stat_info *pstat, UINT8 meshiearray[])
{
	int	counter = 0;
	UINT8	size;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@retval	counter		: Write in array byte count.
 */
static int mesh_ie_PeerLinkClose(struct rtl8190_priv *priv, struct stat_info *pstat, UINT8 meshiearray[], UINT8 reason)
{
	int	counter = 0;
	UINT8	size;
	UINT8	*meshieptr = meshiearray;
 ...
 */
static int mesh_ie_ProxyUpdate(struct rtl8190_priv *priv, struct proxyupdate_table_entry * Entry, UINT8 meshiearray[])
{
	int	length = 0;
	UINT8	*meshieptr = meshiearray;
 ...
 *	@param	meshiearray	: IE buffer
 */
static int mesh_ie_ProxyUpdateConfirm(struct rtl8190_priv *priv, UINT8 * PUSN,UINT8 * MPmac, UINT8 meshiearray[])
{
	int	length = 0;
...
#ifdef	CONFIG_RTK_MESH
void issue_disassoc_MP(struct rtl8190_priv *priv, struct stat_info *pstat, int reason, UINT8 peerLinkReason)
#else
 ...
 *	@retval	void
 */
unsigned int issue_assocreq_MP(struct rtl8190_priv *priv,  struct stat_info *pstat)
{
	unsigned short	val;
	struct wifi_mib *pmib;
	unsigned char	*bssid, *pbuf;
 ...
 */
void issue_assocrsp_MP(struct rtl8190_priv *priv, unsigned short status, struct stat_info *pstat, int pkt_type)
{
	unsigned short	val;
	struct wifi_mib *pmib;
	unsigned char	*bssid,*pbuf;
 ...
 *	@retval	void
 */
static void issue_LocalLinkStateANNOU_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat)
{
	unsigned short	seq;
	UINT8			tmp;
	unsigned char	*pbuf;
...
void issue_proxyupdate_MP(struct rtl8190_priv *priv, struct proxyupdate_table_entry *Entry)
{
	unsigned short	seq;
	UINT8			tmp;
	unsigned char		*pbuf;
...
void issue_proxyupdateconfirm_MP(struct rtl8190_priv *priv, UINT8 *PUSN, char *srcMac, char *destMac)
{
	unsigned short	seq;
	UINT8			tmp;
	unsigned char		*pbuf;
...
void dot11s_mp_set_key(struct rtl8190_priv *priv, unsigned char *mac)
{
	DOT11_SET_KEY Set_Key;
...
extern void pending_add_RATid(struct rtl8190_priv *priv, struct stat_info *pstat);
extern void add_RATid(struct rtl8190_priv *priv, struct stat_info *pstat);
 ...
 *	@retval	void
 */
static void mesh_PeerLinkEstablish(struct rtl8190_priv *priv,  struct stat_info *pstat)
{
	unsigned long		flags;
...
void mesh_auth_timer(unsigned long pVal)
{
	...
	struct rtl8190_priv *priv = (struct rtl8190_priv *)pVal;
	struct stat_info	*pstat;
 ...
 */
int start_MeshPeerLink(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, struct stat_info *pstat, UINT16 cap, UINT8 OFDMparam)
{
	unsigned long	flags;
 ...
 *	@retval	result
 */
int close_MeshPeerLink(struct rtl8190_priv *priv, UINT8 *da)
{
	struct stat_info *pstat;
	unsigned long	flags;
 ...
 */
unsigned int OnAssocReq_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	struct wifi_mib		*pmib;
	struct stat_info	*pstat;
 ...
 */
unsigned int OnAssocRsp_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	unsigned long	ioaddr = priv->pshare->ioaddr;
 ...
 */
unsigned int OnProbeReq_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{	
	struct wifi_mib	*pmib;
 ...
 */
unsigned int OnProbeRsp_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	struct stat_info *pstat;
	int len;
	unsigned char *pframe, *p;
...
unsigned int OnBeacon_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	int i, len;
	...
	if (1 == GET_MIB(priv)->dot1180211sInfo.mesh_enable) {
		...
		if (NULL != pstat) {
				...
				&& (MP_SUPERORDINATE_LINK_UP == pstat->mesh_neighbor_TBL.State || MP_SUPERORDINATE_LINK_DOWN == pstat->mesh_neighbor_TBL.State))
			{
				...
				// Use rssi temporary
				pstat->mesh_neighbor_TBL.ept = pfrinfo->rssi;
...
unsigned int OnDisassoc_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
...
/* Can't resolve, Temporary reserve by popen
static unsigned int OnAuth_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
#if (MESH_DBG_LV & MESH_DBG_COMPLEX)
...
static unsigned int OnDeAuth_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
#if (MESH_DBG_LV & MESH_DBG_COMPLEX)
 ...
 *	@retval	void
 */
void OnLocalLinkStateANNOU_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	struct stat_info	*pstat;
...
#ifdef PU_STANDARD
//by pepsi 20080229
void OnProxyUpdateConfirm_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	unsigned char 	*pframe, *p,  meshHeaderOffset;
	int	len;
...
//by pepsi
void OnProxyUpdate_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	unsigned char 	*pframe, *p,  meshHeaderOffset;
	int	len;
	UINT8 PUflag;
...
*/
unsigned int OnPathSelectionManagFrame(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo, int Is_6Addr)
{
	struct stat_info	*pstat;
 ...
 */
/* Not use now, Reserve
static unsigned int OnHello_MP(struct rtl8190_priv *priv, struct rx_frinfo *pfrinfo)
{
	struct stat_info	*pstat;
	unsigned char		*pframe;
	unsigned long		flags;
...
#ifdef  _11s_TEST_MODE_
int mesh_debug_sme1(struct rtl8190_priv *priv)
{
	static int popen = 0;
	if((OPMODE & WIFI_STATION_STATE) &&
...
int mesh_debug_sme2(struct rtl8190_priv *priv, unsigned int *rate)
{
