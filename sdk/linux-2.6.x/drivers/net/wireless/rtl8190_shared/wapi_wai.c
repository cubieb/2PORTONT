...
extern void wapi_event_indicate(struct rtl8190_priv *priv);
#endif
...
static int32 wapiFragementSend(struct sk_buff *pskb, struct rtl8190_priv *priv);
...
static int	WapiSendActivateAuthenticationPacket(struct rtl8190_priv *priv, struct stat_info *pstat, int len, uint8 *data);

static int	wapiFreeAllSta(struct rtl8190_priv *priv, int forceFree)
{
...
static void wapiMCastUpdateKeyTimeout(unsigned long task_psta)
{
	...
	priv = (struct rtl8190_priv *)task_psta;
	wapiAssert(priv);
	if (priv->wapiMCastKeyUpdate)
	{
		...
		{
			del_timer(&waiMCastKeyUpdateTimer);
			...
			{
				mod_timer(&waiMCastKeyUpdateTimer, jiffies + priv->pmib->wapiInfo.wapiUpdateMCastKeyTimeout*HZ);
			}
			WAPI_LOCK(&priv->pshare->lock);
...
void wapiInit(struct rtl8190_priv *priv)
{
	int	i;
...
void wapiExit(struct rtl8190_priv *priv)
{
	del_timer(&waiMCastKeyUpdateTimer);
/*
	init_timer(&waiMCastKeyUpdateTimer);
...
void
wapiSetIE(struct rtl8190_priv	*priv)
{
...
static int32 wapiFragementSend(struct sk_buff *pskb, struct rtl8190_priv *priv)
{
	...
	for(fragIdx=0;fragIdx<fragnum;fragIdx++)
	{
		...
		if (rtl8190_start_xmit(fragSkb, priv->dev))
		{
...
int DOT11_Process_WAPI_Info(struct rtl8190_priv *priv, uint8 *data, int32 len)
{
	struct stat_info	*pstat;
		...
		if (wapiInfo->priv->wapiMCastKeyUpdate)
		{
			...
			if (ret==WAPI_RETURN_SUCCESS)
			{
				...
				if (allDone==TRUE)
				{
					...
					mod_timer(&waiMCastKeyUpdateTimer, jiffies + HZ);
					WAPI_UNLOCK(&wapiInfo->priv->pshare->lock);
				}
			}
		}
...
int	wapiIEInfoInstall(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	int 			akmNum;
	uint8		*akm;
...
int	WapiSendAuthenticationRspPacket(struct rtl8190_priv *priv, struct stat_info *pstat, int len, uint8 *data)
{
	struct sk_buff		*pskb;
	wapiStaInfo		*wapiInfo;
	wapiWaiHeader		*wai_hdr;
...
int	wapiSendUnicastKeyAgrementRequeset(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	struct sk_buff		*pskb;
	wapiStaInfo		*wapiInfo;
	wapiWaiHeader	*wai_hdr;
...
int	wapiSendUnicastKeyAgrementConfirm(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	struct sk_buff		*pskb;
	wapiStaInfo		*wapiInfo;
	wapiWaiHeader	*wai_hdr;
...
int	wapiSendMulticastKeyNotification(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	struct sk_buff		*pskb;
	wapiStaInfo		*wapiInfo;
	wapiWaiHeader	*wai_hdr;
...
int	wapiUpdateUSK(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	wapiStaInfo		*wapiInfo;
...
int	wapiUpdateMSK(struct rtl8190_priv *priv, struct stat_info *pstat)
{
	struct list_head	*plist;
	struct stat_info	*pstat1;
	unsigned int	index;
...
updateTimer:
	...
	if (priv->pmib->wapiInfo.wapiUpdateMCastKeyType==wapi_time_update||
		priv->pmib->wapiInfo.wapiUpdateMCastKeyType==wapi_all_update)
	{
		mod_timer(&waiMCastKeyUpdateTimer, jiffies + timeout);
	}
