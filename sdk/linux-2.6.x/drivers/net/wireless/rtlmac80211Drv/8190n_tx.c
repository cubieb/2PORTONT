...
void ampdu_timeout(struct mac80211_shared_priv *priv, unsigned int current_time)
{
...
//	unsigned char *da, *pbuf, *pwlhdr;
	...
	}else
	{
		...
		/* printk("[__rtl80211_tx]stainfo:%x, current_tx_rate:%x\n",stainfo,stainfo->current_tx_rate); */
		if (is_MCS_rate(stainfo->current_tx_rate)) {
			int ret;
			//printk("CTL_AMPDU\n");
			//txinfo.aggre_mthd = AGGRE_AMPDU;
			/* brian try to fix data rate
			txinfo.fixed_rate = 1;
			txinfo.tx_rate = MCS15;
			*/		
