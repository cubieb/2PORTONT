...
static void set_mib_default(struct mac80211_shared_priv *priv)
{
	...
	priv->mib->mib_version = MIB_VERSION;
	set_mib_default_tbl(priv);

	// others that are not types of byte and int
	strcpy(priv->mib->dot11StationConfigEntry.dot11DesiredSSID, "close_test");
	priv->mib->dot11StationConfigEntry.dot11DesiredSSIDLen = strlen("close_test");
...
#endif
	{
		...
		if (!priv->EE_AutoloadFail) {
			...
			if (priv->EE_RFTypeID == 9)
				priv->mib->dot11RFEntry.dot11RFType = 7;
			else if (priv->EE_RFTypeID == 10)
				priv->mib->dot11RFEntry.dot11RFType = 8;
			else {
			...
			memcpy(priv->mib->dot11RFEntry.pwrlevelCCK, priv->EE_TxPower_CCK, MAX_CCK_CHANNEL_NUM);
			memcpy(priv->mib->dot11RFEntry.pwrlevelOFDM, priv->EE_TxPower_OFDM, MAX_OFDM_CHANNEL_NUM);
		}
#endif
...
static __inline__ int __rtl8192SE_interrupt(void *dev_instance)
{
	...
	struct mac80211_shared_priv *priv = NULL;
                  ...
                  const struct pci_device_id *ent)
{
	...
	if (rc) {
		printk(KERN_ERR "%s (rtl8192se): Cannot enable new PCI device\n",
				pci_name(pdev));
		return rc;
	}
	...
	rc = pci_request_regions(pdev, DRV_NAME);
 	if (rc) {
		printk(KERN_ERR "%s (rtl8192se): Cannot obtain PCI resources\n",
				pci_name(pdev));
		return rc;
	}
...
{
	int err;
	printk(KERN_INFO "rtl8192se: Driver loaded\n" );
...
static void MDL_EXIT rtl8190_exit (void)
{
	printk(KERN_INFO "rtl8192se: Driver unloaded\n" );
