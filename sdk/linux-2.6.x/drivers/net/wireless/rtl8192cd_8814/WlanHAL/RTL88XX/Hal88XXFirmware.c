...
#ifdef _BIG_ENDIAN_
	pfw_hdr->signature= le16_to_cpu(pfw_hdr->signature);
	pfw_hdr->version = le16_to_cpu(pfw_hdr->version);
	pfw_hdr->SVN_index = le32_to_cpu(pfw_hdr->SVN_index); 
	pfw_hdr->Year = le16_to_cpu(pfw_hdr->Year); 
#endif

	Adapter->pshare->fw_signature	= pfw_hdr->signature;
...
)
{
	...
	RT_TRACE_F(COMP_INIT, DBG_LOUD, ("set 3081 CPU clock to 100M !!! \n"));
