...
//					*((unsigned short *)(skb->data+MACADDRLEN*2)) != __constant_htons(ETH_P_8021Q) &&
	...
	//if ((pstat == NULL) || (pstat && (!priv->pshare->swq_en) && (!(pstat->swq.beq_empty | pstat->swq.viq_empty | pstat->swq.voq_empty | pstat->swq.bkq_empty))))
	if (swq_out == 0
#ifdef GBWC
		|| priv->GBWC_consuming_Q
#endif		
...
//#endif
        ...
        if(
			...
			if (priv->pshare->rf_ft_var.swq_dbg == 2) {
			    if(pstat)
					GDEBUG("pstat=%x, pstat->aggre_mthd=%d, txcfg->aggre_en=%d\n", pstat, pstat->aggre_mthd, txcfg->aggre_en);
				else
					GDEBUG("pstat is NULL\n");
			}
			if (
