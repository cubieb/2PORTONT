	...
	)
{
	...
	case PHYDM_LA_MODE:
		{
			RT_ADCSMP_TRIG_SEL		TrigSel;
			RT_ADCSMP_TRIG_SIG_SEL	TrigSigSel;
			u1Byte					DmaDataSigSel;
			u1Byte					TriggerTime;
			u2Byte					PollingTime;
			u4Byte					var, buf_idx;
			...
			PHYDM_SSCANF(input[i + 1], DCMD_HEX, &var);
			TrigSel = (RT_ADCSMP_TRIG_SEL)var;
			PHYDM_SSCANF(input[i + 2], DCMD_HEX, &var);
			TrigSigSel = (RT_ADCSMP_TRIG_SIG_SEL)var;
			PHYDM_SSCANF(input[i + 3], DCMD_HEX, &var);
			DmaDataSigSel = (u1Byte)var;
			PHYDM_SSCANF(input[i + 4], DCMD_HEX, &var);
			TriggerTime = (u1Byte)var;
			PHYDM_SSCANF(input[i + 5], DCMD_HEX, &var);
			PollingTime = (u2Byte)var;
			
			DbgPrint("TrigSel=0x%x, TrigSigSel=0x%x, DmaDataSigSel=0x%x, TriggerTime=0x%x, PollingTime=0x%x\n",
				TrigSel, TrigSigSel, DmaDataSigSel, TriggerTime, PollingTime);
			ADCSmp_Set(pDM_Odm, TrigSel, TrigSigSel, DmaDataSigSel, TriggerTime, PollingTime);

			for(buf_idx = 0; buf_idx < 0x10000; buf_idx += 2) {
				PHYDM_SNPRINTF_PROC((output+used, out_len-used, "%x%x\n", ADCSmpBuf->Octet[buf_idx+1], ADCSmpBuf->Octet[buf_idx]));
			}
