...
{	
	...
	for(Path =0; Path <=3; Path++){
		...
		if(!LOK_notready){
			...
			ODM_SetRFReg(pDM_Odm, Path, 0x8, 0x07c00, LOK_temp1>>4);
			ODM_SetRFReg(pDM_Odm, Path, 0x8, 0xf8000, LOK_temp2>>4);
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, 
				("==>S%d LOK Fail!!!\n", Path));
			ODM_SetRFReg(pDM_Odm, Path, 0x8, bRFRegOffsetMask, 0x08400);
		}
