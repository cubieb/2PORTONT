...
#define DFS_VERSION		"1.0.18"


#ifdef __ECOS
void rtl8192cd_dfs_chk_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_DFS_TXPAUSE_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_DFS_timer(void *task_priv)
#else
	...
	if (!priv->pmib->dot11DFSEntry.disable_DFS && priv->pmib->dot11DFSEntry.DFS_detected) {
		...
		if (timer_pending(&priv->dfs_chk_timer)) {
			...
			if (dfs_chk){
				...
				PRINT_INFO("DFS CP1.\n");
#ifdef __ECOS
				init_timer(&priv->dfs_chk_timer, (unsigned long)priv, rtl8192cd_dfs_chk_timer);
#else
...
#ifdef __ECOS
void rtl8192cd_ch_avail_chk_timer(void *task_priv)
#else
...
#endif
{
	...
	if(priv->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_ETSI){
		...
		(priv->pmib->dot11RFEntry.dot11channel <= 132)){
			PHY_SetBBReg(priv, 0x918, bMaskDWord, 0x1c17eced);
			PHY_SetBBReg(priv, 0x91c, bMaskDWord, 0x0fa21a20);
			PHY_SetBBReg(priv, 0x924, bMaskDWord, 0x0152a480);
			PHY_SetBBReg(priv, 0x920, bMaskDWord, 0xe0f69104);
...
#ifdef __ECOS
void rtl8192cd_ch52_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch56_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch60_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch64_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch100_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch104_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch108_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch112_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch116_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch120_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch124_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch128_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch132_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch136_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch140_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_ch144_timer(void *task_priv)
#else
...
#ifdef __ECOS
void rtl8192cd_dfs_det_chk_timer(void *task_priv)
#else
...
void DFS_SetReg(struct rtl8192cd_priv *priv)
{
	if (GET_CHIP_VER(priv) == VERSION_8192D) {
		...
		if (priv->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_FCC) {
			...
			if(priv->pshare->rf_ft_var.dfs_det_off == 1){
				PHY_SetBBReg(priv, 0x918, bMaskDWord, 0x1c16aced);
				PHY_SetBBReg(priv, 0x924, bMaskDWord, 0x0152a480);
				PHY_SetBBReg(priv, 0x920, bMaskDWord, 0xe0767231);
			}
			else{
				PHY_SetBBReg(priv, 0x918, bMaskDWord, 0x1c17aced);
				PHY_SetBBReg(priv, 0x924, bMaskDWord, 0x0152a480);
				PHY_SetBBReg(priv, 0x920, bMaskDWord, 0xe076d131);
			}
		}
		else if (priv->pmib->dot11StationConfigEntry.dot11RegDomain == DOMAIN_ETSI) {
			if(priv->pshare->rf_ft_var.dfs_det_off == 1){
				PHY_SetBBReg(priv, 0x918, bMaskDWord, 0x1c16eced);
				PHY_SetBBReg(priv, 0x91c, bMaskDWord, 0x0fa21a20);
				...
				if((priv->pmib->dot11RFEntry.dot11channel >= 120) &&
					...
					PHY_SetBBReg(priv, 0x91c, bMaskDWord, 0x0fa21a20);
					PHY_SetBBReg(priv, 0x924, bMaskDWord, 0x0152a480);
					PHY_SetBBReg(priv, 0x920, bMaskDWord, 0xe0f79104);
				}
			}
		}
			...
			if(priv->pshare->rf_ft_var.dfs_det_off == 1){
				...
				if((priv->pmib->dot11RFEntry.dot11channel >= 52) &&
					...
					PHY_SetBBReg(priv, 0x920, bMaskDWord, 0xe079d134);
					PHY_SetBBReg(priv, 0x924, bMaskDWord, 0x0152a480);
					if(priv->pmib->dot11nConfigEntry.dot11nUse40M == HT_CHANNEL_WIDTH_20){
						PHY_SetBBReg(priv, 0x91c, bMaskDWord, 0xa4721a20);
					}
