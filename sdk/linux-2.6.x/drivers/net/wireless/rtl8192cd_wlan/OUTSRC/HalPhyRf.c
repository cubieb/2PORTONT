	...
	)
{
	...
	PHY_SetBBReg(pAdapter, rFPGA0_IQK, bMaskHWord, 0x0000);
	PHY_SetBBReg(pAdapter, 0x840, bMaskDWord, 0x00010000);
	PHY_SetBBReg(pAdapter, rFPGA0_IQK, bMaskHWord, 0x8080);
}
...
// root cause, so we remove these actions temporarily. Added by tynli and SD3 Allen. 2010.05.31.
	...
	for(path = 0; path < pathbound; path++)
	{
		...
		if(path == RF_PATH_A)
		{
			...
			//page-B1
			PHY_SetBBReg(pAdapter, rFPGA0_IQK, bMaskHWord, 0x4000);
		...
		for(index = 0; index < APK_BB_REG_NUM; index++)
		{
			...
			do
			{
				PHY_SetBBReg(pAdapter, rFPGA0_IQK, bMaskHWord, 0x8000);
				{
