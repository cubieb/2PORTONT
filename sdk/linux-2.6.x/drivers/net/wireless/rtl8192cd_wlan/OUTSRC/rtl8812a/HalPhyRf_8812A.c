...
{	
	...
	if ((Path == ODM_RF_PATH_A) || (Path == ODM_RF_PATH_B))
		ODM_SetRFReg(pDM_Odm, Path, 0x65, bRFRegOffsetMask, gRadioARF65[Path]);
...
VOID phy_IQCalibrate_SetRF65Default(IN ODM_RF_RADIO_PATH_E Path, IN u4Byte val)
{
	if ((Path == ODM_RF_PATH_A) || (Path == ODM_RF_PATH_B))
		gRadioARF65[Path] = val;
}
			...
			)
{
	...
	for(path = 0; path < pathbound; path++)
	{
		...
		if(path == RF_PATH_A)
		{
			...
			//page-B1
			ODM_SetBBReg(pDM_Odm, rFPGA0_IQK, bMaskHWord, 0x4000);
		...
		for(index = 0; index < APK_BB_REG_NUM; index++)
		{
			...
			do
			{
				ODM_SetBBReg(pDM_Odm, rFPGA0_IQK, bMaskHWord, 0x8000);
				{
