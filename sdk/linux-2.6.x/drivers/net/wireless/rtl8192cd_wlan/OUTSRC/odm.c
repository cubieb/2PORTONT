...
)
{
	...
	if(pDM_Odm->SupportICType & ODM_IC_11AC_SERIES)
	{
		...
		ODM_Write2Byte(pDM_Odm, 0x990+2, 0x2710);	//0x990[31:16]=0x2710	Time duration for NHM unit: 4us, 0x2710=40ms
		ODM_Write2Byte(pDM_Odm, 0x994+2, 0xffff);	//0x994[31:16]=0xffff	th_9, th_10
		//ODM_Write4Byte(pDM_Odm, 0x998, 0xffffff5c);	//0x998=0xffffff5c 		th_3, th_2, th_1, th_0
		ODM_Write4Byte(pDM_Odm, 0x998, 0xffffff52);	//0x998=0xffffff52 		th_3, th_2, th_1, th_0
		ODM_Write4Byte(pDM_Odm, 0x99c, 0xffffffff);	//0x99c=0xffffffff		th_7, th_6, th_5, th_4
		ODM_SetBBReg(pDM_Odm, 0x9a0, bMaskByte0, 0xff);		//0x9a0[7:0]=0xff		th_8
		ODM_SetBBReg(pDM_Odm, 0x994, BIT8|BIT9|BIT10, 7);	//0x994[9:8]=3			enable CCX
		ODM_SetBBReg(pDM_Odm, 0x9e8, BIT0, 1);		//0x9e8[7]=1			max power among all RX ants	
	...
	else if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
	{
		...
		ODM_Write2Byte(pDM_Odm, 0x894+2, 0x2710);	//0x894[31:16]=0x2710	Time duration for NHM unit: 4us, 0x2710=40ms
		//ODM_Write2Byte(pDM_Odm, 0x894+2, 0x4e20);	//0x894[31:16]=0x4e20	Time duration for NHM unit: 4us, 0x4e20=80ms
		ODM_Write2Byte(pDM_Odm, 0x890+2, 0xffff);	//0x890[31:16]=0xffff	th_9, th_10
		//ODM_Write4Byte(pDM_Odm, 0x898, 0xffffff5c);	//0x898=0xffffff5c 		th_3, th_2, th_1, th_0
		ODM_Write4Byte(pDM_Odm, 0x898, 0xffffff52);	//0x898=0xffffff52 		th_3, th_2, th_1, th_0
		ODM_Write4Byte(pDM_Odm, 0x89c, 0xffffffff);	//0x89c=0xffffffff		th_7, th_6, th_5, th_4
		ODM_SetBBReg(pDM_Odm, 0xe28, bMaskByte0, 0xff);		//0xe28[7:0]=0xff		th_8
		ODM_SetBBReg(pDM_Odm, 0x890, BIT10|BIT9|BIT8, 7);	//0x890[9:8]=3			enable CCX
		ODM_SetBBReg(pDM_Odm, 0xc0c, BIT7, 1);		//0xc0c[7]=1			max power among all RX ants				
	}
...
)
{
	...
	if (pDM_Odm->SupportICType & ODM_IC_11AC_SERIES)
		value32 = ODM_GetBBReg(pDM_Odm, 0xfa8, bMaskDWord);
	else if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
		value32 = ODM_GetBBReg(pDM_Odm, 0x8d8, bMaskDWord);
...
//	printk("NHM_cnt_0=%d, NHMCurTxOkcnt=%llu, NHMCurRxOkcnt=%llu\n",
		//NHM_cnt_0, pDM_Odm->NHMCurTxOkcnt, pDM_Odm->NHMCurRxOkcnt);
//	DbgPrint("cnt0 = %d, cnt1 = %d\n ", NHM_cnt_0,NHM_cnt_1);
...
//	DbgPrint("byteRX = %llu\n ", *(pDM_Odm->pNumRxBytesUnicast));
	...
	if ( (pDM_Odm->NHMCurTxOkcnt) + 1 > (u8Byte)(pDM_Odm->NHMCurRxOkcnt<<2) + 1)		//Tx > 4*Rx possible for adaptivity test
	{
		...
		if((NHM_cnt_0 >= 190 || pDM_Odm->adaptivity_flag == TRUE) /*&& pFalseAlmCnt->Cnt_all <1000*/)
		{
		...
		{
			if(pDM_Odm->tolerance_cnt<4)
				pDM_Odm->tolerance_cnt = pDM_Odm->tolerance_cnt + 1;
			else
				pDM_Odm->tolerance_cnt = 5;	
			//test_status = 5;
	...
	{
		if((pDM_Odm->adaptivity_flag == TRUE && NHM_cnt_0 <= 200) /*&& pFalseAlmCnt->Cnt_all <1000*/)
		{
			//test_status = 2;
			ODM_SetMACReg(pDM_Odm, REG_TX_PTCL_CTRL, BIT15, 0);	//don't ignore EDCCA	 reg520[15]=0
			ODM_SetMACReg(pDM_Odm, REG_RD_CTRL, BIT11, 1);	//reg524[11]=1	
	...
	//printk("test_status = %d, adaptivity_flag = %d, FA = %d\n ", test_status, pDM_Odm->adaptivity_flag, pFalseAlmCnt->Cnt_all);

	if (pDM_Odm->SupportICType & ODM_IC_11AC_SERIES)	//disable enable NHX
	{           		
		ODM_SetBBReg(pDM_Odm, 0x994, BIT1, 0);
		ODM_SetBBReg(pDM_Odm, 0x994, BIT1, 1);
	}	
	else if (pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
	{
		ODM_SetBBReg(pDM_Odm, 0x890, BIT1, 0);
		ODM_SetBBReg(pDM_Odm, 0x890, BIT1, 1);
	}
}

VOID
odm_DynamicAdjustAdaptivityThreshold(
IN		PVOID		pDM_VOID,
IN		BOOLEAN		bBeaconBlocked
)
{
	PDM_ODM_T		pDM_Odm = (PDM_ODM_T)pDM_VOID;

	pDM_Odm->TxHangWaitPar = pDM_Odm->TxHangWaitPar +1;
	pDM_Odm->SupportAbility = pDM_Odm->SupportAbility & (~ODM_BB_DIG);
	ODM_Write_DIG(pDM_VOID, 0x32);

	if(bBeaconBlocked == TRUE && pDM_Odm->TxHangWaitPar > 3 && pDM_Odm->TH_L2H_ini < (s1Byte)0xfe && pDM_Odm->TxHangFlg == TRUE)
	{
			pDM_Odm->TH_L2H_ini = pDM_Odm->TH_L2H_ini + 2;
	}
	else
	{
		if(pDM_Odm->TxHangFlg == TRUE && pDM_Odm->TxHangWaitPar > 3 )
		{
			pDM_Odm->TxHangFlg = FALSE;
            pDM_Odm->bLinkAdjust = TRUE;
			pDM_Odm->TH_L2H_ini = pDM_Odm->TH_L2H_ini + 1;
			pDM_Odm->SupportAbility = pDM_Odm->SupportAbility|ODM_BB_DIG;
			...
			{
				ODM_SetBBReg(pDM_Odm, rFPGA0_XB_LSSIReadBack, 0xFFFF, ((u1Byte)0x7f<<8) | (u1Byte)0x7f);
			}	
		}
	}
...
)
{
	...
	//
	pDM_Odm->BeaconOkCnt = 0;
	pDM_Odm->TxHangFlg = TRUE;
	pDM_Odm->TxHangWaitPar = 0;
	pDM_Odm->bLinkAdjust = TRUE;
	NHMBBInit(pDM_Odm);
	//

#if (DM_ODM_SUPPORT_TYPE == ODM_AP)
	if(pDM_Odm->priv->pshare->rf_ft_var.adaptivity_enable == 2) 
	{
		pDM_Odm->TH_L2H_ini = 20;
		pDM_Odm->TH_EDCCA_HL_diff = 8;
		pDM_Odm->AdapEn_IGI = 0x26;
	}
#endif	
...
)
{
	...
	s1Byte TH_L2H_dmc, TH_H2L_dmc, adaptivity_th;
	...
	if(*pDM_Odm->pChannel > 149) // Band4 doesn't need adaptivity
	{
		if(pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
	  	{
	...
	else
	{
		...
		{
			if(pDM_Odm->Carrier_Sense_enable && !(pDM_Odm->priv->pmib->dot11nConfigEntry.dot11nUse40M==1 || pDM_Odm->priv->pmib->dot11RFEntry.dot11channel==14 ))
			{
				EDCCA_State = 0;
			}			
			else
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))		//for dynamic adjust adaptivity threshold
	if(pDM_Odm->Carrier_Sense_enable != 1 && pDM_Odm->priv->pshare->rf_ft_var.adaptivity_enable != 2)
	{
		struct rtl8192cd_priv *priv=pDM_Odm->priv;
		u4Byte	CurrentBeaconOkCnt;
		u2Byte	beaconPeriod, beaconNum, beaconTh;
		BOOLEAN	bBeaconBlocked = TRUE;

		beaconPeriod = priv->pmib->dot11StationConfigEntry.dot11BeaconPeriod;
		beaconNum = 1000 /beaconPeriod;
		if(beaconNum%2 != 0)
			beaconTh = beaconNum>>1 +1;
		else
			beaconTh = beaconNum>>1;
		
		if ((pDM_Odm->SupportICType & ODM_RTL8192E) && (priv->pshare->fw_version < 17) ||(pDM_Odm->SupportICType & ODM_RTL8188E) && (priv->pshare->fw_version < 15) ||
           (pDM_Odm->SupportICType & ODM_RTL8812) && (priv->pshare->fw_version < 28) ||(pDM_Odm->SupportICType & ODM_RTL8881A) && (priv->pshare->fw_version < 5))
		{

			CurrentBeaconOkCnt = priv->ext_stats.beacon_ok - pDM_Odm->BeaconOkCnt;
			pDM_Odm->BeaconOkCnt = priv->ext_stats.beacon_ok;
			if(CurrentBeaconOkCnt > beaconTh)
				bBeaconBlocked = FALSE;

			if(pDM_Odm->TxHangFlg == TRUE && pDM_Odm->bLinked)
			{
				odm_DynamicAdjustAdaptivityThreshold(pDM_Odm,bBeaconBlocked);
                pDM_Odm->bLinkAdjust = FALSE;
			}
           	else if(pDM_Odm->bLinkAdjust == FALSE)
            	odm_DynamicAdjustAdaptivityThreshold(pDM_Odm,bBeaconBlocked);
				 
			//panic_printk("TxHangflag = %d, CurrentBeaconOkCnt = %d, bBeaconBlocked= %d\n",pDM_Odm->TxHangFlg,  CurrentBeaconOkCnt,bBeaconBlocked);
			//panic_printk("beaconTh = %d\n",beaconTh);
		}
	}
#endif

//NHM 
	if(pDM_Odm->TxHangFlg ==FALSE && pDM_Odm->Carrier_Sense_enable != 1 &&pDM_Odm->priv->pshare->rf_ft_var.adaptivity_enable != 2)
		NHMBB(pDM_Odm);
//NHM
	...
	if(EDCCA_State == 1)
	{
		...
		if(pDM_Odm->Carrier_Sense_enable != 1 &&pDM_Odm->priv->pshare->rf_ft_var.adaptivity_enable != 2)
		{
			adaptivity_th = 0x32-110+pDM_Odm->TH_L2H_ini-pDM_Odm->TH_EDCCA_HL_diff;
			pDM_Odm->IGI_LowerBound = (u1Byte) (adaptivity_th-10+110);

			TH_H2L_dmc = TH_L2H_dmc - pDM_Odm->TH_EDCCA_HL_diff;
			if(TH_L2H_dmc > 10) 	TH_L2H_dmc = 10;
			if(TH_H2L_dmc > 10)	TH_H2L_dmc = 10;
		}
		else
		{
			if(TH_L2H_dmc > 10) 	TH_L2H_dmc = 10;
		...
		//panic_printk("TH_L2H_ini = 0x%x, IGI_target = 0x%x , IGI_lowerBound = 0x%x\n", pDM_Odm->TH_L2H_ini, IGI_target, pDM_Odm->IGI_LowerBound);
	}
	else
	{
		TH_L2H_dmc = 0x7f;
	...
	)
{
	...
	volatile u1Byte				DIG_MaxOfMin, DIG_Dynamic_MIN, i;
...
#if (DM_ODM_SUPPORT_TYPE & (ODM_AP|ODM_ADSL))
			// Secondary on-shot
			if((pDM_DigTable->CurIGValue > DIG_MaxOfMin) && (pFalseAlmCnt->Cnt_all > 2000))
				CurrentIGI = (pDM_Odm->RSSI_Min < pDM_DigTable->rx_gain_range_max)?pDM_Odm->RSSI_Min:pDM_DigTable->rx_gain_range_max;
			else
#endif
			{
	...
	)
{
	...
	ODM_SetBBReg(pDM_Odm, rFPGA0_IQK, bMaskHWord, 0x8080);
	...
	ODM_SetBBReg(pDM_Odm, rFPGA0_IQK, bMaskHWord, 0x0000);
	PSD_report_tmp = 0x0;
