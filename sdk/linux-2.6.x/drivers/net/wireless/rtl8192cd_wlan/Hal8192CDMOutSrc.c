...
void DIG_process(struct rtl8192cd_priv *priv)
{
	...
	unsigned char Adap_IGI_Upper = priv->pshare->rf_ft_var.IGI_target + 30 + priv->pshare->rf_ft_var.TH_L2H_ini - priv->pshare->rf_ft_var.TH_EDCCA_HL_diff;
#ifdef INTERFERENCE_CONTROL
...
void APK_MAIN(struct rtl8192cd_priv *priv, unsigned int is2T)
{
	...
	for(path = 0; path < pathbound; path++) {
		...
		if(path == RF92CD_PATH_A) {
			 ...
			 *	page-B1
			 */
			PHY_SetBBReg(priv, 0xe28, bMaskHWord, 0x4000);
			...
			PHY_SetBBReg(priv, 0xe28, bMaskHWord, 0x0000);
		...
		for(index = 1; index < APK_BB_REG_NUM; index++) {
			...
			i = 0;
			do {
				PHY_SetBBReg(priv, 0xe28, bMaskHWord, 0x8000);
...
void _PHY_PathAStandBy(struct rtl8192cd_priv *priv)
{
	...
	PHY_SetBBReg(priv, 0xe28, bMaskHWord, 0x8080);
}
