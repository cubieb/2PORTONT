...
void 
phy_FixSpur_8812A(
	struct rtl8192cd_priv	*pAdapter,
	unsigned char			Bandwidth,
	unsigned char			Channel
)
{
	// C cut Item12 ADC FIFO CLOCK
	if(IS_C_CUT_8812(pAdapter))
	{
		if(Bandwidth == HT_CHANNEL_WIDTH_20_40 && Channel == 11)
			PHY_SetBBReg(pAdapter, 0x8ac, 0xC00, 0x3);		// 0x8AC[11:10] = 2'b11
		else
			PHY_SetBBReg(pAdapter, 0x8ac, 0xC00, 0x2);		// 0x8AC[11:10] = 2'b10
		
		// <20120914, Kordan> A workarould to resolve 2480Mhz spur by setting ADC clock as 160M. (Asked by Binson)
		if (Bandwidth == HT_CHANNEL_WIDTH_20 && (Channel == 13 || Channel == 14)) 
		{
			PHY_SetBBReg(pAdapter, 0x8ac, 0x300, 0x3);  	// 0x8AC[9:8] = 2'b11
			PHY_SetBBReg(pAdapter, 0x8c4, BIT30, 1);  		// 0x8C4[30] = 1
		}
		else if (Bandwidth == HT_CHANNEL_WIDTH_20_40 && Channel == 11) 
		{
			PHY_SetBBReg(pAdapter, 0x8c4, BIT30, 1);  		// 0x8C4[30] = 1
		} 
		else if (Bandwidth != HT_CHANNEL_WIDTH_80) 
		{
			PHY_SetBBReg(pAdapter, 0x8ac, 0x300, 0x2);  	// 0x8AC[9:8] = 2'b10
			PHY_SetBBReg(pAdapter, 0x8c4, BIT30, 0);  		// 0x8C4[30] = 0
		} 
	}
	else if (GET_CHIP_VER(pAdapter)==VERSION_8812E)
	{
		// <20120914, Kordan> A workarould to resolve 2480Mhz spur by setting ADC clock as 160M. (Asked by Binson)
		if (Bandwidth == HT_CHANNEL_WIDTH_20 && (Channel == 13 || Channel == 14))
			PHY_SetBBReg(pAdapter, 0x8ac, 0x300, 0x3);  	// 0x8AC[9:8] = 11
		else if (Channel <= 14) // 2.4G only
			PHY_SetBBReg(pAdapter, 0x8ac, 0x300, 0x2);  	// 0x8AC[9:8] = 10
	}
...
void UpdateBBRFVal8812(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	phy_FixSpur_8812A(priv, priv->pshare->CurrentChannelBW, channel);
...
void PHY_SetOFDMTxPower_8812(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	if (!priv->pshare->rf_ft_var.disable_txpwrlmt)
	{
		if (priv->pmib->dot11RFEntry.tx2path) {
			RTL_W32(rTxAGC_A_Ofdm18_Ofdm6_JAguar, (RTL_R32(rTxAGC_A_Ofdm18_Ofdm6_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_A_Ofdm54_Ofdm24_JAguar, (RTL_R32(rTxAGC_A_Ofdm54_Ofdm24_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_A_MCS3_MCS0_JAguar, (RTL_R32(rTxAGC_A_MCS3_MCS0_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_A_MCS7_MCS4_JAguar, (RTL_R32(rTxAGC_A_MCS7_MCS4_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_A_Nss1Index3_Nss1Index0_JAguar, (RTL_R32(rTxAGC_A_Nss1Index3_Nss1Index0_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_A_Nss1Index7_Nss1Index4_JAguar, (RTL_R32(rTxAGC_A_Nss1Index7_Nss1Index4_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_A_Nss2Index1_Nss1Index8_JAguar, (RTL_R32(rTxAGC_A_Nss2Index1_Nss1Index8_JAguar) - (pwrdiff & 0x0000ffff)));

			RTL_W32(rTxAGC_B_Ofdm18_Ofdm6_JAguar, (RTL_R32(rTxAGC_B_Ofdm18_Ofdm6_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_Ofdm54_Ofdm24_JAguar, (RTL_R32(rTxAGC_B_Ofdm54_Ofdm24_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_MCS3_MCS0_JAguar, (RTL_R32(rTxAGC_B_MCS3_MCS0_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_MCS7_MCS4_JAguar, (RTL_R32(rTxAGC_B_MCS7_MCS4_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_Nss1Index3_Nss1Index0_JAguar, (RTL_R32(rTxAGC_B_Nss1Index3_Nss1Index0_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_Nss1Index7_Nss1Index4_JAguar, (RTL_R32(rTxAGC_B_Nss1Index7_Nss1Index4_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_Nss2Index1_Nss1Index8_JAguar, (RTL_R32(rTxAGC_B_Nss2Index1_Nss1Index8_JAguar) - (pwrdiff & 0x0000ffff)));			
		}
...
void PHY_SetCCKTxPower_8812(struct rtl8192cd_priv *priv, unsigned char channel)
{
	...
	{			
		if (priv->pmib->dot11RFEntry.tx2path) {
			RTL_W32(rTxAGC_A_CCK11_CCK1_JAguar, (RTL_R32(rTxAGC_A_CCK11_CCK1_JAguar) - pwrdiff));
			RTL_W32(rTxAGC_B_CCK11_CCK1_JAguar, (RTL_R32(rTxAGC_B_CCK11_CCK1_JAguar) - pwrdiff));
		}
