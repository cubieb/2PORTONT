...
void mp_set_datarate(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	for (eRFPath = RF92CD_PATH_A; eRFPath<priv->pshare->phw->NumTotalRFPath; eRFPath++) {
		if (!PHYCheckIsLegalRfPath8192cPci(priv, eRFPath))
			continue;
		if (priv->pshare->CurrentChannelBW == HT_CHANNEL_WIDTH_20) {
#if defined(CONFIG_RTL_8812_SUPPORT) || defined(CONFIG_RTL_88E_SUPPORT)			
			if (CHECKICIS8812() || (GET_CHIP_VER(priv) == VERSION_8188E))
...
{
	char base_A,base_B, byte0, byte1, byte2, byte3;
...
void mp_set_phypara(struct rtl8192cd_priv *priv, unsigned char *data)
{
	...
	if (GET_CHIP_VER(priv) != VERSION_8192D && GET_CHIP_VER(priv) != VERSION_8188E &&  GET_CHIP_VER(priv) != VERSION_8812E) {
		printk("Fail: xcap is for 92D/88E/8812E only\n");
		return;
	}
