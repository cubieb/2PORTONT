...
#define SMP_LOCK_FIRETX(__x__)
#define SMP_UNLOCK_FIRETX(__x__)		
...
#define SMP_LOCK_FIRETX(__x__)			spin_lock_irqsave(&priv->pshare->lock_firtx, (__x__))
#define SMP_UNLOCK_FIRETX(__x__)		spin_unlock_irqrestore(&priv->pshare->lock_firtx, (__x__))
...
#ifdef __LINUX_2_6__
		start = CPHYSADDR(start)+CONFIG_LUNA_SLAVE_PHYMEM_OFFSET;
#endif
