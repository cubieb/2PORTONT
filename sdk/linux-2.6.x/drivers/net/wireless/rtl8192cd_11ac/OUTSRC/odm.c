	...
	)
{
	...
	else if(pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
	{
		...
		//ODM_EdcaTurboInit(pDM_Odm);
#if !defined(MP_DRIVER) || (MP_DRIVER != 1)  //eric
	...
	)
{
	...
	list_for_each_entry(pstat, &priv->asoc_list, asoc_list) {
		...
		if(ODM_RAStateCheck(pDM_Odm, (s4Byte)pstat->rssi, FALSE, &pstat->rssi_level) ) {
			...
			UpdateHalRAMask8812(priv, pstat, pstat->rssi_level);
			}else
#endif
...
{	
	...
	if ((priv->up_time % 3) == 0 )  {
		for(i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++){
			PSTA_INFO_T pstat = pDM_Odm->pODM_StaInfo[i];
			if(IS_STA_VALID(pstat) ) {
				if ((pstat->hp_level == 0) && (pstat->rssi > TX_POWER_NEAR_FIELD_THRESH_8812))
					pstat->hp_level = 1;
	...
	)
{
	...
	if (!(pDM_Odm->priv->pshare->rf_ft_var.mp_specific && pDM_Odm->priv->pshare->rf_ft_var.mp_dig_enable))
		return;

	ODM_RT_TRACE(pDM_Odm,ODM_COMP_MP, ODM_DBG_LOUD, ("===> ODM_MPT_DIG\n"));
