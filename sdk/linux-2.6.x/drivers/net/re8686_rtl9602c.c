...
*/
	if(pMsgData->intrType == MSG_TYPE_ONU_STATE) // ONU state Change event
	{
		if(pMsgData->intrBitMask < CPU_PORT && pMsgData->intrBitMask >= 0)
		{
			...
			else if (pMsgData->intrSubType == GPON_STATE_O1 && LCDev_mapping[pMsgData->intrBitMask].status == 1) //Link Down 
			{
				if (netif_carrier_ok(LCDev_mapping[pMsgData->intrBitMask].phy_dev))
				{
					netif_carrier_off(LCDev_mapping[pMsgData->intrBitMask].phy_dev);
...
__IRAM_NIC int do_icmpreplay_check(struct sk_buff *skb)
{
	...
	if(((*(u16*)(skb->data+off))==htons(0x0800))||((*(u16*)(skb->data+off))==htons(0x0021)))//IPv4 or IPv4 with PPPoE
	{
		if(skb->data[off+11]  == 0x1 && skb->data[off+22] == 0x0) // ICMP
		{
			...
			// if without payload , set checksum to 0xffff directly.(special case)
			if(((skb->data[off+4] << 16) + skb->data[off+5]) == 0x1c)
			{
				skb->data[off+24] = skb->data[off+25] = 0xff;
				return 1;
			}
			
			off+=22; // ipv4 header + packet type (0x0800 or 0x0021)
			for(idx = 0 ; skb->data[off+8+idx]; idx++)
				if(skb->data[off+8+idx] != 0x0)
					return 0;
...
static int padding_enable_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len=0;
	...
	case 0:
		len = sprintf(page, "0, (padding is disable)\n");
		break;
	case 1:
		len = sprintf(page, "1, (padding is enable)\n");
		break;
	default:
		len = sprintf(page, "Error, (padding_enable is invalid)\n");
	}

	return len;
}

static int padding_enable_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	unsigned char tmpBuf[16] = {0};
	int i=0;
...
static struct gmac_proc_entry gmac_proc_tbl[] = {
	...
	{NULL, "padding_enable", padding_enable_read, padding_enable_write},
	{NULL, NULL, NULL, NULL},
};
