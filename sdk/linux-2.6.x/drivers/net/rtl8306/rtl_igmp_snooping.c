...
#ifdef CONFIG_ETHWAN
//shlee #include "Rtl8306_AsicDrv.h"
...
static uint8 rtl_gatewayMac[4][6];
static uint32 rtl_gatewayIp[4];
		...
		{	
			...
			if((macFrame[0]&0x01)==0)
			{
			      for(i=0;i<4;i++)
				{
					...
					(macInfo->ipBuf->destinationIp==htonl(rtl_gatewayIp[i])))
				       {
...
int32 rtl_setIgmpSnoopingV1Parameter(struct rtl_igmpSnoopingParameter igmpSnoopingParas, uint8* gatewayMac, uint32 gatewayIp)
{
	...
	if(gatewayMac!=NULL)
	{
		...
		#ifndef RTL_IGMP_SNOOPING_TEST
       	if(rtl8306_addLUTUnicastMacAddress(rtl_gatewayMac[i], 0x02, TRUE, TRUE, cpuPortNum)==FAILED)
		{
              	   rtl_igmpGluePrintf("set CPU mac error!\n");
		}
		#endif
	...
	if(gatewayIp!=0)
	{
		for(i=0;i<4;i++)
		{
			if(rtl_gatewayIp[i]==0)
			rtl_gatewayIp[i]=gatewayIp;
		}
...
int8 rtl_igmpSnoopingV1Send(uint32 mdIp)
{
...
//	if(IS_CLASSD_ADDR(destIp) && (macInfo.cpuTagFlag==0))          /*check whether destination ip is a multicast address*/
		...
		hashIndex=mdIp&rtl_fwdHashMask;
		groupEntry=rtl_searchGroupEntry(mdIp, rtl_fwdHashTable[hashIndex]); /*search multicast list to find whether destination IP registered*/ 
		if(groupEntry!=NULL)
		{
			 if(((groupEntry->lookupTableFlag & IN_LOOKUP_TABLE) !=0)  && ((groupEntry->lookupTableFlag & AGGREGATOR_FLAG) ==0))
			 {
			 	/*means in look up table and non-aggregator entry */
				return -1; //8671 support
			 }
		#if 0	 
			...
			*(*newMacFrame+15)=(groupEntry->fwdPortMask)&(~rtl_cpuPortMask);
			#endif

			/*insert cpu tag, add multicast router port and clear cpu port*/
//			insert[14]=CPUTAGPROTOCOL<<4;
			return ((groupEntry->fwdPortMask)&(~rtl_cpuPortMask));
	...
	uint8 multicastMacAddress[6];
       ...
       if(rtl_igmpStatus==ENABLE)
       {
		...
		for(j=0;j<4;j++)
		{
			...
			#ifndef RTL_IGMP_SNOOPING_TEST
			if((rtl_gatewayMac[j][0]|rtl_gatewayMac[j][1]|rtl_gatewayMac[j][2]|rtl_gatewayMac[j][3]|rtl_gatewayMac[j][4]|rtl_gatewayMac[j][5])!=0)
			{

			rtl8306_deleteMacAddress(rtl_gatewayMac[j], &entryaddr);
			}
			 #else
			 rtl8306_deleteMacAddress(rtl_gatewayMac[j]);
			 #endif
              	 for(i=0; i<6; i++)
	     		{
...
void rtl_igmpDump(uint32 dumpOpration, uint32 groupAddress)
{
	...
	switch(dumpOpration)
	{
		...
		for(i=0;i<4;i++)
		{
		rtl_igmpGluePrintf("the gateway mac address is %x:%x:%x:%x:%x:%x\n",rtl_gatewayMac[i][0],rtl_gatewayMac[i][1],rtl_gatewayMac[i][2],rtl_gatewayMac[i][3],rtl_gatewayMac[i][4],rtl_gatewayMac[i][5]);
		rtl_igmpGluePrintf("the gateway ip address is %d.%d.%d.%d\n", ((rtl_gatewayIp[i]&0xff000000)>>24),((rtl_gatewayIp[i]&0x00ff0000)>>16),((rtl_gatewayIp[i]&0x0000ff00)>>8),(rtl_gatewayIp[i]&0x000000ff));
