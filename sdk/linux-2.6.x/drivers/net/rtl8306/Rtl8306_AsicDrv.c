    ...
    *  $Id: Rtl8306_AsicDrv.c,v 1.4 2011/06/21 06:50:27 ql Exp $
    */
...
#include "mdcmdio.h"
#ifdef CONFIG_ETHWAN
#include "rtl8306e_asicdrv.h"   //shlee, for one arm router
#endif
...
int32 rtl8306_phyReg_set(uint32 phyad, uint32 regad, uint32 npage, uint32 value) 
{
    uint32 rdata;
    uint32 regval; 
...
int32 rtl8306_phyReg_get(uint32 phyad, uint32 regad, uint32 npage, uint32 *pvalue)
{
    uint32 rdata;
    uint32 regval; 
...
/* Function Name:
 *      rtl8306_port_eeeAbility_set
 * Description:
 *       enable PHY EEE 10M/100M  abilty
 * Input:
 *      phy                    - Specify the phy(0 ~4) to configure
 *      enabled10M         - EEE 10M ability enable/disable
 *      enabled100M       - EEE 100M ability enable/disable 
 * Output:
 *      none
 * Return:
 *      SUCCESS
 ...
 * Note:
 */
int32 rtl8306_port_eeeAbility_set(uint32 phy, uint32 enabled10M, uint32 enabled100M)
{
    uint32 regval;

    /*eee abilty work when phy auto negoation is enabled*/
    regval = 0;
    rtl8306_getAsicPhyRegBit(phy, 0, 12, 0, &regval);
    if(!regval)
        return FAILED;

    //rtl8306_getAsicPhyReg(phy, 16, 4, &regval);
    rtl8306_phyReg_get(phy, 16, 4, &regval);
    regval &= ~(0x3 << 12);
    regval |= enabled10M ? (0x1 << 13) : 0;
    regval |= enabled100M ? (0x1 << 12) : 0;    
    //rtl8306_setAsicPhyReg(phy, 16, 4, regval);
    rtl8306_phyReg_set(phy, 16, 4, regval);

    /*retart nway*/
    rtl8306_setAsicPhyRegBit(phy, 0, 9, 0, 1);

    return SUCCESS;
}

/* Function Name:
 *      rtl8306_port_eeeAbility_get
 * Description:
 *       Get  PHY EEE 10M/100M  abilty enable or disable
 * Input:
 *      phy                    - Specify the phy(0 ~4) to configure
 * Output:
 *      pEnabled10M        - the pointer of EEE 10M ability enable/disable
 *      pEnabled100M       - the pointer of EEE 100M ability enable/disable 
 * Return:
 *      SUCCESS
 *      FAILED
 * Note:
 */
int32 rtl8306_port_eeeAbility_get(uint32 phy, uint32 *pEnabled10M, uint32 *pEnabled100M)
{
    uint32 regval;
    ...
    //rtl8306_getAsicPhyReg(phy, 16, 4, &regval);
    rtl8306_phyReg_get(phy, 16, 4, &regval);
    *pEnabled10M = (regval & (0x1 << 13)) ? 1: 0;
    *pEnabled100M = (regval & (0x1 << 12)) ? 1: 0;
