...
#include <rtk/acl.h>
#include <rtk/trap.h>
#include <rtl86900/sdk/system/include/common/error.h>
...
#ifdef CONFIG_RTL8686_TEST
#include "re8686_rtl9601b_SW_def.h"
#endif
	...
	if((pTxInfo->opts3.bit.tx_portmask == 0) && ((skb->data[0]&1)==1))
	{
		...
		if(memcmp(skb->dev->name,"wlan1",5)!=0 && memcmp(skb->dev->name,"vwlan", 5)!=0)
		{
			...
			else
			{
				...
				pTxInfo->opts3.bit.tx_portmask=0x3;
			}
#ifdef CONFIG_DUALBAND_CONCURRENT		
		}		
#endif
...
//#endif
    ...
    /* disable delay mode for RTL9601B */
    value = RTL_R32(Rx_Pse_Des_Thres_h);
    value |= 1 << 6;
    RTL_W32(Rx_Pse_Des_Thres_h, value);
	...
	RTL_W32(CPUtag1CR, (PON_CTRL_STREAM << 8) | (PON_PORT));/* , spa_pon is 3[from switch team]*/
...
static int dbg_level_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	len += sprintf(page + len, "RTL8686_RX_WARN\t\t0x%08x\n", RTL8686_RX_WARN);
	len += sprintf(page + len, "RTL8686_TX_WARN\t\t0x%08x\n", RTL8686_TX_WARN);
...
int __init re8670_probe (void)
{
...
#ifdef CONFIG_RTL8686_TEST
    {
        ...
        {
            if((*((uint32 *)(reg_map_def[i].reg_addr | UNCACHE_MASK)) & reg_map_def[i].write_bit) != reg_map_def[i].reset_val)
            {
                printk("default unmatch@0x%08x:0x%08x != 0x%08x\n",
                    reg_map_def[i].reg_addr | UNCACHE_MASK,
                    (*((uint32 *)(reg_map_def[i].reg_addr | UNCACHE_MASK)) & reg_map_def[i].write_bit),
                    reg_map_def[i].reset_val);
                passed = FALSE;
            }
        }
        if(passed)
        {
            printk("NIC defalut check passed!\n");
        }
        else
        {
            printk("NIC defalut check failed!\n");
        }
    }
#endif
