...
{
	unsigned int pinocr = REG32(BSP_MISC_PINOCR);
...
*/
	...
	//spi or nor flash
	if((pinocr & BSP_BOOT_FLASH_TYPE) == SPI_NOR_T) {
#ifdef CONFIG_RTL8672_SPI_FLASH
		if (!rtl8672_mtd && ((pinsr & BSP_BOOT_FLASH_STS) == SPI_T)) {
			printk("spi probe...\n");
			rtl8672_mtd = do_map_probe("spi_probe", &rtl8672_map);
		}
#endif
		if (!rtl8672_mtd && ((pinsr & BSP_BOOT_FLASH_STS) == NOR_T)) {
			printk("nor probe...\n");
	...
	if (rtl8672_mtd) {
		...
		printk("%s: probe failed! pinocr=0x%08x, pinsr=0x%08x\n", __func__, pinocr, pinsr);
	}
/*
