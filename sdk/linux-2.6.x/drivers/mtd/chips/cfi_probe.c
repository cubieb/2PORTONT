...
/*linux-2.6.19*/
	...
	if ((base + 0xfff) >= map->size) {
		printk(KERN_NOTICE
			"Probe at base[0x555](0x%08lx) past the end of the map(0x%08lx)\n",
			(unsigned long)base + 0x555, map->size -1);
		return 0;
	}
#else
				   ...
				   struct cfi_private *cfi)
{
	...
	/*linux-2.6.19*/
	//tylo, debug   sh, need it specially for flash size > 4MB
	cfi->cfiq->BufWriteTimeoutTyp=0;
	cfi->cfiq->BufWriteTimeoutMax=0;
	cfi->cfiq->MaxBufWriteSize=0;
#ifdef DEBUG_CFI
