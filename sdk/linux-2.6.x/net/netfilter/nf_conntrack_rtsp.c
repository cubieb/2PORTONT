...
#define PROCFS_NAME "algonoff_rtsp"
ALGONOFF_INIT(rtsp)
#endif
...
{ 
        ...
        while (dataoff < datalen) { 
                ...
                if (expinfo.pbtype == pb_range) { 
						...
						// Mason Yu. we ignore the src port(incoming). 
						// Because the server will use another port	to send the data.					
                        //exp->mask.src.u.udp.port  = 0xfffe; 
						exp->mask.src.u.udp.port  = 0x00; 
                } 
...
#ifdef CONFIG_IP_NF_ALG_ONOFF
	if(!algonoff_rtsp) {
		// Mason Yu. If disable RTSP ALG, system do not need to parse RTSP packet for e8.
		printk("Not do RTSP ALG!!!\n");
		//return NF_DROP;		
		return NF_ACCEPT;
	}	
#endif
...
{ 
        ...
        for (i = 0; (i < MAX_PORTS) && ports[i]; i++) { 
				...
				// Mason Yu. Assign the help for IPv4			
