...
*/
	if (cp->hw_acc) {
		timeval = rtl865x_naptSync((cp->protocol==IPPROTO_TCP)? 1: 0, 
				cp->daddr.ip, cp->dport, cp->vaddr.ip, cp->vport, cp->caddr.ip, cp->cport,0);
...
/*2007-12-19*/
		...
		if (cp->hw_acc) {
			int rc;
			rc = rtl865x_delNaptConnection((cp->protocol==IPPROTO_TCP)? 1: 0, 
				cp->daddr.ip, cp->dport, cp->vaddr.ip, cp->vport, cp->caddr.ip, cp->cport);
			#ifdef CONFIG_HARDWARE_NAT_DEBUG
			/*2007-12-19*/
