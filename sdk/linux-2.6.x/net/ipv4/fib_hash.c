...
#if defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER_L3)
	if(tb->tb_id == RT_TABLE_MAIN){
		rtl8676_add_route(cfg->fc_oif, fi->fib_dev->name, cfg->fc_dst, inet_make_mask(cfg->fc_dst_len), cfg->fc_gw);
		}
#endif
...
#if defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER_L3)
		if (tb->tb_id == RT_TABLE_MAIN) {
			rtl8676_del_route(cfg->fc_dst, inet_make_mask(cfg->fc_dst_len));
		}
#endif
		fa = fa_to_delete;
...
#if defined(CONFIG_RTL_HARDWARE_NAT) || defined(CONFIG_IP_MULTIPLE_TABLES)
...
#endif
{
	...
	hlist_for_each_entry_safe(f, node, n, head, fn_hash) {
		...
		list_for_each_entry_safe(fa, fa_node, &f->fn_alias, fa_list) {
			...
			if (tb_id==RT_TABLE_MAIN) {
				rtl8676_del_route(f->fn_key, inet_make_mask(fz->fz_order));
			}
#endif
