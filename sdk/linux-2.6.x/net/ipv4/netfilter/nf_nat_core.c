...
void nf_nat_conntrack_replace(const struct nf_conntrack_tuple *tuple,
		  const struct nf_conn *ignored_conntrack)
	{
		...
		for (i = 0; i < nf_conntrack_htable_size; i++) 
		{
			...
			hlist_nulls_for_each_entry_rcu(h, n, &net->ct.hash[i], hnnode)
			{
				...
				if(ct && (ct != ignored_conntrack))
				{
					...
					{
						tmpct = ((ct->timeout.expires-(jiffies-begin_jiffies)) > longest_expires)?ct:tmpct;
		...
		if(tmpct)
		{
			printk("Early drop connection: protocol:%d SRC[%d.%d.%d.%d:%d] GW[%d.%d.%d.%d:%d]DST[%d.%d.%d.%d:%d]\n",tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.protonum,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip>>24)&0xff,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip>>16)&0xff,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip>>8)&0xff,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u3.ip>>0)&0xff,tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.src.u.all,(tmpct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip>>24)&0xff,(tmpct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip>>16)&0xff,(tmpct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip>>8)&0xff,(tmpct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u3.ip>>0)&0xff,tmpct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.all,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip>>24)&0xff,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip>>16)&0xff,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip>>8)&0xff,(tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u3.ip>>0)&0xff,tmpct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.all);
...
out:
	...
	if((maniptype == IP_NAT_MANIP_SRC)	//SNAT
		&& ((orig_tuple->dst.protonum==IPPROTO_TCP) ||(orig_tuple->dst.protonum==IPPROTO_UDP)))
	{
		extern int32 rtk_rg_apollo_naptExtPortGet(int isTcp,uint16 *pPort);
		extern int32 rtk_rg_apollo_naptExtPortFree(int isTcp,uint16 port);
		/* Keep Linux NAT core and RG forwarding engine using separated NAPT external port. */
		int i=0;
		unsigned short *pPort;
		int isTCP;
		int ret;
		//printk("\n====>Tuple:: [dir:%s] [src addr:%d.%d.%d.%d] [src port:%d] [proto:%d] [dst addr:%d.%d.%d.%d] [dst port:%d]\n",tuple->dst.dir?"Orig":"REPLY",tuple->src.u3.ip>>24,(tuple->src.u3.ip>>16)&0xff,(tuple->src.u3.ip>>8)&0xff,tuple->src.u3.ip&0xff,tuple->src.u.all,tuple->dst.protonum,tuple->dst.u3.in.s_addr>>24,(tuple->dst.u3.in.s_addr>>16)&0xff,(tuple->dst.u3.in.s_addr>>8)&0xff,tuple->dst.u3.in.s_addr&0xff,tuple->dst.u.all);
		...
		{
			if(i++>65535) break;
			//printk("\n@@@@@@@@ [%s %d] Get external port:%d\n",__FUNCTION__,__LINE__,*pPort);
			ret = rtk_rg_apollo_naptExtPortGet(isTCP,pPort);
			if(ret==0) //Port is available
			{
				//printk("PS find available port:%d\n",tuple->src.u.all);
			...
			{
				//printk("port in used:%d\n",*pPort);
				*pPort=((*pPort+1)&0xffff);
			}
		}
		//printk("====>Final decided port:%d[0x%x]\n",*pPort,*pPort);
	}
	else //DNAT
	{
