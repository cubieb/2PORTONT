...
// Kaohj -- patched from newer version
static inline int ebt_basic_match(struct ebt_entry *e, const struct sk_buff *skb,
   const struct net_device *in, const struct net_device *out)
{
	...
	#if 1
	if(FWINV2(ebt_dev_check_switch_port(e->in, skb->switch_port), EBT_IIN) &&
		FWINV2(ebt_dev_check(e->in, in), EBT_IIN))
		return 1;
	#else
