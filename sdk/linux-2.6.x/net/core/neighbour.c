...
#if defined(CONFIG_RG_NAPT_ARP_AUTO_LEARN)
...
int rtl869x_checkArp(struct neighbour *n)
{
	int ret=-1;
	int arp_index=-1;
	rtk_rg_arpInfo_t arpInfo;
	//printk("%s-%d primary_key:0x%x\n",__func__,__LINE__, htonl(*((__u32 *)n->primary_key)));	
	arpInfo.arpEntry.ipv4Addr=htonl(*((__u32 *)n->primary_key));
	ret=rtk_rg_arpEntry_find(&arpInfo,&arp_index);
	if(ret==RT_ERR_RG_OK) 
	{
		//printk("arp entry in hardware, don't kill software entry\n");
		//printk("%s-%d arp index:%d\n",__func__,__LINE__,arp_index);
	}
	return ret;
}
#endif
		...
		if (atomic_read(&n->refcnt) == 1 &&
		    (state == NUD_FAILED ||
		     time_after(now, n->used + n->parms->gc_staletime))) {
				...
				int ret;
				if (state & NUD_STALE) {
					ret = rtl8676_setArp(n, ARP_CHECK);
					if(ret==0){
						write_unlock(&n->lock);
...
#endif
#ifdef CONFIG_RG_NAPT_ARP_AUTO_LEARN
//printk("%s-%d, check asic arp entry\n",__func__,__LINE__);
				if (state & NUD_STALE){
					int ret=-1;
					ret = rtl869x_checkArp(n);
					if(ret == 0){
						write_unlock(&n->lock);
						goto next_elt;						
					}
				}
//printk("%s-%d, check asic arp entry done\n",__func__,__LINE__);
#endif
#if 0 // defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER) && defined(CONFIG_RTL_LAYERED_DRIVER_L3)
				if (state & NUD_VALID) {
					unsigned int tval = rtl8676_setArp(n, ARP_SYNC); 
					if (tval > 0)
					{
						n->used = jiffies;
						n->dead=0;
...
#if 0 // defined(CONFIG_RTL_HARDWARE_NAT) && defined(CONFIG_RTL_LAYERED_DRIVER)	&& defined(CONFIG_RTL_LAYERED_DRIVER_L3)
	if (state & (NUD_REACHABLE | NUD_DELAY)) {

		tval = rtl8676_setArp(neigh, ARP_SYNC);
		if (tval > 0) 
		{
