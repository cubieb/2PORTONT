	...
	int rc;
        ...
        if(DumpProtocolStackTx_debug)
        {
                ...
                if(skb_mac_header_was_set(skb))
                {
                        ...
                        if(skb->mac_header > 0)
                                total_len = skb->len + (skb->data-skb->mac_header);
                }
                else
                {
                        data = skb->data;
...
int rteFastForwarding(struct sk_buff *skb)
{
        if(fp_on==2 || (fp_on==1 && skb->dev->priv_flags & IFF_DOMAIN_WAN))
        {
...
int netif_rx(struct sk_buff *skb)
{
	...
	if(skb->dev->priv_flags & IFF_RSMUX) {
		...
		if(smux_pkt_recv(skb, skb->dev) == 0){	//make sure skb not been freed
