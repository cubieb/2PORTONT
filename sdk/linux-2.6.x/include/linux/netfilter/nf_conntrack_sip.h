...
extern rwlock_t ip_qos_rtp_lock;
#define RTP_TIME_OUT (2* 60* HZ)
struct rtp_struct {
struct list_head list;
//add by ramen 20090408
struct timer_list timer;
__be32 dip ;
__be16 dport ;
unsigned char flag;//0--can delete,1--shouldn't been deleted,just for expection conntrack
unsigned char use;
unsigned char dir;
};
extern struct rtp_struct *qos_rtp_alloc(unsigned int ip,unsigned short port,unsigned char dir);
extern int qos_rtp_list_add(struct rtp_struct *rs);
extern struct rtp_struct *qos_rtp_list_find(unsigned int ip,unsigned short port);
extern void  qos_rtp_set_flag(struct rtp_struct *rs,unsigned char flag);
extern int qos_rtp_list_rm(struct rtp_struct *rs);
