...
void __init bsp_timer_init(void)
{
	...
	REG32(BSP_TC0DATA) = ( (BSP_MHZ)*((BSP_DIVISOR)/HZ)) << BSP_TCD_OFFSET;
...
void inline bsp_timer_ack_2(void)
{
    REG32(BSP_TCIR) |= BSP_TC1IP;
}

void __init rlx_setup_timer2(void) {
	if (REG32(BSP_TCIR) & BSP_TC1IP)
		REG32(BSP_TCIR) |= BSP_TC1IP;
	REG32(BSP_TC1DATA) = ( (BSP_MHZ)*((BSP_DIVISOR)/250)) << BSP_TCD_OFFSET;
	rlx_clockevent_init_2(BSP_TC1_IRQ);
