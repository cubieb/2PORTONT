...
static void board_11_handle_set(int which, int op) {
...
static void board_11_handle_init(void) {
	//rtl8367b reset
	GPIO_SET(GPIO_H_1, LED_ON);
	mdelay(15);
	GPIO_SET(GPIO_H_1, LED_OFF);
	mdelay(1000);
	
	board_11_handle_set(LED_INTERNET_GREEN, LED_OFF);
#if defined (CONFIG_RTL8192CD) || defined (CONFIG_RTL8192CD_MODULE)
	board_11_handle_set(LED_WPS_GREEN, LED_OFF);
	board_11_handle_set(LED_WPS_RED, LED_OFF);
	board_11_handle_set(LED_WPS_YELLOW, LED_OFF);
#endif
#ifdef CONFIG_SW_USB_LED1
	board_11_handle_set(LED_USB_1, LED_OFF);
#endif //CONFIG_SW_USB_LED1

};

static struct led_operations board_11_operation = {
	.name = "board_11",
	.handle_init = board_11_handle_init,
	.handle_set = board_11_handle_set,
};


static void board_11_pb_init(void) {
};

static int board_11_pb_is_pushed(int which) {
	switch(which) {
...
static struct pushbutton_operations board_11_pb_op = {
	.handle_init = board_11_pb_init,
	.handle_is_pushed = board_11_pb_is_pushed,
};

static int __init board_11_led_init(void) {
#ifndef CONFIG_JTAG_USABLE
	unsigned int sicr;
...
#endif //CONFIG_JTAG_USABLE
	REG32(BSP_MISC_PINMUX) &= ~(BSP_LEDP0_GPF0 | BSP_UR0_GPG0G1 | BSP_USBLED1_GPF6);
	if (IS_8676V2)
	...
	led_register_operations(&board_11_operation);
	pb_register_operations(&board_11_pb_op);
	return 0;
}

static void __exit board_11_led_exit(void) {
}


module_init(board_11_led_init);
module_exit(board_11_led_exit);
