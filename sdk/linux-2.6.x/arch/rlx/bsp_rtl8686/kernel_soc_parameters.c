...
__initdata static part_info_t part_info[] = {
	{"config",         0, 0}, /* opt1    */
	{"UNUSED_opt2",    0, 0}, /* opt2    */
	{"UNUSED_opt3",    0, 0}, /* opt3    */
	{"UNUSED_opt4",    0, 0}, /* opt3    */
	{"linux",          0, 0}, /* kernel1 */
	{"UNUSED_kernel2", 0, 0}, /* kernel2    */
	{"rootfs",         0, 0}, /* rootfs1 */
	{"UNUSED_rootfs2", 0, 0}, /* rootfs2 */
	{"END",            0, 0}, /* end     */ 
...
__init void flash_layout_to_mtdparts(char *mtdparts_output) {
	...
	for(i = 0; i < ARRAY_SIZE(part_info); i++) {
		if(pi_ptr[i]->size != 0) {
			...
			strlcat(mtdparts_output, tmp_str, CL_SIZE);
		}
	}
