...
__IRAM_SYS_MIDDLE void bsp_irq_dispatch(void) {
	...
	int timer_irq = 0;
        extint0_ip |= REG32(BSP_GIMR0_1) & REG32(BSP_GISR0_1);
        extint1_ip |= REG32(BSP_GIMR1_1) & REG32(BSP_GISR1_1);
                ...
                case BSP_PERIPHERAL_IRQ:
                        ...
                        for(tmp1= PER_LO; tmp1 <= PER_HI; tmp1++)  {
                                if ((extint1_ip & (1<<tmp1)) && (dispatch_table[tmp1+IRQ_HI_BOUND+1]>0)) {
                                        extint1_ip &= ~(1<<tmp1);
                                        if(unlikely((tmp1 + IRQ_HI_BOUND + 1) == BSP_TC4_IRQ))
                                                timer_irq = 1;
                                        else
        ...
        if(timer_irq == 1)
                do_IRQ(dispatch_table[BSP_TC4_IRQ]);
