...
void __init bsp_timer_init(void) {
	if(SOC_ID == 0x0371){
		...
		REG32(BSP_TC5CTL) = 0x0; /* disable timer before setting CDBR */
	
		/* initialize timer registers */
		REG32(BSP_TC5CTL) |= (BSP_DIVISOR) << 0;
		REG32(BSP_TC5DATA) = (( (BSP_MHZ)*((BSP_DIVISOR)/HZ))/2) << 0;
	...
	if(SOC_ID == 0x0371) {
		...
		REG32(BSP_TC5CTL) |= BSP_TCEN | BSP_TCMODE_TIMER;
		REG32(BSP_TC5INT) |= BSP_TCIE;
	}
}
