...
*
******************************************************************************/
#define VOIP_SRC_IP		IP2LONG(10,16,2,65)
#define VOIP_DST_IP		IP2LONG(10,16,2,68)
...
******************************************************************************/
static int ActivatePCMIBSGeneration(void)
{
...
static int ActivatePlayIBSGeneration(void)
{
	Tac49xIbsStringAttr IbsAttr;
	long device = 0;
	long channel = 0;
	...
	IbsAttr.Signal[1].SystemCode = IBS_SYSTEM_CODE__CALL_PROGRESS_0_TO_15;
	IbsAttr.Signal[1].Digit = 1;

	Ac49xSendIbsString(device, channel, &IbsAttr);	//High
...
{
	static long FlagOnOff = 0;

	if(FlagOnOff == 0)
	{
		FlagOnOff = 1;
		ActivatePCMIBSGeneration();
	}
		
	ActivatePlayIBSGeneration();
...
{
	aclxx_demo_dev->SetupChannelAttr[channel].OpenOrUpdateChannel.IbsLevel_minus_dbm = 0;
		
	// Override CP Tone def
	Tac49xCallProgressDeviceConfigurationAttr *pAttr = 	&aclxx_demo_dev->SetupDeviceAttr.CallProgress;

	/* Dial Tone (ANSI TI.401-1993) - High Volume*/
	...
	/* Dial Tone (ANSI TI.401-1993) - Low Volume*/
	pAttr->Signal[1].ToneAFrequency                 = 150;
	pAttr->Signal[1].ToneB_OrAmpModulationFrequency = 440;
	...
	pAttr->Signal[1].ToneALevel                     = 40;
	pAttr->Signal[1].ToneBLevel                     = 40;
