...
//test IBS generation
	{	
		...
		IbsAttr.Signal[0].SystemCode = IBS_SYSTEM_CODE__DTMF/*IBS_SYSTEM_CODE__CALL_PROGRESS_0_TO_15*/;
		...
		for(i=0; i<10000; i++)
		{
			...
			if(ret>0)
			{
				if( ac49xPacket.u.ProprietaryPayload.ProprietaryHeader.Opcode == PACKET_CATEGORY__COMMAND_OR_STATUS_PACKET )
				{
					if( ac49xPacket.u.ProprietaryPayload.ProprietaryHeader.PacketCategory == STATUS_OR_EVENT_PACKET_OP_CODE__IBS_EVENT )
					{
						...
						printk("Digit-%d, System-%s",digit ,system);
					}
				}
			}
...
static int dspDev_setDefaults(void)
{
...

	{
		...
		#define CHANNEL1 1
		Tac49xIbsStringAttr IbsAttr;		
		Tac49xActivateOrDeactivate3WayConferenceConfigurationAttr *active3WCconf;
		
	
		printk("<<<<<<< Advanced CHANNEL 1,2 >>>>>>>\n");		
		Ac49xAdvancedChannelConfiguration(pTxPacketParams->Device, CHANNEL0, &dsp_dev->SetupChannelAttr[CHANNEL0].Advanced);
		Ac49xAdvancedChannelConfiguration(pTxPacketParams->Device, CHANNEL1, &dsp_dev->SetupChannelAttr[CHANNEL1].Advanced);

		printk("<<<<<<< Open CHANNEL 1,2 >>>>>>>\n");				
		Ac49xOpenChannelConfiguration(pTxPacketParams->Device, CHANNEL0, &dsp_dev->SetupChannelAttr[CHANNEL0].OpenOrUpdateChannel);
		Ac49xOpenChannelConfiguration(pTxPacketParams->Device, CHANNEL1, &dsp_dev->SetupChannelAttr[CHANNEL1].OpenOrUpdateChannel);

		printk("<<<<<<< Active PCM CHANNEL 1,2 >>>>>>>\n");				
		PCM_restart(CHANNEL0);
		PCM_restart(CHANNEL1);

		printk("<<<<<<< Active RTP CHANNEL 1,2 >>>>>>>\n");
		Ac49xActivateRegularRtpChannelConfiguration(pTxPacketParams->Device, CHANNEL0, &dsp_dev->SetupChannelAttr[CHANNEL0].ActivateOrUpdateRtp);
		Ac49xActivateRegularRtpChannelConfiguration(pTxPacketParams->Device, CHANNEL1, &dsp_dev->SetupChannelAttr[CHANNEL0].ActivateOrUpdateRtp);		
		
			
		printk("<<<<<<< 3 Way Conf Command : Mode1 Main=CH0, Mate=CH1 >>>>>>>\n");				

		active3WCconf = &dsp_dev->SetupChannelAttr[CHANNEL0].ActivateOrDeactivate3WayConferenceConfiguration;
		active3WCconf->Mode = _3_WAY_CONFERENCE_MODE__1;
		active3WCconf->ConferenceChannelMate = CHANNEL1;
		
		Ac49xActivateOrDeactivate3WayConferenceConfiguration(pTxPacketParams->Device, CHANNEL0, &dsp_dev->SetupChannelAttr[CHANNEL0].ActivateOrDeactivate3WayConferenceConfiguration );
	

		printk("<<<<<<< PLAY IBS CP on Mate Channel >>>>>>>\n");							
		IbsAttr.GeneratorRedirection = IBS_STRING_GENERATOR_REDIRECTION__INTO_DECODER_OUTPUT;
		IbsAttr.NumberOfDigits = 1;
		IbsAttr.OnDuration = 40000;
		IbsAttr.OffDuration = 0;
		IbsAttr.PauseDuration = 0;		
		IbsAttr.Signal[0].Pause = CONTROL__DISABLE;
		IbsAttr.Signal[0].SystemCode = IBS_SYSTEM_CODE__CALL_PROGRESS_0_TO_15;
		IbsAttr.Signal[0].Digit = 1;
		Ac49xSendIbsString(pTxPacketParams->Device, CHANNEL1, &IbsAttr);
		Ac49xSendIbsString(pTxPacketParams->Device, CHANNEL0, &IbsAttr);
