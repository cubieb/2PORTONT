...
<title><% multilang("13" "LANG_DSL_WAN"); %> <% multilang("212" "LANG_CONFIGURATION"); %></title>
...
function vcCheck()
{
 ...
 if (validateInt(document.adsl.vpi.value) == 0) { // Jenny,  buglist B056, check if input VPI is a decimal		
  alert('<% multilang("1947" "LANG_INVALID_VPI_VALUE_VPI_SHOULD_NOT_BE_A_DECIMAL"); %>');
 ...
 if ( validateKey(document.adsl.vpi.value) == 0 ||
  ...
  alert('<% multilang("1948" "LANG_INVALID_VPI_VALUE_YOU_SHOULD_SET_A_VALUE_BETWEEN_0_255"); %>');
  document.adsl.vpi.focus();
  return false;
 }

 if (validateInt(document.adsl.vci.value) == 0) { // Jenny,  buglist B056, check if input VCI is a decimal		
  alert('<% multilang("1949" "LANG_INVALID_VCI_VALUE_VCI_SHOULD_NOT_BE_A_DECIMAL"); %>');
 ...
 if ( validateKey(document.adsl.vci.value) == 0 ||
  ...
  alert('<% multilang("1950" "LANG_INVALID_VCI_VALUE_YOU_SHOULD_SET_A_VALUE_BETWEEN_0_65535"); %>');
  document.adsl.vci.focus();
  return false;
 }

 if ( (digitVPI == 0 && digitVCI == 0) ) {
  alert('<% multilang("1951" "LANG_INVALID_VPI_VCI_VALUE"); %>');
  document.adsl.vpi.focus();
  return false;
 }
...
function autopvcCheckClick()
{
 ...
 if (document.adsl.autopvc.checked == true) {
  ...
  alert('<% multilang("1952" "LANG_YOU_SHOULD_ENABLE_AUTO_PVC_SEARCH_FIRST"); %>');
  return false;
 }
}
...
function apply1q()
{
 if (!check1q(document.adsl.vid.value)) {
  alert('<% multilang("1953" "LANG_INVALID_VLAN_ID"); %>');
...
<BODY>
<blockquote>
<h2><font color="#0000FF"><% multilang("13" "LANG_DSL_WAN"); %><% multilang("212" "LANG_CONFIGURATION"); %></font></h2>
...
<table border="0" cellspacing="4" width="800">
 <tr><td><font size=2>
    <% multilang("228" "LANG_PAGE_DESC_CONFIGURE_PARAMETERS"); %><% multilang("229" "LANG_WAN_MODE"); %>
 </font></td></tr>
...
<table border=0 width="800" cellspacing=4 cellpadding=0>
 <tr>
  ...
  <td><font size=2><b><% multilang("88" "LANG_ENCAPSULATION"); %>: </b>
 ...
 <tr>
  <td>
   <font size=2><b><% multilang("237" "LANG_ADMIN_STATUS"); %>:</b>
   <input type="radio" value=1 name="chEnable" checked><% multilang("222" "LANG_ENABLE"); %>&nbsp;&nbsp;
   <input type="radio" value=0 name="chEnable"><% multilang("221" "LANG_DISABLE"); %></font>
  </td>
 </tr>
 <% ShowConnectionType() %>
</table>
...
<table>
 ...
 <td><font size=2><% multilang("258" "LANG_802_1P_MARK"); %>:
...
<input type="submit" value="<% multilang("276" "LANG_MODIFY"); %>" name="modify" onClick="return vcCheck()">
<BR>
<BR>

<table border="0" width=900>
 <tr><font size=2><b><% multilang("273" "LANG_CURRENT_ATM_VC_TABLE"); %>:</b></font></tr>
 <% atmVcList2(); %>

</table>
<br>
<input type="submit" value="<% multilang("198" "LANG_DELETE_SELECTED"); %>" name="delvc" onClick="return deleteClick()">
