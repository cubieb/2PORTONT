...
<title><% multilang("1101" "LANG_CFM_802_1AG_ACTION"); %></title>
...
function update_ui()
{
 with(document.dot1agAction)
 {
  if(processing == true)
  {
   document.getElementById('status').innerHTML = "<b><% multilang("3" "LANG_STATUS"); %>:</b> <% multilang("1102" "LANG_PROCESSING"); %>";
   start.disabled = true;
  }
  else
  {
   document.getElementById('status').innerHTML = "<b><% multilang("3" "LANG_STATUS"); %>:</b> <% multilang("1103" "LANG_READY"); %>";
...
function startClick()
{
 ...
 if(document.dot1agAction.action[0].checked == true)
 {
  ...
  {
   alert('<% multilang("2086" "LANG_COUNT_CANNOT_BE_EMPTY"); %>');
   document.dot1agAction.count.focus();
   return false;
  }
  if (checkDigit(document.dot1agAction.count.value) == 0)
  {
   alert('<% multilang("2087" "LANG_COUNT_SHOULD_BE_A_POSITIVE_NUMBER"); %>');
   document.dot1agAction.count.focus();
   document.dot1agAction.count.focus();
   return false;
  }
...
<body onLoad="on_init();">
<blockquote>
<h2><font color="#0000FF"><% multilang("1101" "LANG_CFM_802_1AG_ACTION"); %></font></h2>

<form action=/boaform/formDot1agAction method=POST name="dot1agAction">

<font size=2><b><% multilang("1104" "LANG_PLEASE_SELECT_ONE_OF_FOLLOWING_ENTRY"); %>:</b></font>
...
<tr>
 <td width=30%><font size=2><% multilang("629" "LANG_ACTION"); %>:&nbsp;&nbsp;</font></td>
 <td><input type="radio" name="action" value="lbm" onClick="update_ui()"><font size=2> <% multilang("1108" "LANG_SEND_LBM"); %></font></td>
 <td><input type="radio" name="action" value="ltm" onClick="update_ui()"><font size=2> <% multilang("1109" "LANG_SEND_LTM"); %></font></td>
</tr>
<tr id="tr_multicast">
 <td><font size=2><% multilang("1110" "LANG_USE_MULTICAST"); %>:</td>
 <td><input type="checkbox" name="multicast" value=1 onClick="update_ui()"></td>
</tr>
<tr>
    <td width=30%><font size=2><% multilang("1111" "LANG_DESTINATION_MAC_ADDRESS"); %>:&nbsp;&nbsp;</td>
    <td><input type="text" name="mac" size="20" maxlength="17"></td>
</tr>
<tr id="tr_count">
    <td width=30%><font size=2><% multilang("1112" "LANG_LBM_COUNT"); %>:&nbsp;&nbsp;</td>
    <td><input type="text" name="count" size="20" maxlength="5"></td>
</tr>
</table>

<input type="button" value="<% multilang("449" "LANG_START"); %>" name="start" onClick="startClick()">&nbsp;&nbsp;

<p>
<hr size=1 noshade align=top>
<div id="status"></div>
<b><% multilang("1113" "LANG_RESULT"); %>:</b>
