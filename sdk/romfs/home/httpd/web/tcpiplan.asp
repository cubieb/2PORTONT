...
<title><% multilang("110" "LANG_LAN_INTERFACE_SETTINGS"); %></title>
 ...
 with (document.forms[0])
 {
  ...
  if((ip_version1.selectedIndex != 0) && (ipv6_mode1[1].checked)){ //IPv6 enabled && mode is manual
   if (ipv6_addr1.value =="" || ipv6_addr1.value =="::") {
    alert("<% multilang("2331" "LANG_LAN_IPV6_ADDRESS_CANNOT_BE_EMPTY_FORMAT_IS_IPV6_ADDRESS_FOR_EXAMPLE_3FFE_501_FFFF_100_1"); %>");
    ipv6_addr1.focus();
    return false;
   } else {
    if ( validateKeyV6IP(ipv6_addr1.value) == 0) {
     alert("<% multilang("2332" "LANG_INVALID_LAN_IPV6_IP"); %>");
     ipv6_addr1.focus();
     return false;
    }
   }
   ...
   if (ipv6_prefix1.value =="") {
    alert("<% multilang("2333" "LANG_LAN_IPV6_ADDRESS_IPV6_PREFIX1_CANNOT_BE_EMPTY_VALID_NUMBER_IS_0_127"); %>");
    ipv6_prefix1.focus();
    return false;
   } else {
    var prefixInt = parseInt(ipv6_prefix1.value);
    if ( prefixInt>127 ||prefixInt<0) {
     alert("<% multilang("2334" "LANG_INVALID_LAN_IPV6_PREVIX"); %>");
     ipv6_prefix1.focus();
     return false;
    }
   }
...
<body>
<blockquote>
<h2><font color="#0000FF"><% multilang("110" "LANG_LAN_INTERFACE_SETTINGS"); %></font></h2>

<form action=/boaform/formTcpipLanSetup method=POST name="tcpip">
<table border=0 width="500" cellspacing=4 cellpadding=0>
  <tr><td><font size=2>
    <% multilang("111" "LANG_PAGE_DESC_CONFIG_DEVICE_LAN_INTERFACE"); %>
  </font></td></tr>
...
<table border=0 width="500" cellspacing=4 cellpadding=0>
 <input type=hidden name="wlanDisabled" value=<% wlanStatus(); %>>
  <tr>
      <td width="30%"><font size=2><b><% multilang("66" "LANG_INTERFACE"); %><% multilang("619" "LANG_NAME"); %>:</b></td>
 ...
 <tr id=tr_ipv6_mode1 <% checkWrite("rg_only_function"); %>>
  <td width=150><font size=2><b><% multilang("116" "LANG_IPV6_ADDRESS_MODE"); %>:</b></td>
  <td width=350><font size=2>
   <input type="radio" name="ipv6_mode1" value=0 OnChange="ipv6_mode1_change()" <% checkWrite("lan_ipv6_mode_auto"); %> ><% multilang("151" "LANG_AUTO"); %>
   <input type="radio" name="ipv6_mode1" value=1 OnChange="ipv6_mode1_change()" <% checkWrite("lan_ipv6_mode_manual"); %>><% multilang("406" "LANG_MANUAL"); %>
  </td>
 </tr>
  <tr id=tr_ipv6_addr1 <% checkWrite("rg_only_function"); %>>
      <td width="30%"><font size=2><b><% multilang("97" "LANG_IPV6_ADDRESS"); %>:</b></td>
  ...
  <tr id=tr_ipv6_prefix1 <% checkWrite("rg_only_function"); %>>
      <td width="30%"><font size=2><b><% multilang("117" "LANG_IPV6_PREFIX_LENGTH"); %>:</b></td>
      <td width="70%"><input type="text" name="ipv6_prefix1" size="5" maxlength="5" value=0></td>
  </tr>
   <!-- Hide VID setting temporarily -->
 <tr style="display:none">
  <td width="30%"><font size=2><b><% multilang("118" "LANG_VLAN_ID"); %>:</b></td>
 ...
 <tr <% checkWrite("rg_only_function"); %>>
  <td width="30%"><font size=2><b><% multilang("119" "LANG_IP_VERSION"); %>:</b></td>
  <td width="70%">
