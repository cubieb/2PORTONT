...
<title>DNS <% multilang("212" "LANG_CONFIGURATION"); %></title>
...
function saveClick()
{
 ...
 if(sji_checkip(document.dns.dns1.value) == false){
  ...
  alert('<% multilang("2072" "LANG_IP_ADDRESS_IS_INVALID"); %>');
  return;
 }
...
//	if (document.dns.dns2.value=="") {
 if (document.dns.dns2.value=="") {
  if (document.dns.dns3.value=="") { // Jenny,  buglist B059, dns2 couldn't be empty if dns3 is not empty
   ...
   alert('<% multilang("2073" "LANG_THE_SECOND_DNSV4_NAME_SERVER_S_IP_IS_EMPTY"); %>');
...
function saveClickIpv6()
{
 if (document.dns.dnsv61.value=="") {
  ...
  alert('<% multilang("2074" "LANG_THE_FIRST_DNSV6_NAME_SERVER_S_IP_IS_EMPTY"); %>');
  return false;
 }

 if(document.dns.dnsv61.value != "" ){
  if (! isUnicastIpv6Address( document.dns.dnsv61.value) ){
   alert('<% multilang("2075" "LANG_INVALID_THE_FIRST_DNSV6_NAME_SERVER_S_IP"); %>');
   document.dns.dnsv61.focus();
   return false;
  }
 }
 ...
 if (document.dns.dnsv62.value=="") {
  if (document.dns.dnsv63.value=="") // Jenny,  buglist B059, dns2 couldn't be empty if dns3 is not empty
   ...
   alert('<% multilang("2076" "LANG_THE_SECOND_DNSV6_NAME_SERVER_S_IP_IS_EMPTY"); %>');
 ...
 if(document.dns.dnsv62.value != "" ){
  if (! isUnicastIpv6Address( document.dns.dnsv62.value) ){
   alert('<% multilang("2077" "LANG_INVALID_THE_SECOND_DNSV6_NAME_SERVER_S_IP"); %>');
   document.dns.dnsv62.focus();
   return false;
  }
 }
 ...
 if(document.dns.dnsv63.value != "" ){
  if (! isUnicastIpv6Address( document.dns.dnsv63.value) ){
   alert('<% multilang("2078" "LANG_THE_THIRD_DNSV6_NAME_SERVER_S_IP_IS_EMPTY"); %>');
 ...
 if(document.dns.enableDnsBind.checked)
  {
   if(<% checkWrite("WanPvcRouter"); %>) {
    alert('<% multilang("2079" "LANG_NO_ROUTER_WAN_INTERFACE"); %>');
...
<blockquote>
  <h2><font color="#0000FF">DNS <% multilang("212" "LANG_CONFIGURATION"); %></font></h2>
  <DIV align="left" style="padding-left:20px; padding-top:5px">
   <form id="form" action=/boaform/admin/formDNS method=POST name="dns">
   <tr><font size=2>
    <% multilang("312" "LANG_PAGE_DESC_CONFIGURE_DNS_SERVER_IP_ADDRESSES"); %>
   </tr>
   ...
   <tr>
      <tr>
        <td width="40%" colspan="3"><font size=2>
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="dnsAuto" name="dnsMode" <% checkWrite("dns0"); %> onClick="autoDNSclicked()"><% multilang("313" "LANG_ATTAIN_DNS_AUTOMATICALLY"); %>
        </td>
        <td></td>
      </tr>
      <tr>
        <td width="40%" colspan="3"><font size=2>
          &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" value="dnsManual" name="dnsMode" <% checkWrite("dns1"); %> onClick="manualDNSclicked()"><% multilang("314" "LANG_SET_DNS_MANUALLY"); %>
        </td>
