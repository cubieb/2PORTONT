...
<title>DHCP <% multilang("320" "LANG_SETTINGS"); %></title>
...
function showDhcpSvr(ip)
{
 ...
 if (document.dhcpd.dhcpdenable[0].checked == true)
  ...
  document.getElementById('displayDhcpSvr').innerHTML=
   ...
   '<% multilang("297" "LANG_PAGE_DESC_CONFIGURE_DHCP_SERVER_IP_ADDRESS"); %>'+
   '</font></tr>'+
   '<tr><td><hr size=1 noshade align=top></td></tr>'+
   '</table>'+
   '<table border=0 width="500" cellspacing=4 cellpadding=0>'+
   '<tr>'+
   '<td width="40%"><font size=2><b>DHCP <% multilang("85" "LANG_SERVER"); %> <% multilang("83" "LANG_IP_ADDRESS"); %>:</b></font></td>'+
  ...
  html=
   '<table border=0 width="500" cellspacing=4 cellpadding=0>'+
   '<tr><font color="#000000" size=2>'+
   '<% multilang("298" "LANG_PAGE_DESC_ENABLE_DHCP_SERVER"); %>'+


   '</font></tr>'+
   '<tr><td><hr size=1 noshade align=top></td></tr>'+
   '<tr><td><font color="#000000" size=2><b><% multilang("6" "LANG_LAN"); %> <% multilang("83" "LANG_IP_ADDRESS"); %>: </b><% getInfo("dhcplan-ip"); %>&nbsp;&nbsp;&nbsp;'+
   '<b><% multilang("84" "LANG_SUBNET_MASK"); %>: </b><% getInfo("dhcplan-subnet"); %>'+
   '</font></td></tr>'+
   '</table>'+
   '<table border=0 width="500" cellspacing=4 cellpadding=0>'+
   '<tr>'+
  ...
  html+=
   '<input type="button" value="<% multilang("300" "LANG_SHOW_CLIENT"); %>" name="dhcpClientTbl" onClick="dhcpTblClick(\'/dhcptbl.asp\')" >'+
   '</td>'+
   '</tr>';
  ...
  html += '<tr>'+
   '<td width="30%"><font size=2><b><% multilang("301" "LANG_MAX_LEASE_TIME"); %>:</b></td>'+
   '<td width="70%"><font size=2>'+
   '<input type="text" name="ltime" size=10 maxlength=9 value="<% getInfo("lan-dhcpLTime"); %>"><b> <% multilang("302" "LANG_SECONDS"); %></b><b> (<% multilang("303" "LANG_MINUS_1_INDICATES_AN_INFINITE_LEASE"); %>)</b>'+
   '</td>'+
   '</tr>'+
   '<tr>'+
   '<td width="30%"><font size=2><b><% multilang("364" "LANG_DOMAIN"); %><% multilang("619" "LANG_NAME"); %>:</b></td>'+
   '<td width="70%">'+
   '<input type="text" name="dname" size=32 maxlength=29 value="<% getInfo("lan-dhcpDName"); %>">'+
   '</td>'+
   '</tr>'+
   '<tr>'+
   '<td width="30%"><font size=2><b><% multilang("304" "LANG_GATEWAY_ADDRESS"); %>:</b></td>'+
  ...
  html +=
   ...
   '<td width="30%"><font size=2><b><% multilang("309" "LANG_DNS_OPTION"); %>:</b></td>'+
   '<td width=70%><input type=radio name=dhcpdns value=0 onClick=showdns()><% multilang("310" "LANG_USE_DNS_RELAY"); %>&nbsp;&nbsp;'+
   '<input type=radio name=dhcpdns value=1 onClick=showdns()><% multilang("311" "LANG_SET_MANUALLY"); %>&nbsp;&nbsp;</td>'+
   '</tr></table></div>'+
   ...
   '<input type="button" value="<% multilang("306" "LANG_PORT_BASED_FILTER"); %>" name="macIpTbl" onClick="macIpClick(\'/portBaseFilterDhcp.asp\')">'+
   '<input type="button" value="<% multilang("305" "LANG_MAC_BASED_ASSIGNMENT"); %>" name="macIpTbl" onClick="macIpClick(\'/macIptbl.asp\')">';
...
function saveChanges()
{
  ...
  alert('<% multilang("2019" "LANG_INVALID_DOMAIN_NAME"); %>');
 ...
 if (document.dhcpd.dhcpRangeStart.value=="") {
  alert('<% multilang("2020" "LANG_PLEASE_INPUT_DHCP_IP_POOL_RANGE_"); %>');
 ...
 if ( validateKey( document.dhcpd.dhcpRangeStart.value ) == 0 ) {
  alert('<% multilang("2021" "LANG_INVALID_DHCP_CLIENT_START_RANGE_IT_SHOULD_BE_1_254"); %>');
  document.dhcpd.dhcpRangeStart.value = document.dhcpd.dhcpRangeStart.defaultValue;
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 ...
 if ( !checkSubnet(document.dhcpd.lan_ip.value,document.dhcpd.lan_mask.value,document.dhcpd.dhcpRangeStart.value)) {
  alert('<% multilang("2022" "LANG_INVALID_DHCP_CLIENT_START_ADDRESSIT_SHOULD_BE_LOCATED_IN_THE_SAME_SUBNET_OF_CURRENT_IP_ADDRESS"); %>');
  document.dhcpd.dhcpRangeStart.value = document.dhcpd.dhcpRangeStart.defaultValue;
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 if (document.dhcpd.dhcpRangeEnd.value=="") {
  alert('<% multilang("2023" "LANG_PLEASE_INPUT_DHCP_IP_POOL_RANGE"); %>');
 ...
 if ( validateKey( document.dhcpd.dhcpRangeEnd.value ) == 0 ) {
  alert('<% multilang("2024" "LANG_INVALID_DHCP_CLIENT_END_ADDRESS_RANGE_IT_SHOULD_BE_1_254"); %>');
  document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
  document.dhcpd.dhcpRangeEnd.focus();
  return false;
 }
 if ( !checkDigitRange(document.dhcpd.dhcpRangeEnd.value,1,1,254) ) {
  alert('<% multilang("2025" "LANG_INVALID_DHCP_CLIENT_END_RANGE_IT_SHOULD_BE_1_254"); %>');
  document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
  document.dhcpd.dhcpRangeEnd.focus();
  return false;
 }
 if ( !checkSubnet(document.dhcpd.lan_ip.value,document.dhcpd.lan_mask.value,document.dhcpd.dhcpRangeEnd.value)) {
  alert('<% multilang("2026" "LANG_INVALID_DHCP_CLIENT_END_ADDRESSIT_SHOULD_BE_LOCATED_IN_THE_SAME_SUBNET_OF_CURRENT_IP_ADDRESS"); %>');
  document.dhcpd.dhcpRangeEnd.value = document.dhcpd.dhcpRangeEnd.defaultValue;
  document.dhcpd.dhcpRangeEnd.focus();
  return false;
 }
 if ( parseInt(document.dhcpd.dhcpRangeStart.value, 10) >= parseInt(document.dhcpd.dhcpRangeEnd.value, 10) ) {
  alert('<% multilang("2027" "LANG_INVALID_DHCP_CLIENT_ADDRESS_RANGEENDING_ADDRESS_SHOULD_BE_GREATER_THAN_STARTING_ADDRESS"); %>');
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }
 }
 ...
 if (!checkInputIP(document.dhcpd.lan_ip.value)) {
  alert('<% multilang("2028" "LANG_INVALID_IP_POOL_RANGE_LAN_IP_MUST_BE_EXCLUDED_FROM_DHCP_IP_POOL"); %>');
  document.dhcpd.dhcpRangeStart.focus();
  return false;
 }

 if ( document.dhcpd.ltime.value=="") {
  alert('<% multilang("2029" "LANG_PLEASE_INPUT_DHCP_LEASE_TIME"); %>');
  document.dhcpd.ltime.focus();
  return false;
 }
 if ( validateKey_leasetime( document.dhcpd.ltime.value ) == 0 ) {
  alert('<% multilang("2030" "LANG_INVALID_DHCP_SERVER_LEASE_TIME_NUMBER"); %>');
  document.dhcpd.ltime.value = document.dhcpd.ltime.defaultValue;
  document.dhcpd.ltime.focus();
  return false;
 }
 if ( !checkDigitRange_leaseTime(document.dhcpd.ltime.value, -1) ) {
  alert('<% multilang("2031" "LANG_INVALID_DHCP_SERVER_LEASE_TIME"); %>');
  document.dhcpd.ltime.value = document.dhcpd.ltime.defaultValue;
  document.dhcpd.ltime.focus();
  return false;
 }
 ...
 /*if (document.dhcpd.ip.value=="") {
  ...
  if (document.dhcpd.dns1.value=="") {
   alert('<% multilang("2039" "LANG_ENTER_DNS_VALUE"); %>');
...
<h2><font color="#0000FF">DHCP <% multilang("320" "LANG_SETTINGS"); %></font></h2>
...
<table border=0 width="500" cellspacing=4 cellpadding=0>
  <tr><td><font color="#000000" size=2>
    <% multilang("295" "LANG_PAGE_DESC_CONFIGURE_DHCP_SERVER_RELAY"); %>
  </font></td></tr>
  <tr><td><hr size=1 noshade align=top></td></tr>
...
<table border=0 width="500" cellspacing=4 cellpadding=0>
  <tr>
  <td><font size=2><b>DHCP <% multilang("125" "LANG_MODE"); %>: </b>
  <% checkWrite("dhcpMode"); %>
