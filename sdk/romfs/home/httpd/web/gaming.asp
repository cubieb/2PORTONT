...
<title><% multilang("24" "LANG_PORT_TRIGGERING"); %><% multilang("212" "LANG_CONFIGURATION"); %></title></head>
<script type="text/javascript" src="share.js"></script>
<script language=javascript>
 function addClick()
 {
   if (formname.ip.value=="") {
  alert('<% multilang("2136" "LANG_IP_ADDRESS_CANNOT_BE_EMPTY"); %>');
  formname.ip.focus();
  return false;
   }
  ...
  alert('<% multilang("2137" "LANG_INVALID_IP_ADDRESS_RANGE_IN_1ST_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
   if ( !checkDigitRange(formname.ip.value,2,0,255) ) {
  alert('<% multilang("2138" "LANG_INVALID_IP_ADDRESS_RANGE_IN_2ND_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
   if ( !checkDigitRange(formname.ip.value,3,0,255) ) {
        alert('<% multilang("2139" "LANG_INVALID_IP_ADDRESS_RANGE_IN_3RD_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
   if ( !checkDigitRange(formname.ip.value,4,1,254) ) {
        alert('<% multilang("2140" "LANG_INVALID_IP_ADDRESS_RANGE_IN_4TH_DIGIT"); %>');
  formname.ip.focus();
  return false;
   }
 ...
 function formLoad()
 {
  formname.gamelist.options[0] = new Option ("<% multilang("560" "LANG_SELECT_GAME"); %>", -1);
