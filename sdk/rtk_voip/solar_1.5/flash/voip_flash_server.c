...
int voip_flash_server_start()
{
	...
	voip_feature |= ( uint64 )stVoipCfg.enable << 32;
	if ( ( voip_feature & VOIP_FLASH_FEATURE_MASK ) != 
		 ( VOIP_FLASH_FEATURE & VOIP_FLASH_FEATURE_MASK ) )
	{
		fprintf(stderr, "VoIP Feature is invalid (%llx vs %llx)\n",
			( uint64 )VOIP_FLASH_FEATURE, voip_feature);
		return -1;
	}
#endif
