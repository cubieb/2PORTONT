			   ...
			   unsigned char **outpacket, struct resolv_answer *a)
{
	...
	while (retries++ < MAX_RETRIES) {
		...
		for (j=0;j<h.ancount;j++)
		{
			...
			if (a_first.dotted) free(a_first.dotted);
			goto again;
		    }
			...
			else if (a->atype != T_SIG && a_first.dotted == NULL)
			{
		    	/* For all but T_SIG, accept first answer */
				memcpy(&a_first, a, sizeof(a_first));
			}
			else
			{
		    	DPRINTF("skipping pos %d, type=%d\n", i, a->atype);
			    free(a->dotted);
			}
