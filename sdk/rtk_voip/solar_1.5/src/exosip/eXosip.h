 ...
 */
void eXosip_force_proxy(const int nPort, const char *proxyurl);
...
int   __eXosip_lock(char *file, int line);
int   __eXosip_unlock(char *file, int line);

#define eXosip_lock() __eXosip_lock(__FILE__, __LINE__)
#define eXosip_unlock() __eXosip_unlock(__FILE__, __LINE__)
 ...
 */
int   eXosip_build_initial_invite(int nPort, osip_message_t **invite, char *to,
				  char *from, osip_list_t *routes, char *subject);
/**
 * Initiate a call.
 * 
 ...
 */
int eXosip_initiate_call(int nPort, osip_message_t *invite, void *reference, void *sdp_context_reference, 
	char *local_sdp_port);
 ...
 * @param body 			  The body.
 */
int eXosip_initiate_call_with_body(int nPort, osip_message_t *invite, const char *body_type, 
...
int eXosip_off_hold_call(int jid, char *rtp_ip, int port, int sessoin_version);
 ...
 */
int eXosip_register_init (int nPort, char *from, char *proxy, char *contact, osip_list_t *routes);
 ...
 */
int eXosip_publish(int nPort, osip_message_t *message, const char *sip_if_match);
...
typedef enum eXosip_event_type_t {
  ...
  EXOSIP_CALL_UPDATE,          /* call update (codec or other issue)    */
  ...
  // refer response
  EXOSIP_CALL_REFER_REQUESTFAILURE,
