...
#ifdef MALLOC_DEBUG
int __eXosip_dialog_set_200ok(eXosip_dialog_t *jd, osip_message_t *_200Ok, char *file, int line)
{
  int i;
  if (jd==NULL) return -1;
  ...
  i = __osip_message_clone(_200Ok, &(jd->d_200Ok), file, line);
  if (i!=0) {
    return -1;
  }
  return 0;
...
{
OSIP_TRACE (osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL, "eXosip_dialog_free: jd=%x\n", jd));
...
OSIP_TRACE (osip_trace(__FILE__, __LINE__, OSIP_INFO1, NULL, "eXosip_dialog_free: jd->d_200ok=%x\n", jd->d_200Ok));

  osip_message_free(jd->d_200Ok);
  osip_message_free(jd->d_ack);

#if 1
  // rock: force release ixt before destroy dialog
  osip_stop_retransmissions_from_dialog(eXosip.j_osip, jd->d_dialog);
#endif
