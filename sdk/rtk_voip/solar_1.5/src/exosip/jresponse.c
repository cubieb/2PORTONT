			       ...
			       int status, osip_message_t *request)
{
  ...
  osip_message_set_allow(response, "INFO");
	...
	{
		osip_message_set_supported(response, "replaces");
	}
#endif
...
int
complete_answer_that_establish_a_dialog(int nPort, osip_message_t *response, osip_message_t *request)
{
  int i;
  int pos=0;
  char contact[1000];
...
bbbbbbbbbbbbbbbbbbb
  eXosip_guess_ip_for_via(eXosip.ip_family, locip, 49);
	...
	if(eXosip_get_transport_type(request)==TRANSPORT_TLS)
		strcpy(transport_type, "TLS");
	else
		strcpy(transport_type, "UDP");
	
  if (eXosip.answer_contact[0])
		snprintf(contact,1000, "%s;transport=%s", eXosip.answer_contact, transport_type);  
  else if (request->to->url->username==NULL)
	  snprintf(contact,1000, "<sip:%s:%s;transport=%s>", locip, eXosip.localports[nPort], transport_type);
   else
	  snprintf(contact,1000, "<sip:%s@%s:%s;transport=%s>", request->to->url->username,
		   locip, eXosip.localports[nPort], transport_type);
#else /*CONFIG_RTK_VOIP_SIP_TLS*/
  if (eXosip.answer_contact[0])
    snprintf(contact,1000, "%s", eXosip.answer_contact);
  else if (request->to->url->username==NULL)
    snprintf(contact,1000, "<sip:%s:%s>", locip, eXosip.localports[nPort]);
  else
    snprintf(contact,1000, "<sip:%s@%s:%s>", request->to->url->username,
	     locip, eXosip.localports[nPort]);
 #endif /*CONFIG_RTK_VOIP_SIP_TLS*/
  ...
  if (eXosip.j_firewall_ip[nPort][0]!='\0')
      ...
      if (con!=NULL && con->url!=NULL && con->url->host!=NULL)
	{
	  ...
	  if (eXosip_is_public_address(c_address))
		...
		snprintf(contact,1000, "<sip:%s:%s;transport=%s>", eXosip.j_firewall_ip[nPort],
			 eXosip.j_firewall_ports[nPort], transport_type);
	      else
		snprintf(contact,1000, "<sip:%s@%s:%s;transport=%s>", request->to->url->username,
					eXosip.j_firewall_ip[nPort], eXosip.j_firewall_ports[nPort], transport_type);
#else /*CONFIG_RTK_VOIP_SIP_TLS*/
	      if (request->to->url->username==NULL)
		snprintf(contact,1000, "<sip:%s:%s>", eXosip.j_firewall_ip[nPort],
			 eXosip.j_firewall_ports[nPort]);
	      else
		snprintf(contact,1000, "<sip:%s@%s:%s>", request->to->url->username,
					eXosip.j_firewall_ip[nPort], eXosip.j_firewall_ports[nPort]);
#endif /*CONFIG_RTK_VOIP_SIP_TLS*/
	    }
	}
    }
...
char *
generating_sdp_answer(int nPort, osip_message_t *request, osip_negotiation_ctx_t *context)
{
			  ...
			  if (eXosip_is_public_address(c_address))
			    {
				  ...
				  while (conn!=NULL)
				  {
					  if (conn->c_addr!=NULL )
					  {
						  ...
						  conn->c_addr = osip_strdup(eXosip.j_firewall_ip[nPort]);
					  }
					  pos++;
					  conn = sdp_message_connection_get(local_sdp, pos, 0);
				  }
...
_eXosip2_answer_invite_1xx(eXosip_call_t *jc, eXosip_dialog_t *jd, int code, osip_message_t **answer)
{
  ...
  if (code>100)
    {
      i = complete_answer_that_establish_a_dialog(jc->nPort, *answer, tr->orig_request);
    }
  ...
  if (code>100)
    {
      /* request that estabish a dialog: */
      /* 12.1.1 UAS Behavior */
      i = complete_answer_that_establish_a_dialog(jc->nPort, response, tr->orig_request);
...
eXosip_answer_invite_2xx(eXosip_call_t *jc, eXosip_dialog_t *jd, int code, char *local_sdp_port)
{
  ...
  if(NULL != osip_list_get(tr->orig_request->bodies,0))
    {
      body = generating_sdp_answer(jc->nPort, tr->orig_request, jc->c_ctx);
      if (body==NULL)
	code = 488; /* bad sdp */
    }
  else
  ...
  if (code>100)
    {
      /* request that estabish a dialog: */
      /* 12.1.1 UAS Behavior */
      i = complete_answer_that_establish_a_dialog(jn->nPort, response, tr->orig_request);
