...
static void eXosip_process_refer(eXosip_call_t *jc, eXosip_dialog_t *jd,
				 osip_transaction_t *transaction, osip_event_t *evt);
static void eXosip_process_ack(eXosip_call_t *jc, eXosip_dialog_t *jd, osip_transaction_t *transaction, osip_event_t *evt);
...
static void eXosip_process_new_invite(int nPort, osip_transaction_t *transaction, osip_event_t *evt);
...
static int eXosip_event_package_is_supported(osip_transaction_t *transaction,
					     osip_event_t *evt);
static void eXosip_process_new_subscribe(int nPort, osip_transaction_t *transaction,
					 osip_event_t *evt);
				   ...
				   osip_transaction_t *transaction, osip_event_t *evt)
{
  ...
  osip_transaction_set_your_instance(transaction,
				     __eXosip_new_jinfo(jc->nPort, jc,
							NULL /*jd */,
							NULL,
							NULL));
				 ...
				 osip_transaction_t *transaction, osip_event_t *evt)
{
  ...
  i = complete_answer_that_establish_a_dialog(jc->nPort, answer, evt->sip);
...
#ifdef FIX_PACKET_ORDER
  // ensure 202 accept is sent before Notify
  eXosip_execute();
#endif
...
#endif
   /*++remarked by Jack Chan  for transfer++*/
  //_eXosip_transfer_send_notify(jc, jd, EXOSIP_SUBCRSTATE_ACTIVE, "SIP/2.0 100 Trying");  
  /*--end--*/
}
...
#ifdef SUPPORT_SJPHONE	
  // handle SjPhone format: refer;id=xxx 
...
static void eXosip_process_ack(eXosip_call_t *jc, eXosip_dialog_t *jd, osip_transaction_t *transaction, osip_event_t *evt)
{
  ...
  if (je==NULL)
  {
	...
	// rock: fix memory leak if receive multiple ack
	if (transaction->ack)
	{
  	...
  	if( 0 != osip_message_clone (evt->sip, &transaction->ack) ){	
...
#if 1
	osip_stop_200ok_retransmissions(eXosip.j_osip, evt->sip);
#endif
...
printf( "\n local_sdp 1 != NULL " ) ;
		...
		local_sdp = NULL ;
printf( "\n osip_negotiation_ctx_execute_negotiation 1 ...." ) ;
...
printf( "\n eXosip_send_default_answer 25 .." ) ;
	...
	if( local_sdp != NULL )
	{
		...
		// rock add:
		// 1. updae local ip info of sdp
		// 2. add fmtp of telephone-event
		{
			osip_contact_t *contact;

			osip_message_get_contact(answer, 0, &contact);
			osip_free(local_sdp->o_addr);
			local_sdp->o_addr = osip_strdup(contact->url->host);
			osip_free(local_sdp->c_connection->c_addr);
			local_sdp->c_connection->c_addr = osip_strdup(contact->url->host);
		}
					  ...
					  osip_event_t *evt, sdp_message_t *sdp)
{
	...
	osip_event_t * sipevent ;
	eXosip_event_t * je ;
printf( "\n eXosip_process_invite_on_hold 2 .." ) ;
...
printf( "\n sipevent 2 != NULL" ) ;
	...
	// rock: handle hold in upper layer
	sdp_message_free(sdp);
	osip_list_add( jd->d_inc_trs , transaction , 0 ) ;
	je = eXosip_event_init_for_call( EXOSIP_CALL_HOLD , jc , jd ) ;
	eXosip_event_add_sdp_info( je , evt->sip ) ;
	/* be sure the invite will be processed before any API call on this dialog*/
	/* NOTE: evt->sip will be free after execute */
#ifdef FIX_RESPONSE_TIME
					   ...
					   osip_event_t *evt, sdp_message_t *sdp)
{
	...
	osip_event_t * sipevent ;
	eXosip_event_t * je ;
printf( "\n eXosip_process_invite_off_hold 3 .." ) ;
	sipevent = eXosip_process_reinvite( jc , jd , transaction , evt , sdp ) ;
	if( sipevent == NULL )
		return ;	/* ERROR */
printf( "\n sipevent 1 != NULL" ) ;
	...
	// rock: handle off-hold in upper layer
	sdp_message_free(sdp);
	osip_list_add( jd->d_inc_trs , transaction , 0 ) ;
	je = eXosip_event_init_for_call( EXOSIP_CALL_OFFHOLD , jc , jd ) ;
	eXosip_event_add_sdp_info( je , evt->sip ) ;
...
#ifdef ATTENDED_TRANSFER
/* Should this be here ??*/
/*   INPUT: replace_header
*    OUTPUT: matched call and dialog 
*	return : -1 not matched, 0 matched*/  
int exosip_match_replace_dialog(int nport, osip_header_t *replace_header, eXosip_call_t  **rjc, eXosip_dialog_t **rjd)
{
	osip_replaces_t *replaces;
	eXosip_call_t	*jc;
	eXosip_dialog_t *jd;	
	int	match = -1;
	char	*rcallid = NULL;
	
	osip_replaces_init(&replaces);

	if (osip_replaces_parse(replaces, replace_header->hvalue)  == -1)
	{
		osip_replaces_free( replaces);
		return match;
	}
	osip_call_id_to_str(osip_replaces_get_call_id(replaces),  &rcallid);
	
	
	for (jc = eXosip.j_calls; jc!= NULL ; jc=jc->next)
	{
		printf("\r\nnport:%d %d", nport, jc->nPort);
		...
		for (jd = jc->c_dialogs; jd!= NULL ; jd=jd->next)
		{
			if (jd->d_dialog!=NULL)
			{
			    if (strcmp(jd->d_dialog->call_id, rcallid) == 0)
			    {
    			  	 
			        /*The callid matched, from and to tag suppposed to be matched!*/
				  if ((strcmp(jd->d_dialog->local_tag,osip_replaces_get_to_tag(replaces)) == 0 ) &&
					(strcmp(jd->d_dialog->remote_tag, osip_replaces_get_from_tag(replaces)) == 0 ))
				  {
				  	 match = 0;					
					*rjd = jd;
					*rjc = jc;
				  ...
				  else if ((strcmp(jd->d_dialog->local_tag, osip_replaces_get_from_tag(replaces)) == 0 ) &&
					(strcmp(jd->d_dialog->remote_tag, osip_replaces_get_to_tag(replaces)) == 0 ))
				  {				  	
					match = 0;					
					*rjd = jd;
					*rjc = jc;
				  ...
				  {
				  	//debug, call id matched but from/to-tag not matched.
					//match = 0;					
					//*rjd = jd;
					//*rjc = jc;
					//break;
...
static void eXosip_process_new_invite(int nPort, osip_transaction_t *transaction, osip_event_t *evt)
{
  ...
  osip_transaction_add_event(transaction, evt_answer);
	...
	rc = NULL;
	rd = NULL;
  /* because osip_ist_execute will eat evt->sip, so we process header first*/
//  if(osip_message_get_replaces( evt->sip,0, &replace_header) != -1)
  if (osip_message_header_get_byname(evt->sip, "replaces", 0, &replace_header)!= -1)
  {
 	 //match call dialog here?
     	 if (exosip_match_replace_dialog(nPort, replace_header,&rc,&rd) == 0 )
     	 {
     	 	if (rc == NULL)
			OSIP_TRACE (osip_trace
			  (__FILE__, __LINE__, OSIP_ERROR, NULL,
			         "eXosip: cannot find replace dialog!\n"));
		/*Replace call matched*/
	 }
	else
	{
		rc = NULL;
	...
	osip_event_t *evt)
{
  ...
  eXosip_notify_init(nPort, &jn, evt->sip);
  ...
  i = complete_answer_that_establish_a_dialog(jn->nPort, answer, evt->sip);
  if (i!=0)
    {
      osip_message_free(answer);
      OSIP_TRACE (osip_trace
...
#ifdef FIX_RESPONSE_TIME
	  eXosip.j_call_callbacks[EXOSIP_SUBSCRIPTION_NOTIFY](eXosip.user, EXOSIP_SUBSCRIPTION_NOTIFY, je);
#else
...
eXosip_match_notify_for_subscribe(eXosip_subscribe_t *js, osip_message_t *notify)
{
  ...
  g_message("+++++DEBUG:eXosip_match_notify_for_subscribe+++++\n");
  if (js==NULL)
    return -1;
...
#ifdef FIX_RESPONSE_TIME
      eXosip.j_call_callbacks[EXOSIP_MESSAGE_NEW](eXosip.user, EXOSIP_MESSAGE_NEW, je);
#else
...
#endif
{
  ...
  if (MSG_IS_INVITE(evt->sip))
	...
	// check dialog not found but request has remote tag case (invalid request)
	/*if (jd == NULL && MSG_IS_INVITE(evt->sip) )
	{
  		osip_generic_param_t *tag_param_remote;

		i = osip_from_get_tag (evt->sip->to, &tag_param_remote);
		if (i == 0)	// remote tag in request
		{
			osip_event_free(evt);
			return;
		}
  ...
  if (ctx_type != -1)
    ...
    {
	  // No need handle 100 Trying within call
  ...
  if (jd!=NULL)
    {
      ...
      */
      ...
      if (!MSG_IS_BYE(evt->sip) && 
		  !MSG_IS_ACK(evt->sip))	// rock: receive re-invite ack after bye case (transaction will be NULL)
	{
	...
	osip_transaction_set_your_instance(transaction, __eXosip_new_jinfo(nPort, jc, jd, NULL, NULL));
	  if (old_trn!=NULL && old_trn->state!=IST_TERMINATED && old_trn->state!=IST_CONFIRMED)
	    {
	      osip_list_add(eXosip.j_transactions, transaction, 0);
	...
	/*+++++fix the late ack problem by Jack 24/05/07+++++*/
	if( NULL != old_trn && NULL == old_trn->ack ){
		g_message("+++++debug:process lost ack for the pre INVITE+++++\n");
		__eXosip_delete_jinfo(transaction);
		osip_transaction_free(transaction);
		return ;
	}
	/*-----end-----*/
	  ...
	  if (old_trn!=NULL && old_trn->state!=ICT_TERMINATED)
		...
		// handle invite retransmission
		if (evt->sip->cseq &&
			evt->sip->cseq->number)
		{
			int seq;

			seq = atoi(evt->sip->cseq->number);
			if (jd->d_dialog->remote_cseq == seq &&
				jd->d_dialog->state == DIALOG_CONFIRMED)
			{
				// 200 ok retransmission!
				osip_message_t *msg_200Ok;

				osip_message_clone(jd->d_200Ok, &msg_200Ok);
				evt_answer = osip_new_outgoing_sipmessage(msg_200Ok);
				evt_answer->transactionid = transaction->transactionid;
				osip_transaction_add_event(transaction, evt_answer);
				osip_ist_execute(eXosip.j_osip);
				__eXosip_delete_jinfo(transaction);
      ...
      else if (MSG_IS_BYE(evt->sip))
	{
	  old_trn = eXosip_find_last_inc_bye(jc, jd);
		...
		g_message("+++++DEBUG:receive BYE REQUEST+++++\n");
	...
	{
		g_message("+++++debug:ack is receive within a dialog+++++\n");

	  old_trn = eXosip_find_last_inc_invite(jc, jd);
	  if (old_trn == NULL)
	  {
		g_warning("receive ack but has no transaction!\n");
	    osip_event_free(evt);
	    return;
	  }
	  eXosip_process_ack(jc, jd, old_trn, evt);
	}
  ...
  if (MSG_IS_OPTIONS(evt->sip))
    ...
    {
     /*++added by Rock and Jack Chan 30/01/07
    	If the dialog is NULL and the INVITE request has from tag
    	It will be considered as illegical request++*/
	osip_generic_param_t *tag_param_remote;
	...
	/*--end--*/
      eXosip_process_new_invite(nPort, transaction, evt);
      return;
    }
  else if (MSG_IS_BYE(evt->sip))
    {
    	g_message("+++++DEBUG:Go to jd=NULL; MSG_IS_BYE+++++\n");
      osip_list_add(eXosip.j_transactions, transaction, 0);
      eXosip_send_default_answer(jd, transaction, evt, 481, NULL, NULL, __LINE__);
      return;
    }
...
return;
  ...
  if (MSG_IS_SUBSCRIBE(evt->sip))
      ...
      if (0==eXosip_event_package_is_supported(transaction, evt))
	{
      ...
      eXosip_process_new_subscribe(nPort, transaction, evt);
      return;
    }
...
#endif
{
	...
	for (jc=eXosip.j_calls; jc!=NULL; jc=jc->next)
	{
		for (jd=jc->c_dialogs; jd!=NULL; jd=jd->next)
		{
			if (jd->d_dialog != NULL)
			{
				if (osip_dialog_match_as_uac(jd->d_dialog, evt->sip)==0)
					break;
			}
		}
	...
	if (jc && jd)
	{
		// UAC core process 2xx response
  		if (MSG_IS_STATUS_2XX(evt->sip))
		{
			osip_message_t *ack;

			ack = build_ack_for_receiving_2xx_response(jc, jd, jd->d_200Ok);
			if (ack)
			{
	  ...
	  jpipe_read (eXosip.j_socketctl, buf2, 499);
{
  ...
  eXosip_event_init(&je, EXOSIP_WATCHDOG);
  if (je) 
  {
	fprintf(stdout, "[W]");
	eXosip_event_add(je);
  ...
  {
	OSIP_TRACE(osip_trace(__FILE__,__LINE__,OSIP_ERROR,NULL, "eXosip_event_init for watchdog failed\n"));
  }
}	
#else
	  char buf2[500];
