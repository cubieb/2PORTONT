...
{
  eXosip_event_t *je;
  eXosip_event_init(jc->nPort, &je, type);
				...
				eXosip_dialog_t *jd)
{
  char *tmp;
  eXosip_event_t *je;
  eXosip_event_init(js->nPort, &je, type);
  if (je==NULL) return NULL;
  je->js = js;
  je->jd = jd;
...
{
  eXosip_event_t *je;
  eXosip_event_init(jn->nPort, &je, type);
  if (je==NULL) return NULL;
  je->jn = jn;
  je->jd = jd;
...
{
  eXosip_event_t *je;
  eXosip_event_init(jr->nPort, &je, type);
  if (je==NULL) return NULL;
  je->jr = jr;
  je->rid = jr->r_id;
			      ...
			      osip_message_t *sip)
{
  ...
  for (nPort=0; nPort<g_MaxVoIPPorts; nPort++)
  {
		  ...
		  if(eXosip_get_transport_type(sip)==TRANSPORT_TLS){
			...
			proxyIndex=eXosip_find_TCP_socket_form_proxy(nPort, host);
		  	if (eXosip.j_tls_sockets[nPort][proxyIndex] == tr->out_socket)
				break;
		  }else{
			if (eXosip.j_sockets[nPort] == tr->out_socket)
				break;
		  }
  ...
  if (nPort == g_MaxVoIPPorts)
    return NULL;

  eXosip_event_init(nPort, &je, type);
  if (je==NULL) return NULL;
...
int
eXosip_event_init(int nPort, eXosip_event_t **je, int type)
{
