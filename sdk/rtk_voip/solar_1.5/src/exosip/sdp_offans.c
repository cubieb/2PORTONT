...
char *eXosip_sdp_get_audio_port(osip_negotiation_ctx_t *context, int pos_media)
{
  ...
  if (jc==NULL)
    return osip_strdup("9000");
  else if (jc->c_sdp_port[0]=='\0')
    return osip_strdup("9000");
  else
	return osip_strdup(jc->c_sdp_port);
}
...
int eXosip_sdp_negotiation_init(osip_negotiation_t **sn)
{
  ...
  osip_negotiation_set_o_username(*sn, osip_strdup("111"));//ex,111
  osip_negotiation_set_o_session_id(*sn, osip_strdup("00000002"));
  osip_negotiation_set_o_session_version(*sn, osip_strdup("00000002"));
...
extern char local_ip_str[20] ;
extern char wan_ip_addr[20] ;
...
eXosip_get_local_sdp_info(osip_transaction_t *invite_tr)
{
	...
	{
		g_warning( "BUG -> NOT AN INVITE TRANSACTION!!\n" ) ;
		return NULL ;
	}
