...
#endif
#else
const char pHttpHeaderTcpWan[] = { "GET /goform/formWanTcpipSetup?submit-url=ivr_reserved&" }; 
const char pHttpHeaderTcpLan[] = { "GET /goform/formTcpipSetup?submit-url=ivr_reserved&" };
const char pHttpHeaderReboot[] = { "GET /goform/formRebootCheck?submit-url=ivr_reserved" };
#endif
...
int IvrSystemResetToDefaultCfg( void )
{
	...
	//if ((g_VoIP_Feature & PLATFORM_MASK) == PLATFORM_8186)
	{
		return IvrSetRawHttpToWebServer( 
				...
				"GET /goform/formSaveConfig?reset=Reset HTTP/1.1\nUser-Agent: " HTTP_AGENT_NAME "\n\n" );
	}
