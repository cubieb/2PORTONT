...
pthread_mutex_t semBusSem;
#define MUTEXOBTAIN_BusSem pthread_mutex_lock( &semBusSem );
#define MUTEXRELEASE_BusSem pthread_mutex_unlock( &semBusSem );

int Bus_Init(void)
{
	pthread_mutex_init(&semBusSem,0);
	printf("Bus init: create mutex sem for bus\n");
	return 0;
}
...
{
	int result;
	//printf("Bus_Read_blocking, len=%d\n", len);
	MUTEXOBTAIN_BusSem;
