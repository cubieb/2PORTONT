...
E_CMBS_RC         cmbs_api_ie_CallTransferReqAdd( void * pv_RefIEList, PST_IE_CALLTRANSFERREQ pst_CallTrf )
{
    ...
    u8_Buffer[11] = pst_CallTrf->u8_TermInstance;
...
{
   u8 * pu8_Buffer = (u8*)pv_RefIE;
   u16	u16_Size = pu8_Buffer[CFR_IE_SIZE_POS];
...
E_CMBS_RC         cmbs_api_ie_HandsetsAdd( void * pv_RefIE, u8  u8_Handsets )
{
    u8  u8_Buffer[3];
    
    u8_Buffer[CFR_IE_TYPE_POS] = (u8)CMBS_IE_HANDSETS;
    u8_Buffer[CFR_IE_SIZE_POS] = 1;  // sizeof(u8)
    
    u8_Buffer[CFR_IE_HEADER_SIZE] = u8_Handsets;
    
    cfr_ie_ItemAdd( pv_RefIE, u8_Buffer, 3 );
