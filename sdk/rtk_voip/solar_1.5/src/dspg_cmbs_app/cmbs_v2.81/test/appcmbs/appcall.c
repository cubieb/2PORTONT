...
/*!
        ...
        \param[in,out]		u32_CallInstance	 if not used zero
        \param[in,out]		u16_CallId		      Call ID used, if psz_CLI is NULL
        \param[in,out]		u16_StartSlot        Start Slot, if it is 0xFF, means auto
        \param[in,out]		psz_CLI		      pointer to Caller ID, if not needed NULL
        \return				<none>
*/
void        appmedia_CallObjMediaStart( u32 u32_CallInstance, u16 u16_CallId, u16 u16_StartSlot, char * psz_CLI )
{
    ...
    if ( pst_Call && (E_APPCMBS_MEDIA_PEND == _appcall_CallObjMediaGet(pst_Call)) )
    {
        ...
        if ( pv_RefIEList )
        {
            ...
            RTK_DBG("%s, pst_Call->e_Codec = ", __FUNCTION__);
            ...
            {
            	RTK_DBG("CMBS_AUDIO_CODEC_PCM_LINEAR_WB\n");
                if ( 0xFF != u16_StartSlot )
                    st_MediaChannel.u32_ChannelParameter = 0xF << (u16_StartSlot );
                else
            ...
            {
            	RTK_DBG("CMBS_AUDIO_CODEC_PCM_LINEAR_NB\n");
                if ( 0xFF != u16_StartSlot )
                    st_MediaChannel.u32_ChannelParameter = 0x3 << (u16_StartSlot );
                else
            ...
            {
            	RTK_DBG("CMBS_AUDIO_CODEC_PCMA or CMBS_AUDIO_CODEC_PCMU\n");
                if ( 0xFF != u16_StartSlot )
                    st_MediaChannel.u32_ChannelParameter = 0x1 << (u16_StartSlot );
                else
            ...
            {
            	RTK_DBG("CMBS_AUDIO_CODEC_PCMA_WB or CMBS_AUDIO_CODEC_PCMU_WB\n");
                if ( 0xFF != u16_StartSlot )
                    st_MediaChannel.u32_ChannelParameter = 0x3 << (u16_StartSlot );
                else
                    st_MediaChannel.u32_ChannelParameter = 0x3 << (pst_Call->u32_ChannelID *4 );
            ...
            cmbs_api_ie_MediaChannelAdd( pv_RefIEList, &st_MediaChannel );
            printf("\nCall cmbs_dem_ChannelStart, Channel=%d,Slot=%d,ChannelInfo=%X\n",
...
void        app_OnCallEstablish( void * pvAppRefHandle, void * pv_List )
{
    ...
    if ( pv_List )
    {
        ...
        if ( g_call_automat )
        {
            ...
            if ( pst_Call->u8_LineId == APPCALL_NO_LINE )
            {
            u16 u16_LineId = 1;  //TODO  choose default line for current HS
	...
	CMBS_Api_OutboundEstablishCall(pst_Call);
	/*************************************************
      	 [THLin]:add off-hook event of HS to event FIFO (Outgoing call , HS off-hook)
		  	pst_Call can extract line ID info */
	//RTK_DBG("%s, off-hook, Line ID = %d\n", __FUNCTION__, pst_Call->u8_LineId);
	dect_event_in(pst_Call->u8_LineId, pst_Call->ch_CallerID[0]-0x31, 1/*off-hook*/);
	/*************************************************/
#endif
...
void        app_OnCallInbandInfo( void * pvAppRefHandle,void * pv_List )
{
    ...
    if ( pv_List )
    {
        ...
        while ( pv_IE != NULL )
        {
                            ...
                            if ( g_call_automat )
                            {
                                if ( _appcall_CallObjDigitCollectorEndSymbolCheck( pst_Call ) )
                                {
                                    if ( g_EarlyMediaAutoCfm )
                                    {
                                        ...
                                        st_Properties.e_IE      = CMBS_IE_CALLPROGRESS;
                                        st_Properties.psz_Value = (char*)"CMBS_CALL_PROGR_INBAND\0";
			...
			{
				//RTK_DBG("%s, key event, Line ID = %d\n", __FUNCTION__, pst_Call->u8_LineId);
				dect_event_in(pst_Call->u8_LineId, pst_Call->ch_CallerID[0]-0x31, st_IEInfo.Info.st_CallInfo.pu8_Info[i]);
			}
			/*************************************************/
#endif
    ...
    if ( pv_List )
    {
        ...
        if ( g_call_automat )
        {
         ...
         [THLin]:add off-hook event of HS to event FIFO (Incoming call , HS off-hook)
		  	pst_Call can extract line ID info */
	//RTK_DBG("%s, off-hook, Line ID = %d\n", __FUNCTION__, pst_Call->u8_LineId);
	dect_event_in(pst_Call->u8_LineId, pst_Call->ch_CalledID[0]-0x31, 1/*off-hook*/);
	/*************************************************/
#endif
}
...
void        app_OnCallRelease( void * pvAppRefHandle,void * pv_List )
{
	...
	CMBS_Api_ReleaseNotify( _appcall_CallObjIdGet(pst_Call) );
	 ...
	 //RTK_DBG("%s, on-hook, Line ID = %d\n", __FUNCTION__, pst_Call->u8_LineId);
	if (pst_Call->ch_CallerID[0] == 'p')//incoming CLI
	 	dect_event_in(pst_Call->u8_LineId, pst_Call->ch_CalledID[0]-0x31, 0/*on-hook*/);
	 else
		 dect_event_in(pst_Call->u8_LineId, pst_Call->ch_CallerID[0]-0x31, 0/*on-hook*/);
...
void        app_OnCallTransfer( void * pvAppRefHandle,void * pv_List )
{
    ...
    if ( pv_List )
    {
        ...
        while ( pv_IE != NULL )
        {
            if ( CMBS_IE_CALLTRANSFERREQ == u16_IE )
            {
...
int _appcall_CallerIDSet( PST_CALL_OBJ pst_Call, PST_APPCALL_PROPERTIES pst_Properties, int n_Properties , PST_IE_CALLERPARTY pst_CallerParty )
{
    ...
    if ( n_Pos < n_Properties )
    {
        ...
        pst_CallerParty->u8_AddressLen            = (u8)strlen(pst_Call->ch_CallerID)-2;
...
int _appcall_HsMaskSet( PST_CALL_OBJ pst_Call, PST_APPCALL_PROPERTIES pst_Properties, int n_Properties, u8* pu8_HsMask )
{
    ...
    {
        psz_Value = pst_Properties[n_Pos].psz_Value;
        *pu8_HsMask = (u8)app_HandsetMap(psz_Value);
    ...
    if ( pst_Call )
    {
        ...
        if ( pst_Call->b_Incoming )
        {
            if ( pst_Call->e_Call < E_APPCMBS_CALL_ACTIVE )
            {
                ...
                ListChangeNotif_MissedCallListChanged(pst_Call->u8_LineId, TRUE);
            }
            else
            {
                /* Incoming accepted call */
...
int      appcall_ProgressCall ( PST_APPCALL_PROPERTIES pst_Properties, int n_Properties, u16 u16_CallId, char * psz_CLI )
{
    ...
    if ( pst_Call )
    {
        ...
        if ( pv_RefIEList )
        {
            ...
            if ( st_CallProgress.e_Progress == CMBS_CALL_PROGR_PROCEEDING )
            {
                if ( pst_Call->u8_LineId == APPCALL_NO_LINE )
                {
#ifndef RTK_SIP_INTEGRATED
                    u16 u16_LineId = 1;//_keyb_LineIdInput();
#else
               u16 u16_LineId = 0; // THLin
               RTK_DBG("appcall_ProgressCall, use default LineId%d\n", u16_LineId);
#endif
                    if ( u16_LineId != APPCALL_NO_LINE )
                    {
                        _appcall_CallObjLineObjSet( pst_Call, _appcall_LineObjGet(u16_LineId) );
