...
E_CMBS_RC         cmbs_int_OnHsRegistration( void )
{
   ...
   st_HandsetInfo.u8_Hs  = G_st_os00_Act.u16_XInfo;
   CFR_DBG_OUT( "cmbs_int_OnHsRegistration, HS Num =%d\n",
      st_HandsetInfo.u8_Hs
      );
   p_dr13_EEPROMReadBytes(&st_HandsetInfo.u8_State,
	      (SD02_SUB(G_st_os00_Act.u16_XInfo-1)+
	       SD02_SUB_STATUS-SD02_SUB_OFFSET),1);   
   p_dr13_EEPROMReadBytes(&st_HandsetInfo.u8_IPEI,
	      (SD02_SUB(G_st_os00_Act.u16_XInfo-1)+
	       SD02_SUB_IPUI-SD02_SUB_OFFSET),
	       sizeof(st_HandsetInfo.u8_IPEI));   
#if defined( CMBS_CATIQ )
...
{
   u16            u16_Size = 7;     // minimum = sizeof(e_AreaType + u16_Offset + u16_DataLen)
   u8             u8_Idx   = 3;     // offset of IE data after: 8bit IE type + 16bit IE size                                    
   u8             u8_Buffer[600];   // u16_Size + u8_Idx + max var. param size 512
   ...
   u8_Idx += cfr_ie_ser_u32( u8_Buffer + u8_Idx, pst_ParamArea->u32_Offset );
                                                   ...
                                                   ST_IE_LA_SEARCH_CRITERIA * pst_LASearchCriteria )
{
   ...
   u8_Buffer[CFR_IE_SIZE_POS]        = 5 + pst_LASearchCriteria->u8_PatternLength;
   u8_Buffer[CFR_IE_HEADER_SIZE]     = (u8)pst_LASearchCriteria->e_MatchingType;
   u8_Buffer[CFR_IE_HEADER_SIZE + 1] = pst_LASearchCriteria->u8_CaseSensitive;
   u8_Buffer[CFR_IE_HEADER_SIZE + 2] = pst_LASearchCriteria->u8_Direction;
   u8_Buffer[CFR_IE_HEADER_SIZE + 3] = pst_LASearchCriteria->u8_MarkEntriesReq;
   u8_Buffer[CFR_IE_HEADER_SIZE + 4] = pst_LASearchCriteria->u8_PatternLength;

   cfr_ie_ser_pu8( u8_Buffer + (CFR_IE_HEADER_SIZE + 5), pst_LASearchCriteria->pu8_Pattern, (u16)pst_LASearchCriteria->u8_PatternLength );
