...
int CMBS_Api_InBound_Ring_CallerID( char lineId, char* clid_num, char* clid_name )
{
   ...
   if (ch_Audio != NULL)
   {
   	...
   	st_Properties[i].psz_Value = ch_Audio;
   	n_Prop++;
	i++;
    printf( "Audio wideband/narrow band: %s\n", ch_Audio);
   }
...
void CMBS_Api_OutboundCall(int n_Line)
{
   ST_APPCALL_PROPERTIES st_Properties;
#if 0
   // [THLin]: refer CMBS API Version 2.09 C.8
   // This case will set call state to E_APPCMBS_CALL_OUT_INBAND.
   // It is also workable for outbound call.
   char value1[] = "CMBS_CALL_PROGR_INBAND\0";
#else
   ...
   //THLin: Let call state to be E_APPCMBS_CALL_ACTIVE, and state between CMBS and HS to be CC-Connect.
...
E_APPCMBS_CALL CMBS_Api_LineStateGet(int line)
{
	...
	pst_this = _appcall_CallOBJGetByLine( line );
