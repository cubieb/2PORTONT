...
nict_snd_request (osip_transaction_t * nict, osip_event_t * evt)
{
  ...
  if (i == 0)
  ...
  else if (i > 0)
	{
		...
		sipevent = __osip_event_new(evt->type, nict->transactionid);
		sipevent->sip = evt->sip; // link sip message to new event
		evt->sip = NULL; // remove sip message in old event 
  		nict->orig_request = NULL; // avoid free_transaction after rtk_flush_fifo_event problem
		add_gettimeofday (&sipevent->event_start, 1000); // retry after 1 sec
		osip_transaction_add_event(nict, sipevent); // add event to last 
	}
#endif
  else
    {
