 ...
 */
  typedef enum type_t
  {
    ...
    /* ICMP EVENTS for ICT/NICT */
    RCV_ICMP,    /**< ICMP for ICT/NICT */
...
#define ICT_TIMER_B_FIX 500		/* add System lantency Time (500ms) */
 ...
 */
  typedef struct osip_transaction
  {
 ...
 */
  typedef void (*osip_transport_error_cb_t) (int type, osip_transaction_t *,
					     int error);
/**
 * Callback definition for DNS query.
 * @var osip_get_ip_by_name_cb_t
 */
  typedef int (*osip_get_ip_by_name_cb_t)(unsigned long *ip, const char *hostname, int service);
 ...
 */
  typedef struct osip_event
  {
    ...
    struct timeval event_start; // for dns retry
  } osip_event_t ;
 ...
 */
#ifdef MALLOC_DEBUG
  int __osip_transaction_init (osip_transaction_t ** transaction,
			     osip_fsm_type_t ctx_type, osip_t * osip,
			     osip_message_t * request, char *file, int line);
  #define osip_transaction_init(t,c,o,r) __osip_transaction_init(t,c,o,r,__FILE__,__LINE__)
#else
...
#ifdef MUTEX_DEBUG
  int __osip_transaction_add_event (osip_transaction_t * transaction,
				  osip_event_t * evt, char *file, int line);
  #define osip_transaction_add_event(t,e) __osip_transaction_add_event(t,e,__FILE__,__LINE__)
#else
/**
...
/**
 * Check if an ict transactions meets a ICMP event.
 * @param osip The element to work on.
 */
  void osip_icmp_ict_execute (osip_t * osip, osip_get_ip_by_name_cb_t get_ip_by_name_cb,
  						unsigned long ip, unsigned short port );

/**
 * Check if an nict transactions meets a ICMP event.
 * @param osip The element to work on.
 */
  void osip_icmp_nict_execute (osip_t * osip, osip_get_ip_by_name_cb_t get_ip_by_name_cb,
  						unsigned long ip, unsigned short port );
