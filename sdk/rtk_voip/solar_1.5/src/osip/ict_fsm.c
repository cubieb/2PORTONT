...
ict_snd_invite (osip_transaction_t * ict, osip_event_t * evt)
{
  ...
  if (i == 0)
    {
      // reset timeout after send packet
...
#ifdef RTK_DNS
  else if (i > 0) // dns failed
	{
		// retry event
		osip_event_t *sipevent;
		
		sipevent = __osip_event_new(evt->type, ict->transactionid);
		sipevent->sip = evt->sip; // link sip message to new event
		evt->sip = NULL; // remove sip message in old event 
  		ict->orig_request = NULL; // avoid free_transaction after rtk_flush_fifo_event problem
		add_gettimeofday (&sipevent->event_start, 1000); // retry after 1 sec
		osip_transaction_add_event(ict, sipevent); // add event to last 

		// reset ict time
		ict->birth_time = time(NULL) + 1;
		osip_gettimeofday (&ict->ict_context->timer_a_start, NULL);
		add_gettimeofday (&ict->ict_context->timer_a_start, ict->ict_context->timer_a_length + 1000);
		osip_gettimeofday (&ict->ict_context->timer_b_start, NULL);
		add_gettimeofday (&ict->ict_context->timer_b_start, ict->ict_context->timer_b_length + 1000);
	}	
#endif
  else
    {
  ...
  if (ict->state != ICT_COMPLETED)	/* not a retransmission */
      ...
      if (ict->ict_context->destination == NULL)
	{
		...
		OSIP_TRACE (osip_trace
			...
			"no destination in ict_rcv_3456xx\n"));
#endif
