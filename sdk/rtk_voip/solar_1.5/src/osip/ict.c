  ...
  *ict = (osip_ict_t *) osip_malloc (sizeof (osip_ict_t));
  ...
  if (route != NULL && route->url!=NULL)
      ...
      if (lr_param==NULL)
	{
		...
		osip_list_remove(invite->routes, 0);
  ...
  (*ict)->timer_b_length = 64 * DEFAULT_T1 + ICT_TIMER_B_FIX;
...
osip_event_t *
__osip_ict_need_icmp_event (osip_ict_t * ict, state_t state,
			       int transactionid)
{
  if (ict == NULL)
    return NULL;
  if (state == ICT_CALLING)
    {
	return __osip_event_new (RCV_ICMP, transactionid);
    }
  return NULL;
}
