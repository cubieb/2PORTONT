...
#ifdef MALLOC_DEBUG
int
__osip_transaction_init (osip_transaction_t ** transaction,
...
#endif
{
  ...
  *transaction=NULL;
  ...
  printf("allocating transaction  ressource %i %s\n", 
	transactionid, request->call_id->number);
#endif

#ifdef MALLOC_DEBUG
  OSIP_TRACE (osip_trace
	      (file, line, OSIP_INFO2, NULL,
...
#ifdef MALLOC_DEBUG
  *transaction =
    (osip_transaction_t *) __osip_malloc (sizeof (osip_transaction_t), file, line);
#else
  *transaction =
...
osip_transaction_free (osip_transaction_t * transaction)
{
  ...
  if (i != 0)			/* yet removed ??? */
    {
#if defined(RTK_DEBUG) && defined(ENABLE_TRACE)
		printf("transaction already removed from list %i!\n",
	...
	{
		printf("transaction removed from list %i!\n",
			transaction->transactionid);
	}
#endif
...
osip_transaction_free2 (osip_transaction_t * transaction)
{
  ...
  if (transaction->orig_request != NULL
    ...
    {
#if defined(RTK_DEBUG) && defined(ENABLE_TRACE)
		printf("free transaction ressource %i %s\n",
		   transaction->transactionid,
...
#ifdef MUTEX_DEBUG
int
__osip_transaction_add_event (osip_transaction_t * transaction,
			    osip_event_t * evt, char *file, int line)
{
  ...
  __osip_fifo_add (transaction->transactionff, evt, file, line);
  return 0;
}
#else
int
							  ...
							  response)
{
  ...
  if (b_response == NULL)
	...
	return -1;
      return 0;
#else
	fprintf(stderr, "Warning: The branch parameter in Via header is NULL!!.....\n");
      OSIP_TRACE (osip_trace
		  (__FILE__, __LINE__, OSIP_BUG, NULL,
