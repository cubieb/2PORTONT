 ...
 */
#ifdef MALLOC_DEBUG
  int __osip_message_init (osip_message_t ** sip, char *file, int line);
  #define osip_message_init(s) __osip_message_init(s, __FILE__, __LINE__)
#else
 ...
 */
#ifdef MALLOC_DEBUG
  int __osip_message_clone (const osip_message_t * sip, osip_message_t ** dest, char *file, int line);
  #define osip_message_clone(s,d) __osip_message_clone(s,d,__FILE__,__LINE__)
#else
...
#define MSG_IS_PING(msg)   (MSG_IS_REQUEST(msg) && \
			      0==strcmp((msg)->sip_method,"PING"))
