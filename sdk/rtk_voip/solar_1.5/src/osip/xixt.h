...
/**
 * Check if this transaction needs a RCV_ICMP event
 * NOTE: THIS IS AN INTERNAL METHOD ONLY
 * @param ict The element to work on.
 * @param state The actual state of the transaction.
 * @param transactionid The transaction id.
 */
  osip_event_t *__osip_ict_need_icmp_event (osip_ict_t * ict,
					       state_t state,
					       int transactionid);
 ...
 * @param transactionid The transaction id.
 */
  osip_event_t *__osip_nict_need_icmp_event (osip_nict_t * nict,
						state_t state,
						int transactionid);
