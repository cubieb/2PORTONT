...
#if defined(RTK_DEBUG) && defined(ENABLE_TRACE)
  if (i == 0)
    printf("transaction removed from list %i in osip!\n",
      tr->transactionid);
#endif
  ...
  for( index=0; index<len; ++index )
...
#ifdef RTK_DNS
	  fifo_retry = (osip_fifo_t *) malloc(sizeof(osip_fifo_t));
	  if (fifo_retry == NULL)
	  {
	  	OSIP_TRACE (osip_trace
		      (__FILE__, __LINE__, OSIP_ERROR, NULL,
		       "core module: malloc fifo_retry failed\n"));
	    osip_free(array);
	    return -1;
	  }
	  osip_fifo_init(fifo_retry);
#endif
      ...
      do
	{
	  ...
	  else
	  {
	    ...
	    if (osip_timercmp (&now, &se->event_start, >=))
	    {
	      osip_transaction_execute (transaction, se);
	    }
	    else
	    {
	      osip_fifo_add(fifo_retry, se);
	    }
	  }
#else
	  else
	  ...
	  do
	  {
	    se = (osip_event_t *) osip_fifo_tryget (fifo_retry);
	    if (se == NULL)
	      more_event = 0;
	    else
		  osip_fifo_add(transaction->transactionff, se);
	  }
	  while (more_event == 1);

	  osip_fifo_free(fifo_retry);
#endif
...
int rtk_check_fifo_event(osip_fifo_t *ff)
{
	osip_event_t *se;
	struct timeval now;
	int pos;
...
#endif
	pos = 0;
	while (!osip_list_eol(ff->queue, pos))
	{
		se = (osip_event_t *) osip_list_get(ff->queue, pos);
		if (osip_timercmp (&now, &se->event_start, >=))
	    {
		#ifdef OSIP_MT
...
int rtk_flush_fifo_event(osip_transaction_t *tr)
{
	osip_event_t *se;

	if (tr == NULL || tr->transactionff == NULL)
		return 0;

	se = (osip_event_t *) osip_fifo_tryget(tr->transactionff);
	while (se)
	{
...
osip_timers_gettimeout (osip_t * osip, struct timeval *lower_tv)
{
  ...
  while ( osip_list_iterator_has_elem( iterator ) )
    {
#ifdef RTK_DNS
	  if (rtk_check_fifo_event(tr->transactionff))
#else
...
static int osip_ip_port_cmp( const char *szIP1, unsigned short port1,
							unsigned long IP2, unsigned short port2,
							osip_get_ip_by_name_cb_t get_ip_by_name_cb )
{
  if( port1 != port2 )
    return 1;
  ...
  /* convert IP string to a binary represention */
  ipaddr1 = inet_addr( szIP1 );
  
  if( ipaddr1 == INADDR_NONE && 
    ( *get_ip_by_name_cb )( &ipaddr1, szIP1, port1 ) )
  {
    /* No DNS record, so we don't send packet to this ip */
    return 2;
  }
...
void
osip_icmp_ict_execute (osip_t * osip, osip_get_ip_by_name_cb_t get_ip_by_name_cb,
						unsigned long ip, unsigned short port)
{
  ...
  while ( osip_list_iterator_has_elem( iterator ) )
      ...
      else
    {
      if( !tr ->ict_context )
        ;
      else if( osip_ip_port_cmp( tr ->ict_context ->destination,
                                 tr ->ict_context ->port,
                                 ip, port, get_ip_by_name_cb ) == 0 )
      {
 
        OSIP_TRACE (osip_trace
		      (__FILE__, __LINE__, OSIP_INFO4, NULL,
               "ICMP event: target %s:%d\n", tr ->ict_context ->destination,
        										tr ->ict_context ->port ));
        evt = __osip_ict_need_icmp_event(tr->ict_context, tr->state,
					    tr->transactionid);
        if (evt != NULL)
	      osip_fifo_add (tr->transactionff, evt);
      }
...
void
osip_icmp_nict_execute (osip_t * osip, osip_get_ip_by_name_cb_t get_ip_by_name_cb,
						unsigned long ip, unsigned short port)
{
  osip_transaction_t *tr;
  osip_list_iterator_t iterator;
  ...
  while ( osip_list_iterator_has_elem( iterator ) )
    {
      ...
      if( !tr ->nict_context )
        ;
      else if( osip_ip_port_cmp( tr ->nict_context ->destination,
                                 tr ->nict_context ->port,
                                 ip, port, get_ip_by_name_cb ) == 0 )
      {
        OSIP_TRACE (osip_trace
		      (__FILE__, __LINE__, OSIP_INFO4, NULL,
               "ICMP event: target %s:%d\n", tr ->nict_context ->destination,
        										tr ->nict_context ->port ) );
        evt = __osip_nict_need_icmp_event(tr->nict_context, tr->state,
					    tr->transactionid);
        if (evt != NULL)
	      osip_fifo_add (tr->transactionff, evt);
      }
