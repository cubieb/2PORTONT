...
void *__osip_malloc(size_t size, char *file, int line)
{
  void *ptr = mwMalloc(size, file, line);

  if (ptr==NULL)
  {
    fprintf(stderr, "__osip_malloc failed in %d line of %s\n", line, file);
    exit(0);
  }
...
void *__osip_calloc(size_t num, size_t size, char *file, int line)
{
  void *ptr = mwCalloc(num, size, file, line);

  if (ptr==NULL)
  {
    fprintf(stderr, "__osip_calloc failed in %d line of %s\n", line, file);
    exit(0);
  }
...
void *__osip_realloc(void *p, size_t size, char *file, int line)
{
  void *ptr = mwRealloc(p, size, file, line);

  if (ptr==NULL)
  {
    fprintf(stderr, "__osip_realloc failed in %d line of %s\n", line, file);
    exit(0);
  }
...
char *__osip_strdup(const char *ch, char *file, int line)
{
  char *ptr;
  ...
  ptr = mwStrdup(ch, file, line);
  if (ptr == NULL)
  {
    fprintf(stderr, "__osip_strdup failed in %d line of %s\n", line, file);
    exit(0);
  }
