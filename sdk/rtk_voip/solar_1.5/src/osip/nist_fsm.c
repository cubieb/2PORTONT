  ...
  if (i > 0)
	{
		...
		sipevent = __osip_event_new(evt->type, nist->transactionid);
		sipevent->sip = evt->sip; // link sip message to new event
		evt->sip = NULL; // remove sip message in old event 
		nist->last_response = NULL; // avoid free sip message on retry
		add_gettimeofday (&sipevent->event_start, 1000); // retry after 1 sec
		osip_transaction_add_event(nist, sipevent); // add event to last 
		return;
	}
  else
#endif
