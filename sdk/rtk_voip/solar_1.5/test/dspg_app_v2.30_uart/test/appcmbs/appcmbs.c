...
void		appcmbs_CordlessStart( u8 * pu8_EEprom, u16 u16_Size )
{
	if ( u16_Size)
  {
  	...
  	APPCMBS_WARN(( "APP: INFO Push EEprom settings to target is not implemented!\n"));
  }
...
void     app_IEToString( void * pv_IE, u16 u16_IE )
{
   switch( u16_IE )
   {
      case  CMBS_IE_CALLINSTANCE:
      {
         ...
         APPCMBS_IE(( "APP: IETRACK CallInstance: %u\n", u32_CallInstance ));
	 ...
	 APPCMBS_IE(( "APP: IETRACK CallProgress:"));
      ...
      case  CMBS_IE_MEDIACHANNEL:
      {
         ...
         APPCMBS_IE(( "APP: IETRACK Media Channel: " ));
      ...
      case  CMBS_IE_PARAMETER_AREA:
      {
         ...
         APPCMBS_IE(("PARAMETER AREA: Offset:%d, Length:%d\n", st_ParamArea.u16_Offset, st_ParamArea.u16_DataLen ));
