...
void           app_OnDateTimeIndication( void *pv_List )
{
   ...
   if( pv_List )
   {
      ...
      APPCMBS_INFO(( "APPSRV-INFO: Handset: %d set DateTime: %02x.%02x.%02x %02x:%02x:%02x\n", u8_Hs,
   ...
   if( pv_List )
   {
      ...
      while( pv_IE != NULL )
      {
         if( u16_IE == CMBS_IE_PARAMETER_AREA )
         {
            ...
            cmbs_int_ie_ParameterAreaAdd( (void*)p_RespList, &st_ParamArea );
            cmbs_int_ie_ResponseAdd( (void*)p_RespList, &st_Resp );
      ...
      while( pv_IE != NULL )
      {
         ...
         if ( u16_IE == CMBS_IE_SUBSCRIBED_HS_LIST )
         {
            printf( "Number: %d Subscribed    Name: %S\n", st_IEInfo.Info.st_SubscribedHsList.u16_HsID, 
                                                           (wchar_t*)st_IEInfo.Info.st_SubscribedHsList.u16_HsName);
         }
      ...
      while( pv_IE != NULL )
      {
         ...
         if ( u16_IE == CMBS_IE_LINE_SETTINGS_LIST )
         {
			 printf("\nLineId = %d Line name = %S:\nAttached HS mask = 0x%X Call Intrusion = %d Multiple Calls = %d\n", 
                      st_IEInfo.Info.st_LineSettingsList.u8_Line_Id, (wchar_t*)st_IEInfo.Info.st_LineSettingsList.u16_LineName,
...
int            app_ServiceEntity( void * pv_AppRef, E_CMBS_EVENT_ID e_EventID, void * pv_EventData )
{
   ...
   switch ( e_EventID )
   {
      ...
      case CMBS_EV_DSR_TARGET_UP:
            APPCMBS_INFO(( "APPCMBS-SRV: Target Up!\n"));

            //TODO:  send CMBS_CMD_HELLO with media configuration info 
...
u16            app_HandsetMap ( char * psz_Handsets )
{
   ...
   if ( (u16_Handset != 0) && (u16_Handset <= CMBS_HS_SUBSCRIBED_MAX_NUM))
   {
