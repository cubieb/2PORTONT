...
*	\Author		stein
*
*	@(#)	%filespec: applistacc.c-5 %
*
*******************************************************************************
*	\par	History
*	\n==== History ============================================================\n
*	date			name		version	 action                                          \n
*      1 feb 2010    G.Lebastard                    Remove LineID and add Subtype + u16_LineNumberLineID handling
*	----------------------------------------------------------------------------\n
*
*******************************************************************************
...
E_CMBS_RC cmbs_util_LA_PrepareAndSendData(void * pv_AppRef,u16 u16_SessionId);
void cmbs_util_LA_ParseReceivedData(void * pv_AppRef, u16 u16_SessionId);
E_CMBS_RC app_LaDataPacketSend ( void * pv_AppRef, u16 SessionId, u16 u16_EntryID );
u16 app_LaNrOfListEntries ( u16 u16_ListId );
...
typedef struct
{
   ...
   u8           pu8_LineName[128];
   u8           u8_NumberOfCalls;
   E_CMBS_UTIL_LA_LINE_ID_SUBTYPE Subtype;          
   u16         u16_LineNumberLineID;
...
int               app_LaEntity( void * pv_AppRef, E_CMBS_EVENT_ID e_EventID, void * pv_EventData )
{
       ...
       e_EventID == CMBS_EV_DSR_LA_DATA_PACKET_SEND_RES )
   {
	  ...
	  else if( u16_IE == CMBS_IE_LA_ENTRY_ID )
         {
            cmbs_api_ie_LAEntryIdGet( pv_IE, &u16_EntryID );
            printf( "u16_EntryID=%d", u16_EntryID );
         }
      ...
      if( e_EventID == CMBS_EV_DSR_LA_SESSION_START )
      {
         ...
         {
            u16_NrOfEntries = app_LaNrOfListEntries ( u16_ListId );
      ...
      else if( e_EventID == CMBS_EV_DSR_LA_READ_ENTRIES )
      {
         ...
         {
         E_CMBS_RC e_res;
		 e_res = cmbs_util_LA_PrepareAndSendData(pv_AppRef,u16_SessionId);
      ...
      else if( e_EventID == CMBS_EV_DSR_LA_EDIT_ENTRY )
      {
         ...
         g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEntryNr  = u16_EntryID;
         g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEndIndex = u16_EntryID;
      ...
      {
         u16             u16_EntryIndex2;
         u16             u16_NumOfEntries = 3;  // Number of entries is constant in the demo code!
         E_CMBS_RESPONSE e_Response = CMBS_RESPONSE_OK;
         E_CMBS_LA_NACK_ERRORS
                         e_RejectReason = CMBS_LA_NACK_MAX;

		 u16_EntryIndex2 = u16_EntryID; // no index handling in this demo code!

         printf("Save Entry %d\n", u16_EntryID);

         return cmbs_dsr_la_SaveEntryRes( g_cmbsappl.pv_CMBSRef, u16_SessionId,
                                          u16_EntryID, u16_EntryIndex2, u16_NumOfEntries, e_Response, e_RejectReason );
      }
      else if( e_EventID == CMBS_EV_DSR_LA_DELETE_ENTRY )
      {
         ...
         else
         {
            ...
            if ( u16_EntryID > app_LaNrOfListEntries ( u16_ListId ) )
            {
               e_Response     = CMBS_RESPONSE_ERROR;
         ...
         if ( e_Response == CMBS_RESPONSE_OK )
         {
            ...
            u16_TotalNoOfEntries = app_LaNrOfListEntries ( u16_ListId ) - 1;
         }
         else
         {
            u16_TotalNoOfEntries = app_LaNrOfListEntries ( u16_ListId );
         }
	...
	r_res = cmbs_util_LA_AddEntryToFinalBuffer(pv_AppRef,
				                                              u16_SessionId,
				                                              st_DataGet.pu8_Data,
				                                              st_DataGet.u16_DataLen);

         // send response
        cmbs_dsr_la_DataPacketReceivedRes( pv_AppRef, u16_SessionId, r_res );
		
       // if it was the last then start the parsing
       if (u8_IsLast && (r_res==CMBS_RC_OK))
       	{
                   cmbs_util_LA_ParseReceivedData(pv_AppRef, u16_SessionId);
       	}
      ...
      else if( e_EventID == CMBS_EV_DSR_LA_DATA_PACKET_SEND_RES )
      {
         ...
         i_SessionIndex = app_LaGetSessionIndex( u16_SessionId );
      if (g_st_APP_LA_Session[i_SessionIndex].u16_SizeToSend!=0)
           {
           printf("It was not last packet, need to send more\n");
           cmbs_util_LA_SendFinalBuffer(pv_AppRef,u16_SessionId);
         }
      else
           printf("Last packet, No need to send more\n");

// here we need to send some more data if the last catiq 2 packet was not already sent
      /*
         i_SessionIndex = app_LaGetSessionIndex( u16_SessionId );
         if ( i_SessionIndex == APP_LA_MAX_NR_OF_SESSIONS )
         {
...
E_CMBS_RC  app_LaDataPacketSend ( void * pv_AppRef, u16 u16_SessionId, u16 u16_EntryID )
{

   u16             i_SessionIndex;
   
   u16 u16_index = 0;
u8 p_entry[512];   // Max size for 1 entry
u8 *pu8_finalbuff;
...
case CMBS_LA_LIST_MISSED_CALLS:
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_MCL_FLD_NUMBER) )
   {
    ST_UTIL_LA_FLD_NUMBER st_LA_Fld_Number;
      ...
      st_LA_Fld_Number.u16_DataLen   = (u16) strlen(g_st_APP_LA_MissedCallListEntry[u16_EntryID].pu8_Number);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_MCL_FLD_NAME) )
   {
    ST_UTIL_LA_FLD_NAME st_LA_Fld_Name;
      ...
      st_LA_Fld_Name.u16_DataLen     = (u16) strlen(g_st_APP_LA_MissedCallListEntry[u16_EntryID].pu8_Name);
   ...
   ST_UTIL_LA_FLD_DATETIME st_LA_Fld_DateTime;
      ...
      st_LA_Fld_DateTime.u8_Coding= g_st_APP_LA_MissedCallListEntry[u16_EntryID].st_DateTime.e_Coding;
      st_LA_Fld_DateTime.u8_interpretation= g_st_APP_LA_MissedCallListEntry[u16_EntryID].st_DateTime.e_Interpretation;
   ...
   ST_UTIL_LA_FLD_LINE_NAME  st_LA_Fld_LineName;
      ...
      st_LA_Fld_LineName.u16_DataLen   = (u16) strlen(g_st_APP_LA_MissedCallListEntry[u16_EntryID].pu8_LineName);
   ...
   ST_UTIL_LA_FLD_LINE_ID  st_LA_Fld_LineId;
      ...
      st_LA_Fld_LineId.Subtype= g_st_APP_LA_MissedCallListEntry[u16_EntryID].Subtype;
      st_LA_Fld_LineId.u16_LineNumber= g_st_APP_LA_MissedCallListEntry[u16_EntryID].u16_LineNumberLineID;
...
case CMBS_LA_LIST_OUTGOING_CALLS:
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_OCL_FLD_NUMBER) )
   {
    ST_UTIL_LA_FLD_NUMBER st_LA_Fld_Number;
      ...
      st_LA_Fld_Number.u16_DataLen   = (u16) strlen(g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].pu8_Number);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_OCL_FLD_NAME) )
   {
    ST_UTIL_LA_FLD_NAME st_LA_Fld_Name;
      ...
      st_LA_Fld_Name.u16_DataLen     = (u16) strlen(g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].pu8_Name);
   ...
   ST_UTIL_LA_FLD_DATETIME st_LA_Fld_DateTime;
      ...
      st_LA_Fld_DateTime.u8_Coding= g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].st_DateTime.e_Coding;
      st_LA_Fld_DateTime.u8_interpretation= g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].st_DateTime.e_Interpretation;
   ...
   ST_UTIL_LA_FLD_LINE_NAME  st_LA_Fld_LineName;
      ...
      st_LA_Fld_LineName.u16_DataLen   = (u16) strlen(g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].pu8_LineName);
   ...
   ST_UTIL_LA_FLD_LINE_ID  st_LA_Fld_LineId;
      ...
      st_LA_Fld_LineId.Subtype= g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].Subtype;
      st_LA_Fld_LineId.u16_LineNumber= g_st_APP_LA_OutgoingCallListEntry[u16_EntryID].u16_LineNumberLineID;
      cmbs_util_LA_Fld_LineIdAdd( p_entry, &st_LA_Fld_LineId,  &u16_index );
   }
   break;
...
case CMBS_LA_LIST_INCOMING_ACC:
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_IACL_FLD_NUMBER) )
   {
    ST_UTIL_LA_FLD_NUMBER st_LA_Fld_Number;
      ...
      st_LA_Fld_Number.u16_DataLen   = (u16) strlen(g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].pu8_Number);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_IACL_FLD_NAME) )
   {
    ST_UTIL_LA_FLD_NAME st_LA_Fld_Name;
      ...
      st_LA_Fld_Name.u16_DataLen     = (u16) strlen(g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].pu8_Name);
   ...
   ST_UTIL_LA_FLD_DATETIME st_LA_Fld_DateTime;
      ...
      st_LA_Fld_DateTime.u8_Coding= g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].st_DateTime.e_Coding;
      st_LA_Fld_DateTime.u8_interpretation= g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].st_DateTime.e_Interpretation;
   ...
   ST_UTIL_LA_FLD_LINE_NAME  st_LA_Fld_LineName;
      ...
      st_LA_Fld_LineName.u16_DataLen   = (u16) strlen(g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].pu8_LineName);
   ...
   ST_UTIL_LA_FLD_LINE_ID  st_LA_Fld_LineId;
      ...
      st_LA_Fld_LineId.Subtype= g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].Subtype;
      st_LA_Fld_LineId.u16_LineNumber= g_st_APP_LA_IncomingAcceptedCallListEntry[u16_EntryID].u16_LineNumberLineID;
      cmbs_util_LA_Fld_LineIdAdd( p_entry, &st_LA_Fld_LineId,  &u16_index );
   }
   break;
...
case CMBS_LA_LIST_CONTACT:
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_CL_FLD_NAME) )
   {
    ST_UTIL_LA_FLD_NAME st_LA_Fld_Name;
      ...
      st_LA_Fld_Name.u16_DataLen     = (u16) strlen(g_st_APP_LA_ContactListEntry[u16_EntryID].pu8_Name);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_CL_FLD_FIRST_NAME) )
   {
    ST_UTIL_LA_FLD_FIRST_NAME st_LA_Fld_FirstName;
      ...
      st_LA_Fld_FirstName.u16_DataLen     = (u16) strlen(g_st_APP_LA_ContactListEntry[u16_EntryID].pu8_FirstName);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_CL_FLD_CONTACT_NR) )
   {
    ST_UTIL_LA_FLD_CONTACT_NR st_LA_Fld_ContactNumber;
      ...
      st_LA_Fld_ContactNumber.u16_DataLen   = (u16) strlen(g_st_APP_LA_ContactListEntry[u16_EntryID].pu8_Number);
   ...
   ST_UTIL_LA_FLD_LINE_ID  st_LA_Fld_LineId;
      ...
      st_LA_Fld_LineId.Subtype= g_st_APP_LA_ContactListEntry[u16_EntryID].Subtype;
      st_LA_Fld_LineId.u16_LineNumber= g_st_APP_LA_ContactListEntry[u16_EntryID].u16_LineNumberLineID;
      cmbs_util_LA_Fld_LineIdAdd( p_entry, &st_LA_Fld_LineId,  &u16_index );
   }
   break;
...
case CMBS_LA_LIST_ALL_CALLS:
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_ACL_FLD_NUMBER) )
   {
    ST_UTIL_LA_FLD_NUMBER st_LA_Fld_Number;
      ...
      st_LA_Fld_Number.u16_DataLen   = (u16) strlen(g_st_APP_LA_AllCallListEntry[u16_EntryID].pu8_Number);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_ACL_FLD_NAME) )
   {
    ST_UTIL_LA_FLD_NAME st_LA_Fld_Name;
      ...
      st_LA_Fld_Name.u16_DataLen     = (u16) strlen(g_st_APP_LA_AllCallListEntry[u16_EntryID].pu8_Name);
   ...
   ST_UTIL_LA_FLD_DATETIME st_LA_Fld_DateTime;
      ...
      st_LA_Fld_DateTime.u8_Coding= g_st_APP_LA_AllCallListEntry[u16_EntryID].st_DateTime.e_Coding;
      st_LA_Fld_DateTime.u8_interpretation= g_st_APP_LA_AllCallListEntry[u16_EntryID].st_DateTime.e_Interpretation;
   ...
   ST_UTIL_LA_FLD_LINE_NAME  st_LA_Fld_LineName;
      ...
      st_LA_Fld_LineName.u16_DataLen   = (u16) strlen(g_st_APP_LA_AllCallListEntry[u16_EntryID].pu8_LineName);
   ...
   ST_UTIL_LA_FLD_LINE_ID  st_LA_Fld_LineId;
      ...
      st_LA_Fld_LineId.Subtype= g_st_APP_LA_AllCallListEntry[u16_EntryID].Subtype;
      st_LA_Fld_LineId.u16_LineNumber= g_st_APP_LA_AllCallListEntry[u16_EntryID].u16_LineNumberLineID;
      cmbs_util_LA_Fld_LineIdAdd( p_entry, &st_LA_Fld_LineId,  &u16_index );
}
   break;
...
case CMBS_LA_LIST_LINE_SETTINGS:
   ...
   ST_UTIL_LA_FLD_LINE_NAME  st_LA_Fld_LineName;
      ...
      st_LA_Fld_LineName.u16_DataLen   = (u16) strlen(g_st_APP_LA_LineSettingListEntry[u16_EntryID].pu8_LineName);
   ...
   ST_UTIL_LA_FLD_LINE_ID  st_LA_Fld_LineId;
      ...
      st_LA_Fld_LineId.Subtype= g_st_APP_LA_LineSettingListEntry[u16_EntryID].Subtype;
      st_LA_Fld_LineId.u16_LineNumber= g_st_APP_LA_LineSettingListEntry[u16_EntryID].u16_LineNumberLineID;
      cmbs_util_LA_Fld_LineIdAdd( p_entry, &st_LA_Fld_LineId,  &u16_index );
   }
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_LSL_FLD_DIALING_PREFIX) )  // see number
   {
    ST_UTIL_LA_FLD_DIALING_PREFIX st_LA_Fld_DialingPrefixe;
      ...
      st_LA_Fld_DialingPrefixe.u16_DataLen     = (u16) strlen(g_st_APP_LA_LineSettingListEntry[u16_EntryID].pu8_DialingPrefixe);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_LSL_FLD_BLOCKED_NUMBER) )  // see number
   {
    ST_UTIL_LA_FLD_BLOCKED_NUMBER st_LA_Fld_BlockedNumber;
      ...
      st_LA_Fld_BlockedNumber.u16_DataLen     = (u16) strlen(g_st_APP_LA_LineSettingListEntry[u16_EntryID].pu8_BlockedNumber);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_LSL_FLD_CALL_FORWARDING) ) 
   {
    ST_UTIL_LA_FLD_CALL_FORWARDING st_LA_Fld_CallForwarding;
      ...
      st_LA_Fld_CallForwarding.u8_NumberLen   = (u8) strlen(g_st_APP_LA_LineSettingListEntry[u16_EntryID].pu8_CFUNumber);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_LSL_FLD_CFNA) ) 
   {
    ST_UTIL_LA_FLD_CFNA st_LA_Fld_CFNA;
      ...
      st_LA_Fld_CFNA.u8_NumberLen   = (u8) strlen(g_st_APP_LA_LineSettingListEntry[u16_EntryID].pu8_CFNANumber);
   ...
   if ( g_st_APP_LA_Session[i_SessionIndex].u16_requestedFields & (1 << CMBS_LSL_FLD_CFB) ) 
   {
    ST_UTIL_LA_FLD_CFB st_LA_Fld_CFB;
      ...
      st_LA_Fld_CFB.u8_NumberLen   = (u8) strlen(g_st_APP_LA_LineSettingListEntry[u16_EntryID].pu8_CFBNumber);
...
pu8_finalbuff =    cmbs_util_LA_EntryLenght_EntryIdentifierAdd( p_entry, u16_index, u16_EntryID);  
  ...
  return cmbs_util_LA_AddEntryToFinalBuffer(pv_AppRef,
                                              u16_SessionId,
                                              pu8_finalbuff,
                                              u16_index - (pu8_finalbuff-p_entry));
   ...
   for(u16_Index = 1; u16_Index <= APP_LA_MAX_MISSED_CALL_LIST_ENTRIES; u16_Index++)
   {
      if ( g_st_APP_LA_MissedCallListEntry[u16_Index].u8_EntryFilled )
      {
         u16_Cnt++;
	   	...
	   	case CMBS_LA_LIST_CONTACT:
		{
			for(u16_Index = 1; u16_Index <= APP_LA_MAX_CONTACT_LIST_ENTRIES; u16_Index++)
			{
				if ( g_st_APP_LA_ContactListEntry[u16_Index].u8_EntryFilled )
				{
					u16_Cnt++;
				}
			}
	   	...
	   	case CMBS_LA_LIST_INCOMING_ACC:
		{
			for(u16_Index = 1; u16_Index <= APP_LA_MAX_INCOMING_ACCEPTED_CALL_LIST_ENTRIES; u16_Index++)
			{
				if ( g_st_APP_LA_IncomingAcceptedCallListEntry[u16_Index].u8_EntryFilled )
				{
					u16_Cnt++;
				}
			}
	   	...
	   	case CMBS_LA_LIST_OUTGOING_CALLS:
		{
			for(u16_Index = 1; u16_Index <= APP_LA_MAX_OUTGOING_CALL_LIST_ENTRIES; u16_Index++)
			{
				if ( g_st_APP_LA_OutgoingCallListEntry[u16_Index].u8_EntryFilled )
				{
					u16_Cnt++;
				}
			}
	   	...
	   	case CMBS_LA_LIST_LINE_SETTINGS:
		{
			for(u16_Index = 1; u16_Index <= APP_LA_MAX_LINE_SETTING_LIST_ENTRIES; u16_Index++)
			{
				if ( g_st_APP_LA_LineSettingListEntry[u16_Index].u8_EntryFilled )
				{
					u16_Cnt++;
				}
			}
...
void app_LaInitExampleLists( void )
{
   ...
   strcpy((char *)g_st_APP_LA_MissedCallListEntry[1].pu8_Number, "123**112");
   strcpy((char *)g_st_APP_LA_MissedCallListEntry[1].pu8_Name,   "JENDREZEJZAK");
   g_st_APP_LA_MissedCallListEntry[1].st_DateTime.e_Coding= CMBS_DATE_TIME;
   g_st_APP_LA_MissedCallListEntry[1].st_DateTime.e_Interpretation= CMBS_CURRENT_TIME;
   ...
   g_st_APP_LA_MissedCallListEntry[1].Subtype=CMBS_UTIL_LA_LINE_ID_EXT_CALL;         
   g_st_APP_LA_MissedCallListEntry[1].u16_LineNumberLineID=0x31;  
   ...
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.e_Coding= CMBS_DATE_TIME;
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.e_Interpretation= CMBS_CURRENT_TIME;
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.u8_Year = 0x9;  //2009
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.u8_Month = 0x3; // march
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.u8_Day = 0x17; // 17
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.u8_Hours = 0x12; // 12h
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.u8_Mins = 0x7; // 7min
   g_st_APP_LA_MissedCallListEntry[2].st_DateTime.u8_Secs = 0x8; // 8 sec
   g_st_APP_LA_MissedCallListEntry[2].u8_New = 1;
   strcpy((char *)g_st_APP_LA_MissedCallListEntry[2].pu8_LineName, "Provider 1");
   g_st_APP_LA_MissedCallListEntry[2].Subtype=CMBS_UTIL_LA_LINE_ID_EXT_CALL;         
   g_st_APP_LA_MissedCallListEntry[2].u16_LineNumberLineID=12;  
   ...
   g_st_APP_LA_MissedCallListEntry[3].st_DateTime.e_Coding= CMBS_DATE;
   g_st_APP_LA_MissedCallListEntry[3].st_DateTime.e_Interpretation= CMBS_CURRENT_TIME;
   ...
   g_st_APP_LA_MissedCallListEntry[3].Subtype=CMBS_UTIL_LA_LINE_ID_EXT_CALL;         
   g_st_APP_LA_MissedCallListEntry[3].u16_LineNumberLineID=4;  
   ...
   g_st_APP_LA_MissedCallListEntry[4].st_DateTime.e_Coding= CMBS_TIME;
   g_st_APP_LA_MissedCallListEntry[4].st_DateTime.e_Interpretation= CMBS_CURRENT_TIME;
   ...
   g_st_APP_LA_MissedCallListEntry[4].Subtype=CMBS_UTIL_LA_LINE_ID_EXT_CALL;         
   g_st_APP_LA_MissedCallListEntry[4].u16_LineNumberLineID=12;  
   ...
   g_st_APP_LA_MissedCallListEntry[5].st_DateTime.e_Coding= CMBS_DATE_TIME;
   g_st_APP_LA_MissedCallListEntry[5].st_DateTime.e_Interpretation= CMBS_CURRENT_TIME;
   ...
   g_st_APP_LA_MissedCallListEntry[5].Subtype=CMBS_UTIL_LA_LINE_ID_EXT_CALL;         
   g_st_APP_LA_MissedCallListEntry[5].u16_LineNumberLineID=1;    
   g_st_APP_LA_MissedCallListEntry[5].u8_NumberOfCalls = 1;
   g_st_APP_LA_MissedCallListEntry[5].u8_EntryFilled = 1;


   g_st_APP_LA_LineSettingListEntry[1].u8_EntryFilled = 1;
   strcpy((char *)g_st_APP_LA_LineSettingListEntry[1].pu8_LineName,   "gillin");
   g_st_APP_LA_LineSettingListEntry[1].Subtype=CMBS_UTIL_LA_LINE_ID_EXT_CALL;         
   g_st_APP_LA_LineSettingListEntry[1].u16_LineNumberLineID=1;    
   g_st_APP_LA_LineSettingListEntry[1].u8_NumberHandsets=5;
   g_st_APP_LA_LineSettingListEntry[1].pu8_HandsetBitfield[0] = 0x83; // 1rst byte is the bitfield for HS1 to HS8, LSB is HS1, following byte is for HS9 to HS16, etc etc until HS255
   g_st_APP_LA_LineSettingListEntry[1].pu8_HandsetBitfield[1] = 0x41; // 1rst byte is the bitfield for HS1 to HS8, LSB is HS1, following byte is for HS9 to HS16, etc etc until HS255
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_DialingPrefixe, "333");
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_BlockedNumber, "4444");
   g_st_APP_LA_LineSettingListEntry[1].u8_FPMelody = 3;
   g_st_APP_LA_LineSettingListEntry[1].u8_FPVolume = 8;
   g_st_APP_LA_LineSettingListEntry[1].u8_MultiCallMode = 0x31;
   g_st_APP_LA_LineSettingListEntry[1].u8_IntrusionCall = 0x31;
   g_st_APP_LA_LineSettingListEntry[1].u8_CLIRValue = 0x31;
   g_st_APP_LA_LineSettingListEntry[1].u8_CLIRActDataLen = 5; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CLIRActData, "55555");
   g_st_APP_LA_LineSettingListEntry[1].u8_CLIRDesactDataLen=6; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CLIRDesactData, "666666");
   g_st_APP_LA_LineSettingListEntry[1].u8_CFUValue = 0x31;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFUNotSetByUser = 0;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFUActDataLen = 7; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFUActData, "7777777");
   g_st_APP_LA_LineSettingListEntry[1].u8_CFUDesactDataLen = 2; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFUDesactData, "22");
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFUNumber, "333");
   g_st_APP_LA_LineSettingListEntry[1].u8_CFNANotSetByUser = 0;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFNAValue=0x31;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFNANbSecond=20;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFNAActDataLen=4; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFNAActData,"4444");
   g_st_APP_LA_LineSettingListEntry[1].u8_CFNADesactDataLen=5; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFNADesactData,"55555");
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFNANumber,"666666");
   g_st_APP_LA_LineSettingListEntry[1].u8_CFBNotSetByUser=0;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFBValue=0x31;
   g_st_APP_LA_LineSettingListEntry[1].u8_CFBActDataLen=1; 
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFBActData,"1");
   g_st_APP_LA_LineSettingListEntry[1].u8_CFBDesactDataLen=0; 
   //g_st_APP_LA_LineSettingListEntry[1].pu8_CFBDesactData=;
   strcpy(g_st_APP_LA_LineSettingListEntry[1].pu8_CFBNumber, "4444");
...
g_st_APP_LA_Session[i_SessionIndex].u16_Index=0; // init index
            do
            {
               u8_IsLast = (g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEntryNr == g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEndIndex) ? 1 : 0;
               e_res = app_LaDataPacketSend (pv_AppRef,u16_SessionId, g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEntryNr );      
               printf("Add entry num %d\n", g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEntryNr);
               g_st_APP_LA_Session[i_SessionIndex].u16_CMBS_LAEntryNr++;
            }while((!u8_IsLast) && (e_res==CMBS_RC_OK));

            printf("All Entries ready\n");
...
u16 i;
	...
	pu8_BufferToParse = g_st_APP_LA_Session[i_SessionIndex].u8_finalbuffer;

	cmbs_util_LA_EntryLenght_EntryIdentifierGet(pu8_BufferToParse, &u16_EntryIdentifier, &u16_EntryLength, &u16_Index); // index will be updated here to point on fieldId 1
      ...
      switch(g_st_APP_LA_Session[i_SessionIndex].u16_ListId)
		...
		case CMBS_LA_LIST_MISSED_CALLS:
			while (u16_Index < u16_EntryLength) // Check if it was the last field
				{
				pu8_FieldId = pu8_BufferToParse + u16_Index; // get the pointer on fieldid
                           printf("pu8_FieldId=%x\n",*pu8_FieldId);
				switch(*pu8_FieldId) // check the fieldid content
					...
					case CMBS_MCL_FLD_NUMBER:
                     ...
                     APPCMBS_IE(("Number: "));
                     for( i=0; i< st_LA_Fld_Number.u16_DataLen; i++ )
                        APPCMBS_IE(( "%02X ", st_LA_Fld_Number.pu8_Data[i]));
                     APPCMBS_IE(("\n"));
					...
					case CMBS_MCL_FLD_NAME:
                     ...
                     APPCMBS_IE(("Name: "));
                     for( i=0; i< st_LA_Fld_Name.u16_DataLen; i++ )
                        APPCMBS_IE(( "%02X ", st_LA_Fld_Name.pu8_Data[i]));
                     APPCMBS_IE(("\n"));
					...
					case CMBS_MCL_FLD_DATETIME:
                     ...
                     APPCMBS_IE(("Year:%0x Month:%0x Day:%0x "
                                 "Hour:%0x Minutes:%0x Seconds:%0x\n",
                        st_LA_Fld_DateTime.u8_Year,
                        st_LA_Fld_DateTime.u8_Month,
                        st_LA_Fld_DateTime.u8_Day,
                        st_LA_Fld_DateTime.u8_Hours,
                        st_LA_Fld_DateTime.u8_Mins,
                        st_LA_Fld_DateTime.u8_Secs
                        ));
						}
					break;
					...
					case CMBS_MCL_FLD_LINE_NAME:
                     ...
                     APPCMBS_IE(("Line Name: "));
                     for( i=0; i< st_LA_Fld_LineName.u16_DataLen; i++ )
                        APPCMBS_IE(( "%02X ", st_LA_Fld_LineName.pu8_Data[i]));
                     APPCMBS_IE(("\n"));
					...
					case CMBS_MCL_FLD_LINE_ID:
                     ...
                     APPCMBS_IE(("Line ID:%d\n",st_LA_Fld_LineId.u16_LineNumber));
						}
					break;
					...
					case CMBS_LSL_FLD_ATTACHED_HANDSETS:
						{
							...
							u8 pu8_HandsetBitfield[38];
							st_LA_Fld_AttachedHandset.pu8_Data=pu8_HandsetBitfield; // need to allocate memory, cannot use the mem from the buffer since it needs rework
		...
		break;
	            ...
	            g_st_APP_LA_Session[i_SessionIndex].u16_SizeToSend = 0;  // reset buffer
 }
