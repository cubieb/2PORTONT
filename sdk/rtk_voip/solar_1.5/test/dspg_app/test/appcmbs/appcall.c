...
void          appcall_InfoPrint(void)
{
   ...
   for(i=0; i < APPCALL_CALLOBJ_MAX; i++ )
   {
      ...
      APPCMBS_INFO(( "APPCMBS-CALL: INFO ----------------------\n" ));
      if (g_call_automat)
      {
         APPCMBS_INFO(( "APPCMBS-CALL: --AUTOMAT ON ---------------\n" ));
      }
      else
      {
         APPCMBS_INFO(( "APPCMBS-CALL: --AUTOMAT OFF ---------------\n" ));
      }

      if ( g_call_obj[i].u32_CallInstance )
      {
         APPCMBS_INFO((  "APPCMBS-CALL: INFO Line %d\n",i ));

         if ( g_call_obj[i].ch_CallerID )
            APPCMBS_INFO((  "APPCMBS-CALL: INFO CLI %s\n", g_call_obj[i].ch_CallerID ));
         if ( g_call_obj[i].ch_CalledID )
            APPCMBS_INFO((  "APPCMBS-CALL: INFO CLD %s\n", g_call_obj[i].ch_CalledID ));

         APPCMBS_INFO((  "APPCMBS-CALL: INFO Call State  %s\n",_appcall_CallObjStateString(g_call_obj[i].e_Call) ));
         APPCMBS_INFO((  "APPCMBS-CALL: INFO Media State %s\n",_appcall_CallOBJMediaString(g_call_obj[i].e_Media) ));
         APPCMBS_INFO((  "APPCMBS-CALL: INFO\n" ));
         APPCMBS_INFO(( "APPCMBS-CALL: INFO CallInstance %u =%lx\n",g_call_obj[i].u32_CallInstance, (long unsigned int )g_call_obj[i].u32_CallInstance));
         APPCMBS_INFO(( "APPCMBS-CALL: INFO Codec %s\n", _appcall_CallOBJMediaCodecString( g_call_obj[i].e_Codec )));
      }
      else
      {
   ...
   for (i=0; i< APPCALL_CALLOBJ_MAX; i++ )
   {
      if ( !g_call_obj[i].u32_CallInstance )
         return g_call_obj + i;
      	...
      	printf("[THLin]: _appcall_CallOBJNew: can not new Call OBJ\n");
   }
   ...
   if ( pst_This && (E_APPCMBS_MEDIA_PEND) == _appcall_CallOBJMediaGet(pst_This))
   {
         ...
         if( pv_RefIEList )
         {
            ...
            if ( pst_This->e_Codec == CMBS_AUDIO_CODEC_PCM_LINEAR_WB )
            {
               ...
               // st_MediaChannel.u32_ChannelParameter = 0x3 << (pst_This->u32_ChannelID * 2 );
            }
            else
            {
//               st_MediaChannel.u32_ChannelParameter = 0x3 << (pst_This->u32_ChannelID * 2 );
          // [DK] the IOM channel is independent of the Air interface. IOM could be WB and Air interface could be NB!
//                 st_MediaChannel.u32_ChannelParameter = 0xF << (pst_This->u32_ChannelID *4 );
   ...
   if( pv_List )
   {
      ...
      while( pv_IE != NULL )
      {
         ...
         switch ( u16_IE )
         {
            ...
            printf("CALLERID get %02x\n", st_IEInfo.Info.st_CallerParty.u8_AddressProperties);
            if ( pst_Call )
            {
               int i;

               printf ( "CallerID %d\n", st_IEInfo.Info.st_CallerParty.u8_AddressLen );
               for (i=0; i < st_IEInfo.Info.st_CallerParty.u8_AddressLen; i ++ )
                  printf (" %02x", st_IEInfo.Info.st_CallerParty.pu8_Address[i] );
...
void        app_OnCallAnswer( void * pvAppRefHandle,void * pv_List )
{
   ...
   if( pv_List )
   {
      ...
      //printf("app_OnCallAnswer In\n");
            
      while( pv_IE != NULL )
      {
      	 //printf("app_OnCallAnswer: u16_IE (1)=%d\n", u16_IE);
         app_IEToString( pv_IE, u16_IE );

	 //printf("app_OnCallAnswer: u16_IE (2)=%d\n", u16_IE);
         appcmbs_IEInfoGet ( pv_IE, u16_IE, &st_IEInfo );

	 //printf("app_OnCallAnswer: u16_IE (3)=%d\n", u16_IE);
         switch ( u16_IE )
         {
            case CMBS_IE_CALLINSTANCE:
               //Todo: get call resources for this CallInstance...
            ...
            default:
            	//pst_Call = _appcall_CallOBJGet(st_IEInfo.Info.u32_CallInstance, NULL);
            	printf("[THLin]app_OnCallAnswer: get IE=%d. appcmbs NOT handle it.\n", u16_IE);
           	break;

         }

	 if ( pst_Call )//Thlin Fix Crash
         	_appmedia_CallOBJMediaPropertySet ( pst_Call, u16_IE, &st_IEInfo );
...
void _appcall_CalledIDSet(PST_CALL_OBJ pst_This, PST_APPCALL_PROPERTIES pst_Properties, int n_Properties , PST_IE_CALLEDPARTY pst_CalledParty )
{
   ...
   if ( n_Pos < n_Properties)
   {
      ...
      {
         pst_CalledParty->u8_AddressProperties  = CMBS_ADDR_PROPTYPE_UNKNOWN | CMBS_ADDR_PROPPLAN_INTGRP;
      }
...
int      appcall_EstablishCall ( PST_APPCALL_PROPERTIES pst_Properties, int n_Properties )
{
   ...
   if ( !pst_This )
   {
   	printf("[THLin]: ERROR! appcall_EstablishCall can not New Call Obj\n");
      return FALSE;
   }
