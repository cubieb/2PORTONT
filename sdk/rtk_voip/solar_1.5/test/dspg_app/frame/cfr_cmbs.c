...
#include "tcx_log.h"  /* debug handling */
...
/*!
		...
		\return				 < int > return identifier of queue. if failt, returns -1
   ...
   if( pst_DevCtl )
   {
      ...
      switch( pst_DevCtl->e_DevType )
      {
         case CMBS_DEVTYPE_UART:
            g_CMBSInstance.fdDevCtl = cfr_uartInitialze( pst_DevCtl->u_Config.pUartCfg);
            ...
            if( g_CMBSInstance.fdDevCtl != -1 )
            {
               ...
               if( rc != 0 )
               {
                  CFR_DBG_ERROR( "[ERROR]cmbs_api_Init: !!!! Couldn't create UART Thread. ErrorCode:%d\n", rc );
            ...
            {
               CFR_DBG_ERROR( "[ERROR]cmbs_api_Init: !!!! Couldn't open UART device\n" );
            }

            return CMBS_RC_ERROR_GENERAL;
         
         case CMBS_DEVTYPE_SPI:
            g_CMBSInstance.fdDevCtl = cfr_SpiInitialze();

            if( g_CMBSInstance.fdDevCtl != -1 )
            {
               rc = pthread_create( &g_CMBSInstance.serialThreadId, NULL, & cfr_SpiRxThread, &g_CMBSInstance );
               if( rc != 0 )
               {
                  CFR_DBG_ERROR( "[ERROR]cmbs_api_Init: !!!! Couldn't create SPI RX Thread. ErrorCode:%d\n", rc );
                  /*! \todo exception handling is needed */
                  return CMBS_RC_ERROR_GENERAL;
               }
            ...
            {
               CFR_DBG_ERROR( "[ERROR]cmbs_api_Init: !!!! Couldn't open SPI device\n" );
            }
         ...
         for( i = 0; i < (u32)LinMsg.msgData.nLength; i++ )
         {
            ...
            if( nDataIndex >= sizeof(u32) + sizeof(u16) ) // sizeof(u32_Sync) + sizeof(u16_TotalLength)
            {
               ...
               if( nDataIndex == sizeof(CmbsMsg.st_Data.u32_Sync) + u16_Total )
               {
                  ...
                  if( g_CMBSInstance.e_Endian == E_CMBS_ENDIAN_BIG )
                  {
                     cms_int_HdrEndianCvt( &CmbsMsg.st_Data.st_Msg.st_MsgHdr );
                  }

                  tcx_LogIncomingPacketWriteFinish((u8 *)&CmbsMsg.st_Data.st_Msg, CmbsMsg.st_Data.st_Msg.st_MsgHdr.u16_TotalLength);
