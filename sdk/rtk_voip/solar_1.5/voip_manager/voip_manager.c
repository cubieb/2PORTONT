...
uint64 g_VoIP_Feature = 0;

int gCad_on_msec[MAX_VOIP_CH_NUM] = {[0 ... MAX_VOIP_CH_NUM-1] = 2000};		// 2s on
int gCad_off_msec[MAX_VOIP_CH_NUM] = {[0 ... MAX_VOIP_CH_NUM-1] = 4000};	// 4s on
...
int g_sip_register[MAX_VOIP_CH_NUM] = {[0 ... MAX_VOIP_CH_NUM-1] = 0};
...
int32 rtk_SetRtcpConfig(rtp_config_t *cfg, unsigned short rtcp_tx_interval)	//thlin+ for Rtcp
{
    ...
    stVoipRtcpSession.enableXR = cfg->rtcp_xr_enable;
#else
    stVoipRtcpSession.enableXR = 0;
#endif
	stVoipRtcpSession.tx_interval = rtcp_tx_interval;
    if (cfg->isTcp)
    {
        printf("==> SUPPORT UDP ONLY\n");
        return -1;
...
int32 rtk_SetThresholdVadCng( int32 chid, int32 mid, int32 nThresVAD, int32 nThresCNG, int32 nModeCNG, int32 nLevelCNG, int32 nGainCNG )
{
	...
	stVoipThresVadCngConfig.nCNGMode = nModeCNG;
	stVoipThresVadCngConfig.nCNGLevel = nLevelCNG;
	stVoipThresVadCngConfig.nCNGGain = nGainCNG;
...
return 0;
	...
	{
		printf("==>UNKOWN SLIC STATUS (%d)=%X\n", chid, stVoipSlicHook.hook_status);   /* HOOK FIFO empty also belong to this case */
		return -1;
	}
}
	...
	*pval = SIGN_NONE;
	...
	else if (stVoipSlicHook.hook_status == 11) /* FXO_RING_ON */
	{
		*pval = SIGN_OFFHOOK; // off-hook
		...
		stVoipSlicHook.hook_status == 19 )   /* FXO_DIS_TONE */
	{
       	*pval = SIGN_ONHOOK;	// on-hook
		rtk_SetPlayTone(chid, 0, DSPCODEC_TONE_DIAL, 0, DSPCODEC_TONEDIRECTION_LOCAL);
		usleep(100000); // [Important] sleep >= 100ms. MUST add delay for ACMW to stop tone!
...
int32 rtk_GetRealFxoEvent(uint32 chid, FXOEVENT *pval)
{
	...
	*pval = FXOEVENT_NONE;
	...
	if (stVoipSlicHook.hook_status == 10) /* FXO_STILL_OFF_HOOK */
	{
		...
		if (stVoipValue.value >= '1' && stVoipValue.value <= '9')
		    *pval = FXOEVENT_KEY1 + stVoipValue.value - '1';
		else if (stVoipValue.value == '0')
		    *pval = FXOEVENT_KEY0;
		else if (stVoipValue.value == '*')
		    *pval = FXOEVENT_STAR;
	...
	else if (stVoipSlicHook.hook_status == 13)/* FXO_BUSY_TONE */
	{
	...
	else if (stVoipSlicHook.hook_status == 19)   /* FXO_DIS_TONE */
	{
...
int32 rtk_SetRFC2833TxConfig(uint32 chid, uint32 volume, uint32 tx_mode) //tx_mode: 0: DSP mode, 1: AP mode
{
...
int32 rtk_GetRfc2833RxEvent(uint32 chid, uint32 mid, RFC2833_EVENT *pent)
{
	...
	SETSOCKOPT(VOIP_MGR_RFC2833_RX_GET, &stVoipValue, TstVoipValue, 1);
	
	*pent = stVoipValue.value;
...
int32 rtk_Gen_FSK_VMWI(uint32 chid, char *state, char mode)  /* state:	point to the address of value to set VMWI state. 0 : off; 1 : on*/
{
	...
	if(!mode)			// on-hook
		rtk_eanblePCM(chid, 1); // enable PCM before generating VMWI
...
int32 rtk_set_dtmf_det_threshold(uint32 chid, int32 threshold, uint32 dir) /* Threshold: 0 ~ 40, it means 0 ~ -40 dBm */
{
	TstVoipCfg stVoipCfg;

	stVoipCfg.ch_id = chid;
	stVoipCfg.cfg = threshold;
	stVoipCfg.cfg2 = dir;
    
	SETSOCKOPT(VOIP_MGR_DTMF_DET_THRESHOLD, &stVoipCfg, TstVoipCfg, 1);
...
{
	pstVoipSessionStatistics ->chid = chid;
	pstVoipSessionStatistics ->sid = sid;
...
int32 rtk_qos_reset_dscp_priority()
{
	int dscp = 0xFF;
...
int32 rtk_SetDspIdToDsp(unsigned char dsp_id)
{
    TstVoipValue stVoipValue;

    stVoipValue.value = dsp_id;
...
int32 rtk_SetDspPhyId(unsigned char phy_id)
{
    TstVoipValue stVoipValue;

    stVoipValue.value = phy_id;
...
uint8 rtk_CheckDspAllSoftwareReady(unsigned char phy_id)
{
    TstVoipValue stVoipValue;
