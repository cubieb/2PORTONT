...
#define DEV_TX "/dev/voip/pcmtx0"
#define DEV_RX "/dev/voip/pcmrx0"
...
int main(int argc, char *argv[]) {
  ...
  printf("%s: sending data to '%s' Port : %d (%s) (%s)\n", argv[0], h->h_name,port,type?"ALAW":"ULAW",dir?"RX":"TX");
  ...
  inp = fopen(dir?DEV_RX:DEV_TX, "rb");
...
wrong_input:
	...
	printf("usage : %s <server> <port> <ALAW/ULAW> <RX/TX>\n", argv[0]);
	return -1;
}
