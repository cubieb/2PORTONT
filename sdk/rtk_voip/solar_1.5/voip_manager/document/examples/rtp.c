...
int main(int argc, char *argv[])
{
	...
	while (1)
	{
		...
		case SIGN_OFFHOOK:
			...
			// set RTCP session
			memcpy(&rtcp_config, &rtp_config, sizeof(rtcp_config));
			rtcp_config.extPort++; 
			rtcp_config.remPort++;
			rtk_SetRtcpConfig(&rtcp_config,500); //send rtcp every 500ms
