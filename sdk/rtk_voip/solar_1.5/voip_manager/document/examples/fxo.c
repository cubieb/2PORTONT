...
void channel_t38_start(channel_t *channel)
{
	...
	codec_config.local_pt = 0;
	codec_config.remote_pt = 0;
	codec_config.uPktFormat = rtpPayloadT38_Virtual;
...
void channel_handle_input(channel_t *channel, SIGNSTATE val)
{
	...
	case DIAL_FXO_1:
		...
		{
			channel_dial_local(channel, &channels[SLIC_CH_NUM+1]);	// dial fxo 1
		}
		break;
	case DIAL_FXO_2:
		if (fxo_type == FXO_NO_DAA)
		...
		{
			channel_dial_local(channel, &channels[SLIC_CH_NUM+2]);	// dial fxo 2
		}
		break;
	case DIAL_FXO_3:
		if (fxo_type == FXO_NO_DAA)
		...
		{
			channel_dial_local(channel, &channels[SLIC_CH_NUM+3]);	// dial fxo 3
		}
		break;
	case DIAL_FXS_0:
...
void channel_handle_offhook(channel_t *channel)
{
	...
	case STATE_IDLE:
		...
		else
		{
			...
			rtk_Set_FSK_Area(channel->chid - 4, 128/*area*/);   /* area -> 0:Bellcore 1:ETSI 2:BT 3:NTT */
...
{
	int i;
	int fax[VOIP_CH_NUM] = {0};
	int fax_clear[VOIP_CH_NUM] = {[0 ... VOIP_CH_NUM-1] = 1};
	int fax_end[VOIP_CH_NUM] = {0};
	int fax_end_clear[VOIP_CH_NUM] = {[0 ... VOIP_CH_NUM-1] = 1};
...
main_loop:
	...
	for (i=0; i<VOIP_CH_NUM; i++)
	{
		...
		if (CHANNEL_IS_FXS(i))
		{
			rtk_GetFaxModemEvent(i, &fax[i], 0);

			if ((fax[i] == 1) && (fax_clear[i] ==1))
			{
				printf("Get CED event, ch=%d\n", i);
