...
#ifdef WIN32
// Use 'make w32' to generate these definition.
#define SLIC_NUM ???
#define VOIP_CH_NUM ???
#else
#define SLIC_NUM 4
#define VOIP_CH_NUM 4
#endif
...
// ver 58:
// - add CNG noise level configuration
// ver 58:
...
#define VOIP_FLASH_FEATURE_MASK			( ~( DECT_NUM_MASK | SLIC_NUM_MASK | DAA_NUM_MASK ) )
...
struct voipCfgPortParam_s
{
	...
	unsigned char 	cng_gainmode;				///< cng noise gain mode, default: 0(disable configurable), 1(fixed noise gain), 2(based on CN payload)
	unsigned char 	cng_noiselevel;				///< cng noise level value, default: 70(-70dBov), 0~127( 0dB ~ -127dBov)
	signed char 	cng_noisegain;				///< cng noise gain value, default: 0(0dB), -127~127 (negative is gain down and vice versa)
