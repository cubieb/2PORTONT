...
int print_flash_confg( voipCfgParam_t *vcfg )
{
	int i=0 , j=0;
	for( i=0 ; i<2 ; i++ ){
		for(j=0 ; j<2 ; j++){
		    rcm_dp( "port[%d].proxy[%d]\n" , i , j );
	    	rcm_dp( "\tdisplay_name=%s\n" 		, vcfg->ports[i].proxies[j].display_name );
	    	rcm_dp( "\tnumber=%s\n" 			, vcfg->ports[i].proxies[j].number );
	    	rcm_dp( "\tlogin_id=%s\n" 			, vcfg->ports[i].proxies[j].login_id );
	    	rcm_dp( "\tpassword=%s\n" 			, vcfg->ports[i].proxies[j].password );
	    	rcm_dp( "\taddr=%s\n" 				, vcfg->ports[i].proxies[j].addr );
	    	rcm_dp( "\tport=%d\n" 				, vcfg->ports[i].proxies[j].port );
    		rcm_dp( "\tdomain=%s\n"				, vcfg->ports[i].proxies[j].domain_name );
		}
	}
...
#if 0
int voip_flash_get_config(voipCfgParam_t *cfg_pa)
{
//#if 0
    voipCfgAll_t 	 cfg_all;
    char *buf, 	 *text;
    int  buf_len, text_len;
	const char *need_modify=getenv("NEED_MODIFY");
	const char *sip_listen_port=getenv("SIP_LISTEN_PORT");

	char exp_cfg_txt[FILENAME_MAX]={0};

	// read file
    if (flash_voip_read_file(flash_filename, &buf, &buf_len) != 0)
    {
...
#if 0
	snprintf(exp_cfg_txt, sizeof(exp_cfg_txt), "%s_exp.txt", flash_filename );

	// output to text file directly
	if (flash_voip_write_file(exp_cfg_txt, text, text_len) != 0)
	{
    ...
    memcpy( cfg_pa , &cfg_all.current_setting , sizeof(voipCfgParam_t) );

	if( sip_listen_port!=NULL ){
		rcm_change_port( cfg_pa );
	} 
	
	if( need_modify!=NULL ){
		rcm_change_config( cfg_pa );
	}
		
	rcm_dp( "NEED_MODIFY=%s\n" , need_modify );

	//print_flash_confg( cfg_pa );

    free(text); // free unused buffer, Alex, 20111013
//#endif
	return 0;
}


int voip_flash_client_readfile_init(voip_flash_share_t **ppVoIPCfg, cid_t cid)
{
   	TstVoipCfg stVoipCfg;
	int MaxVoIPPorts;
	struct stat sb;
	//voipCfgParam_t *cfg_pa;

	assert( flash_filename[0] != '\0' );

    if( stat(flash_filename, &sb ) == -1) {
		perror("stat");
		return -1;
	}
	//rcm_dp("flash_filename=%s file size=%lld\n" , flash_filename , (long long) sb.st_size );
	...
	memset(g_pVoIPShare , 0 , g_nShareSize);

    if( (voip_flash_get_config(&g_pVoIPShare->voip_cfg)) == -1 ){
		return -1;
		}
	//print_flash_confg( cfg_pa );
