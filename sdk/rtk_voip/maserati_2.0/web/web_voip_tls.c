...
#define UPLOAD_MSG(url) { \
	websHeader(wp); \
	boaWrite(wp, T("<body><blockquote><h4>Upload a file successfully!" \
...
// TODO
	...
	if(0 == wp->lenPostData){
		strcpy(tmpBuf,"Upload file is empty!");
		ERR_MSG(tmpBuf);
		printf("data error!\n");
		return;
	}
	...
	if(wp->lenPostData > CERT_SIZE){
		strcpy(tmpBuf,"Your CA is too large!");
		ERR_MSG(tmpBuf);
		printf("data error!\n");
		return;
	}

	if(strcmp(boaGetVar(wp,"submit_cert1",""),"UPLOAD")==0){
		unlink(CERT1_PATH);
		certNum=1;
	}else if(strcmp(boaGetVar(wp,"submit_cert2",""),"UPLOAD")==0){
		unlink(CERT2_PATH);
		certNum=2;
	}else if(strcmp(boaGetVar(wp,"submit_cert3",""),"UPLOAD")==0){
		unlink(CERT3_PATH);
		certNum=3;
	}else if(strcmp(boaGetVar(wp,"submit_cert4",""),"UPLOAD")==0){
		unlink(CERT4_PATH);
		certNum=4;
	}else{
	...
	fp=open(CERT_DEV_PATH, O_RDWR|O_CREAT|O_TRUNC);
	if (fp == -1){
		strcpy(tmpBuf,"Write cert to flash FAIL!");
		ERR_MSG(tmpBuf);
	...
	strData = boaGetVar(wp, "submit-url", T(tmpBuf));
	UPLOAD_MSG(strData);
#endif
}
...
int asp_voip_TLSGetCertInfo(int eid, request * wp, int argc, char **argv)
{
	...
	certNum = atoi(boaGetVar(wp, "cert", "0"));
	if (strcmp(argv[0], "certNum")==0)
	{
		boaWrite(wp, "%d", certNum);
