...
//[SD6, bohungwu, exosip 3.5 integration
//20110805, SD6, bohungwu, removed it is not a MUST in the review
#if 0
static int rcm_osip_ip_port_cmp( const char *szIP1, unsigned short port1,
...
void rcm_osip_icmp_ict_execute (osip_t * osip, osip_get_ip_by_name_cb_t get_ip_by_name_cb,
		unsigned long ip, unsigned short port)
{
	...
	while ( osip_list_iterator_has_elem( iterator ) )
	{
		...
		else
		{
			if( !tr ->ict_context )
				;
			else if( rcm_osip_ip_port_cmp( tr ->ict_context ->destination,
						tr ->ict_context ->port,
...
void rcm_osip_icmp_nict_execute (osip_t * osip, osip_get_ip_by_name_cb_t get_ip_by_name_cb,
		unsigned long ip, unsigned short port)
{
	osip_transaction_t *tr;
	osip_list_iterator_t iterator;
	...
	while ( osip_list_iterator_has_elem( iterator ) )
	{
		...
		if( !tr ->nict_context )
			;
		else if( rcm_osip_ip_port_cmp( tr ->nict_context ->destination,
