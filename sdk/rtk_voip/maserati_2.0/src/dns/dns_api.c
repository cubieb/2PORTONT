...
#define DNS_PRINTF(...) do { if (dbgprintf) dbgprintf(__VA_ARGS__); } while (0)
...
static int send_dns_request(const int status, const char *hostname, const int port)
{
	...
	DNS_PRINTF(DNS_TRACE, "sending message to queue %d, key=%d\n",
		server_qid, msg.msg_key);

	if (msgsnd(server_qid, (void *) &msg, sizeof(msg), 0) == -1)
	{
		DNS_PRINTF(DNS_ERROR, "send_dns_request failed in %d lines of %s\n", __LINE__, __FILE__);
		return -1;
	}
...
static int recv_dns_response(int port, struct addrinfo **res, struct hostent **host)
{
	...
	if (msg == NULL)
	{
		DNS_PRINTF(DNS_ERROR, "dns_msg_new failed in %d lines of %s\n", __LINE__, __FILE__);
		return DNS_RESPONSE_FAILED;
	}

	DNS_PRINTF(DNS_TRACE, "waiting message in queue %d\n", client_qid);
	if (msgrcv(client_qid, (void *) msg, MAX_DNS_MSGSIZE, getpid(), 0) == -1) 
	{
		DNS_PRINTF(DNS_ERROR, "msgrcv failed in %d lines of %s\n", __LINE__, __FILE__);
		free(msg);
		return DNS_RESPONSE_FAILED;
	}
	DNS_PRINTF(DNS_TRACE, "message is received in queue %d\n", client_qid);

	if (msg->status == DNS_QUERY_BUSY)
	{
		DNS_PRINTF(DNS_ERROR, "dns server is busy\n");
		free(msg);
		return DNS_RESPONSE_BUSY;
	}
	else if (msg->status == DNS_QUERY_TRYING)
	{
		DNS_PRINTF(DNS_INFO, "dns server is trying\n");
		free(msg);
	...
	else if (msg->record.port && msg->record.service.ai_count == 0)
	{
		...
		DNS_PRINTF(DNS_INFO, "getaddrinfo error\n");
		free(msg);
		return DNS_RESPONSE_FAILED;
	}
	else if (msg->record.port == 0 && msg->record.host.h_count == 0)
	{
		...
		DNS_PRINTF(DNS_INFO, "gethostbyname error\n");
		free(msg);
		return DNS_RESPONSE_FAILED;
	}
	...
	if (port == 0)
	{
		...
		{
			DNS_PRINTF(DNS_INFO, "gethostbyname error: too many entry (%d)\n", msg->record.host.h_count);
			free(msg);
			return DNS_RESPONSE_FAILED;
		}
	...
	for (i=msg->record.service.ai_count - 1; i>=0; i--)
	{
		...
		if (addrinfo == NULL)
		{
			DNS_PRINTF(DNS_ERROR, "malloc addrinfo failed in %d lines of %s\n", __LINE__, __FILE__);
		...
		{
			DNS_PRINTF(DNS_ERROR, "malloc ai_addr failed in %d lines of %s\n", __LINE__, __FILE__);
			free(addrinfo);
			my_freeaddrinfo(ai_last);
			free(msg);
			return DNS_RESPONSE_FAILED;
	...
	)
{
	...
	for (cnt=0;;cnt++)
	{
		...
		case DNS_RESPONSE_TRYING:
			sleep(1);
			DNS_PRINTF(DNS_INFO, "resolve %s retry %d times...\n", node, cnt+1);
	...
	)
{
	...
	if (inet_pton(AF_INET, node, &addr)>0)
	{
		...
		if (err != 0)
		{
			DNS_PRINTF(DNS_ERROR, "getaddrinfo failed in %d lines of %s\n", __LINE__, __FILE__);
			return err;
		}
