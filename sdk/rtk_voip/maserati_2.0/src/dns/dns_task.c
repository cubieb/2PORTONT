...
int main(int argc, char *argv[])
{
...
#ifdef RCM_SIMULATION   //SD6, darren_shiue for linphone multiple instances
        const char* iptr = getenv("RCM_NAME");
        if(iptr==NULL)
            pidfile = "/var/run/dns_task.pid";
		else{
            if( !strcmp(iptr,"maserati0") ){
                pidfile = "/var/run/dns_task0.pid";
            }
            else if( !strcmp(iptr,"maserati1") ){
                pidfile = "/var/run/dns_task1.pid"; 
            }
            else{
                pidfile = "/var/run/dns_task.pid";
            }
	...
	if (msg == NULL)
	{
		dbgprintf(DNS_ERROR, "dns_msg_new failed\n");
		return -1;
	}
	...
	while (1)
	{
		...
		{
			dbgprintf(DNS_INFO, "exit dns_task\n");
			goto dns_task_tone;
		}
		...
		case DNS_QUERY_REQUEST3: // cache + non-blocking
			dbgprintf(DNS_INFO, "resolving %s:%d, key=%d\n", 
			...
			else if (cache->status == DNS_FOUND)
			{
				...
				{
					// skip dns query
					dbgprintf(DNS_INFO, "resolving %s:%d is skipped: query time is too short\n", 
						msg->record.hostname, msg->record.port);
				}
				else
				{
					dbgprintf(DNS_INFO, "updating %s:%d\n", msg->record.hostname, msg->record.port);
					msg->status = DNS_QUERY_UPDATE;
			...
			else // DNS_FINDING or DNS_UPDATING
			{
				...
				if (cache->status == DNS_UPDATING)
				{
					...
					dbgprintf(DNS_INFO, "resolving %s:%d is skipped: updating\n", 
						msg->record.hostname, msg->record.port);
					memcpy(&msg->record, &cache->record, dns_record_size(&cache->record));
					msg->status = DNS_QUERY_RESPONSE;
					dns_msg_send(&dns_task, msg);
				...
				{
					// skip dns query
					dbgprintf(DNS_INFO, "resolving %s:%d is skipped: finding\n", 
						msg->record.hostname, msg->record.port);
					msg->status = DNS_QUERY_TRYING;
					dns_msg_send(&dns_task, msg);
				}
		...
		case DNS_QUERY_RESPONSE: // blocking response
			dbgprintf(DNS_INFO, "%s is resolved.\n", msg->record.hostname);
...
int dns_caches_add(TDNS_Task *task, const TDNS_Record *record, int status, int _errno)
{
	...
	{
		dbgprintf(DNS_ERROR, "dns_cache_new failed!\n");
		return -1;
	}
...
void dns_caches_update(TDNS_Task *task)
{
	...
	for (i=0; i<ARRAY_LEN(task->dns_caches); i++)
	{
		...
		{
			dbgprintf(DNS_INFO, "%s in cache is expired\n", task->dns_caches[i]->record.hostname);
			dns_caches_remove(task, i);
		}
	}
}
...
void dns_cache_dump(TDNS_Task *task)
{
	...
	gettimeofday(&now, NULL);
	dbgprintf(DNS_ERROR, "==== dns cache table (now=%d) \n", now.tv_sec);
	for (i=0; i<ARRAY_LEN(task->dns_caches); i++)
	{
		if (task->dns_caches[i] == NULL)
			continue;
		...
		dbgprintf(DNS_ERROR, "idx:%d, status: %d, expire:%d, host:%s, ip0:%s:%d, err=%d\n",
			i,
...
void dns_thread_dump(TDNS_Thread *thread)
{
	...
	dbgprintf(DNS_ERROR, "==== thread%d fifo ====\n", thread->index);
	...
	while (in != out)
	{
		dbgprintf(DNS_ERROR, "idx: %d, status: %d, host:%s:%d, err:%d\n", 
			out,
...
int dns_task_init(TDNS_Task *task)
{
	...
	dbgprintf(DNS_INFO, "dns task starting\n");
	...
	{
		dbgprintf(DNS_ERROR, "msgget server failed = %d\n", errno);
		return -1;
	}
	dbgprintf(DNS_TRACE, "server queue id is %d\n", task->server_qid);

	task->client_qid = msgget(key_client, 0666 | IPC_CREAT);
	if (task->client_qid == -1) 
	{
		dbgprintf(DNS_ERROR, "msgget client failed = %d\n", errno);
		return -1;
	}
	dbgprintf(DNS_TRACE, "client queue id is %d\n", task->client_qid);
...
void dns_task_destroy(TDNS_Task *task)
{
	...
	dbgprintf(DNS_INFO, "dns task ending\n");
	for (i=0; i<MAX_DNS_THREAD; i++)
	{
...
{
	dbgprintf(DNS_TRACE, "waiting message in queue %d\n", task->server_qid);
	if (msgrcv(task->server_qid, (void *) msg, MAX_DNS_MSGSIZE, 0, 0) == -1) 
	{
		...
		dbgprintf(DNS_ERROR, "msgrcv failed = %d (%s)\n", errno, strerror(errno));
		return -1;
	}

	dbgprintf(DNS_TRACE, "message is received in queue %d, key=%d, size=%d\n", task->server_qid,
...
int dns_msg_send(TDNS_Task *task, const TDNS_Message *msg)
{
	...
	dbgprintf(DNS_TRACE, "sending message to queue %d, key=%d, size=%d\n",
		task->client_qid, msg->msg_key, size);	

	if (msgsnd(task->client_qid, (void *) msg, size, 0) == -1) 
	{
		dbgprintf(DNS_ERROR, "msgsnd failed = %d\n", errno);
		return -1;
	}
...
int dns_thread_query(TDNS_Task *task, const TDNS_Message *msg, TDNS_Cache *cache)
{
	...
	{
		dbgprintf(DNS_ERROR, "all fifo is full...(min:%d, in:%d, out:%d)\n", 
			index, thread->dns_query_in, thread->dns_query_out);
		pthread_mutex_unlock(&thread->m_mutex);
//		dns_thread_dump(thread);
		return -1;
	}
	dbgprintf(DNS_INFO, "use thread%d to query %s:%d, key=%d\n", 
	...
	dbgprintf(DNS_TRACE, "signal thread%d to wake up\n", index);
...
static void *dns_thread_func(void *arg)
{
	...
	while (1)
	{
		...
		dbgprintf(DNS_TRACE, "thread%d waiting message\n", thread->index);
		...
		for (i=0; i<active_query_count; i++)
		{
			...
			if (request->record.port)
			{
				...
				if (err)
				{
					...
					dbgprintf(DNS_INFO, "getaddrinfo failed: errno = %d\n", errno);
				}
			}
			else
			{
				...
				if (err)
				{
					...
					dbgprintf(DNS_INFO, "gethostbyname failed: errno = %d\n", h_errno);
				}
			}
			dbgprintf(DNS_TRACE, "thread%d resolving %s done.\n", thread->index, request->record.hostname);

			if (err != 0)
			{
				dbgprintf(DNS_ERROR, "DNS resolve %s failed\n", request->record.hostname);
			...
			if (request->record.port)
			{
				for (ai_count=0, ai=aitop; ai; ai=ai->ai_next)
				{
					...
					{
						dbgprintf(DNS_ERROR, "ai_addr is null!?\n");
						continue;
					}
					if (ai->ai_addrlen > sizeof(request->record.service.addrinfos[0].ai_addr))
					{
						dbgprintf(DNS_ERROR, "ai_addr(%d) is too big\n", ai->ai_addrlen);
						continue;
					}
					ai_count++;
				}
	...
	dbgprintf(DNS_INFO, "thread%d out\n", thread->index);
	pthread_exit(NULL);
}
...
int dns_thread_init(TDNS_Task *task, const int index)
{
	...
	if (i != 0)
	{
		dbgprintf(DNS_ERROR, "pthread_mutex_init failed\n");
		return -1;
	}
	...
	if (i != 0)
	{
		dbgprintf(DNS_ERROR, "pthread_cond_init failed\n");
	...
	if (i != 0)
	{
		dbgprintf(DNS_ERROR, "pthread_create failed\n");
		thread->parent = NULL;
		thread->index = 0;
