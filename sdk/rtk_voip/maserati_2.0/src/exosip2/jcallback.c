...
static void cb_rcvrequest(int type, osip_transaction_t * tr, osip_message_t * sip)
{
	...
	if (jc == NULL) {
		...
		if (MSG_IS_BYE(sip)) {
			/* already sent */
			//printf("message is bye , jd stats is %d\n\n",jd->d_STATE);			
			if((jd!=NULL) && (jd->d_STATE == JD_TRYING)){
				/*ericchung:if early dialog stats, send event to call manager */
				//printf("report EXOSIP_CALL_MESSAGE_NEW\n");
...
//add parse exipre in contect header , some SIP server exipre value setting in Contect header. 
{
	pos = 0;
	co_register = (osip_contact_t *) osip_list_get(&sip->contacts, pos);
	int val;
