...
#ifdef ENABLE_USER_PHONE	//if need user=phone in to/from header , please enable it.
	{
		osip_uri_t *url = &request->from->url;
		osip_uri_set_user_phone(url);
		url = &request->to->url;
		osip_uri_set_user_phone(url);
	}
#endif
...
int generating_cancel(osip_message_t ** dest, osip_message_t * request_cancelled)
{
	...
	if (i != 0)
		return i;
	osip_message_set_outboundproxy(	request,request_cancelled->outbound_proxy,request_cancelled->outbound_port);
	/* prepare the request-line */
