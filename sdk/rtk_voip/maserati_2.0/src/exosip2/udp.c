...
static void
eXosip_process_early_dialog_bye(eXosip_call_t * jc, eXosip_dialog_t * jd,
					 osip_transaction_t * transaction, osip_event_t * evt)
{
	osip_event_t *evt_answer;
	osip_message_t *answer;
...
static void eXosip_process_newrequest(osip_event_t * evt, int socket)
{
	...
	if (jd != NULL) {
		...
		} else if (MSG_IS_BYE(evt->sip)&& (jd->d_STATE != JD_TRYING)) {	/*ericchung: early dialog status receive bye */
		...
		} else if (MSG_IS_BYE(evt->sip)&& (jd->d_STATE == JD_TRYING)) {
		/*ericchung: early dialog status receive bye */
			eXosip_process_early_dialog_bye(jc,jd,transaction,evt);
