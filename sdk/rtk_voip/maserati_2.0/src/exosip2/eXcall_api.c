						 ...
						 osip_message_t ** request)
{
	...
	*request = NULL;
	...
	if (subscription_status == EXOSIP_SUBCRSTATE_PENDING)
		...
		osip_strncpy(subscription_state, "active", 6);
...
#endif
			//marge from claro: for call transfer iot issue, change it on 20141210
			//osip_strncpy(subscription_state, "terminated;reason=noresource", 29);
			osip_strncpy(subscription_state, "terminated;reason=completed", 28);
	}
...
//	eXosip_get_localip_for(jd->d_dialog->remote_contact_uri->url.host, &locip);
	rcm_get_localip_for_voip(solar_ptr->current_voip_interface,&locip);
