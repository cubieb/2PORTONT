...
#endif
{
	...
	{
		boaWrite(wp, "<input type=\"radio\" name=\"pulse_dial_detection\" value=0 %s>%s",
    				( pVoIPCfg ->pulse_dial_det == 0 ? "checked" : "" ), multilang(LANG_DISABLE) );
		boaWrite(wp, "<input type=\"radio\" name=\"pulse_dial_detection\" value=1 %s>%s",
    				( pVoIPCfg ->pulse_dial_det == 1 ? "checked" : "" ), multilang(LANG_ENABLE) );
	}
	else if (strcmp(argv[0], "sip_option_flag")==0)
	{
		...
		boaWrite(wp, "<input type=\"radio\" name=\"sip_option_flag\" value=0 %s>%s",
					( i  ? "" : "checked" ), multilang(LANG_DISABLE) );
		boaWrite(wp, "<input type=\"radio\" name=\"sip_option_flag\" value=1 %s>%s",
					( i ? "checked" : "" ), multilang(LANG_ENABLE) );
	}
	...
	{
		//printf("pVoIPCfg->rfc_flags is %x\n",pVoIPCfg->rfc_flags);
			boaWrite(wp, "%s", pVoIPCfg->rfc_flags & SIP_SERVER_REDUNDANCY ? "checked" : "");
	}
	else if (strcmp(argv[0], "sip_anonymouse_from") == 0)
	{
//		printf("pVoIPCfg->rfc_flags is %x\n",pVoIPCfg->rfc_flags);
			boaWrite(wp, "%s", pVoIPCfg->rfc_flags & SIP_ANONYMOUSE_FROM ? "checked" : "");
	}
	else if (strcmp(argv[0], "sip_internalcall") == 0)
	{
		printf("pVoIPCfg->rfc_flags is %x\n",pVoIPCfg->rfc_flags);
			boaWrite(wp, "%s", pVoIPCfg->rfc_flags & SIP_INTERNALL_CALL ? "checked" : "");
	}	
	...
	else if (strcmp(argv[0], "hookflash_type")==0)
	{
		...
		if(pVoIPCfg->rfc_flags & SIP_FLASHHOOK_INFO){
			boaWrite(wp, "<option>%s</option>", hookflash_type[0]);
			boaWrite(wp, "<option selected>%s</option>", hookflash_type[1]);

		}else{
			boaWrite(wp, "<option selected>%s</option>", hookflash_type[0]);
			boaWrite(wp, "<option>%s</option>", hookflash_type[1]);
		}
...
//eric merge
	...
	if (gstrcmp(boaGetVar(wp, "sip_internalcall", ""), "on") == 0){
		 pVoIPCfg->rfc_flags |= SIP_INTERNALL_CALL;
	}else{
		pVoIPCfg->rfc_flags &= ~SIP_INTERNALL_CALL;
	}
	//support SIP server redundancy
	if (gstrcmp(boaGetVar(wp, "sip_enable_redundancy", ""), "on") == 0){
	...
	if (gstrcmp(boaGetVar(wp, "sip_anonymouse_from", ""), "on") == 0){
...
//printf("sip_option_flag is %d\n",atoi(boaGetVar(wp, "sip_option_flag", "")));
	...
	if(atoi(boaGetVar(wp, "sip_option_flag", ""))==1){
	...
	for (voip_port=0; voip_port< VOIP_PORTS; voip_port++)
	{
		...
		//heart beat cycle
		pCfg->proxies[0].HeartbeatCycle = atoi(boaGetVar(wp, "sip_option_HeartbeatCycle", ""));
		pCfg->proxies[1].HeartbeatCycle = atoi(boaGetVar(wp, "sip_option_HeartbeatCycle", ""));
