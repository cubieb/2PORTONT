...
extern unsigned char rfc2833_trunk_pt_remote[MAX_DSP_SS_NUM];
#endif
...
int Rfc2833EventTxBuf_Reset(uint32 sid)
{
...
#ifdef RFC2833TRUNKEVENT
// for rfc2833 trunk event
static RtpPacket trunkeventPkt_fifo[MAX_DSP_RTK_SS_NUM][EVENT_PKT_FIFO_NUM]={{{0}}};
static int trunkeventPkt_fifo_r[MAX_DSP_RTK_SS_NUM]={0}, trunkeventPkt_fifo_w[MAX_DSP_RTK_SS_NUM]={0};

static int trunkeventPkt_fifoWrite(int sid, RtpPacket *eventPkt)
{
	RtpPacket* p;
	
	if(((trunkeventPkt_fifo_w[sid]+1)%EVENT_PKT_FIFO_NUM) == trunkeventPkt_fifo_r[sid])
	{
		printk("RTP Event PKT FIFO Full\n");
		return 0;
	}
		
	p = &trunkeventPkt_fifo[sid][trunkeventPkt_fifo_w[sid]];
	memcpy(p, eventPkt, sizeof(RtpPacket));
	p->packetData = p->packetbuf;
	p->header = (RtpHeader*) (char*)(p->packetData);
	trunkeventPkt_fifo_w[sid] = (trunkeventPkt_fifo_w[sid] + 1)%EVENT_PKT_FIFO_NUM;
	//printk("w[%d]=%d\n", sid, eventPkt_fifo_w[sid]);
		
	return 1;
}

static int trunkeventPkt_fifoRead(int sid, RtpPacket** eventPacket)
{

	if (trunkeventPkt_fifo_r[sid] == trunkeventPkt_fifo_w[sid])
	{
		//printk("RTP Event PKT FIFO Empty\n");
		//printk(".");
		return 0;
	...
	*eventPacket = &trunkeventPkt_fifo[sid][trunkeventPkt_fifo_r[sid]];
	
	return 1; 
	
}

static void trunkeventPkt_fifoReadDone(int sid)
{
	unsigned long flags;

	save_flags(flags); cli();
	trunkeventPkt_fifo_r[sid] = (trunkeventPkt_fifo_r[sid]+1)%EVENT_PKT_FIFO_NUM;
	restore_flags(flags);
	// printk("r[%d]=%d\n", sid, eventPkt_fifo_r[sid]);
}

static void trunkeventPkt_fifoFlush(int sid)
{
	trunkeventPkt_fifo_r[sid] = 0;
	trunkeventPkt_fifo_w[sid] = 0;
}

void trunkevent_send(void)
{
	static int end_cnt[MAX_DSP_RTK_SS_NUM] = {0};
	unsigned char sid;
	RtpPacket* eventPacket = NULL;
	...
	for(sid=0; sid < MAX_DSP_RTK_SS_NUM; sid++)
	{
		if(trunkeventPkt_fifoRead(sid, &eventPacket))
		{
			eventPayload = getPayloadLoc(eventPacket);
			end_cnt[sid] = 0;
			// backup check variable for later use 
			...
			rfc2833_proc_fifo_read_cnt[sid][eventPayload_event]++;
#endif
			trunkeventPkt_fifoReadDone(sid);
		}
	}
}
#endif
...
int RtpTx_transmitEvent_ISR( uint32 chid, uint32 sid, int event)
{
...
#ifdef RFC2833TRUNKEVENT
	else if ((event >= RFC2833TrunkEventBeginID ) && (event <= RFC2833TrunkEventEndID))
		rfc2833_pt_remote = rfc2833_trunk_pt_remote[sid];
#endif		
...
#ifdef RFC2833TRUNKEVENT
	if (!((event >= RFC2833TrunkEventBeginID) && (event <= RFC2833TrunkEventEndID))){
#endif
	...
	if (!send_2833_start[sid])
	{
  			...
  			((check_removal(sid, event) == 1) && (send_2833_by_ap[chid] == 0))) 
		{
			...
			if ((send_2833_count_down[sid] >= (rfc2833_interval+1)) || 
			    (send_2833_count_down_dsp[sid] >= (rfc2833_interval+1))
				)
			{
...
#ifdef RFC2833TRUNKEVENT	
	else if ((event >= RFC2833TrunkEventBeginID) && (event <= RFC2833TrunkEventEndID)) // DTMF 2833 trunk event
	{
		rfc2833_volume = 0;
	}
#endif	
	...
	if ((event >= RFC2833TrunkEventBeginID) && (event <= RFC2833TrunkEventEndID))
	{
		...
		//transmit( eventPacket, true ) ;
		trunkeventPkt_fifoWrite(sid, eventPacket);
#ifdef RFC2833_PROC_DEBUG_SUPPORT
		...
		//PRINT_R("int_cnt=%d\n", int_cnt); 
#endif
		if ( (event >= RFC2833TrunkABCDEventBeginID) && (event <= RFC2833TrunkABCDEventEndID))
		{	
...
int sprintf_redtx( uint32 sid, struct seq_file *f )
{
	RtpRedundantTx_t *pRtpRedundantTx = &RtpRedundantTx[ sid ];

	seq_printf( f, "RtpRedundant number Audio  : %u/%u\n", 
				pRtpRedundantTx ->audio.set_count, 
				pRtpRedundantTx ->audio.max_count );
	
	seq_printf( f, "RtpRedundant number RFC2833: %u/%u\n",
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int voip_rtp_transmitter_read_proc(struct seq_file *f, void *v)
{
	int ss;
	...
	if( IS_CH_PROC_DATA( v ) ) {
		...
		seq_printf( f, "Prev seq: %u\n", pRtpTransmitter ->prevSequence );
		seq_printf( f, "Sent: packet=%u, byte=%u\n",
						pRtpTransmitter ->packetSent,
						pRtpTransmitter ->payloadSent );
		seq_printf( f, "Format: bitrate=%u, payloadtype=%u, "
		...
		seq_printf( f, "Hack: SSRC=%u, seq=%u\n", 
			pRtpTransmitter ->ssrc, 
			pRtpTransmitter ->prevSequence );
#endif
	}
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int rtp_transmiiter_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_rtp_transmitter_read_proc, NULL);
}

struct file_operations proc_rtp_transmiiter_fops = {
	.owner	= THIS_MODULE,
	.open	= rtp_transmiiter_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_rtp_transmitter_read_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_session_proc_read_entry( "rtptransmitter", &proc_rtp_transmiiter_fops );
#else
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
int voip_rfc2833_read_proc(struct seq_file *f, void *v)
{
	int ch;
	int sid;
	int SessNum;
	...
	if( IS_CH_PROC_DATA( (long)f->private ) )
	{
		...
		if (dtmf_mode[ch] == 0)
			seq_printf( f, "Mode: Outband DTMF(RFC2833).\n");
		else if (dtmf_mode[ch] == 1)
			seq_printf( f, "Mode: Outband DTMF(SIP Info).\n");
		else if (dtmf_mode[ch] == 2)
			seq_printf( f, "Mode: Inband DTMF.\n");
		else if (dtmf_mode[ch] == 3)
			seq_printf( f, "Mode: DTMF delete mode.\n");

		seq_printf( f, "RFC2833 auto send by DSP: %s.\n", send_2833_by_ap[ch] ? "Off" : "On");
		...
		for(i=0; i<SessNum; i++)
		{
			...
			seq_printf( f, "Duration limit :%s\n", bRfc2833_play_limit[sid] ? "On" : "Off");
			seq_printf( f, "Limit duration= %d ms\n", rfc2833_play_limit_ms[sid]);

			seq_printf( f, "Local DTMF 2833 PT = %d\n", rfc2833_dtmf_pt_local[sid]);
			seq_printf( f, "Remote DTMF 2833 PT= %d\n", rfc2833_dtmf_pt_remote[sid]);
			seq_printf( f, "Local Fax/Modem 2833 PT = %d\n", rfc2833_fax_modem_pt_local[sid]);
			seq_printf( f, "Remote Fax/Modem 2833 PT= %d\n", rfc2833_fax_modem_pt_remote[sid]);
		}
	}
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int rfc2833_read_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_rfc2833_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_rfc2833_read_fops = {
	.owner	= THIS_MODULE,
	.open	= rfc2833_read_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_rfc2833_read_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_channel_proc_read_entry( "rfc2833", &proc_rfc2833_read_fops );
#else
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int voip_rfc2833_tx_debug_proc(struct seq_file *f, void *v)
{
	int ss;
	int n = 0;
	int i = 0, digit = 0;
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int rfc2833_tx_debug_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_rfc2833_tx_debug_proc, NULL);
}

struct file_operations proc_rfc2833_tx_debug_fops = {
	.owner	= THIS_MODULE,
	.open	= rfc2833_tx_debug_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_rfc2833_tx_debug_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_session_proc_read_entry( "rfc2833_tx_debug", &proc_rfc2833_tx_debug_fops );
#else
