...
int voip_dsp_channel_info_read_proc(struct seq_file *f, void *v)
{
	int n = 0;
	int i;
	
	seq_printf( f, "DCH  isConf  TrasnSID\n" );
	seq_printf( f, "---------------------\n" );
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int dsp_channel_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_dsp_channel_info_read_proc, NULL);
}

struct file_operations proc_dsp_channel_info_fops = {
	.owner	= THIS_MODULE,
	.open	= dsp_channel_info_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_dsp_channel_info_read_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	proc_create_data( PROC_VOIP_DIR "/channel_info", 0, NULL, &proc_dsp_channel_info_fops, NULL );
#else
