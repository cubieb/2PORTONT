...
#include "../voip_dsp/rtp/rfc2833trunk.h"
#include "v152_api.h"
...
//ericchung: stop dtmf caller id ring
...
//#endif
		...
		voip_event_flush_dsp_fifo(stVoipCfg.ch_id, ssid);
#else
    	voip_event_flush_dsp_fifo(stVoipCfg.ch_id, ssid);
#endif
	}

	voip_event_flush_dtmf_fifo(stVoipCfg.ch_id, 0);
	voip_event_flush_dtmf_fifo(stVoipCfg.ch_id, 1);

	voip_event_flush_energy_fifo(stVoipCfg.ch_id);
...
int do_mgr_VOIP_MGR_SEND_RFC2833_PKT_CFG( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
	...
	if (send_2833_by_ap[stRFC2833.ch_id] == 1)
	{
		...
		idx = Rfc2833EventBufWrite(s_id, g_digit[ s_id ], stRFC2833.duration / ( PCM_PERIOD * 10 ));
		Rfc2833EventBufWriteUpdate(s_id, idx, g_digit[ s_id ], 0, 1);
#else
...
//ericchung: stop dtmf caller id ring
		gRingGenAfterCid[stVoipCfg.ch_id]=0;
#endif
	}
...
/**
 * @ingroup VOIP_DSP_DTMF
 * @brief Enable/Disable RFC2833 trunk event
 * @param TstVoipCfg.enable Sent by DSP (0) by AP (1)
 * @see VOIP_MGR_SEND_RFC2833_BY_AP TstVoipCfg
 */
#if ! defined (AUDIOCODES_VOIP)
int do_mgr_VOIP_MGR_SET_RFC2833_ENABLE_ABCDSIGNAL( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
#ifndef CONFIG_RTK_VOIP_IPC_ARCH_IS_HOST
	...
	extern void RFC2833TrunkEventEnable(int chid, unsigned char bEnable);

	//PRINT_MSG( "VOIP_MGR_SEND_RFC2833_BY_AP:chid=%d\n", stVoipCfg.ch_id);

	PRINT_MSG("VOIP_MGR_SET_RFC2833_ENABLE_ABCDSIGNAL chid = [%d], Enable = [%d]\n", stRFC2833.ch_id, stRFC2833.bEnable);

	RFC2833TrunkEventEnable(stRFC2833.ch_id, (unsigned char)stRFC2833.bEnable);
#endif	
	return 0;
#endif	
}
...
/**
 * @ingroup VOIP_DSP_DTMF
 * @brief Generate RFC2833 trunk event ABCD signal
 * @param TstVoipCfg.ch_id Channel ID
 * @param TstVoipCfg.enable Sent by DSP (0) by AP (1)
 * @see VOIP_MGR_SEND_RFC2833_BY_AP TstVoipCfg
 */
#if ! defined (AUDIOCODES_VOIP)
int do_mgr_VOIP_MGR_SET_RFC2833_GENERATE_ABCDSIGNAL( int cmd, void *user, unsigned int len, unsigned short seq_no )
{
#ifndef CONFIG_RTK_VOIP_IPC_ARCH_IS_HOST
	...
	PRINT_MSG("VOIP_MGR_SET_RFC2833_GENERATE_ABCDSIGNAL channel id = [%d], signal = [%x]\n", stRFC2833.ch_id, stRFC2833.digit);
#ifdef RFC2833TRUNKEVENT
	extern int RFC2833GenerateTrunkEvent(int chid, unsigned char signal);

	ret = RFC2833GenerateTrunkEvent(stRFC2833.ch_id, stRFC2833.digit);
#endif
	return 0;
