				...
				voip_event_queue_t **single_queue )
{
	...
	for( i = 0; i < VOIP_EVENT_QUEUE_TOTAL_NUM; i ++ ) {
		...
		if( ri == queue ->wi )
		{
			//if (action == _voip_mgr_event_out_flush_action )
			//PRINT_G("i=%d, filter=0x%x, mask=0x%x\n", i, filter, mask);
			continue;
		}
		...
		if( action )
		{
			//PRINT_R("i=%d, filter=0x%x, mask=0x%x\n", i, filter, mask);
...
static int __sprintf_voip_events_detail(struct seq_file *f, const voip_event_queue_t *queue )
{
	int n = 0;
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int sprintf_voip_events_detail(struct seq_file *f, int ch )
{
	int n = 0;
	int i;
	
	for( i = 0; i < VOIP_EVENT_QUEUE_TOTAL_NUM; i ++ ) {
			seq_printf( f, "Queue %d:", i );
			__sprintf_voip_events_detail( f, &voip_event_queue[ ch ][ i ] );
	}
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int voip_events_read_proc(struct seq_file *f, void *v)
{
	/*
	...
	if( IS_CH_PROC_DATA( (long)f->private ) ) {
		...
		sprintf_voip_events_detail( f, ch );
	} else {
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int events_read_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_events_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_events_read_fops = {
	.owner	= THIS_MODULE,
	.open	= events_read_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_events_read_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_channel_proc_read_entry( "events", &proc_events_read_fops );
#else
