    ...
    uint8 numSamples)
{
    ...
    while (i < numSamples) {
        ...
        harmonicRow = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzl2ndRow[bestRow]);
        if (pParams->sampleRate != VP_DTMF_SAMPLE_RATE_4KHZ) {
            harmonicCol = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzl2ndCol[bestCol]);
        }
        energyLowGap = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlLowGap);
        energyMidGap = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlMidGap);
        energyHighGap = VpDtmfDetectGoertzelEnergy(pLineCtx, &pDtmfData->grtzlHighGap);
