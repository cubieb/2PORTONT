 ...
 *
 * $Revision: 11712 $
 * $LastChangedDate: 2015-03-30 17:17:20 -0500 (Mon, 30 Mar 2015) $
 */
    ...
    VpLineCtxType           *pLineCtx)
{
    ...
    uint8                 lsdData;
    VpCslacLineCondType   tempAmpSt, tempDiscSt, tempPolRevSt, tempLiuSt;
    VpCslacLineCondType   tempDcFault;
    ...
    if ((pFxoTimers->lastStateChange < VP890_FXO_ONHOOK_CHANGE_DEBOUNCE
        ...
        /* Overcurrent/POH */
        if (onHookState == FALSE && !(intReg & VP890_LDN_MASK)) {
            Vp890ProcessFxoLoopCurrent(pLineCtx);
        } else {
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    VpDeviceIdType deviceId = pDevObj->deviceId;
    uint8 ecVal = pLineObj->ecVal;
    VpCslacLineCondType tempPohSt, tempAcFault;
    uint16 absCurrent[VP890_PCM_BUF_SIZE];
    uint8 bufferVal;
    bool overCurrent = TRUE;

    if (pLineObj->currentMonitor.samplingState == VP890_CMON_SAMPLING_NORMAL
        ...
        for ( bufferVal = 0; bufferVal < VP890_PCM_BUF_SIZE; bufferVal++) {
            absCurrent[bufferVal] = ABS(pLineObj->currentMonitor.currentBuffer[bufferVal]);

            if (absCurrent[bufferVal] < VP890_FXO_OVERCURRENT_THRESHOLD) {
                overCurrent = FALSE;
            }
        ...
        if ((minValue < 0) != (maxValue < 0)) {
            /* If the min and max are different polarities, then this is likely
               just a transient, not an overcurrent */
            overCurrent = FALSE;
        }

        averageCurrent /= (int32)VP890_PCM_BUF_SIZE;

        tempAcFault = VP_CSLAC_CLEAR;
        if (overCurrent) {
            if (pLineObj->currentMonitor.resultState == VP890_CMON_RESULT_OVERCURRENT ||
                pLineObj->currentMonitor.resultState == VP890_CMON_RESULT_POSSIBLE_OC)
            {
                pLineObj->currentMonitor.resultState = VP890_CMON_RESULT_OVERCURRENT;
                tempAcFault = VP_CSLAC_AC_FLT;
            } else {
                uint8 daaCtrl[VP890_LSD_CTL_LEN];
                /* Re-measure the offset to check whether it's an overcurrent or
                   or a current starve.  If it's a true overcurrent, we'll end up
                   in the above case */
                VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Possible Overcurrent or Current Starve (avg %ld)", averageCurrent));
                ...
                pLineObj->lineTimers.timers.fxoTimer.currentMonitorTimer =
                    MS_TO_TICKRATE((32 << pLineObj->dPoh) + 10, pDevObj->devProfileData.tickRate);
                pLineObj->currentMonitor.currentOffset = 0;
                pLineObj->currentMonitor.offsetMeasurements = 0;
                pLineObj->currentMonitor.transient = FALSE;
                pLineObj->currentMonitor.samplingState = VP890_CMON_SAMPLING_OFFSET;
                pLineObj->currentMonitor.resultState = VP890_CMON_RESULT_POSSIBLE_OC;
            }
        }
        ...
        if (overCurrent) {
            /* Skip POH if we suspect overcurrent */
            return;
        }
        
        if ((pLineObj->lineState.condition & VP_CSLAC_RAW_DISC) ||
            (pLineObj->lineState.condition & VP_CSLAC_DISC)) {
            /* Skip POH if we suspect disconnect */
            return;
        }

        /* POH Algorithm */
        deltaMaxMin = maxValue - minValue;
        if (deltaMaxMin > VP890_FXO_TRANSIENT_THRESHOLD) {
            /* This is a period where a line transient occurred. Cannot be
               evaluated for POH, but triggers when a POH might have occurred. */
            pLineObj->currentMonitor.transient = TRUE;
        } else {
            Vp890FxoCMonResultStateType newState = pLineObj->currentMonitor.resultState;
            int32 absOld = ABS(pLineObj->currentMonitor.steadyStateAverage);
            int32 absNew = ABS(averageCurrent);
        
            /* Evaluate steady state current */
            if (absNew < VP890_FXO_STARVE_THRESHOLD) {
                newState = VP890_CMON_RESULT_STARVE;
                tempPohSt = VP_CSLAC_LIU;
            } else if (absNew < VP890_FXO_POH_THRESHOLD) {
                newState = VP890_CMON_RESULT_POH;
                tempPohSt = VP_CSLAC_LIU;
            } else if (absNew < VP890_FXO_PNOH_THRESHOLD) {
                newState = VP890_CMON_RESULT_MIDDLE;
                if (pLineObj->currentMonitor.resultState == VP890_CMON_RESULT_STARVE) {
                    tempPohSt = VP_CSLAC_CLEAR;
                } else {
                    tempPohSt = pLineObj->lineState.condition & VP_CSLAC_LIU;
                }
            } else {
                newState = VP890_CMON_RESULT_PNOH;
                tempPohSt = VP_CSLAC_CLEAR;
            }
            
            if (newState != pLineObj->currentMonitor.resultState) {
                VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("New currentMonitor.resultState %d (old %d, absNew %ld, absOld %ld)",
                    newState, pLineObj->currentMonitor.resultState, absNew, absOld));
            }
            
            /* If we previously saw a transient which was not a polarity reversal,
               evaluate it for POH. */
            if (pLineObj->currentMonitor.transient == TRUE &&
                pLineObj->currentMonitor.resultState != VP890_CMON_RESULT_INIT &&
                ((averageCurrent < 0) == (pLineObj->currentMonitor.steadyStateAverage < 0)))
            {
                int32 lowerRelativeThreshold = (absOld * (100 - VP890_FXO_POH_RELATIVE_PCT)) / 100;
                int32 upperRelativeThreshold = (absOld * 100) / (100 - VP890_FXO_POH_RELATIVE_PCT);

                VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Current shift, absNew %ld, absOld %ld", absNew, absOld));

                switch (newState) {
                    case VP890_CMON_RESULT_STARVE:
                    case VP890_CMON_RESULT_POH:
                        /* Only look for downward shifts that end in these
                           regions.  This does create the possibility of
                           seeing multiple POH events if multiple parallel
                           phones go off hook. */
                        if (absNew < lowerRelativeThreshold) {
                            VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Current shift, POH, absNew %ld, absOld %ld", absNew, absOld));
                            pLineObj->lineEvents.fxo |= VP_LINE_EVID_POH;
                            pLineObj->lineEventHandle = pDevObj->timeStamp;
                            tempPohSt = VP_CSLAC_LIU;
                        }
                        break;
                    case VP890_CMON_RESULT_MIDDLE:
                        /* Look for shifts in either direction that end up in
                           the middle region */
                        if (absNew < lowerRelativeThreshold) {
                            VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Current shift, POH, absNew %ld, absOld %ld", absNew, absOld));
                            pLineObj->lineEvents.fxo |= VP_LINE_EVID_POH;
                        ...
                        if (absNew > upperRelativeThreshold) {
                            VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Current shift, PNOH, absNew %ld, absOld %ld", absNew, absOld));
                            pLineObj->lineEvents.fxo |= VP_LINE_EVID_PNOH;
                            pLineObj->lineEventHandle = pDevObj->timeStamp;
                            tempPohSt = VP_CSLAC_CLEAR;
                        }
                        break;
                    case VP890_CMON_RESULT_PNOH:
                        /* Only look for upward shifts that end in the high
                           region.  This does create the possibility of
                           seeing multiple PNOH events if multiple parallel
                           phones go back on hook. */
                        if (absNew > upperRelativeThreshold) {
                            VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Current shift, PNOH, absNew %ld, absOld %ld", absNew, absOld));
                            pLineObj->lineEvents.fxo |= VP_LINE_EVID_PNOH;
                            pLineObj->lineEventHandle = pDevObj->timeStamp;
                    ...
                    default:
                        /* Should not get here */
                        VP_WARNING(VpLineCtxType, pLineCtx, ("Unexpected cmon state %d, absNew %ld, absOld %ld", newState, absNew, absOld));
                        break;
                }
            }
            ...
            pLineObj->currentMonitor.transient = FALSE;
            pLineObj->currentMonitor.resultState = newState;
        }
    }
} /* Vp890ProcessFxoLoopCurrent */
    ...
    VpLineCtxType *pLineCtx)
{
    ...
    uint8                   ecVal       = pLineObj->ecVal;
    VpDeviceIdType          deviceId    = pDevObj->deviceId;
    uint16                  sampleDelay = (32 << pLineObj->dPoh) + 10;

#undef FXO_LOOP_MONITOR_DEBUG /* print full buffer and offset each sample */
#undef FXO_LOOP_MONITOR_DEBUG_LITE /* print only the latest sample */

#ifdef FXO_LOOP_MONITOR_DEBUG
    VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Vp890FXOLoopCurrentMonitor() - state %d at %d (0x%04X)",
        pLineObj->currentMonitor.samplingState, pDevObj->timeStamp, pDevObj->timeStamp));
#endif
    ...
    switch(pLineObj->currentMonitor.samplingState) {
        case VP890_CMON_SAMPLING_DISABLED:
        ...
        case VP890_CMON_SAMPLING_OFFSET: {
            uint8 i;
            int16 min;
            int16 max;
            ...
            if (pLineObj->currentMonitor.offsetMeasurements < VP890_CM_OFFSET_NUM) {
                pLineObj->lineTimers.timers.fxoTimer.currentMonitorTimer =
                    MS_TO_TICKRATE(sampleDelay, pDevObj->devProfileData.tickRate);
                ...
                max - min > -VP890_CM_OFFSET_TOLERANCE)
            {
                ...
                pLineObj->currentMonitor.samplingState = VP890_CMON_SAMPLING_BUFFER0;
                break;
            }
        ...
        case VP890_CMON_SAMPLING_BUFFER0:
        case VP890_CMON_SAMPLING_BUFFER1:
        case VP890_CMON_SAMPLING_BUFFER2:
        case VP890_CMON_SAMPLING_BUFFER3:
        case VP890_CMON_SAMPLING_BUFFER4: {
                uint8 bufferIndex = pLineObj->currentMonitor.samplingState - VP890_CMON_SAMPLING_BUFFER0;
                ...
                pLineObj->currentMonitor.samplingState++;
        } break;

        case VP890_CMON_SAMPLING_NORMAL: {
                uint8 buffCnt;
                ...
                VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Vp890FXOLoopCurrentMonitor() - Buffer 0, Value %d",
                    pLineObj->currentMonitor.currentBuffer[0]));
#endif
#ifdef FXO_LOOP_MONITOR_DEBUG_LITE
                VP_FXO_FUNC(VpLineCtxType, pLineCtx, ("Vp890FXOLoopCurrentMonitor() - Buffer Value: %d (offset %d)",
                    pLineObj->currentMonitor.currentBuffer[0], pLineObj->currentMonitor.currentOffset));
#endif
   ...
   /* Note that the System State Register may be programmed twice. */
    ...
    switch(state) {
        ...
        case VP_LINE_FXO_LOOP_OPEN:
            ...
            if (pLineObj->currentMonitor.samplingState == VP890_CMON_SAMPLING_OFFSET ||
                pLineObj->currentMonitor.samplingState == VP890_CMON_SAMPLING_DISABLED) {
                VpMpiCmdWrapper(deviceId, ecVal, VP890_LSD_CTL_RD, VP890_LSD_CTL_LEN, daaCtrl);
                daaCtrl[0] &= ~VP890_LSD_CSE_DIS;
                VpMpiCmdWrapper(deviceId, ecVal, VP890_LSD_CTL_WRT, VP890_LSD_CTL_LEN, daaCtrl);
            }
        ...
        case VP_LINE_FXO_TALK:
            ...
            if (pLineObj->currentMonitor.samplingState == VP890_CMON_SAMPLING_DISABLED) {
                /*-----------------7/24/2008 6:23PM-----------------
