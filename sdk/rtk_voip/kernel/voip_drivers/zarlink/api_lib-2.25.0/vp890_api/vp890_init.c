    ...
    if (pLineObj->status & VP890_IS_FXO) {
        ...
        if (pDcFxoCfgProf != VP_PTABLE_NULL) {
            ...
            if (pDcFxoCfgProf[profileIndex] >= VP_CSLAC_FXO_VERSION_POH) {
                ...
                /* Default integrate time and 7.284mA */
                loopSupervision[1] &= ~(VP890_POH_INT_TIME_MASK);
                loopSupervision[1] |= (VP890_FXO_DPOH_DEFAULT << 2);  /* DPOH */
                pLineObj->dPoh = VP890_FXO_DPOH_DEFAULT;
                loopSupervision[0] &= ~(VP890_POH_THRESH_MASK);
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    */
    ...
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp890QuickMpiTest: Test 4 failed. Write buffer corrupted: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            writeBuffer[0], writeBuffer[1], writeBuffer[2], writeBuffer[3], writeBuffer[4], writeBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    if (readBuffer[0] != 0x94 || readBuffer[5] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp890QuickMpiTest: Test 4 failed. Read buffer overflow: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[0], readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    if (readBuffer[1] != 0x00 || readBuffer[2] != 0xFF || readBuffer[3] != 0x00 || readBuffer[4] != 0xFF) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp890QuickMpiTest: Test 4 failed. Read value incorrect: 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4]));
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp890QuickMpiTest: expected 0x00 0xFF 0x00 0xFF"));
        status = VP_STATUS_ERR_SPI;
    }
