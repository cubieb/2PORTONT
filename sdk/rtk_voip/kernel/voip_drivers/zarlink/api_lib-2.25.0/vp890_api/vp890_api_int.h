...
#define VP890_CM_OFFSET_NUM_LIMIT   20  /* Maximum number of offset measurements
                                           before giving up */

#define VP890_FXO_DPOH_DEFAULT    0x01  /* Default loop current averaging time.
                                           (32 << DPOH) ms */

/* Potential overcurrent detection if all of the previous 5 readings are above
   this absolute value */
#define VP890_FXO_OVERCURRENT_THRESHOLD 1230

/* If the average steady current is below this value, assume it is a
   current-starve condition.  Will report POH in this region. */
#define VP890_FXO_STARVE_THRESHOLD 40

/* Report POH if the initial current is measured under this threshold
   and the prior onhook condition was line-not-in-use. */
#define VP890_FXO_POH_THRESHOLD 80

/* Report PNOH if the initial current is measured above this threshold
   and the prior onhook condition was line-in-use. */
#define VP890_FXO_PNOH_THRESHOLD 230

/* If the most recent current samples vary by more than this, consider
   it a transient, and not a steady measurement. */
#define VP890_FXO_TRANSIENT_THRESHOLD 30

/* Declare POH if the new steady state current is this percentage or
   more below the previous steady measurement.
   Declare PNOH if the previous steady measurement is this percentage
   or more below the new steady measurement. */
#define VP890_FXO_POH_RELATIVE_PCT 30
