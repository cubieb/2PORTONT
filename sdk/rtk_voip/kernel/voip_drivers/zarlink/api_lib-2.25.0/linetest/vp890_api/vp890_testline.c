    ...
    if (pLineObj->status & VP890_IS_FXO) {
        ...
        switch (test) {
            ...
            case VP_TEST_ID_VP_MPI_CMD: {
                ...
                if (pMpiCmdArgs == NULL) {
                    VP_ERROR(VpLineCtxType, pLineCtx, ("Vp890TestVpMpiCmd NULL input struct"));
                    rtnval = VP_STATUS_INVALID_ARG;
                } else {
                    ...
                    Vp890GenerateTestEvent(pLineCtx, test, handle, TRUE, FALSE);
                    VP_TEST(VpLineCtxType, pLineCtx, ("Vp890TestVpMpiCmd(): complete"));
