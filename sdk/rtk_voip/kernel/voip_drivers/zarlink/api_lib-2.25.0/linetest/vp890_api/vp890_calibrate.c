 ...
 *
 * $Revision: 4942 $
 * $LastChangedDate: 2014-06-30 10:19:16 -0500 (Mon, 30 Jun 2014) $
 */
    ...
    VpTestIdType testId)
{
    ...
    switch (*pTestState) {
        ...
        case CAL_RESTORE_LINE_STATE:
            ...
            Vp890SetTestStateAndTimer(pDevObj, pTestState, CAL_END,
    ...
    uint8 adcState)
{
    ...
    pLineObj->icr2Values[2] |= 0xA0; /* disable battery, disable met speedup*/
    ...
    VpDeviceIdType deviceId)
{
    ...
    if (VpMemCpyCheck(pLineObj->icr2Values, pCalRegs->icr2, VP890_ICR2_LEN) == TRUE) {
        /* 
         * Prevent the speedup being disabled. This is necessary if going to the active state.
         * If we don't then the CHL voltage change will appear on the VREF pin and cause
         * a thermal fault during line testing. The thermal fault will cause a test abort.
         * This issue only appears if there is leakage from the IHL pin to GND [# 7284]
         */
        ...
        /* remove the met speedup after we are safely in the active state */
        VpMemCpyCheck(pLineObj->icr2Values, pCalRegs->icr2, VP890_ICR2_LEN);
        mpiIndex = VpCSLACBuildMpiBuffer(mpiIndex, mpiBuffer, VP890_ICR2_WRT,
            VP890_ICR2_LEN, pLineObj->icr2Values);
    }
