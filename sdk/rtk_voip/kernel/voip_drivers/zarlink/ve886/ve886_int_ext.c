...
#include "ve886_int.h"
#include "vp886_api_int.h"  /* For VP886_DEV_PCN_88221 device type */
...
** for the Device/Line contexts and objects
*/
static Vp886DeviceObjectType	VpDevObj [ZARLINK_SLIC_DEV_NUM];
static Vp886LineObjectType  	VpLineObj[ZARLINK_SLIC_CH_NUM];
	...
	for (DevType=DEV_FXS; DevType<DEV_LAST; DevType++) {
		...
		for (i=0;i<DevNum;i++) {
			...
			max_line = Ve886CreateDevObj(
				...
				/* Zarlink */
				gDevId,
				VP_DEV_886_SERIES,	/* vincent TODO decided slic type */
				&VpDevObj[gDevId],
				&VpDevCtx[gDevId],
				&VpLineObj[chID],
				&VpLineCtx[chID]); 
			...
			if (DevType == DEV_FXS || DevType == DEV_FXSFXO || DevType == DEV_FXSFXS) {
				/* FXS 1st line */
				Ve886CreateLineObj( chID,  0, LINE_FXS, pDev->pLine[0], pcm_mode, chID);
			} else if (DevType == DEV_FXO) {
				/* FXO 1st line */
				Ve886CreateLineObj( chID,  0, LINE_FXO, pDev->pLine[0], pcm_mode, chID);
			}

			if (DevType == DEV_FXSFXS) {
				/* FXS 2nd line */
				chID = rtkGetNewChID();
				Ve886CreateLineObj( chID, 1, LINE_FXS, pDev->pLine[1], pcm_mode, chID);
			} else if (DevType == DEV_FXSFXO) {
				/* FXO 2nd line */
				chID = rtkGetNewChID();
				Ve886CreateLineObj( chID, 1, LINE_FXO, pDev->pLine[1], pcm_mode, chID);
			}
