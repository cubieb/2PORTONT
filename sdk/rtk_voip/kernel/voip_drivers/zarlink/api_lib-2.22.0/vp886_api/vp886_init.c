 ...
 *
 * $Revision: 11371 $
 * $LastChangedDate: 2014-04-03 18:10:29 -0500 (Thu, 03 Apr 2014) $
 */
    ...
    while (runAnotherState) {
        ...
        switch (pDevObj->initDeviceState) {
            ...
            case VP886_INIT_DEVICE_VREF_CHECK: {
                ...
                timerDuration = VP886_VREF_WAIT_TIME;
                if (pDevObj->devProfileData.cpEnable && !VP886_IS_CR(pDevObj)) {
            ...
            case VP886_INIT_DEVICE_GEN_EVENT: {
                ...
                /* For devices without I/O pins, set the I/Os to output low for safety and
                   power saving. */
                if (VP886_IS_CR(pDevObj)) {
                    uint8 ioDir = 0x05;
                    uint8 ioData = 0x00;
                ...
                } else if (pDevObj->staticInfo.maxChannels == 1) {
                    /* For single-channel non-reduced parts, do the same for only
                       the second channel I/Os. */
                    uint8 ioDir = 0x05;
                    uint8 ioData = 0x00;
                    pDevObj->ecVal = VP886_EC_2;
                    VpSlacRegWrite(pDevCtx, VP_NULL, VP886_R_IODIR_WRT, VP886_R_IODIR_LEN, &ioDir);
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    switch (devicePcn) {
        ...
        case VP886_R_RCNPCN_PCN_LE9651:
            break;
        default:
            VP_ERROR(VpDevCtxType, pDevCtx, ("Device PCN not recognized in ZL880 or VE960 families: 0x%02X",
                devicePcn));
            VP_API_FUNC_INT(VpDevCtxType, pDevCtx, ("Vp886InitDevicePcnRcn-"));
            return VP_STATUS_ERR_SPI;
    }
    ...
    bool initDevice)
{
    ...
    if (initDevice && status == VP_STATUS_SUCCESS && !VP886_IS_CR(pDevObj)) {
        uint8 ioDirReg;
        uint8 io2Use;
