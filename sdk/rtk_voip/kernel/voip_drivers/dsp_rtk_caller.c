...
int voip_clid_read_proc(struct seq_file *f, void *v)
{
	int ch;
	int n = 0;
	...
	if( IS_CH_PROC_DATA( (long)f->private ) )
	{
		...
		seq_printf( f, "DTMF Caller ID Setting:\n");
		seq_printf( f, " - bAuto_Ring = %d (ture=1)\n", dtmf_cid_info[ch].bAuto_Ring);
		seq_printf( f, " - bAuto_SLIC_action = %d (ture=1)\n", dtmf_cid_info[ch].bAuto_SLIC_action);
		seq_printf( f, " - bBefore1stRing = %d (ture=1)\n", dtmf_cid_info[ch].bBefore1stRing);
		seq_printf( f, " - bAuto_StartEnd = %d (ture=1)\n", dtmf_cid_info[ch].bAuto_StartEnd);
		if (dtmf_cid_info[ch].start_digit == DTMF_DIGIT_STAR)
			seq_printf( f, " - start_digit = *\n");
			...
			seq_printf( f, " - start_digit = %c\n", 'A' + dtmf_cid_info[ch].start_digit - DTMF_DIGIT_A);
		else
			seq_printf( f, " - start_digit configuration error!\n");

		if (dtmf_cid_info[ch].end_digit == DTMF_DIGIT_STAR)
			seq_printf( f, " - end_digit = *\n");
			...
			seq_printf( f, " - end_digit = %c\n", 'A' + dtmf_cid_info[ch].end_digit - DTMF_DIGIT_A);
		else
			seq_printf( f, " - end_digit configuration error!\n");
		//seq_printf( f, " - data = %s\n", dtmf_cid_info[ch].data);
		
		seq_printf( f, "FSK Caller ID Setting:\n");
		switch (fsk_spec_areas[ch] & 0x7)
		{
			case 0:
				seq_printf( f, " - Bellcore\n");
				break;
			case 1:
				seq_printf( f, " - ETSI\n");
				break;
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int clid_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_clid_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_clid_read_fops = {
	.owner	= THIS_MODULE,
	.open	= clid_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_clid_read_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_channel_proc_read_entry( "clid", &proc_clid_read_fops );
#else
