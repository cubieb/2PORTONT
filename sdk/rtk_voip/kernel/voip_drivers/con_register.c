...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int voip_con_register_read_proc(struct seq_file *f, void *v)
{
#define S_PTR( ptr, ptr2 )		( ptr ? ptr ->ptr2, NULL )
#define S_CALL( ptr, ptr2, fn )	( ptr && ptr ->ptr2 && ptr ->ptr2 ->fn ? ptr ->ptr2 ->fn( ptr ) : 0 )

	if ( f ){
		printk("f:%p, v:%p, f->private = 0x%x\n", f, v, (long)f->private);
		printk("buf : 0x%x, count 0x%x, index 0x%x\n", *((unsigned char *) f->buf), f->count, f->index);
	}
	
#if 1
	switch( (long)f->private ) {
	case REG_PROC_COOKIE_IOC:
		{
			voip_ioc_t *p_ioc;
			voip_snd_t *p_snd_assigned;
			//fill_proc_buff( ioc, ioch );
			seq_printf( f, "ioch T M S name    ID		pre-assign \n---- - - - ------- -------- ---------- \n" );
			...
			while( p_ioc ) {
				...
				seq_printf( f, "%4X %X %X %X %-7s %08X %2d %-7s \n", 
					p_ioc ->ioch,
					p_ioc ->ioc_type,
					p_ioc ->mode_var, 
					p_ioc ->state_var,
	...
	case REG_PROC_COOKIE_SND:
		{
			...
			seq_printf( f, "sch name	TS1/2 T cpu flgs \n--- ------- ----- - --- --------\n" );
			
			p_snd = ( const voip_snd_t * )voip_snd_link.head;
			
			while( p_snd ) {
				seq_printf( f, "%3d %-7s %2d/%2d %d %-3d %08X\n", 
					p_snd ->sch,
	...
	case REG_PROC_COOKIE_BUS:
		{
			...
			seq_printf( f, "bch name	E TS1/2 \n--- ------- - ----- \n" );
			
			p_bus = ( const voip_bus_t * )voip_bus_link.head;
			
			while( p_bus ) {
				seq_printf( f, "%3d %-7s %d %2d/%2d\n", 
					p_bus ->bch,
...
{
	return single_open(file, voip_con_register_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_register_fops = {
	.owner	= THIS_MODULE,
	.open	= register_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_con_register_read_proc
};
#endif
...
static int __init voip_con_register_proc_init( void )
{
	...
	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/ioc", 0, NULL, 
			&proc_register_fops, ( void * )REG_PROC_COOKIE_IOC );

	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/snd", 0, NULL, 
			&proc_register_fops, ( void * )REG_PROC_COOKIE_SND );

	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/bus", 0, NULL, 
			&proc_register_fops, ( void * )REG_PROC_COOKIE_BUS );

	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/dsp", 0, NULL, 
			&proc_register_fops, ( void * )REG_PROC_COOKIE_DSP );
#else
