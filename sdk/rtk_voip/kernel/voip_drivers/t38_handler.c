...
int voip_t38_read_proc(struct seq_file *f, void *v)
{
	int ch;
	int n = 0;
	...
	if( IS_CH_PROC_DATA( (long)f->private ) ) {
		...
		seq_printf( f, "Running state=%d\n", t38RunningState[ ch ] );
		seq_printf( f, "Fax Tx/Rx Gain=%d/%d\n", FAX_TX_GAINUP, FAX_RX_GAINDN);
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int t38_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_t38_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_t38_fops = {
	.owner	= THIS_MODULE,
	.open	= t38_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_t38_read_proc
};
#endif
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_channel_proc_read_entry( "t38", &proc_t38_fops );
#else
