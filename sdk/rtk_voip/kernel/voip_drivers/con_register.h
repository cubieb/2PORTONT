...
typedef enum {
	...
	IOC_MODE_BLINKING_1HZ,	// special mode for LED application (modify state automatically) 	
	IOC_MODE_BLINKING_4HZ,	// special mode for LED application (modify state automatically) 			
} ioc_mode_t;
...
#if 0
	int ( *PortDetect)(struct voip_snd_s *this, unsigned char * val);
	int ( *LineROH)(struct voip_snd_s *this, unsigned char * val);
	int ( *LineVOLTAGE)(struct voip_snd_s *this, unsigned char * val);
	int ( *LineRESFLT)(struct voip_snd_s *this, unsigned char * val);
#endif	
	int ( *LineTest)(struct voip_snd_s *this, uint16 tID, unsigned char * val);
#endif
#ifdef CONFIG_RTK_VOIP_SLIC_ZARLINK_886_SERIES
	int ( *MeteringPulse)(struct voip_snd_s *this, uint16 hz, uint16 onTime, uint16 offTime, uint16 numMeters);
#endif	
	int	( *GetCalibrationCoeff)(struct voip_snd_s *this, unsigned char * val);
	int	( *SetCalibrationCoeff)(struct voip_snd_s *this, unsigned char * val);
	int	( *DoCalibration)(struct voip_snd_s *this, unsigned char mode);
