...
LtVp880JumpToConclude(
    LtTestCtxType *pTestCtx);

static void
LtVp880AllGr909ClearTemp(
    LtAllGr909ResultType *pTempAllGr909);

static LtTestStatusType
LtVp880AllCheckResult(
    LtTestCtxType *pTestCtx,
    ...
    LtEventType *pEvent)
{
    ...
    LtVp880AllGr909States *pInnerState = &pTempAllGr909->innerState;
    ...
    LT_DOUT(LT_DBG_INFO, ("LtVp880AllGr909Handler(testState:%i)", *pInnerState));

    switch (*pInnerState) {
        case VP880_ALL_GR_909_INIT:
            /* Clear out the temp structure */
            LtVp880AllGr909ClearTemp(pTempResults);
            /* no break needed */

        case VP880_ALL_GR_909_LINE_V:
            *pInnerState = VP880_ALL_GR_909_LINE_V;
            /*
             ...
             * the Vp880909AllAttributesCheck() function. This is done for
             * code clarity and incase the order of the attribute checkers
             * is changed inside of Vp880909AllAttributesCheck().
             */
            ...
            if(retval != LT_STATUS_DONE) {
                return LtVp880AllCheckResult(pTestCtx, retval);
            } else {
                *pTempSubTestMask |=
                ...
                if (LT_TEST_PASSED != (pTempResults->lineV.fltMask & (
                  ...
                  LT_LVM_HEMF_AC_RING | LT_LVM_HEMF_AC_DIFF))) {
                    *pTempAllpfMask |= LT_A909TM_LINE_V_HEMF_FAILED;
                    LT_DOUT(LT_DBG_INFO, ("LtVp880AllGr909Handler() : "\
                        "LT_A909TM_LINE_V_HEMF_FAILED"));

                    if (stopOnFail) {
                        return LtVp880JumpToConclude(pTestCtx);
                    }
                ...
                *pInnerState = VP880_ALL_GR_909_RES_FLT;
            }
            /* no break needed */
        case VP880_ALL_GR_909_RES_FLT:
            ...
            if(retval != LT_STATUS_DONE) {
                ...
                *pTempSubTestMask |=  LT_A909STM_RES_FLT;
                ...
                if (LtVp880AllGr909RunRoh(pTestCtx) == FALSE) {
                    *pInnerState = VP880_ALL_GR_909_COMPLETE;
                    return LtVp880JumpToConclude(pTestCtx);
                } else {
                    *pInnerState = VP880_ALL_GR_909_OFF_HOOK;
                }
            }
            /* no break needed */
        ...
        case VP880_ALL_GR_909_OFF_HOOK:
            ...
            if(retval != LT_STATUS_DONE) {
                ...
                *pTempSubTestMask |=  LT_A909STM_ROH;
                ...
                /* skip the roh and ren test to save time if possible */
                if (LtVp880AllGr909RunRen(pTestCtx) == FALSE) {
                    *pInnerState = VP880_ALL_GR_909_COMPLETE;
                    return LtVp880JumpToConclude(pTestCtx);

                } else {
                    *pInnerState = VP880_ALL_GR_909_RINGERS;
                }
            }
            /* no break needed */
        ...
        case VP880_ALL_GR_909_COMPLETE:
            *pInnerState = VP880_ALL_GR_909_COMPLETE;
    ...
    LtTestCtxType *pTestCtx)
{
    ...
    pTestCtx->pTemp->vp880Temp.allGr909.innerState = VP880_ALL_GR_909_COMPLETE;
...
LtVp880AllCopyResults(
    LtTestCtxType *pTestCtx)
{
    LtAllGr909ResultType *pTempResults = &pTestCtx->pTemp->vp880Temp.allGr909.results;
    LtAllGr909MaskType *pTempAllpfMask = &pTempResults->fltMask;
    LtAllGr909SubTestMaskType *pTempSubTestMask = &pTempResults->subTestMask;
