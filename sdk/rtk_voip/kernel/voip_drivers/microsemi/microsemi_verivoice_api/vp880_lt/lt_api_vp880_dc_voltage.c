...
#if defined(VP880_INCLUDE_TESTLINE_CODE) && defined(LT_VP880_PACKAGE) && defined(LT_DC_VOLTAGE)
    ...
    LtEventType *pEvent)
{
    ...
    LT_DOUT(LT_DBG_INFO, ("LtVp880DcVoltageHandler(testState:%i)", *pState));
    ...
    switch (*pState) {
        ...
        case DC_VOLTAGE_INIT:
            ...
            pTemp880->loopCond.integrateTime =  LT_VP880_DC_VOLTAGE_INTEGRATE_TIME;
            pTemp880->loopCond.settlingTime = LT_VP880_DC_VOLTAGE_SETTLE_TIME;
        ...
        case DC_VOLTAGE_SET_LINE_STATE: {
            ...
            pTempDcVolt->normalPolarity = TRUE;
            ...
            if ((lineStateCurrent == VP_LINE_ACTIVE_POLREV) ||
                ...
                pTempDcVolt->normalPolarity = FALSE;
            }
        ...
        case DC_VOLTAGE_STORE_FEED_VSAB:
            ...
            LtVp880DcVoltageSetup(pTestCtx);
            /* no break */

        case DC_VOLTAGE_RAMP_TO_INPUT_V:
            *pState = DC_VOLTAGE_REQUEST_VSAB_1;
            retval = Vp880TestLineWrapper(pTestCtx, VP_TEST_ID_RAMP_INIT,
        ...
        case DC_VOLTAGE_RAMP_TO_OPPOSITE_V_1:
             ...
             */
            if (VP880_UNIT_CONVERT(-75000, VP880_UNIT_MV, VP880_UNIT_ADC_VAB)
                > ABS(pTempDcVolt->vpApiRampInput1.dcVoltage)) {
                pTempDcVolt->vpApiRampInput1.dcVoltage *= 2;
                *pState = DC_VOLTAGE_REQUEST_VSAB_2;
            }
            retval = Vp880TestLineWrapper(pTestCtx, VP_TEST_ID_RAMP,
        ...
        case DC_VOLTAGE_COMPLETE:
            ...
            LtVp880CalculateResults(pTestCtx, LT_TID_DC_VOLTAGE);
...
{
    int16 inputVADC;
    LtVp880DcVoltageTempType *pTempDcVolt = &pTestCtx->pTemp->vp880Temp.tempData.dcVoltage;
    LtVoltageType inputV = pTempDcVolt->input.testVoltage;
    int16 feedVADC = pTempDcVolt->feedV;
    int32 temp;

    inputVADC = (int16)VP880_UNIT_CONVERT(inputV, VP880_UNIT_MV, VP880_UNIT_ADC_VAB);

    /* Init Metalic Ramp to initial V*/
    pTempDcVolt->vpApiRampInitInput.vRate = LT_VP880_DC_VOLTAGE_RAMP_SLOPE;
    pTempDcVolt->vpApiRampInitInput.dcVstart = -ABS(feedVADC);
/*    pTempDcVolt->vpApiRampInitInput.dcVend = (pTempDcVolt->normalPolarity ? (inputVADC + 137*2) : inputVADC); */
    ...
    pTempDcVolt->vpApiRampInput1.vRate = LT_VP880_DC_VOLTAGE_RAMP_SLOPE;
/*    pTempDcVolt->vpApiRampInput1.dcVoltage = (pTempDcVolt->normalPolarity ? (-inputVADC -137) : (-inputVADC + 137)); */
    ...
    pTempDcVolt->vpApiRampInput2.vRate = LT_VP880_CAP_RINGERS_RAMP_SLOPE;
