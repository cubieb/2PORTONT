 ...
 *
 * $Revision: 10251 $
 * $LastChangedDate: 2012-06-28 15:23:13 -0500 (Thu, 28 Jun 2012) $
 */
...
typedef enum
{
    ...
    ELE3_RES_LG_END             = VP880_TESTLINE_GLB_END_STATE,
    ELE3_RES_LG_QUIT            = VP880_TESTLINE_GLB_QUIT_STATE,
    ...
    VpTestIdType testId)
{
    ...
    VP_TEST(VpLineCtxType, pLineCtx, ("Vp880Test3EleResLG(ts:%i)", *pTestState));
    if (FALSE == callback) {
    ...
    switch (*pTestState) {
        ...
        case ELE3_RES_LG_SET_ADC_CURRENT:
            ...
            *loopCnt = 0;
            ...
            if ((p3EleResData->settlingTime & 0x8000) == 0x0000) {
                VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_RES_LG_MEASURE_CURRENT,
                    MS_TO_TICKRATE(10, tick));
            } else {
                VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_RES_LG_SETUP,
                    MS_TO_TICKRATE(10, tick));
            }
            break;
        ...
        case ELE3_RES_LG_MEASURE_CURRENT: {
            ...
            VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_CONV_CFG_RD, VP880_CONV_CFG_LEN,
                &adcConf);
            ...
            VpStartPcmCollect(pLineCtx, VP_PCM_OPERATION_AVERAGE, ELE3_RES_LG_CHECK_CURRENT,
                0, 146);
            break;
        }
        ...
        case ELE3_RES_LG_SETUP: {
            ...
            VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_RES_LG_CHANGE_FEED, NEXT_TICK);
            break;
        }
        ...
        case ELE3_RES_LG_CHANGE_FEED:
            ...
            if (time > 0) {
                VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_RES_LG_ADC_SETUP,
        ...
        case ELE3_RES_LG_ADC_SETUP:
            ...
            VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_RES_LG_COLLECT_DATA, NEXT_TICK);
            break;
        ...
        case ELE3_RES_LG_RESTORE: {
            uint8 mpiBuffer[8 + VP880_SYS_STATE_LEN + VP880_LOOP_SUP_LEN +
                            VP880_DC_FEED_LEN + VP880_REGULATOR_PARAM_LEN +
                            VP880_ICR1_LEN + VP880_ICR2_LEN + VP880_ICR3_LEN +
                            VP880_ICR4_LEN ];
            uint8 mpiIndex = 0;
            uint8 slacState =  VP880_SS_IDLE | VP880_SS_ACTIVATE_MASK;
            ...
            VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_RES_LG_END,
        ...
        case ELE3_RES_LG_END:
            ...
            *pErrorCode = VP_TEST_STATUS_SUCCESS;
            ...
            VP_TEST(VpLineCtxType, pLineCtx, ("Vp880Test3EleResLG(): complete"));
            break;
        ...
        case ELE3_RES_LG_QUIT:
            ...
            VP_WARNING(VpLineCtxType, pLineCtx, ("Vp880Test3EleResLG(ec:%i): quit",
                 *pErrorCode));
            break;
        ...
        default:
            *pErrorCode = VP_TEST_STATUS_INTERNAL_ERROR;
            VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp880Test3EleResLG(): invalid teststate"));
            break;
    }
...
/*------------------------------------------------------------------------------
 * Vp3EleResLGArgCheck()
 * This functions is called by the Vp880Test3EleResLG() function if
 * Vp880Test3EleResLG() was not called via a callback.
 *
...
/*------------------------------------------------------------------------------
 * Vp3EleResLGSetup()
 * This function is called by the Vp880Test3EleResLG() state machine during
    ...
    VpDeviceIdType deviceId)
{
    ...
    VpDevCtxType *pDevCtx = pLineCtx->pDevCtx;
    Vp880DeviceObjectType *pDevObj = pDevCtx->pDevObj;
    uint8 dcfeed[VP880_DC_FEED_LEN] = {0x6F, 0x1F};
    ...
    uint8 slacState =  VP880_SS_ACTIVE;     /* Active state and codec activated */
    uint8 chanId = pLineObj->channelId;
    int16 biasErr;
     ...
     * If not abs but is channel 0 then icr3 need to set y battery
     */
    if ( (VP880_IS_ABS & pDevObj->stateInt) || (chanId == 0) ) {
        pLineObj->icr3Values[3] = 0xCE;
    } else {
        pLineObj->icr3Values[3] = 0xE6;
    }
    ...
    biasErr += 2 * ((pDevObj->vp880SysCalData.sigGenAError[chanId][0] -
        pDevObj->vp880SysCalData.vocOffset[chanId][VP880_NORM_POLARITY]) * 15 / 10);
    ...
    *loopCnt = 0;
    ...
    switch (*pAdcState) {
        ...
        case ELE3_RES_LG_IMT_REVERSE_FEED: {
            /* switch to low gain */
            uint8 mpiBuffer[VP880_ICR1_LEN+1 +
                            VP880_ICR2_LEN+1 +
                            VP880_ICR3_LEN+1 +
                            VP880_DC_FEED_LEN+1 +
                            VP880_SIGA_PARAMS_LEN+1];
            ...
            *vabComputed = 0;
            ...
            nextTimer = VP880_STATE_CHANGE_SETTLING_TIME_SHORT;  /* 50ms */
            break;
        }

        case ELE3_RES_LG_IMT_ZERO_FEED_RG:
        case ELE3_RES_LG_IMT_REVERSE_FEED_RG: {
            uint8 mpiBuffer[VP880_ICR3_LEN+1 +
                            VP880_ICR2_LEN+1 +
                            VP880_ICR1_LEN+1 +
                            VP880_DC_FEED_LEN+1 +
                            VP880_SIGA_PARAMS_LEN+1];
            ...
            if ((pTestHeap->lowGain == TRUE) && (*pAdcState == ELE3_RES_LG_IMT_ZERO_FEED_RG)) {
                ...
                *pAdcState = ELE3_RES_LG_IMT_REVERSE_FEED;
                 ...
                 * If not abs but is channel 0 then icr3 need to set y battery
                 */
                if ( (VP880_IS_ABS & pDevObj->stateInt) || (pLineObj->channelId == 0) ) {
                    pLineObj->icr3Values[3] = 0xCE;
                } else {
                    pLineObj->icr3Values[3] = 0xE6;
                }
                ...
                biasErr = feedBias;
                biasErr += 2 * ((pDevObj->vp880SysCalData.sigGenAError[pLineObj->channelId][0] -
                    pDevObj->vp880SysCalData.vocOffset[pLineObj->channelId][VP880_NORM_POLARITY]) * 15 / 10);
                sigGenA[1] = (uint8)((biasErr >> 8) & 0x00FF);
                sigGenA[2] = (uint8)(biasErr & 0x00FF);
    ...
    uint8 adcState)
{
    ...
    VpGetPcmResult(pDevObj, VP_PCM_OPERATION_AVERAGE, FALSE, TRUE, chanId, aToDRoute, pResult);

    if (adcState == ELE3_RES_LG_IMT_NORMAL_FEED) {
        VpGetPcmResult(pDevObj, VP_PCM_OPERATION_RMS, FALSE, FALSE, chanId,
            VP880_NO_CONNECT, &imtRms);
