...
typedef enum
{
    ...
    ELE3_CAP_RESTORE         = 35,
    ELE3_CAP_END             = VP880_TESTLINE_GLB_END_STATE,
    ELE3_CAP_QUIT            = VP880_TESTLINE_GLB_QUIT_STATE,
    ...
    VpTestIdType testId)
{
    ...
    /* VP_TEST(VpLineCtxType, pLineCtx, ("Vp880Test3EleCap(ts:%i)", *pTestState)); */
    if (FALSE == callback) {
    ...
    switch (*pTestState) {
        ...
        case ELE3_CAP_SETUP:
            ...
            VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_CAP_SETUP_ADC,
        ...
        case ELE3_CAP_SETUP_ADC:{
            ...
            VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_CAP_GET_DATA,
        ...
        case ELE3_CAP_GET_DATA:{
            ...
            VpSetTestStateAndTimer(pDevObj, pTestState, *pTestState, nbTick);
            break;
        }
        ...
        case ELE3_CAP_RESTORE:
            /* Restore registers */
            VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_CONV_CFG_WRT,
                VP880_CONV_CFG_LEN, pTestHeap->saveConvConfig);
            pDevObj->txBufferDataRate = pTestHeap->saveConvConfig[0] & 0xF0;

            VpSetTestStateAndTimer(pDevObj, pTestState, ELE3_CAP_END, NEXT_TICK);
            break;
        ...
        case ELE3_CAP_END:
            ...
            *pErrorCode = VP_TEST_STATUS_SUCCESS;
            ...
            VP_TEST(VpLineCtxType, pLineCtx, ("Vp880Test3EleCap(): complete"));
            break;

        case ELE3_CAP_QUIT:
            /* The test has eneded early due to the current errorCode */
            VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
            VP_WARNING(VpLineCtxType, pLineCtx, ("Vp880Test3EleCap(ec:%i): quit", *pErrorCode));
            break;

        default:
            *pErrorCode = VP_TEST_STATUS_INTERNAL_ERROR;
            VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp880Test3EleCap(): invalid teststate"));
            break;
    }
...
/*------------------------------------------------------------------------------
 * Vp3EleCapGetData()
 * This functions is called by the Vp880Test3EleCap()
 *
    ...
    uint8 *pTimer)
{
    ...
    if ((pDevObj->ecVal & VP880_WIDEBAND_MODE) == VP880_WIDEBAND_MODE) {
    ...
    if (pTestHeap->xtraBuffer == TRUE) {
        ...
        nbNewSamples = (pDevObj->txBuffer[0] >> 4) & 0x0F;
        if (nbNewSamples == 7) {
            if ((pTestHeap->requestedSamples - nbSamples) <= 6) {
                /* The buffer overflowed but it was the last one, so we have enough samples */
                nbNewSamples = pTestHeap->requestedSamples - nbSamples;
            } else {
                VP_TEST(VpLineCtxType, pLineCtx, ("Impossible to collect back to back buffers, exit"));
                *pErrorCode = VP_TEST_STATUS_INTERNAL_ERROR;
                *pTestState = ELE3_CAP_QUIT;
                *pTimer = 1;
                return;
            }
        } else if ((pTestHeap->requestedSamples - nbSamples) <= nbNewSamples) {
            nbNewSamples = pTestHeap->requestedSamples - nbSamples;
        }
        ...
        for (smp = 0 ; smp < (nbNewSamples * 2) ; smp += 2) {
            ...
            (((int16)(pDevObj->txBuffer[2 + smp])) << 8) | (int16)(pDevObj->txBuffer[2 + smp + 1]);
        }
        ...
        nbSamples = (pDevObj->txBuffer[0] >> 4) & 0x0F;
        if (nbSamples == 7) {
            nbSamples = 6;
        }
...
/*------------------------------------------------------------------------------
 * Vp3EleCapArgCheck()
 * This functions is called by the Vp880Test3EleCap() function if
 * Vp880Test3EleCap() was not called via a callback.
 *
    ...
    VpTestIdType testId)
{
    ...
    *pAdcState = ELE3_CAP_VAL;
    ...
    pDevObj->testResults.result.capAlt.tipCapCal = pDevObj->vp880SysCalData.tipCapCal[chanId];
    pDevObj->testResults.result.capAlt.ringCapCal = pDevObj->vp880SysCalData.ringCapCal[chanId];
    ...
    VP_TEST(VpLineCtxType, pLineCtx, ("amp %u, freqi=%u, freqo=%u", pTestInput->testAmp, p3EleCapData->testFreq, pDevObj->testResults.result.capAlt.freq));
...
/*------------------------------------------------------------------------------
 * Vp3EleCapSetup()
 * This function is called by the Vp880Test3EleCap() state machine during
...
{
    VpTest3EleCapAltResType *pCapData = &(pTestHeap->testArgs).capAlt;
    uint8 sigGenA[VP880_SIGA_PARAMS_LEN] =  /* set 0V */
        {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8 switchRegParam[VP880_REGULATOR_PARAM_LEN];
    uint8 sigGenCtrl = VP880_GEN_CTRL_EN_BIAS | VP880_GEN_CTRL_EN_A;
    uint8 slacState =  VP880_SS_BALANCED_RINGING;
    uint8 devMode = pLineObj->channelId * 2 + 1;    /* setup to use the test data buffer */
    ...
    uint8 mpiBuffer[VP880_REGULATOR_PARAM_LEN+1 +
                    ...
                    VP880_ICR2_LEN+1 +
                    VP880_ICR3_LEN+1 +
                    VP880_DEV_MODE_LEN+1 +
                    VP880_DC_FEED_LEN+1];
    uint8 mpiIndex = 0;

    /* Save the converter configuration sampling frequency */
    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_CONV_CFG_RD,
        VP880_CONV_CFG_LEN, pTestHeap->saveConvConfig);
    ...
    if (pCapData->bias > 0) {               /* Measuring ring->ground */
        slacState = VP880_SS_UNBALANCED_RINGING_PR;
        pLineObj->icr3Values[0] = 0x04;
        pLineObj->icr3Values[1] = 0x04;     /* Reverse longitudinal drive */
        pCapData->bias -= ((pDevObj->vp880SysCalData.sigGenAError[pLineObj->channelId][0] -
           pDevObj->vp880SysCalData.vocOffset[pLineObj->channelId][VP880_REV_POLARITY]) * 16 / 10);
    } else {                                /* Measuring tip->ground */
        slacState = VP880_SS_UNBALANCED_RINGING;
        pLineObj->icr3Values[0] = 0x04;
        pLineObj->icr3Values[1] = 0x00;
        pCapData->bias -= ((pDevObj->vp880SysCalData.sigGenAError[pLineObj->channelId][0] -
           pDevObj->vp880SysCalData.vocOffset[pLineObj->channelId][VP880_NORM_POLARITY]) * 16 / 10);
    }
    ...
    VP_TEST(None, VP_NULL, ("amp %u, freq=%u, freqi=%u", audioAmp, freq, pCapData->testFreq));
