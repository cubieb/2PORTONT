 ...
 *
 * $Revision: 11806 $
 * $LastChangedDate: 2015-09-01 11:14:10 -0500 (Tue, 01 Sep 2015) $
 */
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    if (pDevObj->staticInfo.maxChannels == 1) {
        return Vp880QuickMpiTest1Ch(pDevCtx);
    }
    ...
    */
    ...
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest: Test 4 failed. Write buffer corrupted: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            writeBuffer[0], writeBuffer[1], writeBuffer[2], writeBuffer[3], writeBuffer[4], writeBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    if (readBuffer[0] != 0x94 || readBuffer[5] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest: Test 4 failed. Read buffer overflow: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[0], readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    if (readBuffer[1] != 0x00 || readBuffer[2] != 0xFF || readBuffer[3] != 0x00 || readBuffer[4] != 0xFF) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest: Test 4 failed. Read value incorrect: 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4]));
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest: expected 0x00 0xFF 0x00 0xFF"));
        status = VP_STATUS_ERR_SPI;
    }
...
/** Vp880QuickMpiTest1Ch()
  Single-channel version of Vp880QuickMpiTest().
  Performs some basic tests of the MPI and HAL implementation to make sure that
  we can properly communicate with the device before starting.
*/
VpStatusType
    ...
    VpDevCtxType *pDevCtx)
{
    ...
    */
    ...
    /* Make sure the write buffer was not corrupted */
    if (writeBuffer[0] != 0xD6 || writeBuffer[1] != 0xE5 || writeBuffer[2] != 0xD6) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 1 failed. Write buffer corrupted: 0x%02X 0x%02X 0x%02X",
            writeBuffer[0], writeBuffer[1], writeBuffer[2]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /* Make sure there was no overflow into the data around byte 1 */
    if (readBuffer[0] != 0x94 || readBuffer[2] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 1 failed. Read buffer overflow: 0x%02X 0x%02X 0x%02X",
            readBuffer[0], readBuffer[1], readBuffer[2]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /* The high bit of this register always returns 0, so what we read should
       not be exactly what we wrote.  0xE5 becomes 0x65 */
    if (readBuffer[1] != 0x65) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 1 failed. Read value incorrect: 0x%02X, expected 0x65",
            readBuffer[1]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    */
    ...
    if (writeBuffer[0] != 0xD6 || writeBuffer[1] != 0xAB || writeBuffer[2] != 0xCD ||
        writeBuffer[3] != 0xEF || writeBuffer[4] != 0x01 || writeBuffer[5] != 0xD6)
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 2 failed. Write buffer corrupted: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            writeBuffer[0], writeBuffer[1], writeBuffer[2], writeBuffer[3], writeBuffer[4], writeBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /* Make sure there was no overflow into the surrounding data */
    if (readBuffer[0] != 0x94 || readBuffer[5] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 2 failed. Read buffer overflow: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[0], readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /* Make sure the data read back correctly.
       ...
       0x03CD0701 instead of 0xABCDEF01 */
    if (readBuffer[1] != 0x03 || readBuffer[2] != 0xCD || readBuffer[3] != 0x07 || readBuffer[4] != 0x01) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 2 failed. Read value incorrect: 0x%02X 0x%02X 0x%02X 0x%02X,",
            readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4]));
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: expected 0x03 0xCD 0x07 0x01"));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    */
    ...
    /* Build a large write buffer where the first command is a write to the clock
       slot register, followed by repeated writes to the ch1 GR register. */
    writeBuffer[1] = 0xDA;
    /* Switch to the channel EC */
    writeBuffer[2] = VP880_EC_WRT;
    writeBuffer[3] = VP880_EC_CH1;
    ...
    writeBuffer[i] = 0x78;
    /* Send the whole buffer */
    VpMpiCmdWrapper(deviceId, VP880_EC_CH1, VP880_XR_CS_WRT, i, &writeBuffer[1]);
    
    /* Read back both register values */
    VpMpiCmdWrapper(deviceId, VP880_EC_CH1, VP880_XR_CS_RD, VP880_XR_CS_LEN, &readBuffer[1]);
    VpMpiCmdWrapper(deviceId, VP880_EC_CH1, VP880_CADENCE_TIMER_RD, VP880_CADENCE_TIMER_LEN, &readBuffer[2]);
    ...
    if (readBuffer[1] != 0x5A) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 3 failed. First read value incorrect: 0x%02X, expected 0x5A",
            readBuffer[1]));
        status = VP_STATUS_ERR_SPI;
    }
    if (readBuffer[2] != 0x01 || readBuffer[3] != 0xBB || readBuffer[4] != 0x06 || readBuffer[5] != 0x78) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 3 failed. Second read value incorrect: 0x%02X 0x%02X 0x%02X 0x%02X,",
            readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]));
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: expected 0x01 0xBB 0x06 0x78"));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    */
    ...
    if (writeBuffer[0] != 0xD6 || writeBuffer[1] != 0x00 || writeBuffer[2] != 0xFF ||
        writeBuffer[3] != 0x00 || writeBuffer[4] != 0xFF || writeBuffer[5] != 0xD6)
    {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 4 failed. Write buffer corrupted: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            writeBuffer[0], writeBuffer[1], writeBuffer[2], writeBuffer[3], writeBuffer[4], writeBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /* Make sure there was no overflow into the surrounding data */
    if (readBuffer[0] != 0x94 || readBuffer[5] != 0x94) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 4 failed. Read buffer overflow: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[0], readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4], readBuffer[5]));
        status = VP_STATUS_ERR_SPI;
    }
    ...
    /* Make sure the data was read back correctly. */
    if (readBuffer[1] != 0x00 || readBuffer[2] != 0xFF || readBuffer[3] != 0x00 || readBuffer[4] != 0xFF) {
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: Test 4 failed. Read value incorrect: 0x%02X 0x%02X 0x%02X 0x%02X",
            readBuffer[1], readBuffer[2], readBuffer[3], readBuffer[4]));
        VP_ERROR(VpDevCtxType, pDevCtx, ("Vp880QuickMpiTest1Ch: expected 0x00 0xFF 0x00 0xFF"));
        status = VP_STATUS_ERR_SPI;
    }
