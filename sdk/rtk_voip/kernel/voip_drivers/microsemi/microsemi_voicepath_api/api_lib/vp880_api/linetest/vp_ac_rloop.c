/** file vp_ac_rloop.c
 *
 *  This file contains the ac rloop primitive algorithm
 *
 * Copyright (c) 2011, Microsemi Corporation
 *
 * $Revision: 9115 $
 * $LastChangedDate: 2011-11-15 15:25:17 -0600 (Tue, 15 Nov 2011) $
 */

#include "vp_api_cfg.h"

#if defined (VP_CC_880_SERIES)
#if defined (VP880_INCLUDE_TESTLINE_CODE)

/* Project Includes */
#include "vp_api_types.h"
#include "sys_service.h"
#include "vp_hal.h"
#include "vp_api_int.h"
#include "vp880_api.h"
#include "vp880_api_int.h"
#include "vp_api_testline_int.h"

typedef enum
{
    COMMON_TEST_SETUP               = VP880_TESTLINE_GLB_STRT_STATE,
    AC_RLOOP_OFFSET_SETUP           = 5,
    AC_RLOOP_OFFSET_ADC_SETUP       = 6,
    AC_RLOOP_OFFSET_COLLECT_DATA    = 7,
    AC_RLOOP_SETUP                  = 8,
    AC_RLOOP_ADC_SETUP              = 10,
    AC_RLOOP_COLLECT_DATA           = 15,
    AC_RLOOP_GET_RESULT             = 20,
    AC_RLOOP_END                    = VP880_TESTLINE_GLB_END_STATE,
    AC_RLOOP_QUIT                   = VP880_TESTLINE_GLB_QUIT_STATE,
    AC_RLOOP_ENUM_SIZE              = FORCE_STANDARD_C_ENUM_SIZE /* Portability Req.*/
}  vpOpenVacTestStateTypes;

typedef enum
{
    AC_RLOOP_IMT,
    AC_RLOOP_VAB,
    AC_RLOOP_ILG,
    AC_RLOOP_MAX_ADC,
    VP_AC_RLOOP_ADC_ORDER_ENUM_SIZE = FORCE_STANDARD_C_ENUM_SIZE /* Portability Req.*/
}  vpAcRloopAdcOrderTypes;


VpStatusType
VpAcRloopArgCheck(
    VpLineCtxType *pLineCtx,
    const void *pArgsUntyped,
    uint16 handle,
    uint8 *pAdcState,
    VpTestIdType testId);

void
VpAcRloopOffsetSetup(
	Vp880LineObjectType *pLineObj,
    Vp880DeviceObjectType *pDevObj,
    Vp880TestHeapType *pTestHeap,
    VpDeviceIdType deviceId);

void VpAcRloopSetup(
    Vp880LineObjectType *pLineObj,
    Vp880TestHeapType *pTestHeap,
    VpDeviceIdType deviceId,
    int16 measuredBiasOffset);

static bool
VpAcRloopAdcSetup(
    VpLineCtxType *pLineCtx,
    Vp880DeviceObjectType *pDevObj,
    uint8 adcState,
    bool  axEnabled);

static int16
VpAcRloopOffsetGetResult(
    Vp880DeviceObjectType *pDevObj,
    VpTestTipSelectType lead);

static void
VpAcRloopGetResult(
    Vp880DeviceObjectType *pDevObj,
    int16 *pTestState,
    uint8 *pAdcState);

/*------------------------------------------------------------------------------
 * Vp880TestOpenVdc
 * This functions implements the AC RLOOP Test primitive ...
 *
 * Parameters:
 *  pLineCtx    - pointer to the line context
 *  pArgsUntyped- pointer to the test specific inputs.
 *  handle      - unique test handle
 *  callbak     - indicates if this function was called via a callback
 *
 * Returns:
 * --
 *----------------------------------------------------------------------------*/
EXTERN VpStatusType
Vp880TestAcRloop(
    VpLineCtxType *pLineCtx,
    const void *pArgsUntyped,
    uint16 handle,
    bool callback,
    VpTestIdType testId)

{
    VpDevCtxType *pDevCtx = pLineCtx->pDevCtx;
    Vp880DeviceObjectType *pDevObj = pDevCtx->pDevObj;
    Vp880LineObjectType *pLineObj =  pLineCtx->pLineObj;
    Vp880CurrentTestType *pTestInfo = &pDevObj->currentTest;
    Vp880TestHeapType *pTestHeap = pTestInfo->pTestHeap;
    VpDeviceIdType deviceId = pDevObj->deviceId;
    VpTestStatusType *pErrorCode = &pDevObj->testResults.errorCode;

    int16 *pTestState = &pTestInfo->testState;
    uint8 *pAdcState = &pTestHeap->adcState;
    uint8 chanId = pLineObj->channelId;
    uint8 switchReg[] = {VP880_YRING_TRACK_MASK, VP880_ZRING_TRACK_MASK};
    uint16 tick =  pDevObj->devProfileData.tickRate;
    uint16 measuredBiasOffset = 0;

    VP_TEST(VpLineCtxType, pLineCtx, ("Vp880TestAcRloop(ts:%i)", *pTestState));
    if (FALSE == callback) {
        return VpAcRloopArgCheck(pLineCtx, pArgsUntyped, handle, pAdcState, testId);
    }

    switch (*pTestState) {

        case COMMON_TEST_SETUP:
            /*
             * If this is a tracking device then we need to make sure the channel
             * is prepared for the test
             */
            if ( pTestHeap->testArgs.acRloop.calMode &&
                !(pDevObj->stateInt & VP880_IS_ABS)  &&
                !(pTestHeap->switchReg[0] & switchReg[chanId])) {

                uint32 swz = VP880_UNIT_CONVERT(VP880_AC_RLOOP_MAX_BAT_LVL,
                    VP880_UNIT_MV, VP880_UNIT_TRACK_SWZ);

                uint8 switchParam[VP880_REGULATOR_PARAM_LEN] = {0, 0, 0};
                /* ensure we are in fixed battery mode */
                switchParam[0] = pTestHeap->switchReg[0] | switchReg[chanId];
                /* ensure we have -25V floor for duration of the test */
                switchParam[1] = (uint8)((pTestHeap->switchReg[1] & ~(VP880_FLOOR_VOLTAGE_MASK)) | 0x04);
                /* force the ringing battery to be 75V */
                switchParam[2] = (0xE0 & pTestHeap->switchReg[2]) | (uint8)swz;

                VP_TEST(VpLineCtxType, pLineCtx, ("Vp880TestAcRloop() swy = 0x%02x%02x%02x , swz = 0x%lx",
                    switchParam[0], switchParam[1], switchParam[2], swz));

                VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_REGULATOR_PARAM_WRT,
                    VP880_REGULATOR_PARAM_LEN, switchParam);
                VpMemCpy(pDevObj->swParamsCache, switchParam, VP880_REGULATOR_PARAM_LEN);
            }
            Vp880CommonTestSetup(pLineCtx, deviceId);
            /* no break */

        case AC_RLOOP_OFFSET_SETUP:
            VpAcRloopOffsetSetup(pLineObj, pDevObj, pTestHeap, deviceId);
            VpSetTestStateAndTimer(pDevObj, pTestState, AC_RLOOP_OFFSET_ADC_SETUP,
                MS_TO_TICKRATE(VP880_STATE_CHANGE_SETTLING_TIME *2, tick) );
            break;


        case AC_RLOOP_OFFSET_ADC_SETUP: {
            VpTestTipSelectType lead = pTestHeap->testArgs.acRloop.tip;
             uint8 aToDRoute = VP880_METALLIC_DC_V;

            if (lead == VP_TEST_TIP) {
                aToDRoute = VP880_TIP_TO_GND_V;
            } else if (lead == VP_TEST_RING) {
                aToDRoute = VP880_RING_TO_GND_V;
            }

            if (Vp880AdcSetup(pLineCtx, aToDRoute, FALSE) ) {
                VpSetTestStateAndTimer(pDevObj, pTestState,
                    AC_RLOOP_OFFSET_COLLECT_DATA,
                    MS_TO_TICKRATE(VP880_ADC_CHANGE_SETTLING_TIME, tick));
            } else {
                *pErrorCode = VP_TEST_STATUS_INTERNAL_ERROR;
                VpSetTestStateAndTimer(pDevObj, pTestState, AC_RLOOP_QUIT, NEXT_TICK);
            }
            break;
        }

        case AC_RLOOP_OFFSET_COLLECT_DATA:
            VpStartPcmCollect(pLineCtx,
                (VP_PCM_OPERATION_AVERAGE | VP_PCM_OPERATION_MIN | VP_PCM_OPERATION_MAX),
                AC_RLOOP_SETUP, 0, 80);
            break;


        case AC_RLOOP_SETUP: {
            /* get the bias dc offset */
            measuredBiasOffset = VpAcRloopOffsetGetResult(pDevObj, pTestHeap->testArgs.acRloop.tip);

            /* adjust the sig gen bias */
            VpAcRloopSetup(pLineObj, pTestHeap, deviceId, measuredBiasOffset);
            VpSetTestStateAndTimer(pDevObj, pTestState, AC_RLOOP_ADC_SETUP,
                MS_TO_TICKRATE(VP880_STATE_CHANGE_SETTLING_TIME / 2, tick) );
            break;
        }

        case AC_RLOOP_ADC_SETUP:
            if ( TRUE == VpAcRloopAdcSetup(pLineCtx, pDevObj, *pAdcState,
                    pTestHeap->testArgs.acRloop.calMode) ) {

                VpSetTestStateAndTimer(pDevObj, pTestState,
                    AC_RLOOP_COLLECT_DATA,
                    MS_TO_TICKRATE(VP880_ADC_CHANGE_SETTLING_TIME, tick));
            } else {
                *pErrorCode = VP_TEST_STATUS_INTERNAL_ERROR;
                VpSetTestStateAndTimer(pDevObj, pTestState, AC_RLOOP_QUIT,
                    NEXT_TICK);
            }
            break;

        case AC_RLOOP_COLLECT_DATA: {
            VpTestAcRLoopType *pAcRloopData = &(pTestHeap->testArgs).acRloop;
            VpStartPcmCollect(pLineCtx,
                (VP_PCM_OPERATION_RMS | VP_PCM_OPERATION_FREQ),
                AC_RLOOP_GET_RESULT, 80, pAcRloopData->integrateTime);
            break;
        }

        case AC_RLOOP_GET_RESULT:
            VpAcRloopGetResult(pDevObj, pTestState, pAdcState);
            VpSetTestStateAndTimer(pDevObj, pTestState, *pTestState, NEXT_TICK);
            break;

        case AC_RLOOP_END:
            /*
             * If this is a tracking device then we need to make sure the channel is
             * placed back into the original tracking mode before this test was run
             */
            if ( pTestHeap->testArgs.acRloop.calMode &&
                !(pDevObj->stateInt & VP880_IS_ABS) ) {

                VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_REGULATOR_PARAM_WRT,
                    VP880_REGULATOR_PARAM_LEN, pTestHeap->switchReg);
                VpMemCpy(pDevObj->swParamsCache, pTestHeap->switchReg,
                    VP880_REGULATOR_PARAM_LEN);
            }

            /* The test has completed with no issues and generates an event. */
            *pErrorCode = VP_TEST_STATUS_SUCCESS;
            VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
            VP_TEST(VpLineCtxType, pLineCtx, ("Vp880TestAcRloop(): complete"));
            break;

        case AC_RLOOP_QUIT:
            /* The test has eneded early due to the current errorCode */
            VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
            VP_WARNING(VpLineCtxType, pLineCtx, ("Vp880TestAcRloop(ec:%i): quit", *pErrorCode));
            break;

        default:
            *pErrorCode = VP_TEST_STATUS_INTERNAL_ERROR;
            VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
            VP_ERROR(VpLineCtxType, pLineCtx, ("Vp880TestAcRloop(): invalid teststate"));
            break;
    }
    return VP_STATUS_SUCCESS;
}

/*------------------------------------------------------------------------------
 * VpAcRloopArgCheck
 * This functions is called by the Vp880TestAcRloop() function if
 * Vp880TestAcRloop() was not called via a callback.
 *
 * This function will check that no errorCode is pending and that the input
 * arguments for the AC Rloop primitive are legal. The function also sets up
 * the AC Rloop state machine for the first state and initializes the results
 * from this fucntion to 0.
 *
 * Parameters:
 *  pLineCtx        - pointer to the line context
 *  pArgsUntyped    - pointer to the struct which contains test specfic info.
 *  handle          - unique test handle
 *  pAdcState       - pointer to the value of the current ADC measurement state
 *
 * Returns:
 * VpStatusType
 *
 * Result:
 *  This function initializes all relevent result values to 0. Stores the
 *  TestId and handle into the device Object as well as modifes the current
 *  ADC state to the first measurement.
 *
 *----------------------------------------------------------------------------*/
VpStatusType
VpAcRloopArgCheck(
    VpLineCtxType *pLineCtx,
    const void *pArgsUntyped,
    uint16 handle,
    uint8 *pAdcState,
    VpTestIdType testId)
{
    VpDevCtxType *pDevCtx = pLineCtx->pDevCtx;
    Vp880LineObjectType *pLineObj =  pLineCtx->pLineObj;
    Vp880DeviceObjectType *pDevObj = pDevCtx->pDevObj;
    Vp880CurrentTestType *pTestInfo = &(pDevObj->currentTest);
    VpTestAcRLoopType *pTestInput = (VpTestAcRLoopType*)pArgsUntyped;
    VpTestAcRLoopType *pAcRloopData = &(pTestInfo->pTestHeap->testArgs).acRloop;
    int16 *pTestState = &pTestInfo->testState;

    if (VP_TEST_STATUS_SUCCESS != pDevObj->testResults.errorCode) {
        /* make sure that starting test did not generate any errorCodes */
        VpGenerateTestEvent(pLineCtx, testId, handle, FALSE, TRUE);
        VP_WARNING(VpLineCtxType, pLineCtx, ("VpAcRloopArgCheck bailed ec:%i",
            pDevObj->testResults.errorCode));
        return VP_STATUS_SUCCESS;
    }

    /*
     * Setup current Test members and test pTestInfo data.
     * This portion is called only one time once the primitive is started.
     */
    *pAdcState = AC_RLOOP_IMT;

    pDevObj->testResults.result.acimt.vab = 0;
    pDevObj->testResults.result.acimt.imt = 0;
    pDevObj->testResults.result.acimt.ilg = 0;

    pTestInfo->testId = testId;
    pTestInfo->handle = handle;

    /* This test cannot run if the isolate relay is open*/
    if (VP_RELAY_RESET == pLineObj->relayState) {
        pDevObj->testResults.errorCode = VP_TEST_STATUS_RESOURCE_NA;
        VpGenerateTestEvent(pLineCtx, testId, handle, TRUE, TRUE);
        VP_WARNING(VpLineCtxType, pLineCtx, ("Vp3EleResLGArgCheck bailed: openRelay"));
        return VP_STATUS_SUCCESS;
    }

    /* TODO: check input arguments before assigining them to test struct!!! */
    /*
     * We are using the calMode member to allow the calling code to
     * indicate whether or not to use high gain mode when setting up the ADC
     * and applying the ring signal. The measured result is subsequently halved
     * before it is returned to the user.
     */
    pAcRloopData->calMode = pTestInput->calMode;
    pAcRloopData->integrateTime = pTestInput->integrateTime;
    pAcRloopData->settlingTime = pTestInput->settlingTime;
    pAcRloopData->freq = pTestInput->freq;
    pAcRloopData->vTestLevel = pTestInput->vTestLevel;
    pAcRloopData->vBias = pTestInput->vBias;
    pAcRloopData->tip = pTestInput->tip;
    if (pAcRloopData->tip > VP_TEST_TIP_RING) {
        pAcRloopData->tip = VP_TEST_TIP_RING;
    }

    /*
     * force the function to start running state maching on next tick only if
     * all arguments checkout ok and no events were generated.
     */
    VpSetTestStateAndTimer(pDevObj, pTestState, VP880_TESTLINE_GLB_STRT_STATE, NEXT_TICK);
    return VP_STATUS_SUCCESS;
}



/*------------------------------------------------------------------------------
 * VpAcRloopOffsetSetup
 * This function is called by the Vp880TestAcRloop() state machine during
 * the AC_RLOOP_SETUP state.
 *
 * This function is used to configure the line for the ac rloop test.
 *
 * Parameters:
 *  pDevObj         - pointer to the device object.
 *  deviceId        - device ID
 *  ecVAl           - current enable channel value.
 *
 * Returns:
 * --
 * Result:
 *  Signal Generator A will be setup and slic will be put into
 *  balanced ringing. The loop super vision ringtrip threshold and current
 *  will be lowered in order not to cause a ring tirp during the test.
 *----------------------------------------------------------------------------*/
 void
VpAcRloopOffsetSetup(
    Vp880LineObjectType *pLineObj,
    Vp880DeviceObjectType *pDevObj,
    Vp880TestHeapType *pTestHeap,
    VpDeviceIdType deviceId)
{
    VpTestAcRLoopType *pAcRloopData = &pTestHeap->testArgs.acRloop;
    uint8 slacState;
    int32 freq = 0;
    int32 freqAry[] = {0x0E39, 0x071C, 0x038E, 0x01C7, 0x00E4};
    int16 bias = pAcRloopData->vBias;
    uint8 lpSuper[VP880_LOOP_SUP_LEN] = {0x1B, 0x84, 0x10, 0x00};
    uint8 dcfeed[VP880_DC_FEED_LEN] = {0x12, 0x08};
    uint8 sigGenAB[VP880_SIGA_PARAMS_LEN]
        = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    uint8 byte1, byte2;

    /* convert voltage bias into sig gen bias */
    if (pAcRloopData->tip != VP_TEST_TIP_RING) {
        bias = (int16)VP880_UNIT_CONVERT(VP880_AC_RLOOP_MAX_TEST_LVL,
                VP880_UNIT_MV, VP880_UNIT_DAC_RING);
    } else {
        bias = (int16)VP880_UNIT_CONVERT(bias, VP880_UNIT_MV, VP880_UNIT_DAC_RING);
    }
    VP_TEST(None, NULL, ("VpAcRloopOffsetSetup(bias)=0x%4x", bias));
    byte1 = (uint8)((bias >> 8) & 0xFF);
    byte2 = (uint8)(bias & 0x00FF);
    sigGenAB[1] = byte1;
    sigGenAB[2] = byte2;

    /*
     * determine a freq that gives once cycle in the test buffer (6 samples)
     * using the current tx buffer data rate
     */
    freq = freqAry[((pDevObj->txBufferDataRate & 0x70) >> 4)];
    sigGenAB[3] = (uint8)((freq >> 8) & 0xFF);
    sigGenAB[4] = (uint8)(freq & 0x00FF);

    /*
     * load a very small amplitude into siggen
     * cannot use 0 because it causes issues with the
     * ring enter/exit algorithim
     */
    sigGenAB[5] = 0x00;
/*    sigGenAB[6] = 0x6A; */
    sigGenAB[6] = 0x02;


    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_SIGA_PARAMS_WRT, VP880_SIGA_PARAMS_LEN,
        sigGenAB);

    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_LOOP_SUP_WRT, VP880_LOOP_SUP_LEN, lpSuper);

    /* Make sure we dont overwrite VAS */
    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_DC_FEED_RD, VP880_DC_FEED_LEN, dcfeed);
    dcfeed[0] = (dcfeed[0] & 0x03) | 0x12;
    dcfeed[0] = (dcfeed[1] & 0xC0) | 0x08;
    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_DC_FEED_WRT, VP880_DC_FEED_LEN, dcfeed);

    if (VP_TEST_TIP_RING == pAcRloopData->tip) {
        slacState = VP880_SS_ACTIVATE_MASK | VP880_SS_BALANCED_RINGING;
    } else if (VP_TEST_TIP == pAcRloopData->tip) {
        slacState = VP880_SS_ACTIVATE_MASK | VP880_SS_UNBALANCED_RINGING_PR;
        pLineObj->icr3Values[0] = 0x24;
        pLineObj->icr3Values[1] = 0x20;
        pLineObj->icr3Values[2] = 0x00;
        pLineObj->icr3Values[3] = 0x00;
        VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_ICR3_WRT, VP880_ICR3_LEN, pLineObj->icr3Values);
    } else {
        slacState = VP880_SS_ACTIVATE_MASK | VP880_SS_UNBALANCED_RINGING;
    }

    /* enable ringing */
    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_SYS_STATE_WRT,
        VP880_SYS_STATE_LEN, &slacState);
    pLineObj->slicValueCache = slacState;

    return;
}

void VpAcRloopSetup(
    Vp880LineObjectType *pLineObj,
    Vp880TestHeapType *pTestHeap,
    VpDeviceIdType deviceId,
    int16 measuredBiasOffset)
{
    VpTestAcRLoopType *pAcRloopData = &pTestHeap->testArgs.acRloop;
    uint16 freq = pAcRloopData->freq;
    int16 voltage = pAcRloopData->vTestLevel;
    int16 bias = pAcRloopData->vBias;
    uint8 sigGenAB[VP880_SIGA_PARAMS_LEN]
        = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    uint8 byte1, byte2;

    /* read siggen A */
    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_SIGA_PARAMS_RD, VP880_SIGA_PARAMS_LEN,
        sigGenAB);

    /* convert voltage bias into sig gen bias */
    if (pAcRloopData->tip != VP_TEST_TIP_RING) {
        bias = (int16)VP880_UNIT_CONVERT(VP880_AC_RLOOP_MAX_TEST_LVL,
                VP880_UNIT_MV, VP880_UNIT_DAC_RING);
    } else {
        bias = (int16)VP880_UNIT_CONVERT(bias, VP880_UNIT_MV, VP880_UNIT_DAC_RING);
    }

    /* make the adjustment to the requested bias */
    VP_TEST(None, NULL, ("measuredBiasOffset %i", measuredBiasOffset));
    VP_TEST(None, NULL, ("bias before %i", bias));
    if (measuredBiasOffset != 0) {
        bias -= (measuredBiasOffset - bias);
    }
    VP_TEST(None, NULL, ("bias after %i", bias));


    byte1 = (uint8)((bias >> 8) & 0xFF);
    byte2 = (uint8)(bias & 0x00FF);
    sigGenAB[1] = byte1;
    sigGenAB[2] = byte2;

    /* load freq into siggen */
    freq = (uint16)(((10000ul * freq) / 3662ul) + 1);
    byte1 = (uint8)((freq >> 8) & 0xFF);
    byte2 = (uint8)(freq & 0x00FF);
    sigGenAB[3] = byte1;
    sigGenAB[4] = byte2;

    /* load amplitude into siggen */
    byte1 = (uint8)((voltage >> 8) & 0xFF);
    byte2 = (uint8)(voltage & 0x00FF);
    sigGenAB[5] = byte1;
    sigGenAB[6] = byte2;

    /* send the new parameters */
    VpMpiCmdWrapper(deviceId, pLineObj->ecVal, VP880_SIGA_PARAMS_WRT, VP880_SIGA_PARAMS_LEN,
        sigGenAB);
    return;
}

/*------------------------------------------------------------------------------
 * VpAcRloopAdcSetup
 * This function is called by the Vp880TestAcRloop() state machine during
 * the AC_RLOOP_ADC_SETUP state.
 *
 * This function is used to call the Vp880AdcSetup() depending on the
 * current ac rloop adc state.
 *
 * Parameters:
 *  pLineCtx        - pointer to the line context.
 *  pDevObj         - pointer to the device object.
 *  adcState        - current value of the adc setup state.
 *
 * Returns:
 *  FALSE if something goes wrong during the adc setup.
 *  TRUE if everything goes ok.
 *
 * Result:
 *  The ADC routing is changed
 *----------------------------------------------------------------------------*/
static bool
VpAcRloopAdcSetup(
    VpLineCtxType *pLineCtx,
    Vp880DeviceObjectType *pDevObj,
    uint8 adcState,
    bool  axEnabled)
{
    bool adcFlag;
    switch (adcState) {
        case AC_RLOOP_VAB:
            adcFlag = Vp880AdcSetup(pLineCtx, VP880_METALLIC_DC_V, axEnabled);
            break;
        case AC_RLOOP_IMT:
            adcFlag = Vp880AdcSetup(pLineCtx, VP880_METALLIC_DC_I, axEnabled);
            break;
        case AC_RLOOP_ILG:
            adcFlag = Vp880AdcSetup(pLineCtx, VP880_LONGITUDINAL_DC_I, axEnabled);
            break;
        default:
            return FALSE;
    }
    return adcFlag;
}


static int16
VpAcRloopOffsetGetResult(
    Vp880DeviceObjectType *pDevObj,
    VpTestTipSelectType lead)
{
    uint8 aToDRoute = VP880_METALLIC_DC_V;
    int16 min = 0;
    int16 max = 0;
    int16 offset = 0;

    if (lead == VP_TEST_TIP) {
        aToDRoute = VP880_TIP_TO_GND_V;
    } else if (lead == VP_TEST_RING) {
        aToDRoute = VP880_RING_TO_GND_V;
    }

    VpGetPcmResult(pDevObj, VP_PCM_OPERATION_AVERAGE, FALSE, FALSE, 0, aToDRoute, &offset);

    VpGetPcmResult(pDevObj, VP_PCM_OPERATION_MIN, FALSE, FALSE, 0, aToDRoute, &min);

    VpGetPcmResult(pDevObj, VP_PCM_OPERATION_MAX, FALSE, FALSE, 0, aToDRoute, &max);

    /*
     * check if average is within a certain min max criteria.
     * If its not then just return no offset.
     */
    VP_TEST(None, NULL, ("VpAcRloopOffsetGetResult(bias-offset:%i)", offset));

    /* flip it */
    if (lead != VP_TEST_TIP_RING) {
        offset *= -1;
    }

    /* convert to DAC units */
    return (int16)VP880_UNIT_CONVERT(offset, VP880_UNIT_ADC_VAB, VP880_UNIT_DAC_RING);
}

/*------------------------------------------------------------------------------
 * VpAcRloopGetResult
 * This function is called by the Vp880TestAcRloop() state machine during
 * the AC_RLOOP_GET_RESULT state.
 *
 * This function is used to collect the result form the PCM integrator. And load
 * the result into the results structure.
 *
 * Parameters:
 *  pDevObj         - pointer to the device object.
 *  pTestState      - pointer to the device current test state.
 *  pErrorCode      - pointer to the device results errorCode member
 *  pAdcState       - pointer to the value of the current ADC measurement state
 *
 * Returns:
 *  --
 *
 * Result:
 *  Two possible results of running this function are:
 *  1) not all adc connections have been made so advance the pAdcState and set
 *     the testState to AC_RLOOP_AD_SETUP.
 *  3) all adc connections have been made jump to AC_RLOOP_END.
 *----------------------------------------------------------------------------*/
static void
VpAcRloopGetResult(
    Vp880DeviceObjectType *pDevObj,
    int16 *pTestState,
    uint8 *pAdcState)
{
    switch (*pAdcState) {
        case AC_RLOOP_IMT:
            VpGetPcmResult(pDevObj, VP_PCM_OPERATION_RMS, FALSE, FALSE, 0,
                VP880_METALLIC_DC_I, &pDevObj->testResults.result.acimt.imt);
            VP_TEST(None, NULL, ("VpAcRloopGetResult(imt:%i)", \
                pDevObj->testResults.result.acimt.imt));
            break;
        case AC_RLOOP_ILG:
            VpGetPcmResult(pDevObj, VP_PCM_OPERATION_RMS, FALSE, FALSE, 0,
                VP880_LONGITUDINAL_DC_I, &pDevObj->testResults.result.acimt.ilg);
            VP_TEST(None, NULL, ("VpAcRloopGetResult(ilg:%i)", \
                pDevObj->testResults.result.acimt.ilg));
            break;
        default: /* AC_RLOOP_VAB */
            VpGetPcmResult(pDevObj, VP_PCM_OPERATION_RMS, FALSE, FALSE, 0,
                VP880_METALLIC_DC_V, &pDevObj->testResults.result.acimt.vab);
            VP_TEST(None, NULL, ("VpAcRloopGetResult(vab:%i)", \
                pDevObj->testResults.result.acimt.vab));

            VpGetPcmResult(pDevObj, VP_PCM_OPERATION_FREQ, FALSE, FALSE, 0,
                VP880_METALLIC_DC_V, &pDevObj->testResults.result.acimt.freq);
            VP_TEST(None, NULL, ("VpAcRloopGetResult(freq:%li)", \
                pDevObj->testResults.result.acimt.freq));

            break;
    }

    /* move to the next ADC measurment state */
    if (AC_RLOOP_MAX_ADC > ++(*pAdcState)) {
        *pTestState = AC_RLOOP_ADC_SETUP;
    } else {
        *pTestState = AC_RLOOP_END;
    }
    return;
}

#endif /* VP880_INCLUDE_TESTLINE_CODE */

#endif /* VP_CC_880_SERIES */




