...
static int  DAA_Check_Line_State_mirror_daa( struct voip_snd_s *this )	/* 0: connect, 1: not connect, 2: busy*/
{
	...
	DECLARE_DAA_RPC_VARS( DAA_Check_Line_State, 0 );
...
{
#if MIRROR_DAA_TYPE == 1
	DECLARE_DAA_RPC_VARS( DAA_On_Hook, 0 );
	printk("Daa onhook\n");
	*( ( unsigned int * )p_data ) = *( ( unsigned int * )p_data );	// avoid compiler warning 
...
{
#if MIRROR_DAA_TYPE == 1
	DECLARE_DAA_RPC_VARS( DAA_Off_Hook, 0 );
	printk("Daa offhook\n");
	*( ( unsigned int * )p_data ) = *( ( unsigned int * )p_data );	// avoid compiler warning 
...
{
#if 0
	DECLARE_DAA_RPC_VARS( DAA_Set_PulseDial, sizeof( directly ) );
	
	*( ( unsigned int * )p_data ) = directly;
