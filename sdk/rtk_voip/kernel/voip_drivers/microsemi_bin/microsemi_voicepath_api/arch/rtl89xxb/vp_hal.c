...
#include "mpi_hal.h"
#include "spi.h"
#if defined(CONFIG_RTK_VOIP_DRIVERS_SLIC_ZSI) //|| defined(CONFIG_RTK_VOIP_DRIVERS_SLIC_LE88601)
#include "linux/delay.h"
/* In ZSI mode, there must be a delay between bytes.  The length of this delay
   is determined by the SPI and PCM clock rates.
        delay1 = (8000000/pcmClk_kHz - 7000000/spiClk_kHz) nanoseconds
        delay2 = (11000000/pcmClk_kHz) + 122 nanoseconds
        delay = MAX(delay1, delay2)
   With a PCLK of 8192kHz, the second delay requirement will usually be larger,
   giving us a minimum delay of 1465ns.
   Rounding up to 2000ns to be safer. */
//#define ZSI_BYTE_DELAY_NS 2000
#define ZSI_BYTE_DELAY_NS 6000
#endif
...
extern rtl_spi_dev_t rtlSpi[];

#define CONFIG_RTK_VOIP_DRIVERS_SLIC_ZARLINK_ON_NEW_ARCH
...
#else
		vp_write(&rtlSpi[deviceId], buf);
		vp_write(&rtlSpi[deviceId], ecVal);
...
#else
	vp_write(&rtlSpi[deviceId], cmd);
#endif

    if (isRead)
    {
		#if defined(CONFIG_RTK_VOIP_DRIVERS_SLIC_ZSI)// || defined(CONFIG_RTK_VOIP_DRIVERS_SLIC_LE88601)
         /* ZSI interface implementation -
          * In order to perform ZSI read operations, the host must issue the
          * read command followed by one extra NOP command before reading
          * the rest of the data. The first byte returned during this NOP should
          * be discarded, as it contains no useful data */
		 ecVal = NO_OP;
		 #ifdef CONFIG_RTK_VOIP_DRIVERS_SLIC_ZARLINK_ON_NEW_ARCH
		 vp_write( ( rtl_spi_dev_t * )deviceId, &ecVal);
		 #else
...
#else
			vp_write(&rtlSpi[deviceId], dataPtr[byteCnt]);
#endif
        }
    }
...
#if 0
    printk("(rtl_spi_dev_t *) deviceId: %p \n", deviceId);
    printk("calling _rtl_spi_rawWrite with ( rtl_spi_dev_t * )deviceId %p\n", (( rtl_spi_dev_t * )deviceId));
    printk("calling _rtl_spi_rawWrite with (( rtl_spi_dev_t * )deviceId)->type %d\n", (( rtl_spi_dev_t * )deviceId)->type);
    printk("calling _rtl_spi_rawWrite with (( rtl_spi_dev_t * )deviceId)->spiCS_ %d\n", (( rtl_spi_dev_t * )deviceId)->hw.spiCS_);
   

    printk("calling _rtl_spi_rawWrite with buf %p\n", buf);
#endif

	_rtl_spi_rawWrite( ( rtl_spi_dev_t * )deviceId, &buf, 8);
#else
	_rtl_spi_rawWrite(&rtlSpi[deviceId], &buf, 8);
#endif
//#endif
...
#else
	_rtl_spi_rawWrite(&rtlSpi[deviceId], &ecVal, 8);
#endif
    }
...
#else
    _rtl_spi_rawWrite(&rtlSpi[deviceId], &cmd, 8);
#endif
...
#else
		_rtl_spi_rawRead(&rtlSpi[deviceId], &dataPtr[byteCnt], 8);
#endif
...
#else
           _rtl_spi_rawWrite(&rtlSpi[deviceId], &dataPtr[byteCnt], 8);
#endif
        }
    }
