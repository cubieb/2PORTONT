...
static int Polling_Mirror_SND_con( voip_con_t *p_con, voip_snd_t *p_snd )
{
	...
	case SND_TYPE_FXS:
		...
		if( (p_mirror_data ->udata.slic.hook_status == 0 ) && (MultiRingCadenceEnableCheck_con(p_con) == 1) && (MultiRingStatusCheck_con(p_con) == 1) && ( MultiRingHookStateCheck_con(p_con) == 2 ) ){
			p_snd ->fxs_ops ->FXS_Ring(p_snd, 0);
		}
		else if((p_mirror_data ->udata.slic.hook_status == 1 ) && (MultiRingCadenceEnableCheck_con(p_con) == 1) && (MultiRingStatusCheck_con(p_con) == 1) && ( MultiRingHookStateCheck_con(p_con) == 1 )){
			MultiRingSetHookState_con(p_con, 2);		
		}
