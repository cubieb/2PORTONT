...
static int voip_iis_read_proc(struct seq_file *f, void *v)
{
	int ch;//, ss;
	int n = 0;
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int iis_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_iis_read_proc, NULL);
}

struct file_operations proc_iis_fops = {
	.owner	= THIS_MODULE,
	.open	= iis_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_iis_read_proc
};
#endif
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_channel_proc_read_entry( "iis", &proc_iis_fops );
#else
