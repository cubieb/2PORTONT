...
unsigned int offhook_min_time[CON_CH_NUM]; /* offhook minimum detection threshold*/
static unsigned int on_cnt[CON_CH_NUM], off_cnt[CON_CH_NUM];
...
* off_cnt: count how many time "OFF-HOOK" event happens.
		...
		unsigned int flash_hook_min_duration, unsigned int flash_hook_duration )
{
	...
	if (status)
	{
		...
		if (check_flash[ cch ] == 1)
		{
			if (on_cnt[ cch ] <= flash_hook_min_duration)
			{
				if (!(off_set[ cch ]))
				{
					if ( off_cnt[ cch ] >= offhook_min_time[cch] ){
						hook->hook_status = PHONE_OFF_HOOK;
						off_set[ cch ] = 1;
						on_set[ cch ] = 0;
						//printk("*** OFF %d, off_set[%d]=%d***\n", __LINE__, cch, off_set[cch]);
					}else{
						hook->hook_status = PHONE_STILL_ON_HOOK;
						off_set[ cch ] ++;
						//printk("*** OFF %d, on_cnt %d, off_cnt %d***\n", __LINE__,on_cnt[cch],off_cnt[cch]);
					}
				}
				else
				{
					hook->hook_status = PHONE_STILL_ON_HOOK;
					//printk("*** S-ON 1, on_cnt %d, off_cnt %d***\n",on_cnt[cch],off_cnt[cch]);
				}
		...
		else
		{
			if (off_cnt[cch] >= offhook_min_time[cch] ){
				...
				off_cnt[cch] ++;
				//printk("*** S-OFF 1, on_cnt %d, off_cnt %d***\n",on_cnt[cch],off_cnt[cch]);				
			}else{
				hook->hook_status = PHONE_STILL_ON_HOOK;
				off_cnt[cch] ++;				
				//printk("*** OFF %d, on_cnt %d, off_cnt %d***\n", __LINE__,on_cnt[cch],off_cnt[cch]);
	...
	else
	{
		...
		{
			hook->hook_status = PHONE_STILL_OFF_HOOK;
			//printk("*** S-OFF 2, on_cnt %d, off_cnt %d***\n",on_cnt[cch],off_cnt[cch]);
		}
						...
						voip_snd_t * const p_snd )
{
	...
	extern int RFC2833TrunkEventOut(int chid, int status);
#endif	
	const uint32 cch = p_con ->cch;
...
#ifdef RFC2833TRUNKEVENT			
			// send RFC2833 event
			RFC2833TrunkEventOut(cch, PHONE_ON_HOOK);
#endif			
		}
		//printk("hook_in: ON\n");
	}
...
#ifdef RFC2833TRUNKEVENT			
			// send RFC2833 event			
			RFC2833TrunkEventOut(cch, PHONE_OFF_HOOK);
#endif
	...
	//MultiRingCadenceProcess_con(p_con);	//move to host
...
int voip_hook_read_proc(struct seq_file *f, void *v)
{
	int ch;
	int n = 0;
	...
	if( IS_CH_PROC_DATA( (long)f->private ) )
	{
		...
		seq_printf( f, "Flash Hook Max.: %d ms\n", 10*flash_hook_time[ch]);
		seq_printf( f, "Flash Hook Min.: %d ms\n", 10*flash_hook_min_time[ch]);
...
#ifdef PULSE_DIAL_DET
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
int voip_pulse_dial_read_proc(struct seq_file *f, void *v)
{
	int ch;
	int n = 0;
	...
	if( IS_CH_PROC_DATA( (long)f->private ) )
	{
		...
		seq_printf( f, "Pulse Dial Det Pause Time: %d ms\n", 10*pulse_dial_pause_time[ch]);
		seq_printf( f, "Pulse Dial Det Max. Break Time: %d ms\n", 10*break_max_ths[ch]);
		seq_printf( f, "Pulse Dial Det Min. Break Time: %d ms\n", 10*break_min_ths[ch]);
...
{
	return single_open(file, voip_hook_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_hook_fops = {
	.owner	= THIS_MODULE,
	.open	= hook_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_hook_read_proc
};

static int pulse_dial_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_pulse_dial_read_proc, PDE_DATA(file_inode(file)));
}

struct file_operations proc_pulse_dial_fops = {
	.owner	= THIS_MODULE,
	.open	= pulse_dial_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,
//read:   voip_pulse_dial_read_proc
};
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	create_voip_channel_proc_read_entry( "hook", &proc_hook_fops );
#ifdef PULSE_DIAL_DET
	create_voip_channel_proc_read_entry( "pulse_dial", &proc_pulse_dial_fops );
#endif
