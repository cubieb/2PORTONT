...
static void leds_blinking_timer_func( void *data )
{
	...
	while( p_ioc ) {
		...
		if( p_ioc ->ioc_type == IOC_TYPE_LED &&
			( p_ioc ->mode_var == IOC_MODE_BLINKING || p_ioc ->mode_var == IOC_MODE_BLINKING_1HZ || p_ioc ->mode_var == IOC_MODE_BLINKING_4HZ )
			)
		{
			blinking_count ++;
			timercount ++;
		
			if ( p_ioc->mode_var == IOC_MODE_BLINKING_1HZ ){
				if ( (timercount % 20) != 0 )
					break;
			}else if ( p_ioc->mode_var == IOC_MODE_BLINKING_4HZ ){
				if ( (timercount % 5) != 0 )
					break;			
			}			
