...
static int voip_host_cch_invert_table_read_proc(struct seq_file *f, void *v)
{
	...
	seq_printf( f, "index dsp_cpuid dsp_cch host_cch\n" );
	seq_printf( f, "----- --------- ------- --------\n" );
	
	for( i = 0; i < CON_CH_NUM; i ++ ) {
		seq_printf( f, "  %2d  " "    %2d    " "   %2d   " "   %2d   " "\n",
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	int i = 0;
	printk("index dsp_cpuid dsp_cch host_cch\n" );
	printk("----- --------- ------- --------\n" );
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int host_cch_invert_table_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_host_cch_invert_table_read_proc, NULL);
}

struct file_operations proc_host_cch_invert_table_fops = {
	.owner	= THIS_MODULE,
	.open	= host_cch_invert_table_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
...
{
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/invhostcch", 0, NULL, 
		&proc_host_cch_invert_table_fops, NULL );	
#else
