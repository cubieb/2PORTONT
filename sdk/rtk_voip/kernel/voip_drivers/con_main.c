...
static int voip_con_main_read_proc(struct seq_file *f, void *v)
{
...
#define S_PTR( ptr, fld )	( ptr ? ptr ->fld : NULL )
	...
	seq_printf( f, "cch E B dsp sch T name	  bch name	  bch2 name    dch name 	\n" );
	seq_printf( f, "--- - - --- ------------- ----------- ------------ ------------\n" );
	...
	for( i = 0; i < CON_CH_NUM; i ++ ) {
		...
		seq_printf( f, "%3d %d %X %u%c%u %3d %d %-7s %3d %-7s %4d %-7s %3d %-7s\n",
				S_VAL( p_con, cch ),
				S_VAL( p_con, enabled ),
	...
	seq_printf( f, "cch LED0		 LED1		  Relay \n" );
	seq_printf( f, "--- ----=------- ----=------- ----=------- \n" );
	...
	for( i = 0; i < CON_CH_NUM; i ++ ) {
		...
		seq_printf( f, "%3d " "%4X %-7s " "%4X %-7s " "%4X %-7s " "\n",
				S_VAL( p_con, cch ),
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int voip_global_num_read_proc(struct seq_file *f, void *v)
{
	...
	extern int add_dsp_rtk_global_num_string(struct seq_file *f);
	//extern int add_dsp_ac_global_num_string( char *buff );
	

		
	seq_printf( f, "con_ch_num = %d\n", con_ch_num );
	seq_printf( f, "dsp_ch_num = %d\n", dsp_ch_num );
	seq_printf( f, "dsp_ss_num = %d\n", dsp_ss_num );

	add_dsp_rtk_global_num_string( f );
	return 0;
}
#else
...
#if  defined(CONFIG_DEFAULTS_KERNEL_3_10)
static int con_main_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_con_main_read_proc, NULL);
}

struct file_operations proc_con_main_fops = {
	.owner	= THIS_MODULE,
	.open	= con_main_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
//read:   voip_con_main_read_proc
};

static int global_number_open(struct inode *inode, struct file *file)
{
	return single_open(file, voip_global_num_read_proc, NULL);
}

struct file_operations proc_global_number_fops = {
	.owner	= THIS_MODULE,
	.open	= global_number_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
	.release = single_release,	
//read:   voip_global_num_read_proc
};
...
static int __init voip_con_main_proc_init( void )
{
	...
	proc_create_data( PROC_VOIP_DIR "/" PROC_VOIP_CH_MAPS_DIR "/bind", 0, NULL, 
		&proc_con_main_fops, NULL );

	proc_create_data( PROC_VOIP_DIR "/global_num", 0, NULL, &proc_global_number_fops, NULL );
#else
