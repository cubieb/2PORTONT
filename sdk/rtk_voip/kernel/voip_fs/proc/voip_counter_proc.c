...
static int voip_counter_read(struct seq_file *f, void *v)
{
	int chid, sid;
	int n;
...
#endif
		seq_printf( f, "  *RTP Counter:\n");
		for (sid = 0; sid < DSP_SS_NUM; sid++)
		{
			seq_printf( f, "    - sid%d rx: %d \n", sid, rtp_rx_count[sid]);
			seq_printf( f, "    - sid%d tx: %d \n", sid, rtp_tx_count[sid]);
		}
		
		seq_printf( f, "  *DSP Counter:\n");
		for (sid = 0; sid < DSP_SS_NUM; sid++)
		{
			seq_printf( f, "    - sid%d dsp process: %d \n", sid, dsp_process_count[sid]);
		}
		
		for (sid = 0; sid < DSP_SS_NUM; sid++)
		{
			seq_printf( f, "    - sid%d tone gen: %d \n", sid, tone_gen_count[sid]);
		}
	}
...
{
	return single_open(file, voip_counter_read, NULL);
}

static ssize_t counter_write(struct file *file, const char __user * userbuf, size_t count, loff_t * off) {
        return voip_counter_write(file, userbuf, count, NULL);
}

const struct file_operations proc_counter_fops = {
	.owner	= THIS_MODULE,
	.open	= counter_open,
	.read	= seq_read,
	.llseek	= seq_lseek,
