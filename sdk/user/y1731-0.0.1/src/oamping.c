...
/**
 * Where all the needed RTT info is stored 
 */
struct oam_ep2 *ep2;

/**
 * How many 1000s we have
 */
int rotations_1000;
...
{
    fprintf(stderr, "\nUsage: oamping [options]\n\n"
            "  --mac / -m [MAC], \n"
            "       MAC of the target.\n"
            "  --mel / -l [MEG level], \n"
            "       MEG level of the target.\n"
            "  --count / -c [number of], \n"
            "       Number of LBMs are we sending.\n"
            "  --interval / -i [number of] \n"
            "       The sending interval in usec. (e.g. 1 000 000 usec = 1 s)"
 ...
 */
static int oam_handle_lbr_sa(const struct oam_pdu_common * common_pdu,
                                      const struct oam_recv_information *recv_info)
{
    ...
    /* Skip past the header to the Transaction ID */
    ptr += 4; 
    memcpy(&received_tid, ptr, sizeof(uint32_t));
    received_tid = ntohl(received_tid);
    OAM_DEBUG("Received TID = %d, TID %d\n", received_tid, tid);
    if (received_tid != tid) {
        OAM_DEBUG("Received Transaction ID %u, expected\n", received_tid, tid);
    }
    received++;

    /* internal time */
    rtt = oam_internal_time_diff(&sent_internal_time, &recv_info->sw_time) / 
        (int64_t)OAM_USECS_TO_NANO;
    oam_ep2_add_value(rtt, ep2); 

#ifdef TEST_FEATURE
    /* print every thousandth struct */
    if (ep2->num_of_observations >= 1000 + ep2->num_of_markers) {
        oam_ep2_print_serial(rotations_1000, ep2);
        ep2->num_of_observations = ep2->num_of_markers;
        rotations_1000++;
    }
#endif

    oam_internal_time_str(buf, sizeof(buf), &sent_internal_time);
    OAM_XTRA("LB SENT Timestamp %s\n", buf);
    oam_internal_time_str(buf, sizeof(buf), 
                          (struct oam_internal_time *)&recv_info->sw_time);
    OAM_XTRA("LB RECV Timestamp %s\n", buf);
    OAM_XTRA("LB RTT %d usecs (%d/%d)\n", rtt, received, count); 
                                ...
                                fd_set *fdset)
{
    ...
    if (FD_ISSET(fd, fdset)) {
        ...
        /* stand alone pinger is not a MIP so OFF and the socket to 0 */
        ...
        if (checked) {  
            ...
            switch (opcode) {   
            case OAM_OPCODE_LBM:
                /* This is reserved for the old style pinging */
                break;
            case OAM_OPCODE_LBR:
                err = oam_handle_lbr_sa(pdu_in, &recv_info);
                break;
            default:
                err = 0;
                break;
...
int main (int argc, char **argv) {
    ...
    //struct oam_internal_time    previous;
    ...
    opterr = 0;   
    while ((c = getopt_long_only(argc, argv, "l:c:m:i:f:", 
                                 longopts, NULL)) != -1) {
        switch (c) {
        case 'l':
            /* meg level */
    ...
    OAM_DEBUG("\n\nTarget MAC %s,\n"
              ...
              "count %d,\n"
              "interval %d usec\n\n", 
              presentation_mac, mel, count, interval);

    oam_daemon_state = OAM_RUNNING;
    i = 0;

    if(oam_ep2_allocate(m, (int)sizeof(double), &ep2) != 0) {
        printf("Something wrong in allocation\n");
    } 
    oam_ep2_set_quantiles(ep2, m, 0.25, 0.5, 0.75);
    ...
    while (oam_daemon_state != OAM_STOPPED) {
        ...
        if (i < count) {
            if (oam_internal_time_expired(&timer_stamp) == 0 ) {
                ...
                i++;
                //timer = (int64_t)interval * (int64_t)OAM_USECS_TO_NANO;
                oam_get_internal_time(&timer_stamp);
                oam_add_usecs_to_internal_time((int64_t)interval, &timer_stamp);
            }
        }
        // check if the received is the same or more as the to be send
        if (count <= received) {
