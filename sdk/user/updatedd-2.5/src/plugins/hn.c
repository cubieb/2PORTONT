...
print_version(FILE *fp)
{
	...
	(void)fprintf(fp,
		      "\n" PNAME " plugin for hn.org version " VERSION ",\n"
...
update_dyndns(int s, struct arguments *args)
{
...

	{
		char buffer[1024];
                ...
                b64user = (char *)malloc((2 * strlen(args->login) + 1) * sizeof(char));
                if(b64user == NULL) {
...
check_server_msg(int s, char *hostname)
{
	...
	if(strstr(server_msg, "HTTP/1.1 200 OK") ||
	   strstr(server_msg, "HTTP/1.0 200 OK")) {
		...
		ret_msg(NONE, "hn.org: wrong username or password");
	} else {
		ret_msg(NONE, "hn.org: Internal Server Error");
	}
