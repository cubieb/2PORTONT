...
int g_cs_bak_offset= -1;  // "cs-bak" offset in flash, CURRENT_SETTING_BAKUP_OFFSET
...
#if defined(COMPRESS_CURRENT_SETTING)
	//if ( offset == CURRENT_SETTING_OFFSET ) {//compress current setting
	if ( offset == g_cs_offset || offset == g_cs_bak_offset ) {//compress current setting
...
int __mib_file_write(CONFIG_DATA_T data_type, unsigned char* ptr, int len)
{
	...
	switch(data_type)
	{
		case CURRENT_SETTING:
			...
			ret = __mib_flash_write(ptr, g_cs_offset, len);
			return __mib_flash_write(ptr, g_cs_bak_offset, len) && ret;
...
static void init_mib_global_parm()
{
	...
	g_cs_bak_offset = get_mtd_start("cs-bak");
	if (g_cs_bak_offset == -1) {
		printf("Error! Cannot get \"cs-bak\" offset.\n");
		while (1);
	}
