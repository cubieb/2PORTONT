...
void usl_fd_poll(void)
{
	...
	if (usl_list_empty(&usl_fd_active_list)) {
		...
		num = select(usl_fd_count+1, &r_poll, NULL, NULL, &tv);
		...
		if (num <= 0) {
			...
			if (errno == EBADF) {
				//printf("%s invalid file descriptor was given.\n", __func__);
				usl_fd_update_poll_set();
				return;
			}
			//USL_SYSLOG(LOG_ERR, "%s: select", strerror(errno));
			//continue;
			//printf("%s select return 0", __func__);
			return;
		}
