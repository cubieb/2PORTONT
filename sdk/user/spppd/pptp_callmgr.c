 ...
 *
 * $Id: pptp_callmgr.c,v 1.2 2012/05/30 03:52:15 ql Exp $
 */
#include <sys/time.h>
#include <sys/types.h>
#include <sys/stat.h>
...
int open_inet6sock(struct in6_addr inet6addr);
void close_inet6sock(int fd, struct in6_addr inet6addr);
...
int get_call_ID(PPTP_CONN *conn, u_int16_t *callid, u_int16_t *peer_callid)
{
	...
	if (conninfo && conninfo->call) {
		pptp_call_get_ids(conn, conninfo->call, callid, peer_callid);
		return 1;
	}
...
int conninfo_init(PPTP_CONN *conn)
{
	...
	conninfo = malloc(sizeof(struct local_conninfo));
	if (conninfo==NULL) {
		PPTP_DEBUG("malloc local_conninfo fail.");
		return -1;
	}
	memset(conninfo, 0, sizeof(struct local_conninfo));
...
void conninfo_release(PPTP_CONN *conn)
{
	struct local_conninfo *conninfo;
	...
	pptp_conn_closure_put(conn, NULL);
}
...
void call_callback(PPTP_CONN *conn, PPTP_CALL *call, enum call_state state)
{
	...
	case CALL_CLOSE_DONE:
		...
		PPTP_DEBUG("call closed");
		break;
	default:
		PPTP_DEBUG("Unhandled call callback state [%d].", (int) state);
	break;
	}
}
...
int open_inetsock(struct in_addr inetaddr)
{
	...
	if ((s = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
		PPTP_DEBUG("socket: %s", strerror(errno));
		return s;
	}

	fcntl(s, F_SETFD, fcntl(s, F_GETFD) | FD_CLOEXEC); // FD_CLOEXEC: Let child process won't inherit this fd
	if (connect(s, (struct sockaddr *) &dest, sizeof(dest)) < 0) {
		PPTP_DEBUG("connect: %s", strerror(errno));
		close(s);
		return -1;
	}
	return s;
