...
int l2tp_show_tunnel(FILE *file, struct l2tp_api_tunnel_msg_data *tun, int config_only, int transport_only)
{
	...
	if (0 == tun->peer_addr.flag) {
		ip_addr.s_addr = tun->peer_addr.s_addr.in;
		ip = inet_ntoa(ip_addr);
		strcpy(peer_ip, ip);
		ip_addr.s_addr = tun->our_addr.s_addr.in;
		ip = inet_ntoa(ip_addr);
		strcpy(our_ip, ip);
	}
#ifdef CONFIG_IPV6_VPN
	else {
		memcpy(&ip6_addr.s6_addr, tun->peer_addr.s_addr.in6, 16);
		inet_ntop(AF_INET6, &ip6_addr, peer_ip, 48);
		memcpy(&ip6_addr.s6_addr, tun->our_addr.s_addr.in6, 16);
		inet_ntop(AF_INET6, &ip6_addr, our_ip, 48);
	}
#endif
	idstr[0] = '\0';
	if (tun->config_id != 1) {
...
int l2tp_show_tunnel_profile(FILE *file, struct l2tp_api_tunnel_profile_msg_data *tp)
{
	...
	if (0 == tp->our_addr.flag) {
		if (tp->our_addr.s_addr.in != 0) {
			struct in_addr ip;
			ip.s_addr = tp->our_addr.s_addr.in;
		...
		if (!ipv6_addr_any((struct in6_addr *)tp->our_addr.s_addr.in6)) {
			struct in6_addr *ip6 = (struct in6_addr *)tp->our_addr.s_addr.in6;
			len += fprintf(file, "  local IP address: %s\n", inet_ntop(AF_INET6, ip6, str, 48));
		}
	}
#endif
	...
	if (0 == tp->peer_addr.flag) {
		if (tp->peer_addr.s_addr.in != 0) {
			struct in_addr ip;
			ip.s_addr = tp->peer_addr.s_addr.in;
			strcpy(&str[0], inet_ntoa(ip));
		...
		if (!ipv6_addr_any((struct in6_addr *)tp->peer_addr.s_addr.in6)) {
			struct in6_addr *ip6 = (struct in6_addr *)tp->peer_addr.s_addr.in6;
			len += fprintf(file, "  local IP address: %s\n", inet_ntop(AF_INET6, ip6, str, 48));
		}
	}
#endif
