...
static int ppp_unix_inet6_socket_connect(int session_fd, int tunnel_fd, struct sockaddr_in6 const *addr, 
				   uint16_t tunnel_id, uint16_t session_id, uint16_t peer_tunnel_id, uint16_t peer_session_id)
{
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: session_fd=%d tunnel_fd=%d tid=%hu sid=%hu peer=%hu/%hu addr=%s port=%hu",
		   __func__, session_fd, tunnel_fd, tunnel_id, session_id,
		   peer_tunnel_id, peer_session_id, inet_ntop(AF_INET6, &addr->sin6_addr, buff, 48), htons(addr->sin6_port));
	...
	sax.pppol2tp.pid = 0;
	sax.pppol2tp.fd = tunnel_fd;
	sax.pppol2tp.addr.sin6_addr = addr->sin6_addr;
...
static int ppp_unix_session_up(struct l2tp_session const *session, uint16_t tunnel_id, uint16_t session_id, uint16_t peer_tunnel_id, uint16_t peer_session_id)
{
	...
	session_fd = socket(AF_PPPOX, SOCK_DGRAM, PX_PROTO_OL2TP);
	if (session_fd < 0) {
		l2tp_session_log(session, L2TP_DATA, LOG_ERR, "sess %hu/%hu: unable to open pppox socket", 
				 tunnel_id, session_id);
		result = -errno;
		goto out;
	}
	...
	flags = fcntl(session_fd, F_GETFL);
	if (flags == -1 || fcntl(session_fd, F_SETFL, flags | O_NONBLOCK) == -1) {
		l2tp_session_log(session, L2TP_DATA, LOG_ERR, "sess %hu/%hu: unable to set socket nonblock",
				tunnel_id, session_id);
		result = -errno;
		goto out;
	}
	...
	tunnel = l2tp_session_get_tunnel(session);
	tunnel_fd = l2tp_tunnel_get_fd(tunnel);

	if (l2tp_tunnel_get_ipver(tunnel) == 4)
		result = ppp_unix_socket_connect(session_fd, tunnel_fd, l2tp_tunnel_get_peer_addr(tunnel),
					 tunnel_id, session_id, 
					 peer_tunnel_id, peer_session_id);
#ifdef CONFIG_IPV6_VPN
	else
		result = ppp_unix_inet6_socket_connect(session_fd, tunnel_fd, l2tp_tunnel_get_peer_addr(tunnel),
					 tunnel_id, session_id, 
					 peer_tunnel_id, peer_session_id);
#endif
	if (result < 0) {
		l2tp_session_log(session, L2TP_DATA, LOG_ERR, "sess %hu/%hu: unable to connect pppox socket", 
				 tunnel_id, session_id);
		result = -errno;
		close(session_fd);
		goto out;
	...
	l2tp_session_set_fd(session, session_fd);
...
static int ppp_unix_session_get_stats(struct l2tp_session const *session, uint16_t tunnel_id, uint16_t session_id, struct pppol2tp_ioc_stats *stats)
{
	...
	kernel_fd = l2tp_tunnel_get_fd(tunnel);
	if (kernel_fd >= 0) {
		stats->tunnel_id = tunnel_id;
		stats->session_id = session_id;
		L2TP_DEBUG(L2TP_API, "PPP: sess %s: using fd=%d", l2tp_session_get_name(session), kernel_fd);
		result = ioctl(kernel_fd, PPPIOCGL2TPSTATS, stats);
		if (result < 0) {
			result = -errno;
			l2tp_session_log(session, L2TP_API, LOG_ERR, "PPP: sess %s: ioctl(PPPIOCGL2TPSTATS) failed", l2tp_session_get_name(session));
			goto out;
		}
	} else {
		L2TP_DEBUG(L2TP_DATA, "%s: %s: no ppp context for tid=%hu sid=%hu fd=%d", __func__, l2tp_session_get_name(session),
			   tunnel_id, session_id, kernel_fd);
	}
out:
	return result;
}
