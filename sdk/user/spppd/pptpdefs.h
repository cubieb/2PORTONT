 ...
 *
 * $Id: pptpdefs.h,v 1.1 2012/08/27 07:44:19 ql Exp $
 */
...
(type==START_CTRL_CONN_RQST)?sizeof(struct pptp_start_ctrl_conn_rqst):	\
(type==START_CTRL_CONN_RPLY)?sizeof(struct pptp_start_ctrl_conn_rply):	\
(type==STOP_CTRL_CONN_RQST )?sizeof(struct pptp_stop_ctrl_conn_rqst):	\
(type==STOP_CTRL_CONN_RPLY )?sizeof(struct pptp_stop_ctrl_conn_rply):	\
(type==ECHO_RQST           )?sizeof(struct pptp_echo_rqst):	\
(type==ECHO_RPLY           )?sizeof(struct pptp_echo_rply):	\
(type==OUT_CALL_RQST       )?sizeof(struct pptp_out_call_rqst):	\
(type==OUT_CALL_RPLY       )?sizeof(struct pptp_out_call_rply):	\
(type==IN_CALL_RQST        )?sizeof(struct pptp_in_call_rqst):	\
(type==IN_CALL_RPLY        )?sizeof(struct pptp_in_call_rply):	\
(type==IN_CALL_CONN        )?sizeof(struct pptp_in_call_connect):	\
(type==CALL_CLR_RQST       )?sizeof(struct pptp_call_clr_rqst):	\
(type==CALL_DISCONN_NTFY   )?sizeof(struct pptp_call_disconn_ntfy):	\
(type==WAN_ERR_NTFY        )?sizeof(struct pptp_wan_err_ntfy):	\
(type==SET_LINK_INFO       )?sizeof(struct pptp_set_link_info):	\
0)
