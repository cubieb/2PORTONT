...
#define L2TP_PLUGIN_DIR			"/lib/openl2tp"
...
#else
//#define L2TP_DEBUG(level, fmt, args...) printf("DEBUG: " fmt "\n", ##args);
...
struct l2tp_peer {
	...
	int				flag;	/*0: IPv4  1:IPv6*/
	union {
		struct in_addr		ip; 		/* IP_PKTINFO: src ip to reach peer */
#ifdef CONFIG_IPV6_VPN
		struct in6_addr		ip6;
#endif
	} if_local_addr;
	union {
		struct in_addr		ip;		/* IP_PKTINFO: dest ip to reach peer (addr of next hop if is a router) */
#ifdef CONFIG_IPV6_VPN
		struct in6_addr		ip6;
#endif
	} if_peer_addr;
...
struct l2tp_xprt_tunnel_create_data {
	...
	uint16_t	pad;
	uint32_t	flag;			/*0: IPv4    1: IPv6*/
	union {
		uint32_t	in; 		/* these values must agree with tunnel's socket */
#ifdef CONFIG_IPV6_VPN
		uint8_t		in6[16];
#endif
	} our_addr;
...
extern int (*l2tp_session_created_pppd_hook)(uint16_t tunnel_id, uint16_t session_id);
...
#ifdef CONFIG_IPV6_VPN
extern int ipv6_addr_equal(const struct in6_addr *a1, const struct in6_addr *a2);
extern int ipv6_addr_any(const struct in6_addr *a);
extern void l2tp_net6_recv(int fd, void *arg);
extern void l2tp_net6_recv_unconn(int fd, void *arg);
extern int l2tp_net6_connect_socket(int fd, struct sockaddr_in6 const *peer_addr, struct l2tp_peer *peer, struct l2tp_tunnel *tunnel);
extern int l2tp_net6_create_socket(struct sockaddr_in6 *local_addr, int use_udp_checksums, const struct l2tp_tunnel *tunnel);
extern int l2tp_net6_get_socket_addresses(int fd, struct sockaddr_in6 *src, struct sockaddr_in6 *dest, struct l2tp_tunnel *tunnel);
extern int l2tp_net6_get_local_address_for_peer(struct sockaddr_in6 *peer_addr, struct in6_addr *local_addr);
extern int l2tp_net6_modify_socket(int fd, int use_udp_checksums, int do_pmtu_discovery);
#endif
...
#ifdef CONFIG_IPV6_VPN
extern struct l2tp_peer *l2tp_ipv6_peer_find(struct in6_addr const *src, struct in6_addr const *dest);
extern struct l2tp_peer_profile *l2tp_peerin6_profile_find_by_addr(struct in6_addr peer_addr);
extern struct l2tp_peer *l2tp_ipv6_peer_alloc(struct in6_addr src, struct in6_addr dest);
#endif
...
extern int l2tp_tunnel_xprt_create(struct l2tp_peer *peer, struct l2tp_tunnel *tunnel, struct sockaddr const *peer_addr);
...
#ifdef CONFIG_IPV6_VPN
extern int l2tp_tunnel_get_ipver(struct l2tp_tunnel const *tunnel);
#endif
extern struct sockaddr const *l2tp_tunnel_get_peer_addr(struct l2tp_tunnel const *tunnel);
extern struct sockaddr const *l2tp_tunnel_get_local_addr(struct l2tp_tunnel const *tunnel);
...
extern struct l2tp_tunnel *l2tp_tunnel_find_by_addr(struct sockaddr const *peer_addr, uint16_t peer_tunnel_id, int config_id, int tunnel_id_valid);
...
#ifdef CONFIG_IPV6_VPN
extern void l2tp_tunnel_set_ipv6_addresses(struct l2tp_tunnel *tunnel, struct sockaddr_in6 *src, struct sockaddr_in6 *dest);
#endif
...
extern void l2tp_session_set_fd(struct l2tp_session *session, int fd);
extern int l2tp_session_get_fd(struct l2tp_session const *session);
