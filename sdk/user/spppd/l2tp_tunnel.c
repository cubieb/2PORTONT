...
int l2tp_tunnel_has_session(uint16_t tunnel_id){
	struct l2tp_tunnel *tunnel;
	struct usl_list_head *session_list;

	tunnel = l2tp_tunnel_find_by_id(tunnel_id);
	if(tunnel){
		session_list = l2tp_tunnel_session_list(tunnel);
		if(session_list){
			if(session_list->prev != session_list->next)
				return 1;
		}
	}
	return 0;
...
int l2tp_tunnel_get_ipver(struct l2tp_tunnel const *tunnel)
{
	int ip_version = 4;

#ifdef CONFIG_IPV6_VPN
	if (tunnel->config.peer_addr.sa.sa_family == AF_INET6)
		ip_version = 6;
#endif
	return (ip_version);
}
...
struct sockaddr const *l2tp_tunnel_get_peer_addr(struct l2tp_tunnel const *tunnel)
{
	return &tunnel->config.peer_addr.sa;
}

struct sockaddr const *l2tp_tunnel_get_local_addr(struct l2tp_tunnel const *tunnel)
{
	return &tunnel->config.local_addr.sa;
}
...
void l2tp_tunnel_set_addresses(struct l2tp_tunnel *tunnel, struct sockaddr_in *src, struct sockaddr_in *dest)
{
	...
	if (src != NULL) {
		tunnel->config.local_addr.sin = *src;
		if (peer->if_local_addr.ip.s_addr == INADDR_ANY) {
			peer->if_local_addr.ip.s_addr = src->sin_addr.s_addr;
		}
	}
	if (dest != NULL) {
		tunnel->config.peer_addr.sin = *dest;
	}
...
/* Called when tunnel addresses are known. 
 */
void l2tp_tunnel_set_ipv6_addresses(struct l2tp_tunnel *tunnel, struct sockaddr_in6 *src, struct sockaddr_in6 *dest)
{
	struct l2tp_peer *peer = tunnel->my_peer;

	if (src != NULL) {
		tunnel->config.local_addr.sin6 = *src;
		if (ipv6_addr_any(&peer->if_local_addr.ip6)) {
			peer->flag = 1;
			memcpy(peer->if_local_addr.ip6.s6_addr, src->sin6_addr.s6_addr, 16);
		}
	}
	if (dest != NULL) {
		tunnel->config.peer_addr.sin6 = *dest;
	}
	...
	if (tunnel->config.do_pmtu_discovery) {
		...
		int result;
		socklen_t optlen = sizeof(mtu);
		result = l2tp_net6_modify_socket(tunnel->fd, -1, 1);
		if (result == 0) {
			result = getsockopt(tunnel->fd, SOL_IPV6, IPV6_MTU, &mtu, &optlen);
			if (result == 0) {
				l2tp_tunnel_update_mtu(tunnel, tunnel->fd, mtu);
			}
		}
...
struct l2tp_tunnel *l2tp_tunnel_find_by_addr(struct sockaddr const *peer_addr, uint16_t peer_tunnel_id, 
					     int config_id, int tunnel_id_valid)
{
	...
	usl_list_for_each(walk, tmp, &l2tp_tunnel_list) {
		...
		//	continue;
		//}
		if (tunnel->config.peer_addr.sa.sa_family != peer_addr->sa_family)
			continue;
		if (tunnel->config.peer_addr.sa.sa_family == AF_INET)
		{
			if (tunnel->config.peer_addr.sin.sin_addr.s_addr != ((struct sockaddr_in *)peer_addr)->sin_addr.s_addr)
				continue;
		}
#ifdef CONFIG_IPV6_VPN
		else if (tunnel->config.peer_addr.sa.sa_family == AF_INET6)
		{
			if (!ipv6_addr_equal(&tunnel->config.peer_addr.sin6.sin6_addr, &((struct sockaddr_in6 *)peer_addr)->sin6_addr))
				continue;
		}
#endif
 ...
 */
static int l2tp_tunnel_alloc_config_id(struct sockaddr const *peer_addr)
{
	int config_id = 1;
...
static void l2tp_tunnel_act_send_sccrq(void *tun, void *session, void *arg3)
{
	...
	if (!l2tp_tunnel_is_fd_connected(tunnel)) {
		if (tunnel->config.peer_addr.sa.sa_family == AF_INET)
			l2tp_net_get_socket_addresses(tunnel->fd, &tunnel->config.local_addr.sin, NULL, tunnel);
#ifdef CONFIG_IPV6_VPN
		else
			l2tp_net6_get_socket_addresses(tunnel->fd, &tunnel->config.local_addr.sin6, NULL, tunnel);
#endif
	}
...
static int l2tp_tunnel_peer_ok(struct l2tp_peer *peer, struct l2tp_tunnel *tunnel)
{
	...
	if (0 == peer->flag)
		profile = l2tp_peer_profile_find_by_addr(peer->if_peer_addr.ip);
#ifdef CONFIG_IPV6_VPN
	else
		profile = l2tp_peerin6_profile_find_by_addr(peer->if_peer_addr.ip6);
#endif
	if (profile != NULL) {
		result = 0;
	}
...
static int l2tp_tunnel_check_for_peer_collision(struct l2tp_tunnel *tunnel)
{
	...
	usl_list_for_each(walk, tmp, &l2tp_tunnel_list) {
		...
		if ((tunnel2 != tunnel) &&
		    (tunnel2->sccrq_sent) &&
		    (peer2 != NULL) && 
		    (tunnel2->config.local_addr.sa.sa_family == tunnel->config.local_addr.sa.sa_family) &&
		    ( ((tunnel2->config.local_addr.sa.sa_family == AF_INET) &&
		       (tunnel2->config.local_addr.sin.sin_addr.s_addr == tunnel->config.local_addr.sin.sin_addr.s_addr) &&
		       (tunnel2->config.peer_addr.sin.sin_addr.s_addr == tunnel->config.peer_addr.sin.sin_addr.s_addr) )
#ifdef CONFIG_IPV6_VPN
		       ||
		      ((tunnel2->config.local_addr.sa.sa_family == AF_INET6) &&
		       ipv6_addr_equal(&tunnel2->config.local_addr.sin6.sin6_addr, &tunnel->config.local_addr.sin6.sin6_addr) &&
		       ipv6_addr_equal(&tunnel2->config.peer_addr.sin6.sin6_addr, &tunnel->config.peer_addr.sin6.sin6_addr))
#endif
		     )) {
...
int l2tp_tunnel_queue_event(uint16_t tunnel_id, int event)
{
	...
	if (result < 0) {
		...
		l2tp_log(LOG_ERR, "L2TP: tunnel event pipe full: event %d for tunnel %hu lost\n", event, tunnel_id);
	}
...
int l2tp_tunnel_config_update(struct l2tp_tunnel *tunnel, struct l2tp_api_tunnel_msg_data *msg){
	if(msg->flags & L2TP_API_TUNNEL_FLAG_OUR_ADDR){
		tunnel->config.local_addr.sin.sin_addr.s_addr = msg->our_addr.s_addr.in;
	}
	if(msg->flags & L2TP_API_TUNNEL_FLAG_OUR_UDP_PORT){
		tunnel->config.local_addr.sin.sin_port = msg->our_udp_port;
	}
	l2tp_tunnel_config_set(tunnel, msg, 1);
}
#endif
struct l2tp_tunnel *l2tp_tunnel_alloc(uint16_t tunnel_id, char *tunnel_profile_name, char *peer_profile_name, int created_by_admin, int *ret)
{
	...
	printf( "FUNC: tunl %hu: allocated context using profile '%s'%s\n", 
		tunnel->status.tunnel_id, tunnel->config.tunnel_profile_name,
		tunnel->status.created_by_admin ? "" : ", created by network request");
 ...
 */
int l2tp_tunnel_xprt_create(struct l2tp_peer *peer, struct l2tp_tunnel *tunnel, struct sockaddr const *peer_addr)
{
	...
	if (4 == ip_version) {
		L2TP_DEBUG(L2TP_API, "%s: creating socket to peer %x/%hu using local %x/%hu", __func__,
			   ntohl(tunnel->config.peer_addr.sin.sin_addr.s_addr), ntohs(tunnel->config.peer_addr.sin.sin_port),
			   ntohl(tunnel->config.local_addr.sin.sin_addr.s_addr), ntohs(tunnel->config.local_addr.sin.sin_port));
		new_fd = l2tp_net_create_socket(&tunnel->config.local_addr.sin, tunnel->config.use_udp_checksums, tunnel);
		if (new_fd < 0) {
			result = -errno;
			goto error;
		}
		...
		L2TP_DEBUG(L2TP_API, "%s: creating socket to peer %s/%hu using local %s/%hu", __func__,
			inet_ntop(AF_INET6, &tunnel->config.peer_addr.sin6.sin6_addr, dbuf, 48), 
			ntohs(tunnel->config.peer_addr.sin6.sin6_port),
			inet_ntop(AF_INET6, &tunnel->config.local_addr.sin6.sin6_addr, sbuf, 48), 
			ntohs(tunnel->config.local_addr.sin6.sin6_port));
		new_fd = l2tp_net6_create_socket(&tunnel->config.local_addr.sin6, tunnel->config.use_udp_checksums, tunnel);
		if (new_fd < 0) {
			result = -errno;
			goto error;
		}
	...
	if (4 == ip_version) {
		params.flag = 0;
		params.our_addr.in = tunnel->config.local_addr.sin.sin_addr.s_addr;
		params.peer_addr.in = tunnel->config.peer_addr.sin.sin_addr.s_addr;
		params.our_udp_port = tunnel->config.local_addr.sin.sin_port;
		params.peer_udp_port = tunnel->config.peer_addr.sin.sin_port;
	}
#ifdef CONFIG_IPV6_VPN
	else {
		params.flag = 1;
		memcpy(params.our_addr.in6, tunnel->config.local_addr.sin6.sin6_addr.s6_addr, 16);
		memcpy(params.peer_addr.in6, tunnel->config.peer_addr.sin6.sin6_addr.s6_addr, 16);
		params.our_udp_port = tunnel->config.local_addr.sin6.sin6_port;
		params.peer_udp_port = tunnel->config.peer_addr.sin6.sin6_port;
	}
#endif
	...
	if (4 == ip_version)
		...
		result = usl_fd_add_fd(new_fd, l2tp_net6_recv_unconn, tunnel);
#endif
	if (result < 0) {
...
static int l2tp_tunnel_param_defaults(struct l2tp_tunnel *tunnel, char *tunnel_profile_name, char *peer_profile_name)
{
	...
	if (tunnel_profile->peer_addr.flag == 0) {
		tunnel->config.local_addr.sin.sin_family = AF_INET;
		tunnel->config.local_addr.sin.sin_addr.s_addr = tunnel_profile->our_addr.u.in.s_addr;
		tunnel->config.local_addr.sin.sin_port = htons(tunnel_profile->our_udp_port);
		tunnel->config.peer_addr.sin.sin_family = AF_INET;
		tunnel->config.peer_addr.sin.sin_port = htons(tunnel_profile->peer_udp_port);
	}
#ifdef CONFIG_IPV6_VPN
	else if (tunnel_profile->peer_addr.flag == 1) {
		tunnel->config.local_addr.sin6.sin6_family = AF_INET6;
		tunnel->config.local_addr.sin6.sin6_addr = tunnel_profile->our_addr.u.in6;
		tunnel->config.local_addr.sin6.sin6_port = htons(tunnel_profile->our_udp_port);
		tunnel->config.peer_addr.sin6.sin6_family = AF_INET6;
		tunnel->config.peer_addr.sin6.sin6_port = htons(tunnel_profile->peer_udp_port);
	}
#endif
	else {
		printf("%s %d tunnel profile has not been initialized.\n", __func__, __LINE__);
	}
...
bool_t l2tp_tunnel_create_1_svc(struct l2tp_api_tunnel_msg_data msg, int *retval, struct svc_req *req)
{
...
#ifdef CONFIG_IPV6_VPN
	struct sockaddr_in6 peerin6_addr;
	struct in6_addr localin6_addr;
	char sbuf[48], dbuf[48];
#endif
	...
	if (4 == ip_version) {
		...
		peer_addr.sin_addr.s_addr = msg.peer_addr.s_addr.in;
		peer_addr.sin_port = htons(msg.peer_udp_port);
	}
#ifdef CONFIG_IPV6_VPN
	else {
		memset(&peerin6_addr, 0, sizeof(peerin6_addr));
		peerin6_addr.sin6_family = AF_INET6;
		memcpy(peerin6_addr.sin6_addr.s6_addr, msg.peer_addr.s_addr.in6, 16);
		peerin6_addr.sin6_port = htons(msg.peer_udp_port);
	}
#endif
	...
	if ((msg.flags & L2TP_API_TUNNEL_FLAG_CONFIG_ID) == 0) {
		if (4 == ip_version)
			config_id = l2tp_tunnel_alloc_config_id((struct sockaddr *)&peer_addr);
#ifdef CONFIG_IPV6_VPN
		else
			config_id = l2tp_tunnel_alloc_config_id((struct sockaddr *)&peerin6_addr);
#endif
		if (config_id < 0) {
			l2tp_stats.too_many_tunnels++;
			goto out;
		...
		if ((4 == ip_version) && l2tp_tunnel_find_by_addr((struct sockaddr *)&peer_addr, 0, config_id, 0)) {
			result = -L2TP_ERR_TUNNEL_ALREADY_EXISTS;
			goto out;
		}
#ifdef CONFIG_IPV6_VPN
		else if ((6 == ip_version) && l2tp_tunnel_find_by_addr((struct sockaddr *)&peerin6_addr, 0, config_id, 0)) {
			result = -L2TP_ERR_TUNNEL_ALREADY_EXISTS;
			goto out;
		}
#endif
	...
	if (4 == ip_version) {
		if (msg.our_addr.s_addr.in == INADDR_ANY) {
			result = l2tp_net_get_local_address_for_peer(&peer_addr, &local_addr);
			if (result < 0) {
				goto out;
			}
		} else {
			local_addr.s_addr = msg.our_addr.s_addr.in;
		}
	}
#ifdef CONFIG_IPV6_VPN
	else if (6 == ip_version) {
		if (ipv6_addr_any((struct in6_addr *)&msg.our_addr.s_addr.in6)) {
			result = l2tp_net6_get_local_address_for_peer(&peerin6_addr, &localin6_addr);
			if (result < 0) {
				result = -L2TP_DEBUG_XPRT;
				goto out;
			}
		} else {
			memcpy(localin6_addr.s6_addr, msg.our_addr.s_addr.in6, 16);
		}
	}
#endif
	...
	if (4 == ip_version) {
		...
		peer = l2tp_ipv6_peer_find(&localin6_addr, &peerin6_addr.sin6_addr);
		if (peer == NULL) {
			peer = l2tp_ipv6_peer_alloc(localin6_addr, peerin6_addr.sin6_addr);
			if (peer == NULL) {
				result = -ENOMEM;
				goto out;
			}
	...
	//if (msg.flags & L2TP_API_TUNNEL_FLAG_TUNNEL_NAME)
	//	tunnel.config.tunnel_name = strdup(OPTSTRING_PTR(msg.tunnel_name));

	/* Retain local address info */
	if (4 == ip_version) {
		tunnel->config.local_addr.sin.sin_family = AF_INET;
		if (tunnel->config.local_addr.sin.sin_addr.s_addr == INADDR_ANY) {
			tunnel->config.local_addr.sin.sin_addr.s_addr = local_addr.s_addr;
		}
		if (tunnel->config.local_addr.sin.sin_port == 0) {
			tunnel->config.local_addr.sin.sin_port = htons(msg.our_udp_port);
		}

		/* Retain peer address info */
		tunnel->config.peer_addr.sin.sin_family = AF_INET;
		tunnel->config.peer_addr.sin.sin_addr.s_addr = peer_addr.sin_addr.s_addr;
		tunnel->config.peer_addr.sin.sin_port = peer_addr.sin_port;
	}
#ifdef CONFIG_IPV6_VPN
	else {
		tunnel->config.local_addr.sin6.sin6_family = AF_INET6;
		if ((tunnel->config.local_addr.sin6.sin6_addr.s6_addr32[0] == 0) &&
			(tunnel->config.local_addr.sin6.sin6_addr.s6_addr32[1] == 0) &&
			(tunnel->config.local_addr.sin6.sin6_addr.s6_addr32[2] == 0) &&
			(tunnel->config.local_addr.sin6.sin6_addr.s6_addr32[3] == 0)) {
			tunnel->config.local_addr.sin6.sin6_addr = localin6_addr;
		}
		if (tunnel->config.local_addr.sin6.sin6_port == 0) {
			tunnel->config.local_addr.sin6.sin6_port = htons(msg.our_udp_port);
		}

		/* Retain peer address info */
		tunnel->config.peer_addr.sin6.sin6_family = AF_INET6;
		tunnel->config.peer_addr.sin6.sin6_addr = peerin6_addr.sin6_addr;
		tunnel->config.peer_addr.sin6.sin6_port = peerin6_addr.sin6_port;
	...
	if (4 == ip_version)
		result = l2tp_tunnel_xprt_create(peer, tunnel, (struct sockaddr *)&peer_addr);
#ifdef CONFIG_IPV6_VPN
	else
		result = l2tp_tunnel_xprt_create(peer, tunnel, (struct sockaddr *)&peerin6_addr);
#endif
	if (result < 0) {
		goto out;
	}
	...
	if (4 == ip_version) {
		...
		char dest[16];
		char *ip;
		ip = inet_ntoa(tunnel->config.local_addr.sin.sin_addr);
		strcpy(src, ip);
		ip = inet_ntoa(tunnel->config.peer_addr.sin.sin_addr);
		strcpy(dest, ip);
		...
		char src[48], dest[48];
		L2TP_DEBUG(L2TP_API, "%s: src=%s dest=%s configId=%d secret=%s", __func__,
			   inet_ntop(AF_INET6, &tunnel->config.local_addr.sin6.sin6_addr, src, 48), 
			   inet_ntop(AF_INET6, &tunnel->config.peer_addr.sin6.sin6_addr, dest, 48),
			   tunnel->config.config_id, 
			   tunnel->config.secret ? tunnel->config.secret : "");
	}
#endif
#endif
...
bool_t l2tp_tunnel_delete_1_svc(u_short tunnel_id, optstring tunnel_name, optstring reason, int *result, struct svc_req *req)
{
	...
	*result = 0;
	...
	//QL maybe we should release tunnel right now!
	l2tp_tunnel_dec_use_count(tunnel);
...
bool_t l2tp_tunnel_get_1_svc(u_short tunnel_id, optstring tunnel_name, struct l2tp_api_tunnel_msg_data *result, struct svc_req *req)
{
	...
	if (tunnel->config.peer_addr.sa.sa_family == AF_INET) {
		result->our_addr.flag = 0;
		result->peer_addr.flag = 0;
		result->our_addr.s_addr.in = tunnel->config.local_addr.sin.sin_addr.s_addr;
		result->peer_addr.s_addr.in = tunnel->config.peer_addr.sin.sin_addr.s_addr;
		result->our_udp_port = ntohs(tunnel->config.local_addr.sin.sin_port);
		result->peer_udp_port = ntohs(tunnel->config.peer_addr.sin.sin_port);
	}
#ifdef CONFIG_IPV6_VPN
	else {
		result->our_addr.flag = 1;
		result->peer_addr.flag = 1;
		memcpy(result->our_addr.s_addr.in6, tunnel->config.local_addr.sin6.sin6_addr.s6_addr, 16);
		memcpy(result->peer_addr.s_addr.in6, tunnel->config.peer_addr.sin6.sin6_addr.s6_addr, 16);
		result->our_udp_port = ntohs(tunnel->config.local_addr.sin6.sin6_port);
		result->peer_udp_port = ntohs(tunnel->config.peer_addr.sin6.sin6_port);
	}
#endif
#ifdef L2TP_TEST
...
static int l2tp_tunnel_clone(struct l2tp_tunnel *tunnel, struct l2tp_api_tunnel_msg_data *result)
{
	...
	if (tunnel->config.peer_addr.sa.sa_family == AF_INET) {
		...
		result->our_addr.s_addr.in = tunnel->config.local_addr.sin.sin_addr.s_addr;
		result->peer_addr.s_addr.in = tunnel->config.peer_addr.sin.sin_addr.s_addr;
		result->our_udp_port = htons(tunnel->config.local_addr.sin.sin_port);
		result->peer_udp_port = htons(tunnel->config.peer_addr.sin.sin_port);
	}
#ifdef CONFIG_IPV6_VPN
	else {
		result->our_addr.flag = 1;
		...
		memcpy(result->our_addr.s_addr.in6, tunnel->config.local_addr.sin6.sin6_addr.s6_addr, 16);
		memcpy(result->peer_addr.s_addr.in6, tunnel->config.peer_addr.sin6.sin6_addr.s6_addr, 16);
		result->our_udp_port = htons(tunnel->config.local_addr.sin6.sin6_port);
		result->peer_udp_port = htons(tunnel->config.peer_addr.sin6.sin6_port);
	}
#endif
	result->config_id = tunnel->config.config_id;
	result->trace_flags = tunnel->config.trace_flags;
...
static int l2tp_tunnel_profile_set(struct l2tp_api_tunnel_profile_msg_data *msg, struct l2tp_tunnel_profile *profile)
{
	...
	if (msg->flags & L2TP_API_TUNNEL_PROFILE_FLAG_OUR_ADDR) {
		...
		memcpy(&profile->our_addr, &msg->our_addr, sizeof(struct l2tp_api_ip_addr));
	}
	if (msg->flags & L2TP_API_TUNNEL_PROFILE_FLAG_PEER_ADDR) {
		//profile->peer_addr.s_addr = msg->peer_addr.s_addr;
		memcpy(&profile->peer_addr, &msg->our_addr, sizeof(struct l2tp_api_ip_addr));
	}
...
bool_t l2tp_tunnel_profile_create_1_svc(struct l2tp_api_tunnel_profile_msg_data msg, int *result, struct svc_req *req)
{
	...
	memcpy(&profile->our_addr, &l2tp_tunnel_defaults->our_addr, sizeof(l2tp_tunnel_defaults->our_addr));
	profile->our_udp_port = l2tp_tunnel_defaults->our_udp_port;
	memcpy(&profile->peer_addr, &l2tp_tunnel_defaults->peer_addr, sizeof(l2tp_tunnel_defaults->peer_addr));
...
bool_t l2tp_tunnel_profile_get_1_svc(char *profile_name, struct l2tp_api_tunnel_profile_msg_data *result, struct svc_req *req)
{
	...
	if (0 == profile->our_addr.flag) {
		result->our_addr.flag = 0;
		result->our_addr.s_addr.in = profile->our_addr.u.in.s_addr;
	}
#ifdef CONFIG_IPV6_VPN
	else {
		result->our_addr.flag = 1;
		memcpy(result->our_addr.s_addr.in6, profile->our_addr.u.in6.s6_addr, 16);
	}
#endif
	result->our_udp_port = profile->our_udp_port;
	if (0 == profile->peer_addr.flag) {
		result->peer_addr.flag = 0;
		result->peer_addr.s_addr.in = profile->peer_addr.u.in.s_addr;
	}
#ifdef CONFIG_IPV6_VPN
	else {
		result->peer_addr.flag = 1;
		memcpy(result->peer_addr.s_addr.in6, profile->peer_addr.u.in6.s6_addr, 16);
	}
#endif
...
bool_t l2tp_tunnel_profile_unset_1_svc(struct l2tp_api_tunnel_profile_unset_msg_data msg, int *result, struct svc_req *req)
{
	...
	if (msg.flags & L2TP_API_TUNNEL_PROFILE_FLAG_OUR_ADDR) {
		memset(&profile->our_addr, 0, sizeof(profile->our_addr));
	}
	if (msg.flags & L2TP_API_TUNNEL_PROFILE_FLAG_PEER_ADDR) {
		//profile->peer_addr.s_addr = INADDR_ANY;
		memset(&profile->peer_addr, 0, sizeof(profile->peer_addr));
	}
	if (msg.flags & L2TP_API_TUNNEL_PROFILE_FLAG_HIDE_AVPS) {
...
void l2tp_tunnel_update_state(struct sppp *sp)
{
	...
	if (NULL == tunnel)
	{
		L2TP_DEBUG(L2TP_FUNC, "%s %d fatal error, tunnel %s is NULL.\n", __func__, __LINE__, vpnr->tunnel_nam);
	}
	else
	{
		p->tunnel_id = tunnel->status.tunnel_id;
		
		fsmi = &tunnel->fsmi;
		if (fsmi) {
			switch (fsmi->state)
			{
				case L2TP_CCE_STATE_ESTABLISHED:
					if (L2TP_ST_TUNNEL_INIT == p->call_state)
						p->call_state = L2TP_ST_TUNNEL_ESTABLISH;
					break;
				case L2TP_CCE_STATE_CLOSING:
					p->call_state = L2TP_ST_DEAD;
					break;
			}
		}
		#if 0
		if (fsmi && (L2TP_CCE_STATE_ESTABLISHED == fsmi->state)) {
			p->call_state = L2TP_ST_TUNNEL_ESTABLISH;
		}
		#endif
	}
...
void l2tp_tunnel_init(void)
{
	...
	(void) fcntl(l2tp_tunnel_event_pipe[0], F_SETFL, O_NONBLOCK);
	(void) fcntl(l2tp_tunnel_event_pipe[1], F_SETFL, O_NONBLOCK);
	//printf("usl_fd_add_fd %d callback l2tp_tunnel_do_queued_event\n", l2tp_tunnel_event_pipe[0]);
