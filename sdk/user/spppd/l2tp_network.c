...
static inline void l2tp_net_parse_header(struct l2tp_packet *pkt, uint16_t *ver, int *is_data, int *has_seq, void **payload)
{
	...
	*payload = ((void *) optval) + offset;
	if (hdr->l_bit) {
		// QL 20111221: I think avp_len should exclude offset padding
		#if 0
		pkt->avp_len = dlen - (((void *) optval) - ((void *) hdr));
		#else
		pkt->avp_len = dlen - (((void *) *payload) - ((void *) hdr));
		#endif
	} else {
		pkt->avp_len = 0;
	}
...
extern struct VPN_SERVER_AUTH_St pppd_auth[SPPPD_AUTH_NUM];
#endif
static void l2tp_net_recv_core(int fd, struct sockaddr_in const *from, struct msghdr *msg, int recv_len, struct in_pktinfo *ipi)
{
	...
	if (pkt->tunnel_id != 0) {
		...
		if (msg_type == L2TP_AVP_MSG_SCCRQ) {
			tunnel = l2tp_tunnel_find_by_addr((struct sockaddr *)from, pkt->tunnel_id, 0, 1);
 ...
 #ifdef CONFIG_USER_L2TPD_LNS
		//update tunnel AUTH_MODE by config
		struct l2tp_api_tunnel_msg_data tunnel_msg;
		memset(&tunnel_msg, 0, sizeof(struct l2tp_api_tunnel_msg_data));
		if(pppd_auth[L2TPD_AUTH_IDX].tunnel_auth == 1){
			tunnel_msg.flags |= L2TP_API_TUNNEL_FLAG_AUTH_MODE;
			tunnel_msg.auth_mode = L2TP_API_TUNNEL_AUTH_MODE_CHALLENGE;
			tunnel_msg.flags |= L2TP_API_TUNNEL_FLAG_SECRET;
			tunnel_msg.secret.valid = 1;
			tunnel_msg.secret.optstring_u.value = strdup(pppd_auth[L2TPD_AUTH_IDX].tunnel_secret);			
		}
		tunnel_msg.flags |= L2TP_API_TUNNEL_FLAG_OUR_ADDR;
		tunnel_msg.our_addr.flag = 0;
		tunnel_msg.our_addr.s_addr.in = ipi->ipi_spec_dst.s_addr;
		tunnel_msg.flags |= L2TP_API_TUNNEL_FLAG_OUR_UDP_PORT;
		tunnel_msg.our_udp_port = 1701;
		l2tp_tunnel_config_update(tunnel, &tunnel_msg);

#endif

		l2tp_tunnel_link(tunnel);

		result = l2tp_tunnel_xprt_create(peer, tunnel, (struct sockaddr *)from);
	...
	if (!l2tp_tunnel_is_fd_connected(tunnel)) {
		if (from == NULL) {
			from = (struct sockaddr_in *)l2tp_tunnel_get_peer_addr(tunnel);
		}
...
static void l2tp_net6_recv_core(int fd, struct sockaddr_in6 const *from, struct msghdr *msg, int recv_len, struct in6_pktinfo *ipi)
{
	int result = -EBADMSG;
	struct l2tp_peer *peer = NULL;
	struct l2tp_tunnel *tunnel = NULL;
	...
	if (pkt->tunnel_id != 0) {
		...
		if ((result < 0) || (msg_type != L2TP_AVP_MSG_SCCRQ)) {
			if (result != -ENOMEM) {
				if (from != NULL) {
					L2TP_DEBUG(L2TP_PROTOCOL, "%s: dropping non-SCCRQ type=%d from %s/%hu on fd %d", __func__, 
						   msg_type, inet_ntop(AF_INET6, &from->sin6_addr, buf, 48), ntohs(from->sin6_port), fd);
				} else {
					L2TP_DEBUG(L2TP_PROTOCOL, "%s: dropping non-SCCRQ type=%d on fd %d", __func__, msg_type, fd);
				}
				l2tp_stats.no_matching_tunnel_id_discards++;
		...
		peer_profile = l2tp_peer_profile_find(peer_host_name);
		if (peer_profile == NULL) {
			peer_profile = l2tp_peerin6_profile_find_by_addr(from->sin6_addr);
			if (peer_profile == NULL) {
				peer_profile = l2tp_peer_profile_find(L2TP_API_PEER_PROFILE_DEFAULT_PROFILE_NAME);
			}
		}
		L2TP_DEBUG(L2TP_PROTOCOL, "%s: using peer profile %s for %s (%s/%hu) on fd %d", __func__, 
			   peer_profile->profile_name, peer_host_name, 
			   inet_ntop(AF_INET6, &from->sin6_addr, buf, 48), ntohs(from->sin6_port), fd);

		/* Register a new peer context and record his addr */
		if (ipi != NULL) {
			peer = l2tp_ipv6_peer_find(&ipi->ipi6_addr, &from->sin6_addr);
			if (peer == NULL) {
				peer = l2tp_ipv6_peer_alloc(ipi->ipi6_addr, from->sin6_addr);
				if (peer == NULL) {
					result = -ENOMEM;
					l2tp_stats.no_control_frame_resources++;
					goto out;
		...
		if (l2tp_opt_trace_flags & L2TP_PROTOCOL) {
			l2tp_log(LOG_DEBUG, "PROTO: Creating new tunnel context with profile '%s' for %s (%s/%hu)",
				 peer_profile->default_tunnel_profile_name, peer_host_name, 
				 inet_ntop(AF_INET6, &from->sin6_addr, buf, 48), ntohs(from->sin6_port));
		}
	...
	if (!l2tp_tunnel_is_fd_connected(tunnel)) {
		if (from == NULL) {
			from = (struct sockaddr_in6 *)l2tp_tunnel_get_peer_addr(tunnel);
		}
		result = l2tp_net6_connect_socket(l2tp_tunnel_get_fd(tunnel), from, peer, tunnel);
		if (result < 0) {
			l2tp_stats.socket_errors++;
			goto out_unlink_tunnel;
		}
...
void l2tp_net6_recv_unconn(int fd, void *arg)
{
	int recv_len;
	unsigned char *buf;
	...
	struct {
		...
		.cm = {
			.cmsg_len = sizeof(struct cmsghdr) + sizeof(struct in6_pktinfo),
			.cmsg_level = SOL_IPV6,
	...
	L2TP_DEBUG(L2TP_DATA, "%s: %d bytes received on fd %d, to %x",
		   __func__, recv_len, fd, inet_ntop(AF_INET6, &cmsg.ipi.ipi6_addr, dstbuf, 48));

	l2tp_net6_recv_core(fd, &from, &msg, recv_len, &cmsg.ipi);

out:
	return;
}

void l2tp_net6_recv(int fd, void *arg)
{
	int recv_len;
	unsigned char *buf = NULL;
        struct iovec iov;
	...
	for (;;) {
		...
		if (recv_len < 0) {
			if ((errno == EINTR) || (errno == ERESTART)) {
				...
				int result;
				socklen_t optlen = sizeof(mtu);
				result = getsockopt(fd, SOL_IPV6, IPV6_MTU, &mtu, &optlen);
				if (result == 0) {
					l2tp_tunnel_update_mtu(NULL, fd, mtu);
				}
				free(buf);
	...
	l2tp_net6_recv_core(fd, NULL, &msg, recv_len, NULL);
...
int l2tp_net_create_socket(struct sockaddr_in *local_addr, int use_udp_checksums, const struct l2tp_tunnel *tunnel)
{
	...
	fcntl(new_fd, F_SETFD, fcntl(new_fd, F_GETFD) | FD_CLOEXEC); // FD_CLOEXEC: Let child process won't inherit this fd
	...
	if (flags == -1 || fcntl(new_fd, F_SETFL, flags | O_NONBLOCK) == -1)
		printf("Couldn't set l2tp ctrl socket to nonblock\n");
...
/*
	memset(&saddr, 0, sizeof(saddr));
	saddr.sin_family = AF_INET;
	saddr.sin_port = htons(1701);
	saddr.sin_addr.s_addr = INADDR_ANY;
	result = bind(fd, (struct sockaddr *) &saddr, sizeof(saddr));
	if (result < 0) {
		L2TP_DEBUG(L2TP_XPRT, "%s: bind error %s", __func__, strerror(errno));
		result = -errno;
		goto out;
	}
*/
	for(;;) {
		...
		if(result < 0) {
			...
			L2TP_DEBUG(L2TP_XPRT, "%s: connect error: %s", __func__, strerror(errno));
			result = -errno;
			goto out;
		}
		break;
	...
	if (getsockname(fd, (struct sockaddr*) &saddr, &sock_len) < 0) {
		result = -errno;
		L2TP_DEBUG(L2TP_XPRT, "%s: getsockname error: %s", __func__, strerror(errno));
		goto out;
	}
...
#ifdef CONFIG_IPV6_VPN
int l2tp_net6_get_socket_addresses(int fd, struct sockaddr_in6 *src, struct sockaddr_in6 *dest, struct l2tp_tunnel *tunnel)
{
	socklen_t sock_len;
	int result = 0;
...
int l2tp_net6_connect_socket(int fd, struct sockaddr_in6 const *peer_addr, struct l2tp_peer *peer, struct l2tp_tunnel *tunnel)
{
	struct sockaddr_in6 saddr;
	struct sockaddr_in6 daddr;
	unsigned char sbuf[48], dbuf[48], pbuf[48];
	int result;
	...
	result = l2tp_net6_get_socket_addresses(fd, &saddr, &daddr, tunnel);
	if (result < 0) {
		goto out;
	}

	L2TP_DEBUG(L2TP_DATA, "%s: tunl %hu: daddr=%s/%hu saddr=%s/%hu peer=%s/%hu", __func__,
		   l2tp_tunnel_id(tunnel),
		   inet_ntop(AF_INET6, &daddr.sin6_addr, dbuf, 48), ntohs(daddr.sin6_port), 
		   inet_ntop(AF_INET6, &saddr.sin6_addr, sbuf, 48), ntohs(saddr.sin6_port), 
		   inet_ntop(AF_INET6, &peer_addr->sin6_addr, pbuf, 48), ntohs(peer_addr->sin6_port));

	/* connect() again if peer responds on an ephemeral port */
	if (daddr.sin6_port != htons((uint16_t) l2tp_opt_udp_port)) {
		L2TP_DEBUG(L2TP_DATA, "%s: connecting to peer ephemeral port %hu, fd=%d, tid=%hu", __func__,
			   ntohs(daddr.sin6_port), fd, l2tp_tunnel_id(tunnel));
		for (;;) {
			result = connect(fd, (struct sockaddr*) &daddr, sizeof(struct sockaddr_in6));
			if(result < 0) {
				if ((errno == EINTR) || (errno == ERESTART)) {
					continue;
				}
	...
	result = usl_fd_modify_fd(fd, l2tp_net6_recv, tunnel);
	if (result < 0) {
		l2tp_log(LOG_ERR, "tunl %hu: unable to modify socket %d for connected state", 
			 l2tp_tunnel_id(tunnel), fd);
		goto out;
	...
	l2tp_tunnel_set_ipv6_addresses(tunnel, &saddr, &daddr);

	//if (l2tp_net_socket_connect_hook != NULL) {
	//	(*l2tp_net_socket_connect_hook)(tunnel, fd, &saddr, &daddr);
	//}

	L2TP_DEBUG(L2TP_PROTOCOL, "%s: peer %s/%hu has src %s/%hu on fd %d", __func__,
		   inet_ntop(AF_INET6, &daddr.sin6_addr, dbuf, 48), ntohs(daddr.sin6_port), 
		   inet_ntop(AF_INET6, &saddr.sin6_addr, sbuf, 48), ntohs(saddr.sin6_port), fd);
...
int l2tp_net6_create_socket(struct sockaddr_in6 *local_addr, int use_udp_checksums, const struct l2tp_tunnel *tunnel)
{
	struct sockaddr_in6 saddr;
	socklen_t sock_len = sizeof(saddr);
	int result;
	...
	new_fd = socket(PF_INET6, SOCK_DGRAM, 0);
	if (new_fd < 0) {
		L2TP_DEBUG(L2TP_XPRT, "%s: socket: %s", __func__, strerror(errno));
		result = new_fd;
		goto out;
	...
	/* Enable IPV6_PKTINFO socket message to retrieve peer address info. See l2tp_net_recv_xxx(). */
	(void) setsockopt(new_fd, SOL_IPV6, IPV6_PKTINFO, (char *)&on, sizeof(on));

	memset(&saddr, 0, sizeof(saddr));
	saddr.sin6_family = AF_INET;
	saddr.sin6_port = local_addr->sin6_port;
	saddr.sin6_addr = local_addr->sin6_addr;

	result = bind(new_fd, (struct sockaddr *) &saddr, sizeof(saddr));
	if (result < 0) {
		l2tp_log(LOG_ERR, "%s: bind %s/%hu: %s", __func__, 
			inet_ntop(AF_INET6, &saddr.sin6_addr, buf, 48), ntohs(saddr.sin6_port), strerror(errno));
		result = -errno;
		goto out;
	}
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: fd=%d for local %s/%hu", __func__, new_fd,
		   inet_ntop(AF_INET6, &saddr.sin6_addr, buf, 48), ntohs(saddr.sin6_port));
...
/* Utility function to derive source address to reach a peer.
 */
int l2tp_net6_get_local_address_for_peer(struct sockaddr_in6 *peer_addr, struct in6_addr *local_addr)
{
	int result = 0;
	int fd = -1;
	socklen_t sock_len;
	...
	fcntl(fd, F_SETFD, fcntl(fd, F_GETFD) | FD_CLOEXEC); // FD_CLOEXEC: Let child process won't inherit this fd
	memset(&sin6addr, 0, sizeof(sin6addr));
	sin6addr.sin6_family = AF_INET6;
	sin6addr.sin6_port = htons(1701);
	sin6addr.sin6_addr = anyaddr;
	result = bind(fd, (struct sockaddr *) &sin6addr, sizeof(sin6addr));
	if (result < 0) {
		result = -errno;
		goto out;
	}
	...
	for(;;) {
		...
		if(result < 0) {
			...
			result = -errno;
			L2TP_DEBUG(L2TP_FUNC, "%s %d connect to %s fail.\n", __func__, __LINE__, 
				inet_ntop(AF_INET6, &peer_addr->sin6_addr, buff, 48));
			goto out;
		}
		break;
	}
	...
	sock_len = sizeof(struct sockaddr_in6);
	if (getsockname(fd, (struct sockaddr*) &sin6addr, &sock_len) < 0) {
		result = -errno;
		goto out;
	}
	*local_addr = sin6addr.sin6_addr;
...
int l2tp_net6_modify_socket(int fd, int use_udp_checksums, int do_pmtu_discovery)
{
	int result = 0;
	int on;
	...
	if (do_pmtu_discovery >= 0) {
		on = do_pmtu_discovery ? IPV6_PMTUDISC_DO : IPV6_PMTUDISC_DONT;
		result = setsockopt(fd, SOL_IPV6, IPV6_MTU_DISCOVER, &on, sizeof(on));
		if (result < 0) {
			result = -errno;
			goto out;
		}
...
/*
 * l2tp_net_init opens a l2tp server
 */
...
{
	usl_fd_remove_fd(l2tp_fd);
	l2tp_net_cleanup();
}

int getServerSocketStatus()
{
	return(l2tp_fd >= 0);
}
