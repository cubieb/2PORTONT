...
{
	if (0 == peer->flag) {
		L2TP_DEBUG(L2TP_FUNC, "%s: %x/%x", __func__, ntohl(peer->if_local_addr.ip.s_addr), ntohl(peer->if_peer_addr.ip.s_addr));
	}
#ifdef CONFIG_IPV6_VPN
	else {
		char sbuf[48], dbuf[48];
		L2TP_DEBUG(L2TP_FUNC, "%s: %s/%s", __func__, 
			inet_ntop(AF_INET6, &peer->if_local_addr.ip6, sbuf, 48), 
			inet_ntop(AF_INET6, &peer->if_peer_addr.ip6, dbuf, 48));
	}
#endif
...
struct l2tp_peer *l2tp_peer_find(struct in_addr const *src, struct in_addr const *dest)
{
	...
	usl_list_for_each(walk, tmp, &l2tp_peer_list) {
		...
		if (((src == NULL) || (peer->if_local_addr.ip.s_addr == src->s_addr)) &&
		    ((dest == NULL) || (peer->if_peer_addr.ip.s_addr == dest->s_addr))) {
			return peer;
		}
	}
...
void l2tp_peer_inc_use_count(struct l2tp_peer *peer)
{
	peer->use_count++;
	if (0 == peer->flag) {
		L2TP_DEBUG(L2TP_FUNC, "%s: %x/%x: count now %d", __func__, ntohl(peer->if_local_addr.ip.s_addr), 
			   ntohl(peer->if_peer_addr.ip.s_addr), peer->use_count);
	}
#ifdef CONFIG_IPV6_VPN
	else {
		char sbuf[48], dbuf[48];
		L2TP_DEBUG(L2TP_FUNC, "%s: %s/%s: count now %d", __func__, 
				inet_ntop(AF_INET6, &peer->if_local_addr.ip6, sbuf, 48), 
				inet_ntop(AF_INET6, &peer->if_peer_addr.ip6, dbuf, 48),
				peer->use_count);
	}
...
void l2tp_peer_dec_use_count(struct l2tp_peer *peer)
{
	...
	if (peer->use_count < 0) {
		l2tp_log(LOG_ERR, "Peer %x/%x use count gone negative! Caller %p",
			 ntohl(peer->if_local_addr.ip.s_addr), ntohl(peer->if_peer_addr.ip.s_addr), __builtin_return_address(0));
		return;
	}
	if (peer->use_count == 0) {
	...
	if (peer->use_count != 0) {
		if (0 == peer->flag) {
			l2tp_log(LOG_ERR, "Free peer %x/%x when use_count=%d", 
				 ntohl(peer->if_local_addr.ip.s_addr), ntohl(peer->if_peer_addr.ip.s_addr), peer->use_count);
		}
#ifdef CONFIG_IPV6_VPN
		else {
			l2tp_log(LOG_ERR, "Free peer %s/%s when use_count=%d", 
				 inet_ntop(AF_INET6, &peer->if_local_addr.ip6, sbuf, 48), 
				 inet_ntop(AF_INET6, &peer->if_peer_addr.ip6, sbuf, 48), 
				 peer->use_count);
		}
#endif
		return;
...
struct l2tp_peer *l2tp_peer_alloc(struct in_addr src, struct in_addr dest)
{
	...
	peer->flag = 0;
	peer->if_local_addr.ip.s_addr = src.s_addr;
	peer->if_peer_addr.ip.s_addr = dest.s_addr;
...
struct l2tp_peer *l2tp_ipv6_peer_find(struct in6_addr const *src, struct in6_addr const *dest)
{
	struct l2tp_peer *peer;
	struct usl_list_head *tmp;
	struct usl_list_head *walk;
	...
	usl_list_for_each(walk, tmp, &l2tp_peer_list) {
		peer = usl_list_entry(walk, struct l2tp_peer, list);
		if ( (peer->flag == 1)/*IPv6 Peer*/ && 
			((src == NULL) || ipv6_addr_equal(&peer->if_local_addr.ip6, src)) &&
		    ((dest == NULL) || ipv6_addr_equal(&peer->if_peer_addr.ip6, dest))) {
			return peer;
		}
	}
...
struct l2tp_peer *l2tp_ipv6_peer_alloc(struct in6_addr src, struct in6_addr dest)
{
	struct l2tp_peer *peer;
	char sbuf[48], dbuf[48];
	...
	peer->flag = 1;
	peer->if_local_addr.ip6 = src;
	peer->if_peer_addr.ip6 = dest;

	USL_LIST_HEAD_INIT(&peer->list);

	L2TP_DEBUG(L2TP_FUNC, "%s: %s/%s", __func__, 
		inet_ntop(AF_INET6, &src, sbuf, 48), inet_ntop(AF_INET6, &dest, dbuf, 48));
...
struct l2tp_peer_profile *l2tp_peer_profile_find_by_addr(struct in_addr peer_addr)
{
	...
	usl_list_for_each(walk, tmp, &l2tp_peer_profile_list) {
		profile = usl_list_entry(walk, struct l2tp_peer_profile, list);
		if (profile->peer_addr.in.sin_addr.s_addr == peer_addr.s_addr) {
		...
		if ((profile->peer_addr.in.sin_addr.s_addr & profile->netmask.s_addr) == (peer_addr.s_addr & profile->netmask.s_addr)) {
...
#ifdef CONFIG_IPV6_VPN
struct l2tp_peer_profile *l2tp_peerin6_profile_find_by_addr(struct in6_addr peer_addr)
{
	struct l2tp_peer_profile *profile;
	struct usl_list_head *tmp;
	struct usl_list_head *walk;
	...
	L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s", __func__, inet_ntop(AF_INET6, &peer_addr, buff, 48));
	usl_list_for_each(walk, tmp, &l2tp_peer_profile_list) {
		profile = usl_list_entry(walk, struct l2tp_peer_profile, list);
		if (profile->ipType != 1)
			continue;
		if (ipv6_addr_equal(&profile->peer_addr.in6.sin6_addr, &peer_addr)) {
			L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s: profile %s is exact match", __func__, 
				   inet_ntop(AF_INET6, &peer_addr, buff, 48), profile->profile_name);
			return profile;
		}
	}

	L2TP_DEBUG(L2TP_FUNC, "%s: peer_addr=%s: no profile found", __func__, 
		   inet_ntop(AF_INET6, &peer_addr, buff, 48));
...
static int l2tp_peer_profile_fill(struct l2tp_api_peer_profile_msg_data *msg, struct l2tp_peer_profile *profile)
{
	...
	if (msg->flags & L2TP_API_PEER_PROFILE_FLAG_PEER_IPADDR) {
		if (0 == msg->peer_addr.flag)
			profile->peer_addr.in.sin_addr.s_addr = msg->peer_addr.s_addr.in;
#ifdef CONFIG_IPV6_VPN
		else
			memcpy(profile->peer_addr.in6.sin6_addr.s6_addr, msg->peer_addr.s_addr.in6, 16);
#endif
	}

	if (msg->flags & L2TP_API_PEER_PROFILE_FLAG_PEER_PORT) {
		if (0 == msg->peer_addr.flag)
			profile->peer_addr.in.sin_port = msg->peer_port;
#ifdef CONFIG_IPV6_VPN
		else
			profile->peer_addr.in6.sin6_port = msg->peer_port;
#endif
	}
	if (msg->flags & L2TP_API_PEER_PROFILE_FLAG_NETMASK) {
		if (0 == msg->peer_addr.flag) {
			struct in_addr addr;
			addr.s_addr = msg->netmask.s_addr.in;
...
bool_t l2tp_peer_profile_get_1_svc(char *name, l2tp_api_peer_profile_msg_data *result, struct svc_req *req)
{
	...
	result->peer_addr.flag = profile->ipType;
	if (0 == profile->ipType) {
		result->peer_addr.s_addr.in = profile->peer_addr.in.sin_addr.s_addr;
		result->peer_port = profile->peer_addr.in.sin_port;
	}
#ifdef CONFIG_IPV6_VPN
	else {
		memcpy(result->peer_addr.s_addr.in6, profile->peer_addr.in6.sin6_addr.s6_addr, 16);
		result->peer_port = profile->peer_addr.in6.sin6_port;
	}
#endif
	...
	result->netmask.s_addr.in = profile->netmask.s_addr;
...
bool_t l2tp_peer_profile_unset_1_svc(l2tp_api_peer_profile_unset_msg_data msg, int *result, struct svc_req *req)
{
	...
	if (msg.flags & L2TP_API_PEER_PROFILE_FLAG_PEER_IPADDR) {
		if (0 == profile->ipType)
			profile->peer_addr.in.sin_addr.s_addr = INADDR_ANY;
#ifdef CONFIG_IPV6_VPN
		else {
			struct in6_addr anyaddr = IN6ADDR_ANY_INIT;
			profile->peer_addr.in6.sin6_addr = anyaddr;
		}
#endif
	}
	...
	if (msg.flags & L2TP_API_PEER_PROFILE_FLAG_PEER_PORT) {
		if (0 == profile->ipType)
			profile->peer_addr.in.sin_port = 0;
#ifdef CONFIG_IPV6_VPN
		else
			profile->peer_addr.in6.sin6_port = 0;
#endif
	}
	if (msg.flags & L2TP_API_PEER_PROFILE_FLAG_NETMASK) {
		profile->netmask.s_addr = INADDR_BROADCAST;
...
bool_t l2tp_peer_get_1_svc(struct l2tp_api_ip_addr local_addr, struct l2tp_api_ip_addr peer_addr, struct l2tp_api_peer_msg_data *result, struct svc_req *req)
{
	...
	usl_list_for_each(walk, tmp, &l2tp_peer_list) {
		peer = usl_list_entry(walk, struct l2tp_peer, list);
		if (peer->flag != peer_addr.flag)
			continue;
		if ((0 == peer->flag) && (peer->if_peer_addr.ip.s_addr == peer_addr.s_addr.in) &&
		    ((local_addr.s_addr.in == INADDR_ANY) || (peer->if_local_addr.ip.s_addr == local_addr.s_addr.in))) {
			result->result_code = 0;
			result->peer_addr.flag = 0;
			result->local_addr.flag = 0;
			result->peer_addr.s_addr.in = peer->if_peer_addr.ip.s_addr;
			result->local_addr.s_addr.in = peer->if_local_addr.ip.s_addr;
			result->num_tunnels = peer->use_count;
			goto out;
		}
#ifdef CONFIG_IPV6_VPN
		else if ((1 == peer->flag) && 
			ipv6_addr_equal(&peer->if_peer_addr.ip6, (struct in6_addr *)peer_addr.s_addr.in6) &&
		    (ipv6_addr_any((struct in6_addr *)local_addr.s_addr.in6) || 
		     ipv6_addr_equal(&peer->if_local_addr.ip6, (struct in6_addr *)local_addr.s_addr.in6))) {
			result->result_code = 0;
			result->peer_addr.flag = 1;
			result->local_addr.flag = 1;
			memcpy(result->peer_addr.s_addr.in6, peer->if_peer_addr.ip6.s6_addr, 16);
			memcpy(result->local_addr.s_addr.in6, peer->if_local_addr.ip6.s6_addr, 16);
			result->num_tunnels = peer->use_count;
			goto out;
		}
#endif
...
bool_t l2tp_peer_list_1_svc(struct l2tp_api_peer_list_msg_data *result, struct svc_req *req)
{
	...
	usl_list_for_each(walk, tmp, &l2tp_peer_list) {
		...
		entry->peer_addr.flag = peer->flag;
		entry->local_addr.flag = peer->flag;
		if (0 == peer->flag) {
			entry->peer_addr.s_addr.in = peer->if_peer_addr.ip.s_addr;
			entry->local_addr.s_addr.in = peer->if_local_addr.ip.s_addr;
		}
#ifdef CONFIG_IPV6_VPN
		else {
			memcpy(entry->peer_addr.s_addr.in6, peer->if_peer_addr.ip6.s6_addr, 16);
			memcpy(entry->local_addr.s_addr.in6, peer->if_local_addr.ip6.s6_addr, 16);
		}
#endif
