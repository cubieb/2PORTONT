...
int (*l2tp_session_created_pppd_hook)(uint16_t tunnel_id, uint16_t session_id) = NULL;
...
int l2tp_session_get_fd(struct l2tp_session const *session)
{
	return session->fd;
}

void l2tp_session_set_fd(struct l2tp_session *session, int fd)
{
	session->fd = fd;
}
...
static void l2tp_session_fsm_log(struct usl_fsm_instance const *fsmi, int level, const char *fmt, ...)
{
	...
	if (1/*session->config.trace_flags & L2TP_FSM*/) {
		va_list ap;
...
static int l2tp_session_created_ind(struct l2tp_tunnel *tunnel, struct l2tp_session *session)
{
	...
	if(l2tp_session_created_pppd_hook != NULL){
		result = (*l2tp_session_created_pppd_hook)(l2tp_tunnel_id(tunnel), session->status.session_id);
	}
...
#if defined(L2TP_FEATURE_LNIC_SUPPORT) || defined(L2TP_FEATURE_LAOC_SUPPORT)
...
#ifdef CONFIG_LUNA
	//printf l2tp tunnelId and sessionId information for diag cmd user: Luke
	if(result>=0)
	{
		int entrynum, i;
		MIB_L2TP_T entry;
		unsigned int serverip;
		unsigned short outer_port,tunnel_id,session_id;
		unsigned short gateway_outer_port,gateway_tunnel_id,gateway_session_id;
		entrynum = mib_chain_total(MIB_L2TP_TBL);
		struct sockaddr *sa;
        struct sockaddr_in *addr;
		printf("#############L2TP INFO:################\n");
		sa=l2tp_tunnel_get_local_addr(tunnel);
		if(sa->sa_family == AF_INET) {
			addr = (struct sockaddr_in *)sa;
			printf("###Client addr =%x\n", addr->sin_addr.s_addr);
			printf("###Outer port: %d\n",addr->sin_port);
			printf("###Tunnel ID: %hu(0x%x)\n",l2tp_tunnel_id(tunnel),l2tp_tunnel_id(tunnel));
			printf("###Session ID: %hu(0x%x)\n",session->status.session_id,session->status.session_id);
			outer_port = addr->sin_port;
		...
		{
			printf("FIXME: RG should support IPV6 here..\n");
		}
#endif
		sa=l2tp_tunnel_get_peer_addr(tunnel);
		if(sa->sa_family == AF_INET) {
			addr = (struct sockaddr_in *)sa;
			printf("###Gateway addr =%x\n", addr->sin_addr.s_addr);
			printf("###Gateway Outer port: %d\n",addr->sin_port);
			printf("###Gateway Tunnel ID: %hu(0x%x)\n",l2tp_tunnel_peer_id(tunnel),l2tp_tunnel_peer_id(tunnel));
			printf("###Gateway Session ID: %hu(0x%x)\n",session->status.peer_session_id,session->status.peer_session_id);
			gateway_outer_port = addr->sin_port;
			gateway_tunnel_id = l2tp_tunnel_peer_id(tunnel);
			gateway_session_id = session->status.peer_session_id;
		}
#ifdef CONFIG_IPV6_VPN		
		else
		{
		...
		for (i=0; i<entrynum; i++)
		{
			...
			//printf("server ip=%s hex=0x%x\n",entry.server,serverip);
			//printf("GatewayIP=0x%x\n", addr->sin_addr.s_addr);
			if(serverip == addr->sin_addr.s_addr){
				entry.outer_port=outer_port;
				entry.tunnel_id=tunnel_id;
				entry.session_id=session_id;
				entry.gateway_outer_port=gateway_outer_port;
				entry.gateway_tunnel_id=gateway_tunnel_id;
				entry.gateway_session_id=gateway_session_id;
				mib_chain_update(MIB_L2TP_TBL, &entry, i);
				break;
			}
		}
...
static void l2tp_session_unlink(struct l2tp_session *session, int force)
{
	...
	if ((session->use_count == 0) || force) {
		...
		if (session->config.session_name)
			sscanf(session->config.session_name, "session_%d", &sessionIdx);
		
		l2tp_session_free(session);

		//session is freed, we should release ppp conn too.
		if (-1 == sessionIdx)
		{
			printf("%s %d, error meet, session name unknown %s\n", __func__, __LINE__, 
					session->config.session_name);
		}
		else
		{
			extern void stop_ppp_interface(int pppUnit);
			pppUnit = 11 + sessionIdx;
			stop_ppp_interface(pppUnit);
		}
	}
}
...
void l2tpd_session_update_state(struct sppp *sp)
{
	L2TPD_DRV_CTRL *p = (L2TP_DRV_CTRL *)sp->pp_lowerp;
	struct l2tp_tunnel *tunnel;
	struct l2tp_session *session;
	struct usl_fsm_instance *fsmi;
	...
	if (NULL == tunnel) {
		printf("%s %d fatal error, tunnel is NULL.\n", __func__, __LINE__);
	...
	if (NULL == session)
	{
		printf("%s %d fatal error, session is NULL.\n", __func__, __LINE__);
		p->call_state = L2TP_ST_DEAD;
	}
	else
	{
		fsmi = &session->fsmi;
		if (fsmi && (L2TP_LNIC_STATE_ESTABLISHED == fsmi->state)) {
			p->call_state = L2TP_ST_SESSION_ESTABLISH;
		}
	}
}
#endif

void l2tp_session_update_state(struct sppp *sp)
{
	L2TP_DRV_CTRL *p = (L2TP_DRV_CTRL *)sp->pp_lowerp;
	struct vpn_param_s *vpnr = (struct vpn_param_s *)sp->dev;
	struct l2tp_tunnel *tunnel;
	struct l2tp_session *session;
	struct usl_fsm_instance *fsmi;
	...
	{
		fsmi = &session->fsmi;
		if (fsmi && (L2TP_LAIC_STATE_ESTABLISHED == fsmi->state)) {
			p->session_id = session->status.session_id;
			p->call_state = L2TP_ST_SESSION_ESTABLISH;
		}
	}
}
...
void l2tp_session_init(void)
{
	...
	fcntl(l2tp_session_event_pipe[0], F_SETFL, O_NONBLOCK);
	fcntl(l2tp_session_event_pipe[1], F_SETFL, O_NONBLOCK);
	//printf("usl_fd_add_fd %d callback l2tp_session_do_queued_event\n", l2tp_session_event_pipe[0]);
