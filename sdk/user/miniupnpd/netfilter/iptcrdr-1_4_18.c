/* $Id: iptcrdr.c,v 1.2 2010/06/30 07:15:23 jiunming Exp $ */
/* MiniUPnP project
	...
	else
	{
			...
			e = iptc_next_rule(e, h))
		{
			if(proto==e->ip.proto)
			{
				...
				target = ipt_get_target((struct ipt_entry *)e);
				mr = (const struct nf_nat_ipv4_multi_range_compat *)&target->data[0];
...
static int
delete_rule_and_commit(unsigned int index, struct xtc_handle *h,
...
get_dnat_target(const char * daddr, unsigned short dport)
{
	...
	/* one nf_nat_ipv4_range already included in nf_nat_ipv4_multi_range_compat */
	mr = (struct nf_nat_ipv4_multi_range_compat *)&target->data[0];
	mr->rangesize = 1;
	range = &mr->range[0];
