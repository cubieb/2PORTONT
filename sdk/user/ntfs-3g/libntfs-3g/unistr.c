...
#ifdef CONFIG_E8B
// rewrite from ntfs_ucstombs
static ntfs_unicode2gbk(const ntfschar *ins, const int ins_len, char **outs, int outs_len)
{
	char *mbs;
	int  mbs_len;
	wchar_t wc[2];
	...
	// need enlarge ins_len to 2 times 
	mbs_len = (ins_len + 1) * MB_CUR_MAX * 2 ;
	...
	for (i = o = 0; i < ins_len; i++) 
	{
		...
		wc[0] = (wchar_t)le16_to_cpu(ins[i]);
		if (!wc[0])
				break;
		if( (i < ins_len-1) && (wc[0] == 0xD840) )
		{
			i++;
			wc[1] = (wchar_t)le16_to_cpu(ins[i]);
			if (!wc[1])
					break;
		}
		
		/* Convert the unicode wide character to GBK multibyte. */
		cnt = Unicode2GBK(wc, mbs + o,4);
		if (cnt == -1)
				goto err_out;
		if (cnt <= 0)
		{
...
// rewrite from ntfs_mbstoucs
static int ntfs_gbk2unicode(const char *ins, ntfschar **outs)
{
	ntfschar *ucs;
	const char *s;
	wchar_t wc[2];
	...
	for (i = o = cnt = 0; i < ins_size; i += cnt, o++) 
	{
		...
		/* Convert the GBK multibyte character to a unicode wide character. */
		cnt = GBK2Unicode(ins + i, ins_size - i,wc);
		if (!cnt)
			break;
		if (cnt == -1)
			goto err_out;
		...
		if ((unsigned long)wc[0] >= (unsigned long)(1 <<(8 * sizeof(ntfschar)))) 
		{
			errno = EILSEQ;
			goto err_out;
		}
		/* Convert the CPU wide character to a LE Unicode character. */
		ucs[o] = cpu_to_le16(wc[0]);
		if (wc[0] == 0xD840)
		{
			if ((unsigned long)wc[1] >= (unsigned long)(1 <<(8 * sizeof(ntfschar)))) 
			{
				errno = EILSEQ;
				goto err_out;
			}
			o++;
			ucs[o] = cpu_to_le16(wc[1]);
		}
	}
		...
		int outs_len)
{
	...
	if(use_utf8)
	{
		return ntfs_unicode2gbk(ins, ins_len, outs, outs_len);
	}
#endif
...
int ntfs_mbstoucs(const char *ins, ntfschar **outs)
{
	...
	if(use_utf8)
	{
		return ntfs_gbk2unicode(ins,outs);
	}
#endif
