...
"$Id: pppoe.c,v 1.2 2011/07/14 10:17:24 jenny Exp $";
...
asyncReadFromEth(int sock, int clampMss)
{
    ...
    if (DebugFile) {
	fprintf(DebugFile, "asyncReadFromEth ");
	fprintf(DebugFile, "RCVD ");
	dumpPacket(DebugFile, &packet);
	fprintf(DebugFile, "\n");
	fflush(DebugFile);
...
#ifdef CONFIG_USER_PPPOE_PROXY
	//ramen--20080805
	//let pppoe know which port the incoming packet is from?
	if (fromport < 0) {
		ioctl(sock, SIOCGFORMPORT, &fromport);
		ioctl(1, TIOCSFROMPORT, &fromport);
	}
#endif
...
syncReadFromEth(int sock, int clampMss)
{
    ...
    if (DebugFile) {
	fprintf(DebugFile, "syncReadFromEth ");
	fprintf(DebugFile, "RCVD ");
	dumpPacket(DebugFile, &packet);
	fprintf(DebugFile, "\n");
	fflush(DebugFile);
