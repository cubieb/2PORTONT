...
*
* $Id: pppoe-server.c,v 1.6 2011/07/14 10:17:06 jenny Exp $
*
***********************************************************************/
...
unsigned char LocalIP[IPV4ALEN] = {192, 168, 1, 1};
unsigned char RemoteIP[IPV4ALEN] = {192, 168, 1, 168}; /* Counter STARTS here */
...
//alex 
int enable_pppoe_server = 0;

#define HOSTNAMELEN 256

static unsigned int localipaddr = 0;
static unsigned char localmacaddr[6] = {0};
...
findSession(pid_t pid)
{
	...
	for (i = 0; i < NumSessionSlots; i ++) {
		if (Sessions[i].pid == pid) {
			//ramen 2008/07/16-fix the bug that pppoe-server may alloc one ip which has been used by other pc.
			if (arpping(*((int *)(&Sessions[i].ip)), localipaddr, localmacaddr, IfName) != 0)
				return &Sessions[i];
			else
...
//#define PPPOE_SERVER_RUNFILE		"/var/run/pppoe-server.pid"
static void log_pid()
{
	FILE *f;
	pid_t pid;
	char *pidfile = PPPOE_SERVER_RUNFILE;
...
static void no_lan_pppoe(int signum){
	fprintf(stderr,"pppoe server process dial signal %x sigusr1\n", signum);
	enable_pppoe_server = 0;
}
static void lan_pppoe_enable(int signum ){
	fprintf(stderr,"pppoe server process dial signal %x sigusr2\n", signum);
	enable_pppoe_server = 1;
...
main(int argc, char **argv)
{
    ...
    while((opt = getopt(argc, argv, "hI:C:L:R:T:m:FN:f:s:W")) != -1) {
	switch(opt) {
	case 's':
	    Synchronous = 1;
	...
	openlog("pppoe-server", LOG_PID, LOG_DAEMON);
      ...
      signal(SIGUSR1, no_lan_pppoe);
      signal(SIGUSR2, lan_pppoe_enable);
    for(i=0; i<NumSessionSlots; i++) {
	Sessions[i].pid = 0;
    ...
    read_interface(IfName, &localipaddr, localmacaddr);
	...
	switch(packet.code) {
	case CODE_PADI:
	    if(enable_pppoe_server) //alex enable pppoe_server	
...
startPPPD(struct ClientSession *session)
{
 ...
 //   argv[9] = "nodeflate";
    ...
    argv[9] = "novj";
    argv[10] = "novjccomp";
    argv[11] = "default-asyncmap";
    argv[12] ="session";
    snprintf(buffer2, SMALLBUF, "%d:%02x:%02x:%02x:%02x:%02x:%02x",
		session->sess,
	        session->eth[0], session->eth[1], session->eth[2],
		session->eth[3], session->eth[4], session->eth[5]);

    
    argv[13] = strdup(buffer2);
    if (Synchronous) {
	argv[14] = "sync";
	argv[15] = NULL;
    } else {
