...
#if defined(CONFIG_USER_BOA_SRC_BOA)
...
static int do_command(const char *path, const char *filename, int dowait)
{
	...
	if((pid = vfork()) == 0) {
		...
		env[0] = "PATH=/bin:/usr/bin:/sbin:/usr/sbin:/etc/scripts";
#ifdef INCLUDE_TIMEZONE
		strcpy(tz, "TZ=");
		strcat(tz, tzone);
		env[1] = tz;
...
static int do_rc(void)
{
	...
	/*star:20090414 START to mount /var first, because "/bin/flash check" will use /var*/
	rc = do_command(_PATH_BSHELL, "/etc/rc-first", 1);
	if (rc)
		return(rc);
	//10/23/05' hrchen, check if config in flash is valid and wait it finish
	//to prevent race condition when invalid flash config been reseted
#if defined(CONFIG_USER_BOA_SRC_BOA) && !defined(CONFIG_USER_XMLCONFIG)
	rc = do_command("/bin/flash", "check", 1);
	if (rc)
		return(rc);
...
int boot_single(int singlearg, int argc, char *argv[])
{
	...
	if(singlearg == 1)
	{
		do_command(_PATH_BSHELL, "/etc/rc-first", 1);
	}
...
void spawn(int i)
{
	...
	if(pid) {
		...
		env[0] = term;
		env[1] = "PATH=/bin:/usr/bin:/sbin:/usr/sbin:/etc/scripts";
#ifdef INCLUDE_TIMEZONE
		strcpy(tz, "TZ=");
		strcat(tz, tzone);
		env[2] = tz;
