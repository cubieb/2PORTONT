/*
 *   $Id: radvd.c,v 1.1 2010/02/08 07:43:12 masonyu Exp $
 *
 *   Authors:
...
main(int argc, char *argv[])
{
...
#endif
	{
        ...
        ddump(stderr, "chrootdir=%s\n", chrootdir?chrootdir:"(null)");
        ddump(stderr, "logfile=%s\n", logfile?logfile:"(null)");
        ddump(stderr, "conf_file=%s\n", conf_file?conf_file:"(null)");
        ddump(stderr, "username=%s\n", username?username:"(null)");
	...
	if (sock < 0) {
	    ddump(stderr, "open_icmpv6_socket: FAIL");
	    exit(1);
        }
	...
	if (check_conffile_perm(username, conf_file) < 0) {
		if (get_debuglevel() == 0) {
		        ddump(stderr, "check_conffile_perm()/get_debuglevel()\n");
			exit(1);
		} else {
			flog(LOG_WARNING, "Insecure file permissions, but continuing anyway");
			ddump(stderr, "Insecure file permissions, but continuing anyway\n");
                }
	}
	...
	if (check_ip6_forwarding()) {
		if (get_debuglevel() == 0) {
			...
			ddump(stderr, "IPv6 forwarding seems to be disabled, exiting\n");
			exit(1);
		}
		else {
			flog(LOG_WARNING, "IPv6 forwarding seems to be disabled, but continuing anyway.");
			ddump(stderr, "IPv6 forwarding seems to be disabled, but continuing anyway.\n");
                }
	}

	/* parse config file */
	if (readin_config(conf_file) < 0) {
	    ddump(stderr, "readin_config(): FAIL\n");
		exit(1);
        }
	...
	if (username) {
		...
		if (drop_root_privileges(username) < 0) {
		        ddump(stderr, "drop_root_privileges(): FAIL\n");
			exit(1);
                }
	}
	...
	if (fd < 0)
	{
		...
		ddump(stderr, "open: cannot create radvd pid file, terminating: %s\n", strerror(errno));
		exit(1);
	}
	...
	if (get_debuglevel() == 0) {
		...
		if (log_method == L_STDERR_SYSLOG)
			log_method = L_SYSLOG;
		if (log_open(log_method, pname, logfile, facility) < 0) {
		    ddump(stderr, "get_debuglevel()/log_open: FAIL\n");
		    exit(1);
                }
...
check_conffile_perm(const char *username, const char *conf_file)
{
	...
	if (!username)
        ...
        /* skip the check of if the owner is root
        {   struct passwd *pw = NULL;
    	    pw = getpwnam(username);
    	    if (stat(conf_file, st) || pw == NULL) {
    	        ddump(stderr, "stat(): FAIL\n");
    		return(-1);
            }
        }
        */

	if (stbody.st_mode & S_IWOTH) {
                flog(LOG_ERR, "Insecure file permissions (writable by others): %s", conf_file);
                ddump(stderr, "Insecure file permissions (writable by others): %s\n", conf_file);
		return(-1);
        }
	...
	if (strncmp(username, "root", 5) != 0 &&
	    ((st->st_mode & S_IWGRP && pw->pw_gid == st->st_gid) ||
	     (st->st_mode & S_IWUSR && pw->pw_uid == st->st_uid))) {
                ...
                ddump(stderr, "Insecure file permissions (writable by self/group): %s\n", conf_file);
		return(-1);
        }
        */
        return 0;
