...
#define		apsVersionCode		(1)	//for v2c
...
AsnNumberType inputVersion;	//SNMP version id

// Added by Mason Yu for write community string
char g_reqCommName[ (apsMaxNameSize + 1) ];
...
ApsIdType		apsVerify (AsnIdType asn)
{
	...
	inputVersion = asnNumber (asnValue (asnVer), n);	
	if (( inputVersion > (AsnNumberType) apsVersionCode)) {		
		return ((ApsIdType) 0);
	}
	...
	strcpy(g_reqCommName, name);
	// Commented by Mason Yu for return the correct comm string		
	//strcpy(name, "ADSL");
	...
	// Added by Mason Yu	
	//printf("ap->apsCommName(Before) = %s\n", ap->apsCommName);		
...
AsnIdType		apsEncode (ApsIdType aps, AsnIdType asn)
{
	...
	ApsCommPtrType		ap;
	AsnIdType		code;
	//AsnIdType		apsVersion;	
	// Mason Yu. Init "apsVersion" for every snmp request to support V1 and V2. Start +++++
	AsnDatumPtrType  apsVersion_ptr;
	
	// Mason Yu. In order to avoid memory leak, We should free the memory of old "apsVersion" then init another new "apsVersion"
	// and set 0 as the initial value.
	apsVersion_ptr = asnIdToPtr(apsVersion);
	asnFree(apsVersion_ptr);
	
	apsVersion = asnPtrToId(apsVersion_ptr);
	apsVersion = (AsnIdType) 0;
	// Mason Yu. Init "apsVersion" for every snmp request to support V1 and V2. End +++++
	...
	if (apsVersion == (AsnIdType) 0) {
		apsVersion = asnIntl (asnClassUniversal,
			(AsnTagType) 2, (CIntlType) inputVersion);
		if (apsVersion == (AsnIdType) 0) {
			return ((AsnIdType) 0);
		}
	}	
