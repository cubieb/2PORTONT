...
extern AsnNumberType inputVersion;  //for v2c
...
AsnIdType		mixNext (MixIdType mix, MixNamePtrType name, MixLengthPtrType namelenp, int *v2EOMV)
{
	AvlInfoType		info;
	MixLengthType		newlen;
	AsnIdType		result;
	
	if (v2EOMV!=NULL) *v2EOMV = 0;
	if (mix == (MixIdType) 0) {
		return ((AsnIdType) 0);
	}
	...
	if (info != (AvlInfoType) 0) {
		*namelenp += newlen;
	} else {  //for v2c endOfMibView
	    if (inputVersion==1) {  //v2c, responce with endOfMibView
		if (v2EOMV!=NULL) *v2EOMV = 1;
	        return asnV2Error(asnV2endOfMibView);
	    };
	};
...
AsnIdType		mixGet (MixIdType mix, MixNamePtrType name, MixLengthType namelen)
{
	...
	info = avlFind ((AvlIdType) mixCookieGet (mix),
		(AvlNamePtrType) name, (AvlLengthType) namelen);
	if (info == (AvlInfoType) 0) {  //OID not exist
	        if (inputVersion==1) {  //v2c, responce with noSuchObject
	        	return asnV2Error(asnV2noSuchObject);
	        } else 
			return ((AsnIdType) 0);
	}
	else {
		rtnVal= (*mixOpGet ((MixIdType) info))
			(mixCookieGet ((MixIdType) info),
			name + mixLenGet ((MixIdType) info),
			namelen - mixLenGet ((MixIdType) info));
		if (rtnVal == (AsnIdType)0) {  //get value fail
	        	if (inputVersion==1) {  //v2c, responce with noSuchInstance
	        		return asnV2Error(asnV2noSuchInstance);
	        	}
	        };
		return rtnVal;
	}
