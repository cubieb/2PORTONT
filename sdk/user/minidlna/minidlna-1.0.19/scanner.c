...
//add by siyuan
int isEmptyDirectory(const char * path)
{
	struct dirent **namelist;
	int i, n, result = 0;
	n = scandir(path, &namelist, filter_media, alphasort);
	if(n <= 0)
	{
		result =1;
		DPRINTF(E_WARN, L_SCANNER, "scandir %s, inode number:%d, empty dir not added!\n", path,n);
	}
	else
	{
		for (i=0; i < n; i++)
...
ScanDirectory(const char * dir, const char * parent, enum media_types dir_type)
{
	...
	for (i=0; i < n; i++)
	{
		...
		if( type == TYPE_DIR )
		{
#if 1//filter empty directory in mnt/
			if(parent || !isEmptyDirectory(full_path))
			{
				insert_directory(name?name:namelist[i]->d_name, full_path, BROWSEDIR_ID, (parent ? parent:""), index+startID);
				sprintf(parent_id, "%s$%X", (parent ? parent:""), index+startID);
				DPRINTF(E_WARN, L_SCANNER, "scandir %s, parent id is %s!\n", full_path,parent_id);
		...
		{
#if 1
			if( insert_file(name?name:namelist[i]->d_name, full_path, (parent ? parent:""), index+startID) == 0 )
				fileno++;
			index++;
#else
