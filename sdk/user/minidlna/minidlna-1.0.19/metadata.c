...
GetImageMetadata(const char * path, char * name)
{
...
#endif
	{
		...
		ret = sql_exec(db, "INSERT into DETAILS"
                   " (PATH, TITLE, SIZE, TIMESTAMP, RESOLUTION, DLNA_PN, MIME) "
                   "VALUES"
                   " (%Q, '%q', %lld, %ld, %Q, %Q, %Q);",
                   path, name, file.st_size, file.st_mtime, m.resolution, m.dlna_pn, m.mime);
...
GetVideoMetadata(const char * path, char * name)
{
...
#if 1
	//siyuan don't parse all kinds of vedio files	
	strcpy(nfo, path);
	ext = strrchr(nfo, '.');
	if( ext )
	{
	...
	if(ends_with(path, ".avi") || ends_with(path, ".divx"))
	...
	else if(ends_with(path, ".mpeg") || ends_with(path, ".mpg"))
		asprintf(&m.mime, "video/mpeg");
	else if(ends_with(path, ".3gp"))
		asprintf(&m.mime, "video/3gpp");
	else if(ends_with(path, ".wmv") || ends_with(path, ".asf"))
	...
	if( video_stream >= 0 )
	{
		...
		else if( strcmp(ctx->iformat->name, "mov,mp4,m4a,3gp,3g2,mj2,m2v") == 0 &&
		         ends_with(path, ".mov") )
			asprintf(&m.mime, "video/quicktime");
