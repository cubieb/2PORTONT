...
//#define IPRELAY_RUNFILE "/var/run/ip_relay.pid"
static void log_pid(int index)
{
        FILE *f;
        pid_t pid;
	char pidfile[64];
	snprintf(pidfile, sizeof(pidfile), "/var/run/ip_relay-%d.pid", index);
        pid = getpid();
