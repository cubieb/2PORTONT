            ...
            if ((strstr(str, "directory")) && (strchr(str, '{')) && (directories)) {
                ...
                *strchr(tmp, '"') = 0;
                ...
                create_options(configfile, &(dir->options), NULL);
       			...
       			sscanf(str, "%[^=]=\"%[^\n\"]", opt->name, opt->value);
				DEBUG_PRINT("opt->name=%s; opt->value=%s\n", opt->name, opt->value);
            }
   		}
		str = config_read_line(configfile);
	}
...
//    char *result;
	char *foo;
	DEBUG_PRINT("config_getoption(%s);userinfo_set=%d", name, userinfo_set);
    if (userinfo_set) {
        if ((foo = getoption_user(name)))
		{
			DEBUG_PRINT("getoption_user(%s);foo=%s", name, foo);
            return foo;
		}
        if ((foo = getoption_group(name)))
		{
			DEBUG_PRINT("getoption_group(%s);foo=%s", name, foo);
            return foo;
		}
    }
