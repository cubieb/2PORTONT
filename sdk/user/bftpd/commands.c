...
int pasvsock;
char *philename = NULL;
off_t offset = 0; /* lijian: 20080904: Bug id 0003809: bftpd not support >4G file */
...
void do_stor(char *filename, int flags)
{
    ...
    */
	...
	if (!((i = recv(sock, buffer, my_buffer_size - 1, 0))))
        ...
        #endif
           if(! attempt_gzip)
	#ifdef BFTPD_ROOTDIR_UNWRITABLE
	   if(!isRootDir())
	#endif
              write(fd, buffer, i);
...
/* lijian: 20080904 START: Bug id 0003809: bftpd not support >4G file */
void command_rest(char *params)
{
    offset = (off_t)strtoull(params, NULL, 10);
	control_printf(SL_SUCCESS, sizeof(offset) == 8 ? "350 Restarting at offset %llu." : "350 Restarting at offset %u.", offset);
}
...
void command_size(char *filename)
{
	...
	if (!stat(mapped, &statbuf)) {
		control_printf(SL_SUCCESS, sizeof(statbuf.st_size) == 8 ? "213 %llu." : "213 %u.", statbuf.st_size);
	} else {
