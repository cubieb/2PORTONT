 ...
 *
 *
 * $Id: docfdisk.c,v 1.1 2008/03/03 07:28:32 ql Exp $
 *
...
"  partitions).  Last size = 0 means go to end of device.\n",
	...
	if (meminfo.oobsize == 16)
		bs = 512;
	else if (meminfo.oobsize == 8)
		bs = 256;
	else if (meminfo.oobsize == 64)
		bs = 2048;

	buf = malloc(meminfo.erasesize);
	oobbuf = malloc((meminfo.erasesize / bs) * meminfo.oobsize);
	if (!buf || !oobbuf) {
	...
	oob.ptr = oobbuf;
	oob.start = mhoffs;
	for (i = 0; i < meminfo.erasesize; i += bs) {
...
"-------------------------------------------------------------------------\n");
	...
	for (i = 0; i < meminfo.erasesize; i += bs) {
		...
		if ((ret = pwrite(fd, buf, bs, oob.start)) < 0) {
			perror("Write page");
			printf("Your MediaHeader may be hosed.  UHOH!\n");
			return 1;
		}
