...
int do_command(int argc, char *argv[], char **table, iptc_handle_t *handle)
{
	...
	#ifdef CONFIG_RAMEN_DEBUG
			printf("sizeof(struct ipt_entry_target)=%d,IPT_ALIGN(sizeof(struct ipt_entry_target))=%d,target->size=%d\n",
			sizeof(struct ipt_entry_target),IPT_ALIGN(sizeof(struct ipt_entry_target)),target->size);
			printf("target->t->u.target_size=%d\n",target->t->u.target_size);
	#endif
