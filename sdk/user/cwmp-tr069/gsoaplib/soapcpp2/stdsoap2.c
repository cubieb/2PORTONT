...
{ 
	...
	if(soap->keys==NULL)
	{
		...
		if (matrixSslNewKeys(&soap->keys) < 0) {
		...
		if (matrixSslLoadRsaKeys(soap->keys, soap->keyfile, soap->keyfile, soap->password, soap->cafile) < 0)  {
...
#endif//end of CONFIG_USER_CWMP_WITH_MATRIXSSL_NEW
	}
	return SOAP_OK;
}
#endif
...
//star:20100825 IPV6_RELATED END
  ...
  {
	struct ifreq ifr = {0};
	strcpy(ifr.ifr_name, soap->itfname);
	if (setsockopt((SOAP_SOCKET)fd, SOL_SOCKET, SO_BINDTODEVICE, (char *)(&ifr), sizeof(ifr)))
	{
		soap->errnum = soap_socket_errno;
...
/*ping_zhang:20090209 END*/
  ...
  if (!strncmp(endpoint, "https:", 6))
  {
    ...
    { X509 *peer;
      ...
      X509_free(peer);
      if (soap_tag_cmp(host, soap->msgbuf))
...
#ifdef CONFIG_USER_CWMP_WITH_MATRIXSSL_NEW
		matrixSslDeleteSessionId(soap->sessionId);
#else
