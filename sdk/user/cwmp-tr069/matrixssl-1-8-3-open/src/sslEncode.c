...
/*jiunming*/
static int32 writeVerify(ssl_t *ssl, sslBuf_t *out);
...
/*jiunming*/
							...
							(2 * ssl->hshakeHeadLen);
					{
						...
						while( cert!=NULL )
						{
							messageSize += 3+cert->certLen;
							cert = cert->next;
						}
					 	//+ ssl->keys->cert.certLen
					}
					messageSize += 2 + ssl->keys->cert.privKey->size;
#else
...
*/
			...
			/*jiunming*/
			if (rc == SSL_SUCCESS) {
				if( (ssl->flags&SSL_FLAGS_CLIENT_AUTH) && (ssl->sec.certMatch>0) ) {
					rc = writeVerify(ssl, out);
				}
			}
			if (rc == SSL_SUCCESS) {
				rc = writeChangeCipherSpec(ssl, out);
...
static int32 writeVerify(ssl_t *ssl, sslBuf_t *out)
{
	unsigned char	*c, *end, *encryptStart;
	char		padLen;
	int32		messageSize, verifyLen, rc, keysize;
	unsigned char	ihash[SSL_MD5_HASH_SIZE + SSL_SHA1_HASH_SIZE];

	c = out->end;
	end = out->buf + out->size;
	keysize = ssl->keys->cert.privKey->size;
	verifyLen = 2 + keysize;
	messageSize = ssl->recordHeadLen + ssl->hshakeHeadLen + verifyLen;
...
/*
	Output the hash of messages we've been collecting so far into the buffer
*/
	if( sslSnapshotHSHash(ssl, ihash, -1)!=sizeof(ihash) )
	{
		fprintf( stderr, "sslSnapshotHSHash error\n" );
		return SSL_ERROR;
	}

	*c++ = (unsigned char)((keysize & 0xFF00) >> 8);
	*c++ = (unsigned char)(keysize & 0xFF);
	//fprintf( stderr, "keysize:%d, %02x%02x\n", keysize, *(c-2), *(c-1) );
	
	if( matrixRsaEncryptPriv( ssl->hsPool, ssl->keys->cert.privKey,ihash, sizeof(ihash), c, keysize  )<0 )
	{
		fprintf( stderr, "matrixRsaEncryptPriv error\n" );
		return SSL_ERROR;
	}
