...
*/
#define POOL_SIGN_192		(5 * 1024) + PS_OH 
#define POOL_VERIFY_192		(5 * 1024) + PS_OH
#define POOL_MAKE_KEY_192	(5 * 1024) + PS_OH
#define POOL_MISC_192		(1 * 1024) + PS_OH

#define POOL_SIGN_224		(5 * 1024) + PS_OH 
#define POOL_VERIFY_224		(5 * 1024) + PS_OH
#define POOL_MAKE_KEY_224	(5 * 1024) + PS_OH
#define POOL_MISC_224		(1 * 1024) + PS_OH

#define POOL_SIGN_256		(6 * 1024) + PS_OH 
#define POOL_VERIFY_256		(6 * 1024) + PS_OH
#define POOL_MAKE_KEY_256	(5 * 1024) + PS_OH
#define POOL_MISC_256		(1 * 1024) + PS_OH

#define POOL_SIGN_384		(7 * 1024) + PS_OH 
#define POOL_VERIFY_384		(7 * 1024) + PS_OH
#define POOL_MAKE_KEY_384	(7 * 1024) + PS_OH
#define POOL_MISC_384		(1 * 1024) + PS_OH

#define POOL_SIGN_521		(7 * 1024) + PS_OH 
#define POOL_VERIFY_521		(7 * 1024) + PS_OH
#define POOL_MAKE_KEY_521	(7 * 1024) + PS_OH
#define POOL_MISC_521		(1 * 1024) + PS_OH
...
int main(int argc, char **argv)
{
	...
	tsize = 0;
#ifdef DO_SECP256R1
	tsize += 16; /* 4 key exponents and 4 time measurements */
#endif
...
#ifdef USE_PEERSEC_MEMORY_MANAGEMENT
/*
		Each key uses a 'misc' pool to keep the measurement pools clean
*/
		misc = psOpenPool("misc", keys[i].poolMisc, 0, NULL);
#endif	
		_psTraceStr("Test %s...\n", keys[i].name);
#ifdef STATS
		...
		psGetEntropy(in, SHA1_HASH_SIZE);
		...
		iter = 0;
#ifdef USE_PEERSEC_MEMORY_MANAGEMENT
		pool = psOpenPool(keys[i].name, keys[i].poolMakeKey, 0, NULL);
#endif		
		psGetTime(&start);
		...
		_psTraceInt(TIME_UNITS "/keyGen\n",
			t = psDiffMsecs(start, end)/keys[i].iter);
#ifdef STATS
		...
		iter = 0;
#ifdef USE_PEERSEC_MEMORY_MANAGEMENT
		pool = psOpenPool(keys[i].name, keys[i].poolSign, 0, NULL);
#endif
		psGetTime(&start);
		while (iter < keys[i].iter) {
		...
		iter = 0;
#ifdef USE_PEERSEC_MEMORY_MANAGEMENT
		pool = psOpenPool(keys[i].name, keys[i].poolVerify, 0, NULL);
#endif
		psGetTime(&start);
		while (iter < keys[i].iter) {
