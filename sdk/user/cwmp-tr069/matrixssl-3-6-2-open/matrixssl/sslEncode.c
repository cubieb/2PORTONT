...
static int32 nowDoSkePka(ssl_t *ssl)
{
	...
	if (pka->type == PKA_AFTER_RSA_SIG_GEN_ELEMENT ||
		...
		if (ssl->flags & SSL_FLAGS_TLS_1_2) {
			if ((rc = privRsaEncryptSignedElement(pkiPool, ssl->keys->privKey,
				...
				psFree(pka->inbuf); pka->inbuf = NULL;	
				pka->type = 0; 
				return PS_PENDING;
			}
		} else {
	...
	if (pka->type == PKA_AFTER_ECDSA_SIG_GEN) {
		...
		if ((err = psEccSignHash(pkiPool, pka->inbuf, pka->inlen, pka->outbuf,
				pka->user, &ssl->keys->privKey->key->ecc, &rc, 1,
				pka->data)) != 0) {
...
*/
		...
		if (ssl->cipher->type == CS_ECDH_ECDSA ||
			...
			/* This case is unique becuase it has two PKA ops for a single CKE
				message.  The key generation is done and then secrent is 
			...
			if (pka->type == PKA_AFTER_ECDH_KEY_GEN) {
				...
				psAssert(pka->user == (uint32)*(pka->outbuf - 1));
