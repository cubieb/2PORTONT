...
*/
			if (ssl->cipher == NULL || ssl->cipher->ident != cipher || 
				...
				printf("[%s %d] Can't support requested cipher: %d\n", __func__, __LINE__, cipher);
...
/* There is no Ext information for some stiuation. */	
#if 0		
#ifdef ENABLE_SECURE_REHANDSHAKES
		if (renegotiationExt == 0) {
#ifdef REQUIRE_SECURE_REHANDSHAKES		
...
*/
/* there is no cacert for some situation. */	
#if 0
		if (ssl->err == SSL_ALERT_NONE &&
...
*/
	...
	case SSL_HS_SERVER_HELLO_DONE: 
		...
		/* Unset PENDING_PKA_R since we are back where we left off */
		if (ssl->flags & SSL_FLAGS_PENDING_PKA_R) {
			ssl->flags &= ~SSL_FLAGS_PENDING_PKA_R;
		}
#ifdef USE_DHE_CIPHER_SUITE
		if (ssl->flags & SSL_FLAGS_DHE_KEY_EXCH) {
#ifdef USE_ECC_CIPHER_SUITE
