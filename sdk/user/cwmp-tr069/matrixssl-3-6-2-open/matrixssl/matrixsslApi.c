...
int32 matrixSslSetPlain(ssl_t *ssl, unsigned char *ptbuf, uint32 len)
{
	if (!ssl || !ptbuf) {
		return PS_ARG_FAIL;
	}
...
int32 matrixSslGetPlain(ssl_t *ssl, unsigned char **ptbuf, uint32 *ptlen)
{
	if (!ssl || !ptbuf || !ptlen) {
		return PS_ARG_FAIL;
	}
...
int32 matrixSslProcessedData(ssl_t *ssl, unsigned char **ptbuf, uint32 *ptlen)
{
	...
	*ptlen = 0;
	...
	#if 0//QL some data remained in inbuf, should no be dropped
