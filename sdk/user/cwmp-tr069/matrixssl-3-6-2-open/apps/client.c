...
static uint32 rehandshakeCipher = 0;
#endif
	...
	struct g_sslstats *stats)
{
	...
	int32			rc, transferred, len, sessionFlag, SNIextLen;
	ssl_t			*ssl;
	...
	if (g_disableCertNameChk == 0) {
		rc = matrixSslNewClientSession(&ssl, keys, sid, g_cipher, g_ciphers,
			certCb, g_ip, extension, NULL, sessionFlag);
	} else {
		rc = matrixSslNewClientSession(&ssl, keys, sid, g_cipher, g_ciphers,
			certCb, NULL, extension, NULL, sessionFlag);
	}
...
#else
			stats->datatime += psDiffMsecs(t1, t2);
#endif
		} else {
#ifdef USE_HIGHRES_TIME
			stats->hstime += psDiffUsecs(t1, t2);
#else
			stats->hstime += psDiffMsecs(t1, t2);
#endif
		}
		goto L_CLOSE_ERR;
	}
...
*/
		...
		case MATRIXSSL_APP_DATA_COMPRESSED:
			if (cp.flags != HTTPS_COMPLETE &&
					(rc = httpBasicParse(&cp, buf, len, g_trace)) < 0) {
				closeConn(ssl, fd);
...
static int32 httpWriteRequest(ssl_t *ssl)
{
	...
	if (g_closeServer) {
		...
		requested = (uint32)strlen((char *)g_matrixShutdownServer) + 1;
...
static void usage(void)
{
	printf("\nusage: client { option }\n"
           ...
           "-c <ciperList>          - Comma separated list of ciphers nums\n"
           ...
           "where keyLen must be one of 1024, 2048 or 4096, and\n"
           "tlsVersion is 0 for SSL V3, 1 for TLS V1.0, 2 for TLS V1.1 and\n"
           "3 for TLS V1.2.  serverPortNum is usually 443 or 4433. The\n"
           "cipherList is a comma separated list of cipher numbers to\n"
           "propose.  It should NOT contain any spaces.\n\n");
...
{
    int   optionChar, key_len, version, numCiphers, g_ip_set;
	...
	opterr = 0;
    while ((optionChar = getopt(argc, argv, "b:c:dhk:n:p:r:s:v:")) != -1)
    {
        switch (optionChar)
        {
        case '?':
