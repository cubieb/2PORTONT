...
function checkDefaultGW() {
 with (document.forms[0]) {
  if (droute[0].checked == false && droute[1].checked == false && gwStr[0].checked == false && gwStr[1].checked == false) {
   alert('<% multilang("1906" "LANG_A_DEFAULT_GATEWAY_HAS_TO_BE_SELECTED"); %>');
   return false;
  }
...
function check_dhcp_opts()
{
 with (document.forms[0])
 {
  ...
  {
   if (opt60_val.value=="") {
    alert('<% multilang("1907" "LANG_VENDOR_ID_CANNOT_BE_EMPTY"); %>');
    opt60_val.focus();
    return false;
   }
   if (checkString(opt60_val.value) == 0) {
    alert('<% multilang("1908" "LANG_INVALID_VENDOR_ID"); %>');
    opt60_val.focus();
    return false;
   }
  }
  ...
  {
   if (iaid.value=="") {
    alert('<% multilang("1909" "LANG_IAID_CANNOT_BE_EMPTY"); %>');
    iaid.focus();
    return false;
   }
   if (checkDigit(iaid.value) == 0) {
    alert('<% multilang("1910" "LANG_IAID_SHOULD_BE_A_NUMBER"); %>');
    iaid.focus();
    return false;
   }
   ...
   if(duid_type[1].checked)
   {
    ...
    if (duid_ent_num.value=="") {
     alert('<% multilang("1911" "LANG_ENTERPRISE_NUMBER_CANNOT_BE_EMPTY"); %>');
     duid_ent_num.focus();
     return false;
    }
    if (checkDigit(duid_ent_num.value) == 0) {
     alert('<% multilang("1912" "LANG_ENTERPRISE_NUMBER_SHOULD_BE_A_NUMBER"); %>');
     duid_ent_num.focus();
     return false;
    }

    if (duid_id.value=="") {
     alert('<% multilang("1913" "LANG_DUID_IDENTIFIER_CANNOT_BE_EMPTY"); %>');
     duid_id.focus();
     return false;
    }
    if (checkString(duid_id.value) == 0) {
     alert('<% multilang("1914" "LANG_INVALID_DUID_IDENTIFIER"); %>');
     duid_id.focus();
     return false;
    }
   }
  ...
  if(typeof enable_opt_125 !== 'undefined' &&enable_opt_125.checked)
  {
   if (manufacturer.value=="") {
    alert('<% multilang("1915" "LANG_MANUFACTURER_OUI_CANNOT_BE_EMPTY"); %>');
    manufacturer.focus();
    return false;
   }
   if (checkString(manufacturer.value) == 0) {
    alert('<% multilang("1916" "LANG_INVALID_MANUFACTURER_OUI"); %>');
    manufacturer.focus();
    return false;
   }

   if (product_class.value=="") {
    alert('<% multilang("1917" "LANG_PRODUCT_CLASS_CANNOT_BE_EMPTY"); %>');
    product_class.focus();
    return false;
   }
   if (checkString(product_class.value) == 0) {
    alert('<% multilang("1918" "LANG_INVALID_PRODUCT_CLASS"); %>');
    product_class.focus();
    return false;
   }

   if (model_name.value=="") {
    alert('<% multilang("1919" "LANG_MODEL_NAME_CANNOT_BE_EMPTY"); %>');
    model_name.focus();
    return false;
   }
   if (checkString(model_name.value) == 0) {
    alert('<% multilang("1920" "LANG_INVALID_MODEL_NAME"); %>');
    model_name.focus();
    return false;
   }

   if (serial_num.value=="") {
    alert('<% multilang("1921" "LANG_SERIAL_NUMBER_CANNOT_BE_EMPTY"); %>');
    serial_num.focus();
    return false;
   }
...
function applyCheck()
{
 ...
 if (document.ethwan.vlan.checked == true) {
  if (document.ethwan.vid.value == "") {
   alert('<% multilang("1923" "LANG_VID_SHOULD_NOT_BE_EMPTY"); %>');
 ...
 if ( document.ethwan.adslConnectionMode.value == 2 ) {
  if (document.ethwan.pppUserName.value=="") {
   alert('<% multilang("1924" "LANG_PPP_USER_NAME_CANNOT_BE_EMPTY"); %>');
   document.ethwan.pppUserName.focus();
   return false;
  }
  if (includeSpace(document.ethwan.pppUserName.value)) {
   alert('<% multilang("1925" "LANG_CANNOT_ACCEPT_SPACE_CHARACTER_IN_PPP_USER_NAME"); %>');
   document.ethwan.pppUserName.focus();
   return false;
  }
  if (checkString(document.ethwan.pppUserName.value) == 0) {
   alert('<% multilang("1926" "LANG_INVALID_PPP_USER_NAME"); %>');
   document.ethwan.pppUserName.focus();
   return false;
  }
  ...
  if (document.ethwan.pppPassword.value=="") {
   alert('<% multilang("1927" "LANG_PPP_PASSWORD_CANNOT_BE_EMPTY"); %>');
  ...
  if(!isAllStar(document.ethwan.pppPassword.value)){
   if (includeSpace(document.ethwan.pppPassword.value)) {
    alert('<% multilang("1928" "LANG_CANNOT_ACCEPT_SPACE_CHARACTER_IN_PPP_PASSWORD"); %>');
    document.ethwan.pppPassword.focus();
    return false;
   }
   if (checkString(document.ethwan.pppPassword.value) == 0) {
    alert('<% multilang("1929" "LANG_INVALID_PPP_PASSWORD"); %>');
    document.ethwan.pppPassword.focus();
    return false;
   }
  ...
  if (document.ethwan.pppConnectType.selectedIndex == 1) {
   if (document.ethwan.pppIdleTime.value <= 0) {
    alert('<% multilang("1930" "LANG_INVALID_PPP_IDLE_TIME"); %>');
  ...
  if(document.ethwan.IpProtocolType.value & 2)
  {
   if (document.ethwan.adslConnectionMode.value != 0 && document.ethwan.adslConnectionMode.value != 6 && document.ethwan.adslConnectionMode.value != 8) {
    if(document.ethwan.slacc.checked == false && document.ethwan.itfenable.checked == false && document.ethwan.staticIpv6.checked == false){
     alert('<% multilang("1931" "LANG_PLEASE_INPUT_IPV6_ADDRESS_OR_SELECT_DHCPV6_CLIENT_OR_CLICK_SLAAC"); %>');
   ...
   if(document.ethwan.itfenable.checked) {
    if(document.ethwan.iana.checked == false && document.ethwan.iapd.checked == false ) {
     alert('<% multilang("1932" "LANG_PLEASE_SELECT_IANA_OR_IAPD"); %>');
   ...
   if(document.ethwan.staticIpv6.checked) {
    ...
    if(document.ethwan.Ipv6Gateway.value != "" ){
     if (! isUnicastIpv6Address( document.ethwan.Ipv6Gateway.value) ){
      alert('<% multilang("1936" "LANG_INVALID_IPV6_GATEWAY_ADDRESS"); %>');
    ...
    if(document.ethwan.Ipv6Dns1.value != "" ){
     if (! isUnicastIpv6Address( document.ethwan.Ipv6Dns1.value) ){
      alert('<% multilang("1937" "LANG_INVALID_PRIMARY_IPV6_DNS_ADDRESS"); %>');
    ...
    if(document.ethwan.Ipv6Dns2.value != "" ){
     if (! isUnicastIpv6Address( document.ethwan.Ipv6Dns2.value) ){
      alert('<% multilang("1938" "LANG_INVALID_SECONDARY_IPV6_DNS_ADDRESS"); %>');
    ...
    if (document.ethwan.adslConnectionMode.value == 8) // 6rd
    {
     if(document.ethwan.SixrdBRv4IP.value == ""){
      alert('<% multilang("1939" "LANG_INVALID_6RD_BOARD_ROUTER_V4IP_ADDRESS"); %>');
      document.ethwan.SixrdBRv4IP.focus();
      return false;
     }

     if(document.ethwan.SixrdIPv4MaskLen.value == ""){
      alert('<% multilang("1940" "LANG_INVALID_6RD_IPV4_MASK_LENGTH"); %>');
      document.ethwan.SixrdIPv4MaskLen.focus();
      return false;
     }

     if(document.ethwan.SixrdPrefix.value == ""){
      alert('<% multilang("1941" "LANG_INVALID_6RD_PREFIX_ADDRESS"); %>');
      document.ethwan.SixrdPrefix.focus();
      return false;
     }

     if(document.ethwan.SixrdPrefixLen.value == ""){
      alert('<% multilang("1942" "LANG_INVALID_6RD_PREFIX_LENGTH"); %>');
    ...
    if (document.ethwan.adslConnectionMode.value == 6) // DS-Lite
    {
     if(document.ethwan.DSLiteLocalIP.value != ""){
      if (! isGlobalIpv6Address( document.ethwan.DSLiteLocalIP.value) ){
       alert('<% multilang("1943" "LANG_INVALID_DSLITELOCALIP_ADDRESS"); %>');
     ...
     if(document.ethwan.DSLiteRemoteIP.value != ""){
      if (! isGlobalIpv6Address( document.ethwan.DSLiteRemoteIP.value) ){
       alert('<% multilang("1944" "LANG_INVALID_DSLITEREMOTEIP_ADDRESS"); %>');
     ...
     if(document.ethwan.DSLiteGateway.value != ""){
      if (! isGlobalIpv6Address( document.ethwan.DSLiteGateway.value) ){
       alert('<% multilang("1945" "LANG_INVALID_DSLITEGATEWAY_ADDRESS"); %>');
...
function deleteCheck()
{
 ...
 {
  alert('<% multilang("1946" "LANG_NO_LINK_SELECTED"); %>');
  return false;
 }
...
<BODY onLoad="on_init();">
<blockquote>
<h2><font color="#0000FF"><% getWanIfDisplay(); %> <% multilang("11" "LANG_WAN"); %></font></h2>
<form action=/boaform/admin/formWanEth method=POST name="ethwan">
<table border="0" cellspacing="4" width="800">
  <tr><td><font size=2>
    <% multilang("228" "LANG_PAGE_DESC_CONFIGURE_PARAMETERS"); %><% getWanIfDisplay(); %><% multilang("11" "LANG_WAN"); %>
  </font></td></tr>
...
<table border=0 width="800" cellspacing=4 cellpadding=0>
 ...
 <tr>
  <td>
   <font size=2><b><% multilang("231" "LANG_ENABLE_VLAN"); %>: </b><input type="checkbox" name="vlan" size="2" maxlength="2" value="ON" onClick=vlanClicked()>
  </td>
 </tr>
 <tr>
  <td>
   <font size=2><b><% multilang("232" "LANG_VLAN"); %> ID: </b><input type="text" name="vid" size="10" maxlength="15">
  </td>
  <td><font size=2><b><% multilang("258" "LANG_802_1P_MARK"); %> </b>
 ...
 <tr>
  ...
  <td <% checkWrite("IPQoS"); %>><font size=2>
   <b><% multilang("271" "LANG_ENABLE_QOS"); %>: </b>
   <input type="checkbox" name="qosEnabled" size="2" maxlength="2" value="ON" >
  </font></td>
 </tr>
 <tr>
  <td><font size=2><b><% multilang("237" "LANG_ADMIN_STATUS"); %>:</b>
   <input type=radio value=1 name="chEnable"><% multilang("222" "LANG_ENABLE"); %>
   <input type=radio value=0 name="chEnable" checked><% multilang("221" "LANG_DISABLE"); %></font>
  </td>
 </tr>
 <% ShowConnectionType() %>
 <tr>
...
<table>
<td><font size=2><b><% multilang("236" "LANG_DEFAULT_ROUTE"); %>:</b>
 <input type=radio value=0 name="droute"><% multilang("221" "LANG_DISABLE"); %>
 <input type=radio value=1 name="droute" checked><% multilang("222" "LANG_ENABLE"); %></font>
</td>
</table>
</div>
...
<input type="submit" value="<% multilang("134" "LANG_APPLY_CHANGES"); %>" name="apply" onClick="return applyCheck()">&nbsp; &nbsp; &nbsp; &nbsp;
<input type="submit" value="<% multilang("254" "LANG_DELETE"); %>" name="delete" onClick="return deleteCheck()">
