...
#define QOS_VLAN_ID_MASK		(0xFFF<<QOS_DNS_Bind_OFFSET)
...
#ifndef CONFIG_RTK_RG_INIT //Temperorily mark for LUNA platform, ebtable rule make let pc could not ping gateway.
#ifdef BR_ROUTE_ONEPVC
...
/*star:20090407 START only parse rtp src port*/
		...
		if((0 != entry.m_1p)) {
			...
			mark |= (((i+1)<<QOS_ENTRY_OFFSET) |SW_QOS_ENABLE);
		}
		else {
			mark |= ((i+1)<<8);
		}
