...
int cli_input_tab()
{
	...
	input_end();
	strcpy(cmdBuf,command_ps);
	if((n = listNextCmdNodeOrArgs(cmdBuf)) > 0){
		if((len+n) > (BUFSIZ - 2))
			return 0;
		cmdLen = strlen(command_ps);
		for(i=0;i<n;i++){
			len++;
			*(command_ps+cmdLen+i) = cmdBuf[i];
			*(command_ps+cmdLen+i+1) = 0; 
			cmdedit_set_out_char(0);
		}
	}else if(n == 0){
		fflush(stdout);	
...
prepare_to_die:
	...
	/*xl_yue*/
	if((cmdLen = strlen(command)) > 0 && command[cmdLen-1] == '\n')
		command[cmdLen-1] = 0;
