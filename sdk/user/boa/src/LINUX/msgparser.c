...
static void cfg_desc_get_addr(struct mymsgbuf *qbuf);
...
static struct command commands[] = {
	...
	*/
	...
	{1, CMD_DESC_GET_ADDR, cfg_desc_get_addr},
...
static void cfg_chain_desc_id(struct mymsgbuf *qbuf)
{
	...
	qbuf->msg.arg1 = size;
	#ifdef USE_SHM
	memcpy(shm_start, (void *)rec_desc, copylen);
	#else
...
// Get Object descriptor by address
static void cfg_desc_get_addr(struct mymsgbuf *qbuf)
{
	int index;
	mib_chain_member_entry_T *rec_desc;
	...
	rec_desc = (mib_chain_member_entry_T *)qbuf->msg.arg1;
	// get total size of the record descriptor
	i = 0; size = 0;
	while (rec_desc[i++].name[0] != 0)
		size++;
...
int doDelegation(const char *fname)
{
	...
	if ( ret == 1 ) {
		...
		if(radvdpid>0) {//TERM it, let previous prefix be deprecated.
			...
			unlink(RADVD_PID); //remove pid file, since if the pid file still existing will impact nex
		}
		va_cmd( "/bin/radvd", 3, 0, "-s", "-C", (char *)RADVD_CONF );
