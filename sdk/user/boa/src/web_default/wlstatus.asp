...
<h2><b><font color="#0000FF"><% multilang(LANG_WLAN_STATUS); %></font></b></h2>
...
<table width=400 border=0>
  <script>
	...
	document.write('\
	<tr bgcolor="#EEEEEE">\
	<td width=100% colspan="2" bgcolor="#008000"><font color="#FFFFFF" size=2><b><% multilang(LANG_WLAN); %> \
	<% multilang(LANG_CONFIGURATION); %></b></font></td>\
	</tr>\
	<tr bgcolor="#EEEEEE">\
	<td width=40%><font size=2><b><% multilang(LANG_MODE); %></b></td>\
...
else
{
	...
	document.write('</td>\
		<tr bgcolor="#DDDDDD">\
	    	<td width=40%><font size=2><b><% multilang(LANG_BAND); %></b></td>\
	    	<td width=60%><font size=2>');
	if (band[i] == 1)
	...
	document.write('</tr>\
	<tr bgcolor="#EEEEEE">\
    	<td width=40%><font size=2><b><% multilang(LANG_SSID); %></b></td>\
    	<td width=60%><font size=2>');
	if (wlanMode[i] != 2) {
	...
	<tr bgcolor="#DDDDDD">\
	<td width=40%><font size=2><b><% multilang(LANG_CHANNEL_NUMBER); %></b></td>\
	<td width=60%><font size=2>'+channel_drv[i] +'</td>\
	</tr>\
	<tr bgcolor="#EEEEEE">\
	<td width=40%><font size=2><b><% multilang(LANG_ENCRYPTION); %></b></td>\
	<td width=60%><font size=2>');
	...
	document.write('</td>\
  	...
  	<tr bgcolor="#DDDDDD">\
    	<td width=40%><font size=2><b><% multilang(LANG_BSSID); %></b></td>\
	...
	if (wlanMode[i]!=2) {	//2 means WDS mode
		...
		if (wlanMode[i]==0 || wlanMode[i]==3 || wlanMode[i]==4) {
			document.write("<td width=40%%><font size=2><b><% multilang(LANG_ASSOCIATED_CLIENTS); %></b></td>\n");
			document.write("<td width=60%%><font size=2>"+clientnum[i]+"</td></tr>");
		}
		else {
			document.write("<td width=40%%><font size=2><b><% multilang(LANG_STATE); %></b></td>\n");
	...
	if (!wlanDisabled[i] && (wlanMode[i]==0 || wlanMode[i]==3 )) {
		for (idx=0; idx<mssid_num; idx++) {
			if (!mssid_disable[idx]) {
				...
				<b><% multilang(LANG_VIRTUAL_AP); %>'+(idx+1)+' <% multilang(LANG_CONFIGURATION); %></b></font></td>\
				</tr>');
				if(!is_wlan_qtn){
					document.write('<tr bgcolor="#EEEEEE">\
	    			<td width=40%><font size=2><b><% multilang(LANG_BAND); %></b></td>\
				...
				document.write('</tr>\
				<tr bgcolor="#EEEEEE">\
				<td width=40%%><font size=2><b><% multilang(LANG_ASSOCIATED_CLIENTS); %></b></td>\
    ...
    if (rp_enabled[i])	// start of repeater
    {
    	...
    	<tr bgcolor="#EEEEEE">\
    	<td width=100% colspan="2" bgcolor="#008000"><font color="#FFFFFF" size=2><b><% multilang(LANG_WLAN); %> \
    	<% multilang(LANG_REPEATER_INTERFACE); %><% multilang(LANG_CONFIGURATION); %></b></font></td>\
    	</tr>\
    	<tr bgcolor="#EEEEEE">\
    	<td width=40%><font size=2><b><% multilang(LANG_MODE); %></b></td>\
    	<td width=60%><font size=2>' );
