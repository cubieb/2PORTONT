...
<title><% multilang(LANG_ETHERNETOAM_Y_1731); %></title>
<script type="text/javascript" src="share.js">
</script>
<SCRIPT>
function saveChanges(){
 if ((document.Y1731.myid<1)) {  
   alert('<% multilang(LANG_MYID_MUST_MORE_THAN_0); %>');
   return false;
 }
 if (document.Y1731.megid.length == 0) {  
   alert('<% multilang(LANG_MEGID_MUST_HAVE_VALUE); %>');
   return false;
 }
 return true;
}
...
<body>
<blockquote>
<h2><font color="#0000FF"><% multilang(LANG_ETHERNETOAM_Y_1731); %></font></h2>

<form action=/boaform/formY1731 method=POST name="Y1731">
<table border=0 width="500" cellspacing=4 cellpadding=0>
  <tr><td><font size=2>
    <% multilang(LANG_HERE_YOU_CAN_CONFIGURE_ETHERNETOAM_Y_1731); %>
  </font></td></tr>
...
<table border=0 width="500" cellspacing=4 cellpadding=0>
 ...
 <tr>
  <td width="30%"><font size=2><b>MEG <% multilang(LANG_LEVEL); %>:</b></td>
  <td width="70%">
