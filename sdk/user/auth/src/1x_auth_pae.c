...
void lib1x_process_radiusdisc( Global_Params * global, struct lib1x_packet * spkt);
void lib1x_resp_disconnect_request( Auth_Pae * auth_pae, struct lib1x_packet * pkt, ErrorCauseNo error_no);
#endif
...
TxRx_Params * lib1x_init_txrx(Dot1x_Authenticator * auth, u_char * oursvr_addr, u_char * svr_addr,
				...
				u_char * dev_svr,
#ifndef DYNAMIC_AUTHORIZATION_SUPPORT
				u_char * dev_supp)
#else
				u_char * dev_supp, u_short udp_dasport)
#endif
{
	...
	//----endif
	{
		...
		if(auth->AccountingEnabled)
		{
			...
			if (inet_aton( acctip_addr, & dev_txrx->acctip_inaddr ) == 0 )
				lib1x_message(MESS_ERROR_FATAL, "Invalid ACCT IP addr ");
		...
		if(auth->DynamicAuthEnable)
		{
			...
			dev_txrx->dasouraddr.sin_family = PF_INET;
			dev_txrx->dasouraddr.sin_port = htons( udp_dasport );
			dev_txrx->dasouraddr.sin_addr.s_addr = htonl(INADDR_ANY);

			if ( bind( dev_txrx->network_svr->dassock, (struct sockaddr *)(&(dev_txrx->dasouraddr)), sizeof(struct sockaddr_in)) < 0)
				lib1x_message( MESS_ERROR_FATAL,"Could not BIND DAS UDP socket.");
			
			memset(&(dev_txrx->network_svr->dassvraddr), 0, sizeof(struct sockaddr_in));
			dev_txrx->network_svr->dassvrlen = sizeof(struct sockaddr_in);

			flags = fcntl(dev_txrx->network_svr->dassock, F_GETFL );
			if ( fcntl( dev_txrx->network_svr->dassock, F_SETFL, flags | O_NONBLOCK) != 0 )
				lib1x_message( MESS_ERROR_FATAL, "lib1x_nal_connect : FCNTL failed on DAS UDP socket" );
		}
#endif
	}
...
void lib1x_authsm_capture_svr( Global_Params * global, struct lib1x_nal_intfdesc * nal, struct lib1x_packet * pkt )
{
	...
	switch( rhdr->code  )
	{
		...
		case 	LIB1X_RAD_ACCTRSP:
						...
						break;
#ifdef DYNAMIC_AUTHORIZATION_SUPPORT
		case	LIB1X_RAD_DISCREQ:
		{
			lib1x_message( MESS_DBG_SPECIAL, "AUTHENTICATION RADIUS Disconnect Request");
			lib1x_process_radiusdisc(global, pkt);				
			break;
		}
#endif
	}
...
void lib1x_process_radiusdisc( Global_Params * global, struct lib1x_packet * spkt)
{
	...
	u_char 				done;
	u_short  				unexplen;
	ErrorCauseNo 			error_no;
	u_char				szUTF8[6];
	u_char				usr_name[80], acct_session_id[80];
	u_char				tmp_addr[100], target_addr[ETHER_ADDRLEN], orig_addr[ETHER_ADDRLEN];
	u_char				usr_name_len, acct_session_id_len, addr_len;
	int 					i, suppid;		
	u_long				ulUTF8Len;
	...
	unexplen = ntohs(rhdr->length) - LIB1X_RADHDRLEN;
	error_no = No_Error;
	usr_name_len = 0, acct_session_id_len = 0, addr_len = 0, suppid = -1, ulUTF8Len = 0;
	
	lib1x_message(MESS_DBG_RAD, "---> lib1x_process_radiusdisc()");
	
	while (!done)
	{
		switch(rattr->type)
		{
			case LIB1X_RAD_USER_NAME:
			{
				lib1x_message(MESS_DBG_RAD," Received User-Name Attribute from radius server");
				if( (0 <= rattr->length) && (rattr->length <= sizeof(usr_name)) )
				{
					memcpy(usr_name, (u_char *)rattr + 2, rattr->length - 2);
					usr_name[rattr->length - 2] = '\0';
					usr_name_len = rattr->length - 2;
					lib1x_message(MESS_DBG_RAD, " Type\tLen\tContent");
					lib1x_message(MESS_DBG_RAD, " %d\t%d\t%s", rattr->type, usr_name_len, usr_name);
				}
				else
					lib1x_message(MESS_DBG_RAD, " Received User-Name length = %d invalid", rattr->length);
				
				break;
			}
			case LIB1X_RAD_CALLING_STID:
			{
				lib1x_message(MESS_DBG_RAD," Received Calling-Station-Id Attribute from radius server");
				if( (0 <= rattr->length) && (rattr->length <= sizeof(tmp_addr)) )
				{
					memcpy(tmp_addr, (u_char *)rattr + 2, rattr->length - 2);
					tmp_addr[rattr->length - 2] = '\0';
					if( hwaddr_ascii2mac(tmp_addr, target_addr) < ETHER_ADDRLEN )
					{
						error_no = 601;
						lib1x_message(MESS_DBG_RAD," Unkwonwn mac addr : %s", tmp_addr);
						goto fail;
					}
					else
					{
						addr_len = rattr->length - 2;
						//PRINT_MAC_ADDRESS(target_addr, "Disconncet target addr");
					}
				}
				else
					lib1x_message(MESS_DBG_RAD, " Received Calling-Station-Id length = %d invalid", rattr->length);

				break;
			}
			case LIB1X_RAD_ACCT_SESSION_ID:
			{
				lib1x_message(MESS_DBG_RAD," Received Acct-Session-Id Attribute from radius server");
				if( (0 <= rattr->length) && (rattr->length <= sizeof(acct_session_id)) )
				{
					memcpy(acct_session_id, (u_char *)rattr + 2, rattr->length - 2);
					acct_session_id[rattr->length - 2] = '\0';
					acct_session_id_len = rattr->length - 2;
					//lib1x_message(MESS_DBG_RAD, " Type\tLen\tContent");
					//lib1x_message(MESS_DBG_RAD, " %d\t%d\t", rattr->type, acct_session_id_len);
					//for( i = 0 ; i != acct_session_id_len ; ++i )
					//	printf("%02x ", (u_char)acct_session_id[i]);
					//printf("\n");
				}
				else
					lib1x_message(MESS_DBG_RAD, " Received Acct-Session-Id length = %d invalid", rattr->length);
				
				break;
			}
			default:
			{
				lib1x_message( MESS_DBG_RAD, " Received unprocess attribute type(%d) from radius server\n", rattr->type);
				break;
			}
		}
	...
	if( (suppid == -1) && (addr_len != 0) )
	{
		for( i = 0 ; i < auth->MaxSupplicant ; i++ )
		{
			...
			if( memcmp(auth_pae->supp_addr, EMPTY_ADDR, ETHER_ADDRLEN) == 0 )
				continue;

			if( memcmp(auth_pae->supp_addr, target_addr, ETHER_ADDRLEN) == 0 )
			{
				suppid = i;
				lib1x_message(MESS_DBG_RAD, " Find target client by mac addr");
				break;
			}
		}
	}
	
	if( (suppid == -1) && (acct_session_id_len != 0) )
	{
		for( i = 0 ; i < auth->MaxSupplicant ; i++ )
		{
			if (auth->Supp[i] == NULL)
			...
			if( ulUTF8Len == acct_session_id_len )
			{
				if( memcmp(szUTF8, acct_session_id, acct_session_id_len) == 0 )
				{
					suppid = i;
					memcpy(target_addr, auth_pae->supp_addr, ETHER_ADDRLEN);
					lib1x_message(MESS_DBG_RAD, " Find target client by accounting id");
					break;
				}
			}
		}
	...
	if( (suppid == -1) && (usr_name_len != 0) )
	{
		for( i = 0 ; i < auth->MaxSupplicant ; i++ )
		{
			...
			if( auth_pae->rinfo->username_len == usr_name_len )
			{
				if( memcmp(auth_pae->rinfo->username, usr_name, usr_name_len) == 0 )
				{
					suppid = i;
					memcpy(target_addr, auth_pae->supp_addr, ETHER_ADDRLEN);
					lib1x_message(MESS_DBG_RAD, " Find target client by user name");
					break;
				}					
			}
		}
	...
	if( suppid == -1 )
	{
		lib1x_message( MESS_DBG_RAD, " suppid = -1");
		error_no = Session_Context_Not_Found;
		goto fail;
	}
	else
	{
		lib1x_message( MESS_DBG_RAD, " suppid = %d", suppid);
		global = auth->Supp[suppid]->global;
		auth_pae = auth->Supp[suppid]->global->theAuthenticator;
	}

	global->EventId = akmsm_EVENT_Disconnect;
	memcpy(global->theAuthenticator->supp_addr, target_addr, ETHER_ADDRLEN);
	lib1x_akmsm_Disconnect(global);
fail:
	lib1x_resp_disconnect_request(global->theAuthenticator, spkt, error_no);
	lib1x_message(MESS_DBG_RAD, "<--- lib1x_process_radiusdisc()");
}
...
void lib1x_resp_disconnect_request( Auth_Pae * auth_pae, struct lib1x_packet * pkt, ErrorCauseNo error_no)
{
	...
	u_char					szAttr[4];	//	service type attr
	u_char					ecAttr[4];	//	error cause attr
	u_char					*sendptr;
	u_char					authenticator[16];
	int 						size;

	lib1x_message(MESS_DBG_RAD, "---> lib1x_resp_disconnect_request()");

	rinfo = auth_pae->rinfo;
	bzero( auth_pae->sendBuffer, LIB1X_AP_SENDBUFLEN);
	
	if( (0 <= error_no) && (error_no <= 299) )
	{
		lib1x_message(MESS_DBG_RAD, "Response Disconnect-Ack to auth server");
		rconst = lib1x_radconst_create( auth_pae, auth_pae->sendBuffer , LIB1X_RAD_DISCACK, rinfo->identifier, LIB1X_IT_UDPSOCK_DAS);
	}
	else
	{
		lib1x_message(MESS_DBG_RAD, "Response Disconnect-Nak to auth server, Error Cause = %d", error_no);
		rconst = lib1x_radconst_create( auth_pae, auth_pae->sendBuffer , LIB1X_RAD_DISCNAK, rinfo->identifier, LIB1X_IT_UDPSOCK_DAS);
		
		lib1x_L2N(LIB1X_RAD_SERVICE_TYPE_Authenticate_Only, szAttr);
		lib1x_radconst_addattr(rconst, LIB1X_RAD_SERVICE_TYPE, 4, (u_char *)szAttr );

		lib1x_L2N(error_no, ecAttr);
		lib1x_radconst_addattr(rconst, LIB1X_RAD_ERROR_CAUSE, 4, (u_char *)ecAttr);
	}

	lib1x_radconst_calradlength(rconst);
	sendptr = rconst->pkt + ETHER_HDRLEN + LIB1X_IPHDRLEN + LIB1X_UDPHDRLEN;
	size = rconst->pktlen - (ETHER_HDRLEN + LIB1X_IPHDRLEN + LIB1X_UDPHDRLEN);

	//	response diconnect-request need request authenticator field for generate response authenticator
	memcpy(rconst->rhdr->authenticator_str, pkt->data+4, 16);
	lib1x_create_respauth(auth_pae, rconst);

	lib1x_nal_send_das(auth_pae->global->TxRx->network_svr, sendptr, size);	
	lib1x_message(MESS_DBG_RAD, "<--- lib1x_resp_disconnect_request()");
}
#endif
