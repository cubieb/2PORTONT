...
//--------------------------------------------------
// create the response authenticator field for the
// radius packets.
// stores it into rinfo->req_authenticator
// MD5(Code||Identifier||Length||Request Authenticator field||response Attributes||shared secret)
//--------------------------------------------------

void lib1x_create_respauth(Auth_Pae * auth_pae, struct lib1x_radius_const * rconst)
{
	u_char	szBuf[1024];
	u_long	ulRadPacketLen;
	MD5_CTX the_md5;
	...
	memcpy(szBuf, rconst->pkt + ETHER_HDRLEN + LIB1X_IPHDRLEN + LIB1X_UDPHDRLEN, ulRadPacketLen);

	lib1x_hexdump2(MESS_DBG_RAD, "lib1x_create_respauth",szBuf, ulRadPacketLen,"Calculate Auth");

	MD5_Init(&the_md5);
	MD5_Update(&the_md5, (unsigned char *) szBuf, ulRadPacketLen);
	MD5_Update(&the_md5, (unsigned char *) auth_pae->global->auth->RadShared.Octet, auth_pae->global->auth->RadShared.Length);
	MD5_Final(rhdr->authenticator_str, &the_md5);          /* set the final vector */
...
#ifdef DYNAMIC_AUTHORIZATION_SUPPORT
	else if(udp_type == LIB1X_IT_UDPSOCK_DAS)
		udp->dport = htons( auth_pae->global->TxRx->network_svr->dassvraddr.sin_port);
#endif
