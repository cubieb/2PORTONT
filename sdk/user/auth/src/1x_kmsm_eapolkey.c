...
LIB1X_EAPOL_HDRLEN);
	...
	if (buf != NULL && EAPOLKEY_DEBUG >=2) {
		...
		//PrintHex((u_char*)"EtherType", (u_char*)eth->ether_type, sizeof(eth->ether_type));

		//PrintHex((u_char*)"EAPOLHeader", (u_char*)(buf +  ETHER_HDRLEN),LIB1X_EAPOL_HDRLEN);
		//printf("\tKeyDescVer:%x\n",  eapol_key->key_desc_ver);
		PrintHex((u_char *)"KeyInfo", eapol_key->key_info, sizeof(eapol_key->key_info));
		//PrintHex((u_char *)"KeyLength", eapol_key->key_len, sizeof(eapol_key->key_len));
		PrintHex((u_char *)"KeyReplayCounter", eapol_key->key_replay_counter, sizeof(eapol_key->key_replay_counter));
		//PrintHex((u_char *)"KeyNonce", eapol_key->key_nounce, sizeof(eapol_key->key_nounce));
		//PrintHex((u_char *)"KeyIV", eapol_key->key_iv, sizeof(eapol_key->key_iv));
		//PrintHex((u_char *)"KeyReplaySequenceCounter", eapol_key->key_rsc, sizeof(eapol_key->key_rsc));
		//PrintHex((u_char *)"KeyID", eapol_key->key_id, sizeof(eapol_key->key_id));
		//PrintHex((u_char *)"KeyMIC", eapol_key->key_mic, sizeof(eapol_key->key_mic));
		//PrintHex((u_char *)"KeyDataLength", eapol_key->key_data_len, sizeof(eapol_key->key_data_len));
