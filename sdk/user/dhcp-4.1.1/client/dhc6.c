...
start_bound(struct client_state *client)
{
...
#if defined(CONFIG_IPV6) && defined(DUAL_STACK_LITE)
	printf("[%s] clear aftr\n",__func__);
	memset(aftrinfo,0,sizeof(aftrinfo));
#endif
	...
	printf("[%s:%d] aftrinfo=%s\n",__func__,__LINE__,aftrinfo);
	oldia = NULL;
	for (ia = lease->bindings ; ia != NULL ; ia = ia->next) {
		if (old != NULL)
...
#if defined(CONFIG_IPV6) && defined(DUAL_STACK_LITE)
	if(aftrinfo[0]){
		//Now send command, to let configd handle aftr	
		printf("Now send command, to let configd handle aftr %s\n",aftrinfo);
		cmd_got_aftr(aftrinfo,strlen(aftrinfo));		
	}
#endif
