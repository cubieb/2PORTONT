	...
	if (evaluate_option_cache (&ds, packet, lease, client_state,
		...
		fprintf(leaseFile, "%soption %s%s%s %s;\n", preamble,
			...
			pretty_print_option(oc->option, ds.data, ds.len,
					    1, 1));
#if defined(CONFIG_IPV6) && defined(DUAL_STACK_LITE)
	if(strcmp(oc->option->name,"dslite")==0){
		char *aftr_tmp = pretty_print_option(oc->option, ds.data, ds.len, 1, 1);
		char aftr[100]={0};

		strncpy(aftr,aftr_tmp+1,ds.len-2); //because original data format is EX:"aftr.ruby.org.", len=15
		snprintf(aftrinfo,sizeof(aftrinfo), "%s,%s",client_state->interface->name,aftr);
		printf("[%s]Got aftr %s\n",__func__,aftrinfo);
	}
#endif
...
//Hack this to prevent multiple process created! Becasue system will have duplicated
//NEWLINK event in netlink in a very short period of time.
void kill_old_process()
{
	FILE *pidfd;
	pid_t oldpid;
	long temp;
