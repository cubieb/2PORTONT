	...
	struct universe *universe;
{
	...
	for (offset = 0;
		...
		// vsio is only for IPv6, IPv6 has no pad options.
		// we should remove this statement if we have correct enterprise number.
		if (universe != &vsio_universe && code == DHO_PAD)
			continue;
#else
