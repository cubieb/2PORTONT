...
static struct option vsio_options[] = {
	...
	/* ctcom can be pure string or combination of sub_code + sub_data
	 * We get it as string first, and parse it using "Ectom." format when matching sub_code + sub_data.
	 */
	{ "ctcom", "X",		&vsio_universe, VENDOR_CT_ENT_NUM, 1},
#endif
...
#ifdef CTCOM_DHCPV6_OPT17
/* Used for parsing vsio.ctcom into format sub_code + sub_data */
struct universe ctcom_universe;
static struct option ctcom_options[] = {
	{ "vendor",		"t",		&ctcom_universe,     1, 1 },
	{ "type",		"t",		&ctcom_universe,	    2, 1 },
	{ "model",		"t",		&ctcom_universe,	    3, 1 },
	{ "internet-vid",	"S",		&ctcom_universe,	    10, 1 },
	{ "iptv-vid",		"S",		&ctcom_universe,	    11, 1 },
	{ "itms-vid",		"S",		&ctcom_universe,	    12, 1 },
	{ "voice-vid",	"S",		&ctcom_universe,	    13, 1 },
	{ "reserved",		"X",		&ctcom_universe,	    83, 1 },
	{ NULL, NULL, NULL, 0, 0 }
};
#endif
...
void initialize_common_option_spaces()
{
	...
	/* No END option. */
#ifdef CTCOM_DHCPV6_OPT17
	// Becuase temprary CT-COM enterprise code is 0,
	// we use 0xffff as end tage to replace 0 to avoid option parsing error.
	// This satement should be removed if we have correct CT-COM enterprise number
	vsio_universe.end = 0xFFFF;
#else
...
#ifdef CTCOM_DHCPV6_OPT17
	/* Add ISC VSIO sub-sub-option space. */
	ctcom_universe.name = "ctcom";
	ctcom_universe.concat_duplicates = 0;
	ctcom_universe.lookup_func = lookup_hashed_option;
	ctcom_universe.option_state_dereference =
		hashed_option_state_dereference;
	ctcom_universe.save_func = save_hashed_option;
	ctcom_universe.delete_func = delete_hashed_option;
	ctcom_universe.encapsulate = hashed_option_space_encapsulate;
	ctcom_universe.foreach = hashed_option_space_foreach;
	ctcom_universe.decode = parse_option_buffer;
	ctcom_universe.length_size = 1;
	ctcom_universe.tag_size = 1;
	ctcom_universe.get_tag = getUChar;
	ctcom_universe.store_tag = putUChar;
	ctcom_universe.get_length = getUChar;
	ctcom_universe.store_length = putUChar;
	ctcom_universe.site_code_min = 0;
	/* No END option. */
	ctcom_universe.end = 0x00;

	code = VENDOR_CT_ENT_NUM;
	if (!option_code_hash_lookup(&ctcom_universe.enc_opt,
				     vsio_universe.code_hash, &code, 0, MDL))
		log_fatal("Unable to find CTCOM parent option (%s:%d).", MDL);
	ctcom_universe.index = universe_count++;
	universes[ctcom_universe.index] = &ctcom_universe;
	if (!option_name_new_hash(&ctcom_universe.name_hash,
				  13, MDL) ||
	    !option_code_new_hash(&ctcom_universe.code_hash,
				  13, MDL))
		log_fatal("Can't allocate Vendor Specific Information "
			  "Options space.");
	for (i = 0 ; ctcom_options[i].name != NULL ; i++) {
		option_code_hash_add(ctcom_universe.code_hash,
				     &ctcom_options[i].code, 0,
				     &ctcom_options[i], MDL);
		option_name_hash_add(ctcom_universe.name_hash,
				     ctcom_options[i].name, 0,
				     &ctcom_options[i], MDL);
	}
#endif
	...
	universe_hash_add(universe_hash, isc6_universe.name, 0,
			  &isc6_universe, MDL);
#ifdef CTCOM_DHCPV6_OPT17
	universe_hash_add(universe_hash, ctcom_universe.name, 0,
			  &ctcom_universe, MDL);
#endif	
