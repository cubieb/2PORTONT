...
build_address6(struct in6_addr *addr, 
		   ...
		   const struct data_string *input, struct packet *packet) {
	MD5_CTX ctx;
	int net_bytes;
	int i;
	char *str;
	...
	if ( packet !=NULL ) {
		for ( i=8; i<=15; i++)
			str[i] = packet->client_addr.iabuf[i];	
	}
	#endif
}
...
// Mason Yu. for EUI64 format
isc_result_t
create_lease6_2(struct ipv6_pool *pool, struct iasubopt **addr, 
	      unsigned int *attempts,
	      const struct data_string *uid, time_t soft_lifetime_end_time, struct packet *packet) {
	struct data_string ds;
	struct in6_addr tmp;
	struct iasubopt *test_iaaddr;
	struct data_string new_ds;
	...
	*attempts = 0;
	for (;;) {
		...
		case D6O_IA_NA:
			...
			log_debug("2:tmp %s, ds.data=%x",
				  inet_ntop(AF_INET6, &tmp,
				  	    tmp_buf, sizeof(tmp_buf)), (unsigned long *)ds.data);
			
			log_debug("2:Pool start %s, net_bits=%d, uid->data=%s",
				  inet_ntop(AF_INET6, &pool->start_addr,
				  	    tmp_buf, sizeof(tmp_buf)), pool->bits, (unsigned long *)uid->data);
