/*
 * "$Id: job.c,v 1.1 2011/02/22 15:42:38 cathy Exp $"
 *
         ...
         printer_t *printer)	/* I - Printer to print job */
{
  ...
  */
  ...
  attr = ippFindAttribute(current->attrs, "document-format",
                          IPP_TAG_MIMETYPE);
  if (attr != NULL && (optptr = strstr(attr->values[0].string.text, "charset=")) != NULL)
  ...
  envp[0]  = "PATH=/bin:/usr/bin:/etc:/sbin:/usr/sbin";	//"PATH=/bin:/usr/bin";
  ...
  */
  ...
  DEBUG_printf(("statusfds = %d, %d, num_filters=%d\n", statusfds[0], statusfds[1], num_filters));
  ...
  */
  ...
  if (strncmp(printer->device_uri, "file:", 5) != 0)
  {
    ...
    //sprintf(command, "%s/backend/%s", ServerRoot, method);		//cathy
    sprintf(command, "/bin/%s", method);
...
/*
 * End of "$Id: job.c,v 1.1 2011/02/22 15:42:38 cathy Exp $".
 */
