     ...
     it shouldn't break on other Unices, and their SIOGIFCONF might work. */
  {
	...
	while (fscanf(f, "%32s %02x %02x %02x %02x %20s\n",
		      addrstring, &if_idx, &plen, &scope, &flags, devname) != EOF) 
	  {
		...
		// We just only create LAN interface socket
		if ( strncmp(LANIF, devname, 3) !=0 )
			continue;
			
			
		//printf("%s, %x, %x, %x, %x, %s\n", addrstring, if_idx, plen, scope, flags, devname);
		// If the interface is not ready, it can not bind its IP address. we should wait. 
		// see the if_addr.h (#define IFA_F_PERMANENT		0x80)
		if ( flags != 0x80)
		{			
			usleep(4000000); // wait 4 sec 
			fclose(f);	
			goto do_again;
		}
	    ...
	    addr.in6.sin6_port = htons(port);
	    addr.in6.sin6_flowinfo = htonl(0);
	    addr.in6.sin6_scope_id = if_nametoindex(LANIF);
