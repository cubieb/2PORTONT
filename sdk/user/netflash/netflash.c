...
int donftl;				/* Target device is NFTL (such as DiskOnChip) */
#ifdef CONFIG_USER_NETFLASH_DECOMPRESS
...
#ifdef CONFIG_MTD
static void program_segment(int rd, char *sgdata, int sgpos, int sglength, int sgsize)
{
	...
	if (lseek(rd, sgpos, SEEK_SET) != sgpos) {
		...
		if (lseek(rd, sgpos, SEEK_SET) != sgpos) {
			...
			for (len = 0; len < sglength; len += 512)
				if (write(rd, sgdata + len, 512) == -1)
...
static void program_nftl_segment(int rd, char *sgdata, int sgpos, int sglength)
{
	if (sglength > 0) {
		if (lseek(rd, sgpos, SEEK_SET) != sgpos) {
			printf("netflash: lseek(%x) failed\n", sgpos);
...
#ifdef CONFIG_NFTL_RW
			if (donftl)
				program_nftl_segment(rd, sgdata, sgpos, sglength);
			else
#endif
...
int netflashmain(int argc, char *argv[])
{
	...
	if (donftl) {
		...
		if (ioctl(rd, BLKGETSIZE, &l) < 0) {
			error("ioctl(BLKGETSIZE) failed, errno=%d", errno);
		}
		devsize = l*512;  /* sectors are always 512 bytes */
		sgsize = 64*1024; /* Just a random power of 2, it doesn't matter */
	}
	else
#endif
	{
