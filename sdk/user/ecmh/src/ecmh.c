...
***************************************
 $Author: masonyu $
 $Id: ecmh.c,v 1.7 2012/12/12 03:17:55 masonyu Exp $
 $Date: 2012/12/12 03:17:55 $
***************************************
...
volatile int	g_needs_leave = false;		// Mason Yu. fast leave
...
// Added by Mason Yu. for IPV6 phase 2 ready logo Test
void l2_ethtype_2(struct intnode *intn, const uint8_t *packet, const unsigned int len, const unsigned int ether_type, unsigned char *SrcMac);
...
// Mason Yu. MLD Proxy. Start
int raw_icmpv6_sock = -1;
#define IVIF 1
#define OVIF 2
#define	IFNAMSIZ	16
...
int mrt6_del_mfc(struct sockaddr_in6 src, struct sockaddr_in6 grp)
{
 	struct mf6cctl mc;
 	...
 	if (setsockopt(raw_icmpv6_sock, IPPROTO_IPV6, MRT6_DEL_MFC,
                (char *)&mc, sizeof(mc))) {
   		perror("MRT6_DEL_MIF, setsockopt: ");
   		return -1;
 	}
...
void 
mrt6_join_group(struct in6_addr grp, u_int ifindex)
{
    ...
    if (setsockopt(raw_icmpv6_sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP,
		   (char *) &mreq, sizeof(mreq)) < 0)
		perror("IPV6_ADD_MEMBERSHIP, setsockopt: ");
}
...
void 
mrt6_leave_group(struct in6_addr grp, u_int ifindex)
{
    struct ipv6_mreq mreq;
    ...
    if (setsockopt(raw_icmpv6_sock, IPPROTO_IPV6, IPV6_DROP_MEMBERSHIP,
		   (char *) &mreq, sizeof(mreq)) < 0)
		perror("IPV6_DROP_MEMBERSHIP, setsockopt: ");
}

// We can use "ip link show" to see the ifindex. Mason Yu
...
int mrt6_add_mif()
{
 	...
 	// Server site
 	ifindex = getPhysicalIfindexbyName(g_conf->upstream);
 	//printf("Upstream ifindex is %d. Name is %s\n", ifindex, g_conf->upstream);
 	bzero(&mc, sizeof(mc));
 	...
 	// Client site
 	ifindex = getPhysicalIfindexbyName(g_conf->downstream);
 	//printf("Downstream ifindex is %d. Name is %s\n", ifindex, g_conf->downstream);
 	bzero(&mc, sizeof(mc));
...
int add_mfc(struct in6_addr mca)
{
	struct sockaddr_in6 src, grp;
	char a1[48];
	inet_ntop(AF_INET6, &mca, a1, 48);
	//printf("Add MFC, grp=%s\n", a1);
...
int del_mfc(struct in6_addr mca)
{
	struct sockaddr_in6 src, grp;
	char a1[48];
	inet_ntop(AF_INET6, &mca, a1, 48);
	//printf("Del MFC, grp=%s\n", a1);
	
    bzero(&src, sizeof(src));
    bzero(&grp, sizeof(grp));
	
    grp.sin6_addr = mca;
	mrt6_del_mfc(src, grp);
}

int add_membership(struct in6_addr mca)
{
	...
	ifindex = getPhysicalIfindexbyName(g_conf->upstream);
	//printf("add_membership: Inf=%s, ifindex=%d, grp=%s\n", g_conf->upstream, ifindex, a1);
	mrt6_join_group(mca, ifindex);
}

int del_membership(struct in6_addr mca)
{
	unsigned int ifindex;
	char a1[48];
	inet_ntop(AF_INET6, &mca, a1, 48);	
	...
	ifindex = getPhysicalIfindexbyName(g_conf->upstream);
	//printf("del_membership: Inf=%s, ifindex=%d, grp=%s\n", g_conf->upstream, ifindex, a1);
	mrt6_leave_group(mca, ifindex);
}

// Mason Yu. MLD Proxy. End
...
//void sendpacket6(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len)
int sendpacket6(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len);
int sendpacket6(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len)
{
	int     sent;
#ifndef ECMH_BPF
...
// Added by Mason Yu. for IPV6 phase 2 ready logo Test
/* Send a icmp packet */
int sendpacket6_icmp(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, unsigned char *SrcMac);
int sendpacket6_icmp(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, unsigned char *SrcMac)
{
	int     sent;
#ifndef ECMH_BPF
	 ...
	 */
	// Added by Mason Yu. for IPV6 phase 2 ready logo Test		
	sa.sll_addr[0] = SrcMac[0];
	sa.sll_addr[1] = SrcMac[1];
	sa.sll_addr[2] = SrcMac[2];
	sa.sll_addr[3] = SrcMac[3];
	sa.sll_addr[4] = SrcMac[4];
	sa.sll_addr[5] = SrcMac[5];	
...
// Added by Mason Yu. for IPV6 phase 2 ready logo Test
void icmp6_send2(struct intnode *intn, const struct in6_addr *src, int type, int code, void *data, unsigned int dlen, char *to_name, unsigned char *SrcMac);
void icmp6_send2(struct intnode *intn, const struct in6_addr *src, int type, int code, void *data, unsigned int dlen, char *to_name, unsigned char *SrcMac)
{
	struct icmp6_hoplimit_packet
	{
	...
	memcpy(&ifreq.ifr_name, to_name, sizeof(ifreq.ifr_name));
	
	if (ioctl(sock, SIOCGIFMTU, &ifreq) != 0)
	{
		dolog(LOG_ERR, "Couldn't determine MTU size for %s, : %s\n", to_name, strerror(errno));
		perror("Error");		
		close(sock);
		return NULL;
	}
	close(sock);	
	packet.icmp6.icmp6_mtu = ifreq.ifr_mtu;	
	
	/* Add the data, we start at the data in the icmp6 packet */
	// Added by Mason Yu. for IPV6 phase 2 ready logo Test
	//memcpy(&packet.icmp6.icmp6_data32, data, (sizeof(packet.data) > dlen ? dlen : sizeof(packet.data)));
	memcpy(&packet.icmp6.icmp6_data32+1, data, (sizeof(packet.data) > dlen ? dlen : sizeof(packet.data)));

	/* Calculate and fill in the checksum */
	packet.icmp6.icmp6_cksum	= ipv6_checksum(&packet.ip6, IPPROTO_ICMPV6, (uint8_t *)&packet.icmp6, sizeof(packet.icmp6) + dlen - sizeof(packet.icmp6.icmp6_data32));

	dolog(LOG_DEBUG, "Sending ICMPv6 Type %s (%u) code %s (%u) on %s/%u\n", icmpv6_type(type), type, icmpv6_code(type, code), code, intn->name, intn->ifindex);
	sendpacket6_icmp(intn, (const struct ip6_hdr *)&packet, sizeof(packet) - (sizeof(packet.data) - dlen) - sizeof(packet.icmp6.icmp6_data32), SrcMac);
...
void l4_ipv6_icmpv6_mld1_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld1 *mld1, const uint16_t plen)
{
	...
	if (	!IN6_IS_ADDR_MULTICAST(&mld1->mca) ||
		...
		// Added by Mason Yu. for IPV6 phase 2 ready logo Test
		IN6_IS_ADDR_MC_RESERVED(&mld1->mca) ||
	...
	// Mason Yu. MLD Proxy
	// Do not do send_report by user space. It must be done by kernel after we set membership.
	...
	if (isnew) {
		...
		// ADD_MEMBERSHIP		
		if ( strncmp(intn->name, ALIASNAME_ELAN_PREFIX, strlen(ALIASNAME_ELAN_PREFIX)) ==0 )
			add_membership(mld1->mca);
	}
...
void l4_ipv6_icmpv6_mld2_report(struct intnode *intn, const struct ip6_hdr *iph, const uint16_t len, struct mld2_report *mld2r, const uint16_t plen)
{
	...
	while (ngrec > 0)
	{
		...
		if (grec->grec_type == MLD2_CHANGE_TO_EXCLUDE) {
			//printf("%s(%d) isnew:%d\n",__func__,__LINE__, isnew);
			//if (isnew) {
            if (grpintn && grpintn->subscriptions)
                subscrn = subscr_find(grpintn->subscriptions, &any);
			if(!subscrn){
				printf("%s(%d) can't find subscription\n",__func__,__LINE__);
				return;
			}
			if(subscrn->mode == MLD2_MODE_IS_INCLUDE)
			{
				// Add MFC
				add_mfc(grec->grec_mca);
		
				// ADD_MEMBERSHIP					
				if ( strncmp(intn->name, ALIASNAME_ELAN_PREFIX, strlen(ALIASNAME_ELAN_PREFIX)) ==0 ) {
					//printf("**It is a new group and intf is %s. Add membership.\n", intn->name);
					add_membership(grec->grec_mca);
				}
			}
		}
...
//void l4_ipv6_multicast(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len)
int l4_ipv6_multicast(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, char *to_name);
int l4_ipv6_multicast(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, char *to_name)
{
...
D(
	...
	LIST_LOOP(groupn->interfaces, grpintn, in)
	{
		...
		LIST_LOOP(grpintn->subscriptions, subscrn, in2)
		{
			...
			ret = sendpacket6(interface, iph, len);
			strcpy(to_name, interface->name);			
...
void l4_ipv6_icmpv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen)
{
	...
	{		
		...
		//else l4_ipv6_multicast(intn, iph, len);
			...
			ret = l4_ipv6_multicast(intn, iph, len, to_name);
 ...
 * plen		= Payload length (should match up to at least icmpv6
 */
void l4_ipv6_icmpv6_2(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen, unsigned char *SrcMac);
void l4_ipv6_icmpv6_2(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, struct icmp6_hdr *icmpv6, const uint16_t plen, unsigned char *SrcMac)
{
	uint16_t		csum;
	...
	// Added by Mason Yu. for IPV6 phase 2 ready logo Test
	if ( *(&icmpv6->icmp6_type-6) == 0x87) {
		//printf("This is an Unrecognized Option\n");
		return;
	}
	...
	if (icmpv6->icmp6_type == ICMP6_ECHO_REQUEST)
	{
		...
		if ( ret == 97 ) 			
			icmp6_send2(intn, &iph->ip6_src, 2, 0, iph, plen-sizeof(*icmpv6)+sizeof(icmpv6->icmp6_data32), to_name, SrcMac);
	}
	else
	{
		if (!(IN6_IS_ADDR_LINKLOCAL(&iph->ip6_src)))
...
void l3_ipv6(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len)
{
	...
	if (IN6_IS_ADDR_MULTICAST(&iph->ip6_dst))
	{
		...
		l4_ipv6_multicast(intn, iph, len, to_name);
		return;
	}
...
// Added by Mason Yu. for IPV6 phase 2 ready logo Test
void l3_ipv6_2(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, unsigned char *SrcMac);
void l3_ipv6_2(struct intnode *intn, struct ip6_hdr *iph, const uint16_t len, unsigned char *SrcMac)
{
	struct ip6_ext		*ipe;
	uint8_t			ipe_type;
	...
	if (ipe_type == IPPROTO_ICMPV6)
	{
		...
		// Added by Mason Yu. for IPV6 phase 2 ready logo Test
		//l4_ipv6_icmpv6(intn, iph, len, (struct icmp6_hdr *)ipe, plen);
		l4_ipv6_icmpv6_2(intn, iph, len, (struct icmp6_hdr *)ipe, plen, SrcMac);
		return;
	}

	// Mason Yu. MLD Proxy
	// We do not need to handle multicast packets on user space. It is kernel job.
#if 0
	/* Handle multicast packets */
	if (IN6_IS_ADDR_MULTICAST(&iph->ip6_dst))
	{
...
void l2_ethtype_2(struct intnode *intn, const uint8_t *packet, const unsigned int len, const unsigned int ether_type, unsigned char *SrcMac)
{
	...
	{
		l3_ipv6_2(intn, (struct ip6_hdr	*)packet, len, SrcMac);
	}
	/* We don't care about anything else... */
}
...
void update_interfaces(struct intnode *intn)
{
	...
	char 			buf[1024]; // Mason Yu. for compiler ecmh
#else
		...
		if (intn)
		{
			...
			else
			{
...
#endif  //#ifdef ECMH_GETIFADDR
				// Mason Yu. for compiler ecmh
				...
				else
				{
					...
					dolog(LOG_ERR, "Unknown Address Family - Ignoring\n");
				}
			}
		}
...
bool handleinterfaces(uint8_t *buffer)
{
	...
	// Added by Mason Yu. for IPV6 phase 2 ready logo Test	
	memcpy(SrcMac, sa.sll_addr, sizeof(SrcMac));	
	...
	if (intn)
	{
		...
		l2_ethtype_2(intn, buffer, len, ntohs(sa.sll_protocol), SrcMac);
	}
	else
	{
...
static struct option const long_options[] = {
	...
	{"downstream",		required_argument,	NULL, 'o'},
...
int main(int argc, char *argv[])
{
	...
	/* Handle arguments */
	while ((i = getopt_long(argc, argv, "fi:o:pPu:"
#ifdef ECMH_BPF
		"tT"
#endif
		"vV"
		...
		, long_options, &option_index)) != EOF)
	{
		...
		default:
			fprintf(stderr,
				...
				"-o, --downstream interface   downstream interface\n" 
#ifdef ECMH_BPF
	...
	while (!g_conf->quit && !quit)
	{
		...
		// Mason Yu. fast leave
		//if (g_needs_timeout)
		if (g_needs_timeout || g_needs_leave)
		{
			/* Run timeout routine */
			timeout();
			...
			g_needs_leave = false;		// Mason Yu. fast leave
	...
	//cleanpid(SIGINT); //iulian, it will cuase multiple ecmh process bug
