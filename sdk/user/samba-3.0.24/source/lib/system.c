...
#define SMB_ACCOUNT_FILE	"/var/passwd.smb"
static char pw_line[100];
...
struct passwd *getpwent(void)
{
	...
	if (NULL == pw_fp)
		pw_fp = fopen(SMB_ACCOUNT_FILE, "r");

	if (NULL != pw_fp)
	{
		memset(pw_line, 0, sizeof(pw_line));
		if ( (p = fgets(pw_line, sizeof(pw_line), pw_fp)) == NULL )
			return NULL;
