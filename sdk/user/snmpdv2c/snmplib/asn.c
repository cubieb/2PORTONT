...
AsnDatumPtrType	asnV2Error (AsnV2ErrType errId)
{
	AsnPtrType		ap;
	AsnDatumPtrType		dp;
	AsnDatumPtrType		mp;
	...
	if (mp != (AsnDatumPtrType) 0) {
		...
		dp->asnDatumMaxLen = asnLengthIndef;
		dp->asnDatumCmd = asnTypeNone;
		dp->asnDatumTag = (AsnTagType) errId;
		dp->asnDatumValue = (AsnLengthType) 0;
		dp->asnDatumClass = asnClassContext;
	}
	return (dp);
}
...
//for trap
char *getAsnString(AslIdType rval)
{
    return (char*)(asnPtrToRoot ((asnIdToPtr (rval)))->asnArea);
}

int getAsnStringLen(AslIdType rval)
{
    return (asnIdToPtr (rval))->asnDatumActualLen;
}

int getAsnInteger(AslIdType rval)
{
    return (int)(asnPtrToRoot ((asnIdToPtr (rval)))->asnArea);
}
