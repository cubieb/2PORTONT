...
static int read_opt120(char *line, char *buf, int *len)
{
	struct in_addr inp;
	int encode = line[0] - '0';
	int old_len=0;
	...
	for(tok = strtok(line, " ,\t"); tok; tok = strtok(NULL, " ,\t"))
	{
		...
		{
			strcpy(tmpstr,tok );
			pToken = strtok_r(tmpstr, ".", &pSave);
			while(pToken){
				/* attach label length of domain name */
				*buf = strlen(pToken);
				buf++;
				*len += 1;
				/* attach the domain name */
				memcpy(buf, pToken, strlen(pToken));
				buf += strlen(pToken);
				*len += strlen(pToken);
				pToken = strtok_r(NULL, ".", &pSave);
			}
			/* attach the terminate character */
			*buf = '\0';
			buf +=  1;
			*len += 1;
		}
...
static int read_opt125(char *line, void *arg, int *len)
{
	...
	*len+=4;
	...
	if(OptDataLen)	/* e8 enterprise code is temprary set to 0.*/
#else
	...
	do {
...
#ifdef _CWMP_TR111_
		else if( option->code==0x7d )
			val = line + strlen(opt)+1;
#endif
		else if(option->code == DHCP_POSIX_TZ_STRING)
			val = strtok(NULL, "\t");
		else
			val = strtok(NULL, ", \t");
		...
		if (val) {
			...
			case OPTION_S32:
				...
				break;
			case OPTION_120:
				retval = read_opt120(val, buffer, &length);
				break;
#ifdef _CWMP_TR111_
			case OPTION_125:
...
void add_serving_pool(struct server_config_t* pserving_pool )
{
	...
	/*ql 20090119 END*/
...
#ifdef IP_BASED_CLIENT_TYPE  // Magician : For E8B
					...
					//strcpy(pserving_pool->clientRange->option60, pserving_pool->vendorclass);
#endif // End Magician : For E8B
...
void write_leases(void)
{
	...
	for (i = 0; i < server_config.max_leases; i++) {
		if (leases[i].yiaddr != 0) {
			...
			fwrite(&(leases[i].interfaceType), 4, 1, fp);
...
void update_force_portal(struct dhcpOfferedAddr *pstLease)
{
	...
	if (server_config.force_portal_file)
    {
    	if ((fp = fopen(server_config.force_portal_file, "w")))
	{
    		/* LOG(LOG_ERR, "Unable to open %s for writing", server_config.force_portal_file); */

        fprintf(fp, "device: %lu: %02x-%02x-%02x-%02x-%02x-%02x\n",
...
#ifdef CONFIG_RTK_RG_INIT
		/*
		/proc/rg/redirect_first_http_req_by_mac
		command:
		a device_type mac: e.g. "a 0 00:00:39:d3:34:d7" means add PC with mac 00:00:39:d3:34:d7
		*/
		if ((fp = fopen("/proc/rg/redirect_first_http_req_by_mac", "w")))
		{
			fprintf(stderr, "a %lu %02x:%02x:%02x:%02x:%02x:%02x\n", 
				pstLease->stClientInfo.ulDevice - 1,
	                pstLease->chaddr[0], pstLease->chaddr[1], pstLease->chaddr[2],
	                pstLease->chaddr[3], pstLease->chaddr[4], pstLease->chaddr[5]);

			fprintf(fp, "a %lu %02x:%02x:%02x:%02x:%02x:%02x\n",
	                pstLease->stClientInfo.ulDevice - 1,
	                pstLease->chaddr[0], pstLease->chaddr[1], pstLease->chaddr[2],
	                pstLease->chaddr[3], pstLease->chaddr[4], pstLease->chaddr[5]);
...
#endif // of _PRMT_X_CT_COM_PORTALMNT_
...
/*star:20081219 START add for client limit for type CTC_UNKNOWNs*/
	if(pstClientInfo->category==CTC_UNKNOWN){
#ifdef CONFIG_RTK_RG_INIT
		//RG let 0 set as default category, all traffic will learn mac at category 0.
		//then compare with other category. So, we shift all category by 1.
		RTK_RG_AccessWanLimitCategory_Set(pstLease->chaddr,CTC_Computer+1);
#endif
		fprintf(fp, "device: %u: %02x-%02x-%02x-%02x-%02x-%02x\n",
            CTC_Computer,
            pstLease->chaddr[0], pstLease->chaddr[1], pstLease->chaddr[2],
...
/*star:20081219 END*/
#ifdef CONFIG_RTK_RG_INIT
		RTK_RG_AccessWanLimitCategory_Set(pstLease->chaddr,pstClientInfo->category+1);
#endif
