...
write_out_plr(){
    ...
    for (unsigned int r=0;r<num_plr;++r) {
        ...
        oob.total_chunk=htonl(round_to_page(file_length(h)-first_load_bytes)+num_of_fl_page);
        oob.startup_num = htonl(num_of_fl_page);
...
write_out_uboot(){
    ...
    oob.total_chunk=htonl(round_to_page(file_length(h)));
...
#endif
    if (unit_per_page*oob_size!=sizeof(oob_t)) {
