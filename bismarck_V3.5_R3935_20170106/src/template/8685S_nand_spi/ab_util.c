...
static char _banner_msg[] SECTION_RECYCLE_DATA = {"PRELOADER.%08x.LPLR.%08x.\n"};

SECTION_RECYCLE void 
plr_init_utility(void) {
    // 1. binding
    // 1.1 bind proto_printf
    const symbol_table_entry_t *s=symb_retrive_lplr(SF_PROTO_PRINTF);
    if (s) _proto_printf=s->v.pvalue;
    // 1.2 bind delays
    s=symb_retrive_lplr(SF_SYS_UDELAY);
    if (s) _udelay=s->v.pvalue;
    s=symb_retrive_lplr(SF_SYS_MDELAY);
    if (s) _mdelay=s->v.pvalue;
    s=symb_retrive_lplr(SF_SYS_GET_TIMER);
