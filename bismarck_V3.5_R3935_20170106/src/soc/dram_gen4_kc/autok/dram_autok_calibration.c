...
{
	u32_t i_dq;
	volatile u32_t *ddcrdqr_base = (volatile u32_t *)0xB8001510;
    ...
    if(PRODUZIONE){
        u32_t m_reg;
        volatile u32_t *dwdqor_base= (volatile u32_t *)0xB80015D0;
    	for (m_reg = 0;m_reg < 4;m_reg++){
    		*dwdqor_base = (w_delay | (w_delay << 8) | (w_delay << 16) | (w_delay << 24) );
    //		printf("0x%08x: 0x%08x\n\r", dwdqor_base,*dwdqor_base);
    		dwdqor_base++;
		}
    }
...
#ifdef CONFIG_DRAM_SILENCE
/* For DDR2, DDR3 read delay tap*/
	if(max_r_len>20 && max_r_seq_start==0){
		r_delay_tap = max_r_len/2 -1;
		max_tap = max_r_len-1;
		min_tap = r_delay_tap-5;

	}else if(max_r_len>12 && max_r_seq_start==0){
		r_delay_tap = max_r_len-11;
		max_tap = max_r_len-1;
		min_tap = r_delay_tap-1;
		//max_r_seq_start +=2;	
		//if((max_r_len-12) >max_r_seq_start)
		//	r_delay_tap = max_r_len-12-max_r_seq_start;
		//else
		//	r_delay_tap =0;
		//max_r_len -=2;
	}else if(max_r_seq_start!=0){
		r_delay_tap = max_r_len/2;
		max_tap = max_r_seq_start+max_r_len;
		min_tap = max_r_seq_start;		
		//r_delay_tap = max_r_len/2;		
	}else{
		r_delay_tap=0;
		max_tap = max_r_seq_start+max_r_len;
	...
	*ddcrdqr_base = ((w_delay_tap & 0x1f) << 24) | \
    ...
    if(PRODUZIONE){   
        volatile u32_t *dwdqor;
        volatile u32_t *dwdqor_base = (volatile u32_t *)0xB80015D0;

    	/* Enable delay tap sync with write delay tap*/
    	if (bit_loc < 16){
            //	printf("w_delay_tap = %d	",w_delay_tap);
            dwdqor = dwdqor_base + (bit_loc / 4);
            *dwdqor = *dwdqor | (w_delay_tap << ((bit_loc % 4) * 8));
            //	printf("dwdqor(%p)= 0x%08x",dwdqor, *dwdqor);		
    	}
    }
#endif //CONFIG_DRAM_SILENCE
...
static u32_t __DDR_Calibration_Full_Scan_Window(u32_t target_addr, u32_t len, u32_t resolution, u32_t w_start, u32_t r_start)
{
...
#ifdef _memctl_hw_silence_autok_test	
    u32_t silence_pattern_len,silence_pattern_start_addr,silence_period,silence_period_unit;
    u32_t cnt_tmp;
#endif
    ...
    if(PRODUZIONE){
    	// clear DWDQOR 
    	printf("AK: clear dwdqor\n\r");
        u8_t ci;
        volatile u32_t *dwdqor_base = (volatile u32_t *)0xB80015D0;
    	for (ci=0; ci<4; ci++){
    		*dwdqor_base = 0;
    		//printf("AK: dwdqor_base(%p) = 0x%x\n\r",dwdqor_base,*dwdqor_base);
    		dwdqor_base++;
    	}
    }
    
    for(bit_loc = 0; bit_loc < 32; bit_loc++){
        printf("AK: bit:%02d, max_w_s(%02d), max_w_len(%02d), max_r_s(%02d), max_r_len(%02d): ", bit_loc, max_w_seq_start_array[bit_loc], max_w_len_array[bit_loc], max_r_seq_start_array[bit_loc], max_r_len_array[bit_loc]);
        //write back the calibrated delay taps
    ...
    if(PRODUZIONE){
    	printf("AK: DQ enable delay sync with DQ delay tap.\n    0xb80015D0=0x%08x, 0xb80015D4=0x%08x, 0xb80015D8=0x%08x, 0xb80015DC=0x%08x\n\r",REG32(0xb80015D0),REG32(0xb80015D4),REG32(0xb80015D8),REG32(0xb80015DC));
    }
#ifdef _memctl_hw_silence_autok_test
	silence_period = 
	silence_period_unit =
	silence_pattern_len =
	silence_pattern_start_addr=0x80900000;
	REG32(0xb8001504) = 0x8;			//silence period n19..n16, silence period unit n15..n8, silence len n7..n0 => 0=4bytes

	
	REG32(0xb8001508) = silence_pattern_start_addr;	//silence pattern starting address.
	for(silence_pattern_len=0;silence_pattern_len)
		REG32(0x0x80900000+) = 0xffff0000
#endif	
    return 0;
}
	...
	if(full_scan){
    	...
    	while( 0 != _DDR_Calibration_Full_Scan(target_addr, len, resolution, w_start, r_start)){
	        ...
	        if(retry_limit > MEMCTL_CALI_RETRY_LIMILT){
				printf("EE: %s-%d full scan Fail, need to define failure status. retry_limit:%d\n",retry_limit);
