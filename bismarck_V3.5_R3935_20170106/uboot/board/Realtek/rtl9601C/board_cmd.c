...
extern int concur_btg_test (int flag, int argc, char * const argv[]);
/*
 * Do DRAM test.
 */
int do_concur_btg_test (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
    return concur_btg_test(flag, argc, argv);
}
#endif
...
#endif
#ifdef CONFIG_CMD_CONCURENT_BTG_TEST
    do_concur_btg_test(cmdtp, flag, argc, argv);
#endif
#ifdef CONFIG_CMD_DRAM_TEST
...
#ifdef CONFIG_CMD_CONCURENT_BTG_TEST
U_BOOT_CMD(
        mConBtg_test,    10,    1,     do_concur_btg_test,
        "mConBtg_test   - do Concurrent BTG test.",
        "    - do Concurrent BTG test."
);
#endif
#ifdef CONFIG_CMD_DRAM_TEST
U_BOOT_CMD(
