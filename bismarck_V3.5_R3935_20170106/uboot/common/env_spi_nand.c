...
int writeenv(size_t offset, u_char *buf)
{
    ...
    blocksize = spi_nand_chip_size(0);
    len = min(blocksize, CONFIG_ENV_SIZE);

    while (amount_saved < CONFIG_ENV_SIZE && offset < end) {
        char_ptr = &buf[amount_saved];
		if (spi_nand_write_ecc(offset, len, char_ptr))
            return 1;
    ...
    if (amount_saved != CONFIG_ENV_SIZE)    return 1;
...
#define env_new (*(_env_new))
    ...
    puts("II: erasing spi-nand... ");
    if (spi_nand_erase(offset, CONFIG_ENV_RANGE))
        return 1;

    puts("II: writing to spi-nand... ");
    if (writeenv(offset, (u_char *)&env_new)) {
        puts("failed!\n");
        return 1;
    }
...
#define env_new (*(_env_new))
    ...
    puts("II: erasing spi-nand... ");
    if (spi_nand_erase(CONFIG_ENV_OFFSET, CONFIG_ENV_RANGE))
        return 1;
...
int readenv(size_t offset, u_char *buf)
{
    ...
    while (amount_loaded < CONFIG_ENV_SIZE && offset < end) {
        ...
        spi_nand_read_ecc(offset, len, char_ptr);
        offset += blocksize;
        amount_loaded += len;
	}
