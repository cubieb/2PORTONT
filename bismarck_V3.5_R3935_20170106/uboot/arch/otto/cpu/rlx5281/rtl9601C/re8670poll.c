...
void Lan_RXENABLE(void)
{
    ...
    if(0 == analog_patch_flag) {
        ...
        MACReg(0x10004, 0xa0000000);
        if((MACRegR(0x10004) & 0xffff) == 0x6485)
        {
            MACReg(0, 0xfffb);
            MACReg(4, 0x61b844);
            MACReg(4, 0x21b906);
        ...
        // patch phy done - WRAP_GPHY_MISC bit 0, PATCH_PHY_DONE
        MACReg(0x110, 1);
        __udelay(1000000);

        //phy patch: power down
        MACReg(0x0, 0x3000);
        MACReg(0x4, 0x60a400);
        __udelay(1000000);
        MACReg(0x0, 0x1140);
        ...
        MACReg(0x230c4, 0);
        //port isolation
        MACReg(0x27000, 0x3fffff);
        MACReg(0x27004, 0x3fffff);
        //cpu port ability
        MACReg(0x18c, 0x196);
        //cpu port force mode
        MACReg(0x1c0, 0xfff);
        //meter set tick-token tick-period 74 token 151
        MACReg(0x25000, 0x12bbd);
        //meter set pon-tick-token tick-period 53 token 58
        //MACReg(0x25108, 0x1353a);
        //vlan set state disable
        MACReg(0x13008, 0x00000000);
       //vlan set tag-mode port 0-3 keep-format
        MACReg(0x2a000, 0x00000001);
        MACReg(0x2a004, 0x00000001);
        MACReg(0x2a008, 0x00000001);
        MACReg(0x2a00c, 0x00000001);
        //cpu set trap-insert-tag state and aware disable
        MACReg(0x23030, 0x00000000);
        //MOCIR_FRC_MD, 0711
        MACReg(0xf02170, 0x1ffff);
        //MOCIR_FRC_VAL, 0711
        MACReg(0xf02174, 0x1ffff);
